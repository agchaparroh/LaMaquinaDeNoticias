...
Uso: python .claude/check.py TASK-XXX
"""
import yaml
import subprocess
import sys
...
def load_tasks():
    ...
    tasks_file = Path('.claude/tasks.yaml')
    if not tasks_file.exists():
        print("❌ Error: No se encuentra .claude/tasks.yaml")
...
def main():
    """Función principal"""
    if len(sys.argv) != 2:
        print("Uso: python .claude/check.py TASK-XXX")
        print("\nEjemplo:")
        print("  python .claude/check.py TASK-001")
        sys.exit(1)
    ...
    if not Path('.claude').exists():
        print("❌ Error: No se encuentra el directorio .claude")
