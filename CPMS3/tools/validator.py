...
class PlanValidator:
            ...
            if action == 'modify_file':
                ...
                if not parent.exists():
                    try:
                        ...
                        self.info.append(
                            f"{step_id}: Directorio se creará: {parent}"
                        )
    ...
    def _validate_search_patterns(self):
        ...
        for step in steps:
            ...
            try:
                ...
                self.errors.append(
                    f"{step_id}: No se puede leer archivo: {path} - {e}"
                )
                continue
            ...
            for i, change in enumerate(changes):
                ...
                if find_text not in content:
                    ...
                    if normalized_find in normalized_content:
                        ...
                        preview = find_text[:50] + "..." if len(find_text) > 50 else find_text
                        self.errors.append(
        ...
        for step in steps:
            ...
            if executable:
                ...
                if not self._command_exists(executable):
                    ...
                    self.info.append(
                        f"{step_id}: Comando verificado: {executable}"
                    )
        ...
        for directory in directories:
            if directory.exists():
                ...
                if not os.access(parent, os.W_OK):
                    self.errors.append(
                        f"Sin permisos para crear: {directory}"
                    )
    ...
    def _find_similar_text(self, pattern: str, content: str, max_results: int = 3) -> List[str]:
...
def print_validation_report(is_valid: bool, errors: List[str], 
                          warnings: List[str], info: List[str]):
