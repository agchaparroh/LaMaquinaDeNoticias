...
from psycopg2.extras import RealDictCursor, execute_batch  # noqa: F401
...
class EmbeddingBatchProcessor:
    ...
    def get_records_to_process(
        ...
        try:
            ...
            logger.info(
                f"Encontrados {len(records)} registros en {table_name} para procesar"
            )
            return records
    ...
    def process_table(
        ...
        id_column: str = "id",
        additional_columns: List[str] = None,
        text_combiner: Callable[[Dict], str] = None,
    ) -> ProcessingStats:
        """
        ...
        try:
                ...
                try:
                    ...
                    if self.config.validate_embeddings:
                        ...
                        if validation["invalid_embeddings"] > 0:
                            logger.warning(
                                f"Lote {self.stats.batch_count}: {validation['invalid_embeddings']} embeddings inválidos"
                            )
                    ...
                    if (
                        current_transaction_batches >= self.config.transaction_size
                        or i + self.config.batch_size >= len(records)
                    ):
                        ...
                        logger.info(
                            f"Actualizados {updated_count} registros en transacción"
                        )
                    ...
                    if self.stats.batch_count % self.config.log_interval == 0:
                        logger.info(
                            f"Procesados {self.stats.batch_count} lotes, "
                    ...
                    while retry_count < self.config.max_retries:
                        try:
                            ...
                            logger.info(
                                f"Reintentando lote {self.stats.batch_count} (intento {retry_count + 1})"
                            )

                            # Repetir procesamiento
                            if text_combiner:
                                texts = [
                                    text_combiner(record) for record in batch_records
                                ]
                            else:
                                texts = [
                                    record[text_column] or ""
                            ...
                            logger.info(
                                f"✅ Lote {self.stats.batch_count} reintentado exitosamente"
                            )
                            break
                            ...
                            retry_count += 1
                            logger.error(
                                f"Error en reintento {retry_count}: {retry_error}"
                            )

                            if retry_count >= self.config.max_retries:
                                logger.error(
                                    f"❌ Lote {self.stats.batch_count} falló después de {self.config.max_retries} reintentos"
                                )
            ...
            logger.info(
                f"  ⚡ Velocidad: {self.stats.records_per_second:.1f} registros/seg"
            )
...
def test_batch_processing():
    ...
    try:
        ...
        records = processor.get_records_to_process(
            "hilos_narrativos", "titulo", "id", ["descripcion"]
        )
        ...
        if records:
            ...
            if len(embeddings) == len(sample_texts):
                ...
                logger.info(
                    f"📊 Validación: {validation['valid_embeddings']}/{validation['total_embeddings']} válidos"
                )

                if validation["valid_embeddings"] > 0:
                    logger.info(
                        "🎉 Framework de procesamiento batch funcionando correctamente"
                    )
                    return True
                else:
                ...
                logger.error(
                    f"❌ Error en generación: {len(embeddings)} vs {len(sample_texts)}"
                )
                return False
        else:
            logger.warning(
                "⚠️  No hay registros para probar, pero la configuración es correcta"
            )
            return True
