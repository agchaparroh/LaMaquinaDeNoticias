...
def test_compatibility_methods():
    ...
    for method in compatibility_methods:
        ...
        expected_params = [
            "contenido", "modelo_spacy", "request_id", 
            "groq_api_key", "contexto_articulo"
        ]
...
def test_article_context_extraction():
    ...
    required_context_fields = [
        "titulo", "fecha_publicacion", "fuente", "pais", 
        "tipo_medio", "idioma", "autor", "seccion", "es_opinion", "url"
    ]
...
def test_type_detection_logic():
    ...
    for type_name, is_article, is_fragment in types_to_test:
        ...
        assert not (is_article and is_fragment), f"Tipo {type_name} no puede ser ambos tipos"
        
        if not is_article and not is_fragment:
            print(f"  - Tipo no soportado: debe lanzar ValueError")
        else:
            print(f"  - Tipo soportado: procesamiento normal")
