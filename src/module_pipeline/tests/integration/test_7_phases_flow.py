...
# Config
from ...src.config import get_model_for_content, pipeline_config

# Controller y fases
from ...src.controller import PipelineController
from ...src.models.procesamiento import (
    CitaTextual,  # noqa: F401
    DatosCuantitativos,  # noqa: F401
...
from ...src.models.response import EstadoProcesamiento, ProcesarArticuloResponse
...
from ...src.pipeline.fase_5_datos import ejecutar_fase_5_datos  # noqa: F401
from ...src.pipeline.fase_6_citas import ejecutar_fase_6_citas  # noqa: F401
from ...src.pipeline.fase_7_normalizacion import (
    ejecutar_fase_7_normalizacion,  # noqa: F401
)

# Servicios
from ...src.services.chunking_service import ChunkingService, ChunkType  # noqa: F401
from ...src.services.consolidation_service import ConsolidationService  # noqa: F401
from ...src.utils.fragment_processor import FragmentProcessor  # noqa: F401
        ...
        with patch("src.pipeline.fase_1_triaje.Groq") as mock_groq:
            mock_groq.return_value.chat.completions.create.return_value = Mock(
                choices=[
                    Mock(
                        message=Mock(
                            content='{"relevancia": 8, "justificacion": "Artículo relevante"}'
                        )
                    )
                ],
        ...
        with patch("src.pipeline.fase_2_simplificacion.Groq") as mock_groq:
            mock_groq.return_value.chat.completions.create.return_value = Mock(
                choices=[
                    Mock(
                        message=Mock(
                            content='{"texto_simplificado": "Texto simplificado"}'
                        )
                    )
                ],
                usage=Mock(prompt_tokens=100, completion_tokens=50),
        ...
        with patch("src.pipeline.fase_4_hechos.Groq") as mock_groq:
            mock_groq.return_value.chat.completions.create.return_value = Mock(
                choices=[
                    Mock(
                        message=Mock(
                            content='{"hechos": [{"id": 1, "contenido": "Reunión presidencial", "tipo_hecho": "EVENTO"}]}'
                        )
                    )
                ],
                usage=Mock(prompt_tokens=100, completion_tokens=50),
...
class TestCompleteFlow:
    ...
    @pytest.mark.asyncio
    async def test_complete_pipeline_short_article(
        self, sample_article_short, pipeline_controller
    ):
        """Test del pipeline completo con artículo corto (sin chunking)."""
        request = ProcesarArticuloRequest(**sample_article_short)

        # Mock de todas las llamadas a Groq
        with patch("src.pipeline.fase_1_triaje.Groq") as mock_groq1, patch(
            "src.pipeline.fase_2_simplificacion.Groq"
        ) as mock_groq2, patch(
            "src.pipeline.fase_3_entidades.Groq"
        ) as mock_groq3, patch("src.pipeline.fase_4_hechos.Groq") as mock_groq4, patch(
            "src.services.supabase_service.SupabaseService"
        ) as mock_supabase:
            ...
            mock_groq1.return_value.chat.completions.create.return_value = Mock(
                choices=[
                    Mock(
                        message=Mock(
                            content='{"relevancia": 8, "justificacion": "Relevante"}'
                        )
                    )
                ],
                usage=Mock(prompt_tokens=100, completion_tokens=50),
            ...
            mock_groq4.return_value.chat.completions.create.return_value = Mock(
                choices=[
                    Mock(
                        message=Mock(
                            content='{"hechos": [{"id": 1, "contenido": "Reunión", "tipo_hecho": "EVENTO"}]}'
                        )
                    )
                ],
                usage=Mock(prompt_tokens=100, completion_tokens=50),
    ...
    @pytest.mark.asyncio
    async def test_complete_pipeline_with_chunking(
        self, sample_article_long, pipeline_controller
    ):
        ...
        with patch("src.pipeline.fase_1_triaje.Groq") as mock_groq, patch(
            "src.services.supabase_service.SupabaseService"
        ) as mock_supabase:
            ...
            mock_groq.return_value.chat.completions.create.return_value = Mock(
                choices=[
                    Mock(
                        message=Mock(
                            content='{"relevancia": 8, "entidades": [], "hechos": []}'
                        )
                    )
                ],
                usage=Mock(prompt_tokens=100, completion_tokens=50),
            ...
            if (
                len(sample_article_long["contenido"])
                > pipeline_config.chunking.chars_threshold
            ):
                assert len(chunks) > 1, (
                    "El artículo largo debería generar múltiples chunks"
                )
...
class TestPerformanceAndScalability:
    ...
    async def test_parallel_chunk_processing_performance(self):
        ...
        with patch("src.pipeline.fase_3_entidades.Groq") as mock_groq:
            ...
            tiempo_secuencial = (datetime.now() - start_time).total_seconds()  # noqa: F841
            ...
            tiempo_paralelo = (datetime.now() - start_time).total_seconds()  # noqa: F841
            ...
            assert (
                resultado_paralelo["metadatos_extraccion"]["parallel_processing"]
                is True
            )
            assert (
                resultado_secuencial.get("metadatos_extraccion", {}).get(
...
class TestIntegrationSuite:
    ...
    async def test_full_integration_with_real_flow(self, sample_article_short):
        ...
        with patch("src.services.groq_service.Groq") as mock_groq, patch(
            "src.services.supabase_service.SupabaseService"
        ) as mock_supabase:
            ...
            mock_groq.return_value.chat.completions.create.return_value = Mock(
                choices=[
                    Mock(
                        message=Mock(
                            content='{"relevancia": 8, "justificacion": "Artículo político relevante"}'
                        )
                    )
                ],
