...
from ...src.config import (
    get_chunking_config,
    get_parallel_processing_config,
    pipeline_config,
)
...
from ...src.pipeline.fase_4_hechos import (
    ...
    extraer_hechos_con_chunking,  # noqa: F401
    extraer_hechos_con_chunking_paralelo,  # noqa: F401
)

# Imports del sistema de chunking
from ...src.services.chunking_service import (  # noqa: F401
    ChunkingService,
    ChunkType,
    ContentChunk,
)
...
class TestChunkingDecisions:
    ...
    def test_multiple_criteria_chunking(self, chunking_service):
        ...
        config = get_chunking_config()  # noqa: F841
...
class TestParallelChunkProcessing:
    ...
    async def test_parallel_entity_extraction(self, resultado_fase2_mock):
        ...
        chunks = [
            "Chunk 1: Juan Pérez",
            "Chunk 2: María González",
            "Chunk 3: Carlos López",
        ]
        ...
        with patch("src.pipeline.fase_3_entidades.Groq") as mock_groq:
            ...
            mock_groq.return_value.chat.completions.create.return_value = Mock(
                choices=[
                    Mock(
                        message=Mock(
                            content='{"entidades": [{"id": 1, "nombre": "Test", "tipo": "PERSONA"}]}'
                        )
                    )
                ],
    ...
    async def test_parallel_vs_sequential_performance(self, resultado_fase2_mock):
        ...
        with patch("src.pipeline.fase_3_entidades.Groq") as mock_groq:
            ...
            tiempo_secuencial = time.time() - start_time  # noqa: F841
            ...
            tiempo_paralelo = time.time() - start_time  # noqa: F841
...
class TestCrossChunkConsolidation:
    ...
    def test_consolidate_similar_facts(self, consolidation_service):
        ...
        hechos_chunk1 = [
            {
                "id": 1,
                "contenido": "El presidente se reunió con el ministro",
                "tipo_hecho": "EVENTO",
            }
        ]
        ...
        hechos_chunk2 = [
            {
                "id": 2,
                "contenido": "Reunión presidencial con ministro de economía",
                "tipo_hecho": "EVENTO",
            }
        ]
            ...
            [
                {
                    ...
                    "nombre": "Juan Pérez",
                    "tipo": "PERSONA",
                    "descripcion": "Presidente del país",
                }
            ],
        ]
    ...
    def test_consolidation_threshold_configuration(self, consolidation_service):
        ...
        config = get_parallel_processing_config()  # noqa: F841
        ...
        with patch("src.pipeline.fase_4_hechos.Groq") as mock_groq:
            mock_groq.return_value.chat.completions.create.return_value = Mock(
                choices=[
                    Mock(
                        message=Mock(
                            content='{"hechos": [{"id": 1, "contenido": "Reunión presidencial", "entidades_relacionadas": ["Juan Pérez", "María González"]}]}'
                        )
                    )
                ],
                usage=Mock(prompt_tokens=100, completion_tokens=50),
...
class TestChunkingPerformance:
    ...
    def test_memory_usage_with_large_content(self, chunking_service):
        ...
        try:
            ...
            for chunk in chunks:
                assert (
                    len(chunk.text) <= pipeline_config.chunking.chars_threshold * 1.2
                )  # 20% margen
    ...
    async def test_parallel_processing_scales_with_chunks(self, resultado_fase2_mock):
        ...
        assert all(t >= 0 for t in processing_times), (
            "Todos los procesamientos deberían completarse"
        )
