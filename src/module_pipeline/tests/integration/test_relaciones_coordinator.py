...
class TestRelacionesCoordinator(unittest.TestCase):
    ...
    def test_extraer_relaciones_hecho_entidad(self):
        ...
        for rel in self.mock_resultado_fase7.metadata_normalizacion[
            "relaciones_completas"
        ]["relaciones_estructurales"]["hecho_entidad"]:
        ...
        self.assertEqual(
            len(relaciones_hecho_entidad[1]), 2
        )  # Hecho 1 tiene 2 entidades
    ...
    def test_mapear_tipo_relacion_hecho(self):
        ...
        self.assertEqual(
            coordinator._mapear_tipo_relacion_hecho("causa-efecto"), "causa"
        )
        self.assertEqual(
            coordinator._mapear_tipo_relacion_hecho("temporal_secuencial"),
            "seguimiento_de",
        )
        self.assertEqual(
            coordinator._mapear_tipo_relacion_hecho("aclaracion"), "aclaracion_de"
        )

        # Test sin mapeo (debe devolver el original)
        self.assertEqual(
            coordinator._mapear_tipo_relacion_hecho("consecuencia"), "consecuencia"
        )
        self.assertEqual(
            coordinator._mapear_tipo_relacion_hecho("contexto_historico"),
            "contexto_historico",
        )
    ...
    def test_mapear_tipo_contradiccion(self):
        ...
        # Test sin mapeo (debe devolver el original)
        self.assertEqual(
            coordinator._mapear_tipo_contradiccion("entidades"), "entidades"
        )
        self.assertEqual(
            coordinator._mapear_tipo_contradiccion("ubicacion"), "ubicacion"
        )
    ...
    def test_extraer_todas_relaciones(self):
        ...
        relaciones_temporales = self.mock_resultado_fase7.metadata_normalizacion[
            "relaciones_completas"
        ]["relaciones_temporales"]
        ...
        relaciones_estructurales = self.mock_resultado_fase7.metadata_normalizacion[
            "relaciones_completas"
        ]["relaciones_estructurales"]
    ...
    def test_relaciones_parciales(self):
        ...
        # Extraer relaciones
        relaciones = mock_resultado_parcial.metadata_normalizacion[
            "relaciones_completas"
        ]
        hecho_entidad = relaciones["relaciones_estructurales"].get("hecho_entidad", [])
        hecho_relacionado = relaciones["relaciones_temporales"].get(
            "hecho_relacionado", []
        )
