...
def reset_article_states(urls=None):
    ...
    if not supabase_url or not supabase_key:
        print(
            "⚠️  No se encontraron credenciales de Supabase. Saltando reset de estados."
        )
        return
...
def test_article_with_stats(json_file):
    ...
    """
    stats = {"exitoso": False, "hechos": 0, "entidades": 0, "citas": 0}
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if elementos:
                stats["hechos"] = elementos.get(
                    "hechos", elementos.get("total_hechos", 0)
                )
                stats["entidades"] = elementos.get(
                    "entidades", elementos.get("total_entidades", 0)
                )
...
Nota: El reset de estados se hace automáticamente antes de cada prueba.
    ...
    parser.add_argument(
        "target", help='Archivo JSON específico o "all" para probar todos'
    )
    ...
    if args.target == "all":
        ...
        if not json_dir.exists():
            json_dir = Path(
                "src/module_pipeline/../fixtures/articles/json"
            )  # Ruta absoluta
        ...
        if urls:
            ...
            print(
                "⚠️  No se pudieron obtener URLs, reseteando todos los artículos de Infobae"
            )
        ...
        print(f"📝 Total de elementos extraídos:")  # noqa: F541
        print(f"   - Hechos: {total_hechos}")
        print(f"   - Entidades: {total_entidades}")
        print(f"   - Citas: {total_citas}")
        if exitosos > 0:
            print(f"📈 Promedios por artículo:")  # noqa: F541
