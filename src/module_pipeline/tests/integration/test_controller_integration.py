    ...
    hechos = [
        ...
        HechoProcesado(
            ...
            metadata_hecho=MetadatosHecho(
                tipo_hecho="ANUNCIO", precision_temporal="exacta"
            ),
        ),
    ]
    ...
    entidades = [
        EntidadProcesada(
            ...
            metadata_entidad=MetadatosEntidad(
                tipo="PERSONA", descripcion_estructurada=["líder del proyecto"]
            ),
        ),
        EntidadProcesada(
            ...
            relevancia=7,
            metadata_entidad=MetadatosEntidad(
                tipo="PERSONA", descripcion_estructurada=["coautora del estudio"]
            ),
        ),
        EntidadProcesada(
            ...
            relevancia=6,
            metadata_entidad=MetadatosEntidad(
                tipo="LUGAR", descripcion_estructurada=["sede de la cumbre del clima"]
            ),
        ),
    ]
    ...
    citas = [
        CitaTextual(
            ...
            metadata_cita=MetadatosCita(
                contexto="Declaración sobre el impacto del descubrimiento", relevancia=5
            ),
        )
    ]
    ...
    datos = [
        DatosCuantitativos(
            ...
            metadata_dato=MetadatosDato(
                categoria="ambiental", tipo_periodo="acumulado", tendencia="aumento"
            ),
        )
    ]
...
class TestPipelineControllerIntegration:
    ...
    async def test_process_article_complete_flow(
        ...
        with patch("src.controller.GroqService", return_value=mock_groq_service), patch(
            "src.controller.get_supabase_service", return_value=mock_supabase_service
        ), patch(
            "src.pipeline.fase_1_triaje.ejecutar_fase_1",
            return_value=mock_fase1_resultado,
        ), patch(
            "src.pipeline.fase_2_extraccion.ejecutar_fase_2",
            return_value=mock_fase2_resultado,
        ), patch(
            "src.pipeline.fase_3_citas_datos.ejecutar_fase_3",
            return_value=mock_fase3_resultado,
        ), patch(
            "src.pipeline.fase_4_normalizacion.ejecutar_fase_4",
            return_value=mock_fase4_resultado,
            ...
            mock_builder_instance.construir_payload_fragmento.return_value = (
                mock_payload
            )
            ...
            assert all(
                fase in metricas["tiempos_fases"]
            ...
            assert resultado["persistencia"]["exitosa"] == True  # noqa: E712
            ...
            assert resultado["procesamiento_parcial"] == False  # noqa: E712
    ...
    async def test_process_fragment_direct_flow(
        ...
        with patch("src.controller.GroqService", return_value=mock_groq_service), patch(
            ...
            assert resultado["procesamiento_exitoso"] == True  # noqa: E712
            assert resultado["procesamiento_parcial"] == False  # noqa: E712
    ...
    async def test_process_article_with_fase2_error_fallback(
        ...
        with patch("src.controller.GroqService", return_value=mock_groq_service), patch(
            ...
            "src.pipeline.fase_2_extraccion.ejecutar_fase_2", side_effect=error_fase2
        ), patch(
            "src.pipeline.fase_3_citas_datos.ejecutar_fase_3",
            return_value=mock_fase3_resultado,
        ), patch(
            ...
            assert resultado["procesamiento_exitoso"] == True  # noqa: E712
            assert (
                resultado["procesamiento_parcial"] == True
            )  # Indica uso de fallback  # noqa: E712

            # Verificar advertencias
            assert len(resultado["advertencias"]) > 0
            assert any(
                "Fase 2 ejecutada con fallback" in adv
            ...
            metricas = resultado["metricas"]
            assert (
                metricas["tasas_exito"]["fase2"] == False
            )  # Fase 2 falló  # noqa: E712
            ...
            assert fase2_data["metadata_extraccion"]["es_fallback"] == True  # noqa: E712
            assert (
                len(fase2_data["hechos_extraidos"]) >= 1
            )  # Al menos el hecho del título

    @pytest.mark.asyncio
    async def test_process_fragment_all_phases_with_errors(
        self, mock_groq_service, mock_supabase_service, sample_fragment_simple
    ):
        ...
        with patch("src.controller.GroqService", return_value=mock_groq_service), patch(
            ...
            "src.pipeline.fase_1_triaje.ejecutar_fase_1", side_effect=error_fase1
        ), patch(
            "src.pipeline.fase_2_extraccion.ejecutar_fase_2", side_effect=error_fase2
        ), patch(
            "src.pipeline.fase_3_citas_datos.ejecutar_fase_3", side_effect=error_fase3
        ), patch(
            "src.pipeline.fase_4_normalizacion.ejecutar_fase_4", side_effect=error_fase4
        ), patch("src.services.payload_builder.PayloadBuilder") as mock_payload_builder:
            # Configurar mock del PayloadBuilder
            mock_builder_instance = Mock()
            mock_payload = Mock()
            ...
            assert resultado["procesamiento_exitoso"] == True  # noqa: E712
            assert resultado["procesamiento_parcial"] == True  # noqa: E712
    ...
    async def test_process_article_persistence_error(
        ...
        mock_supabase_service.insertar_fragmento_completo.side_effect = Exception(
            "Error de BD"
        )
        ...
        with patch("src.controller.GroqService", return_value=mock_groq_service), patch(
            ...
            assert resultado["procesamiento_exitoso"] == True  # noqa: E712
            assert "persistencia" in resultado
            assert resultado["persistencia"]["exitosa"] == False  # noqa: E712
            assert "Error de BD" in resultado["persistencia"]["error"]

            # Verificar advertencias
            assert any(
                "Error en persistencia" in adv
    ...
    async def test_process_fragment_no_facts_skip_persistence(
        ...
        with patch("src.controller.GroqService", return_value=mock_groq_service), patch(
            ...
            "src.pipeline.fase_2_extraccion.ejecutar_fase_2",
            return_value=mock_fase2_sin_hechos,
        ), patch(
            "src.pipeline.fase_3_citas_datos.ejecutar_fase_3",
            return_value=mock_fase3_resultado,
        ), patch(
            ...
            assert resultado["persistencia"]["exitosa"] == False  # noqa: E712
            assert (
                "No hay datos suficientes para persistir"
                in resultado["persistencia"]["mensaje"]
            )
...
if __name__ == "__main__":
    ...
    print(
        "  pytest tests/test_controller_integration.py::TestPipelineControllerIntegration::test_process_article_complete_flow -v"
    )
    print("\nTests disponibles:")
