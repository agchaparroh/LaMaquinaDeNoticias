...
def generate_test_article(char_count: int = 4000) -> str:
    ...
    entidades = ['Pedro Sánchez', 'María García', 'José Luis Rodríguez', 'Ana Torres', 'Madrid', 'Barcelona', 'España']
    verbos = ['anunció', 'declaró', 'confirmó', 'estableció', 'propuso', 'rechazó', 'aprobó']
    sustantivos = ['reforma', 'medida', 'política', 'proyecto', 'iniciativa', 'programa', 'plan']
...
def benchmark_chunking_consolidation(article: str) -> Dict[str, float]:
    ...
    try:
        ...
        processor = OptimizedSimilarityProcessor()
        ...
        return {
            ...
            'reduction_percent': (1 - len(consolidated_groups) / len(all_entities)) * 100 if all_entities else 0,
            'entities_per_second': len(all_entities) / elapsed_time if elapsed_time > 0 else 0
        }
    except Exception as e:
...
def main():
    ...
    for size in test_sizes:
        ...
        results.append({
            'size': size,
            'actual_size': len(article),
            **metrics
        })
        
        # Mostrar resultados
        print(f"✅ Completado en {metrics['total_time_seconds']:.2f}s")
        print(f"   Target: <30s | Actual: {metrics['total_time_seconds']:.2f}s | {'✅ PASS' if metrics['meets_requirement'] else '❌ FAIL'}")
    ...
    print(f"✅ Tests pasados: {sum(1 for r in results if r['meets_requirement'])}/{len(results)}")
