...
def generate_long_article(length: int = 15_000) -> Dict[str, Any]:
    ...
    while current_length < length:
        ...
        if paragraph_count % 4 == 0:
            paragraph += f'"Este es un momento histórico para la investigación en IA", afirmó el investigador principal. '  # noqa: F541
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(
                ...
                if response_data.get("status") == "processing":
                    print(f"✅ Procesamiento asíncrono confirmado")  # noqa: F541
                    ...
                    print(f"\n📊 Información de tracking:")  # noqa: F541
                    print(
                        f"   - Tiempo estimado: {tracking.get('estimated_time_seconds')} segundos"
                    )
                    print(
                        f"   - Endpoint de status: {tracking.get('check_status_endpoint')}"
                    )
                    ...
                    print(f"\n📋 Metadata:")  # noqa: F541
                    print(
                        f"   - Longitud: {metadata.get('longitud_caracteres')} caracteres"
                    )
                    print(
                        f"   - Es artículo largo: {metadata.get('es_articulo_largo')}"
                    )
                    ...
                    print(f"❌ No se detectó procesamiento asíncrono")  # noqa: F541
...
async def check_job_status(job_id: str, max_attempts: int = 30, wait_seconds: int = 2):
        ...
        for attempt in range(max_attempts):
            try:
                async with session.get(f"{API_BASE_URL}/status/{job_id}") as response:
                    ...
                    print(
                        f"   Intento {attempt + 1}/{max_attempts}: Status = {job_status}"
                    )

                    # Si está completado o falló, retornar resultado
                    if job_status == "completed":
                        print(f"✅ Job completado exitosamente")  # noqa: F541
                        result = status_data.get("data", {}).get("result", {})
                        print(f"   - Fragmento ID: {result.get('fragmento_id')}")
                        print(
                            f"   - Tiempo procesamiento: {result.get('tiempo_procesamiento_segundos')}s"
                        )
                        print(
                            f"   - Elementos extraídos: {result.get('elementos_extraidos', {})}"
                        )
                        return status_data

                    elif job_status == "failed":
                        print(f"❌ Job falló")  # noqa: F541
    ...
    print(
        f"⏱️  Timeout: Job no completó después de {max_attempts * wait_seconds} segundos"
    )
    return None
...
async def test_multiples_requests_concurrentes():
    ...
    try:
        ...
        for i, size in enumerate(sizes):
            ...
            articulo["titular"] = (
                f"Artículo concurrente #{i + 1}: {articulo['titular'][:30]}..."
            )
        ...
        for result in results:
            if "error" in result:
                print(
                    f"   ❌ Artículo {result['index'] + 1}: Error - {result['error']}"
                )
            else:
                print(
                    f"   ✅ Artículo {result['index'] + 1}: Job ID = {result['job_id']}"
                )
                if result["job_id"]:
        ...
        print(
            f"   - Tiempo promedio por request: {total_time / len(articulos):.2f} segundos"
        )

        # Verificar que el sistema maneja múltiples jobs
        if len(job_ids) == len(articulos):
            print(f"\n✅ El sistema maneja correctamente múltiples jobs concurrentes")  # noqa: F541
        else:
            print(f"\n⚠️  Algunos jobs no se crearon correctamente")  # noqa: F541
...
async def test_articulo_pequeno_sincrono():
    ...
    try:
        ...
        print(
            f"✅ Artículo pequeño generado: {len(articulo['contenido_texto'])} caracteres"
        )
        print(f"   - Bajo el threshold de {ASYNC_PROCESSING_THRESHOLD} caracteres")

        # Enviar artículo
        async with aiohttp.ClientSession() as session:
            print(f"\n📤 Enviando artículo pequeño...")  # noqa: F541
            start_time = time.time()
            ...
            async with session.post(
                ...
                if "data" in response_data and "fase_1_triaje" in response_data["data"]:
                    print(f"✅ Procesamiento síncrono confirmado")  # noqa: F541
                    print(f"   - Resultado completo recibido inmediatamente")  # noqa: F541

                    # Verificar algunos elementos del resultado
                    data = response_data["data"]
                    metricas = data.get("metricas", {})
                    print(f"\n📊 Métricas del procesamiento:")  # noqa: F541
                    print(
                        f"   - Tiempo total: {metricas.get('tiempo_total_segundos', 0):.2f}s"
                    )
                    print(
                        f"   - Hechos extraídos: {metricas.get('conteos_elementos', {}).get('hechos_extraidos', 0)}"
                    )
                    print(
                        f"   - Entidades extraídas: {metricas.get('conteos_elementos', {}).get('entidades_extraidas', 0)}"
                    )

                    return True

                elif response_data.get("status") == "processing":
                    print(f"❌ Se detectó procesamiento asíncrono (no esperado)")  # noqa: F541
                    print(f"   - El artículo pequeño debería procesarse síncronamente")  # noqa: F541
                    return False

                else:
                    print(f"❌ Respuesta inesperada")  # noqa: F541
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(f"{API_BASE_URL}/health") as response:
                if response.status == 200:
                    print(f"✅ API está activa y respondiendo")  # noqa: F541
                else:
                    print(
                        f"❌ API no responde correctamente (status: {response.status})"
                    )
                    return 1
    except Exception as e:
    ...
    if job_id:
        ...
        if final_status:
            ...
            if data.get("status") == "completed":
                print(f"\n✅ Test 1: Test asíncrono exitoso")  # noqa: F541
            else:
                print(f"\n❌ Test 1: Error - Job no completó exitosamente")  # noqa: F541
        else:
            print(f"\n❌ Test 1: Error - No se pudo verificar estado del job")  # noqa: F541
    else:
        print(f"\n❌ Test 1: Error - No se obtuvo job_id")  # noqa: F541

    # Test 2: Múltiples requests concurrentes
    job_ids = await test_multiples_requests_concurrentes()

    if job_ids:
        print(f"\n✅ Test 2: Múltiples requests concurrentes exitoso")  # noqa: F541
        print(f"   - Se crearon {len(job_ids)} jobs concurrentemente")
    else:
        print(f"\n❌ Test 2: Error - No se pudieron crear jobs concurrentes")  # noqa: F541

    # Test 3: Artículo pequeño síncrono
    sync_result = await test_articulo_pequeno_sincrono()

    if sync_result:
        print(f"\n✅ Test 3: Procesamiento síncrono verificado")  # noqa: F541
    else:
        print(f"\n❌ Test 3: Error - Procesamiento síncrono falló")  # noqa: F541
