...
class RecoveryTestResult:
    ...
    def add_recovery_attempt(
        self, success: bool, recovery_time: float, details: Dict[str, Any] = None
    ):
    ...
    def print_summary(self):
        ...
        print(
            f"Recuperaciones exitosas: {self.successful_recoveries} ({self.calculate_recovery_rate():.1f}%)"
        )
        ...
        if self.system_state_changes:
            print(f"\nCambios de estado del sistema:")  # noqa: F541
...
class TestRecovery:
    ...
    def sample_article(self):
        ...
        return {
            ...
            "contenido_texto": "Este es un artículo de prueba para verificar la recuperación del sistema. "
            * 20,
            "idioma": "es",
    ...
    def sample_fragment(self):
        ...
        return {
            ...
            "texto_original": "Fragmento de prueba para verificar recuperación ante fallos. "
            * 10,
    ...
    async def test_recuperacion_fallo_groq(
        self, mock_groq, api_base_url, sample_article
    ):
        """
                            ...
                            "content": json.dumps(
                                {
                                    ...
                                    "entidades": [
                                        {"nombre": "Sistema", "tipo": "ORGANIZACION"}
                                    ],
                                }
                            )
                        }
        ...
        try:
            response = await self._send_request_with_retry(
                f"{api_base_url}/procesar_articulo", sample_article
            )
            ...
            if response["status"] == 200:
                ...
                result.add_recovery_attempt(
                    True,
                    recovery_time,
                    {"attempts": call_count, "final_status": response["status"]},
                )
            else:
                print(f"  ❌ Sistema no se recuperó completamente")  # noqa: F541
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                f"{api_base_url}/procesar_articulo", json=sample_article
            ) as response:
                verification_time = time.time() - verification_start

                if response.status == 200:
                    print(f"  ✅ Sistema operativo post-recuperación")  # noqa: F541
                    ...
                    print(f"  ❌ Sistema aún degradado")  # noqa: F541
                    result.system_state_changes.append(
                        "Sistema no completamente recuperado"
                    )
        ...
        try:
            response = await self._send_request_with_retry(
                f"{api_base_url}/procesar_articulo", sample_article, max_retries=2
            )

            # Si llegamos aquí, el sistema tiene un fallback
            if response["status"] == 200:
                print(f"  ✅ Sistema usó fallback exitosamente")  # noqa: F541
                result.add_recovery_attempt(
                    True,
                    time.time() - persistent_start,
                    {"method": "fallback", "status": response["status"]},
                )
            else:
            ...
            result.add_recovery_attempt(
                False,
                time.time() - persistent_start,
                {"expected_failure": True, "error_type": type(e).__name__},
            )
        ...
        assert result.get_average_recovery_time() < 30, (
            "Tiempo de recuperación muy alto"
        )

    @pytest.mark.asyncio
    @patch("src.services.supabase_service.SupabaseService.call_rpc")
    async def test_recuperacion_fallo_supabase(
        self, mock_supabase, api_base_url, sample_fragment
    ):
        """
        ...
        try:
            response = await self._send_request_with_retry(
                f"{api_base_url}/procesar_fragmento", sample_fragment
            )
            ...
            if response["status"] == 200:
                ...
                result.add_recovery_attempt(
                    True, recovery_time, {"attempts": call_count}
                )
            else:
                print(f"  ❌ No se pudo recuperar completamente")  # noqa: F541
                result.add_recovery_attempt(False, recovery_time)
        ...
        validation_start = time.time()  # noqa: F841

        try:
            # Este error no debería ser recuperable
            response = await self._send_request_with_retry(
                f"{api_base_url}/procesar_fragmento", sample_fragment, max_retries=1
            )

            # Si obtenemos respuesta, verificar que sea un error
            if response["status"] >= 400:
                print(
                    f"  ✅ Error de validación manejado correctamente (HTTP {response['status']})"
                )
                result.system_state_changes.append(
                    "Error de validación no recuperable detectado"
                )
            else:
                print(f"  ⚠️  Respuesta inesperada para error de validación")  # noqa: F541

        except Exception as e:
            print(
                f"  ✅ Error de validación propagado correctamente: {type(e).__name__}"
            )
        ...
        for i, failure_prob in enumerate(degradation_levels):
            ...
            try:
                response = await self._send_request_with_retry(
                    f"{api_base_url}/procesar_fragmento", sample_fragment, max_retries=2
                )

                if response["status"] == 200:
                    result.system_state_changes.append(
                        f"Nivel {i}: Éxito (prob fallo: {failure_prob})"
                    )
                else:
                    result.system_state_changes.append(
                        f"Nivel {i}: Degradado (prob fallo: {failure_prob})"
                    )

            except:  # noqa: E722
                result.system_state_changes.append(
                    f"Nivel {i}: Fallo total (prob fallo: {failure_prob})"
                )
        ...
        assert len(result.system_state_changes) > 0, (
            "No se registraron cambios de estado"
        )
    ...
    async def test_recuperacion_memoria(self, api_base_url, sample_article):
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    response_data = await response.json()  # noqa: F841
                    recovery_time = time.time() - start_time

                    if response.status == 200:
                        print(f"  ✅ Artículo grande procesado exitosamente")  # noqa: F541
        ...
        print(
            f"📊 Memoria actual: {current_memory:.1f} MB (incremento: {memory_increase:.1f} MB)"
        )
        ...
        if successful > len(large_articles) * 0.7:  # 70% éxito
            result.add_recovery_attempt(
                True,
                concurrent_time,
                {"successful": successful, "total": len(large_articles)},
            )
        else:
        ...
        result.system_state_changes.append(
            f"Memoria liberada por GC: {pre_gc_memory - post_gc_memory:.1f} MB"
        )
        ...
        recovery_check_start = time.time()  # noqa: F841
        ...
        try:
            ...
            if response["status"] == 200:
                print(f"  ✅ Sistema operativo después de presión de memoria")  # noqa: F541
                result.system_state_changes.append(
                    "Sistema recuperado post-presión de memoria"
                )
            else:
                print(f"  ❌ Sistema degradado post-presión")  # noqa: F541
    ...
    async def test_circuit_breaker(self, api_base_url, sample_article):
        ...
        if circuit_open_detected:
            result.add_recovery_attempt(
                True,
                failed_requests * 0.1,
                {"circuit_breaker_triggered": True, "failed_requests": failed_requests},
            )
        else:
            print(f"  ⚠️  Circuit breaker no detectado explícitamente")  # noqa: F541
        ...
        try:
            response = await self._send_request_with_retry(
                f"{api_base_url}/procesar_articulo", sample_article, max_retries=3
            )

            recovery_time = time.time() - recovery_start

            if response["status"] == 200:
                print(f"  ✅ Circuit breaker recuperado")  # noqa: F541
                result.add_recovery_attempt(
                    True,
                    recovery_time,
                    {"recovery_successful": True, "wait_time": wait_time},
                )
                result.system_state_changes.append(
                    "Circuit breaker cerrado - sistema operativo"
                )
            else:
                print(f"  ❌ Sistema aún no recuperado")  # noqa: F541
                result.add_recovery_attempt(False, recovery_time)
    ...
    @pytest.mark.asyncio
    async def test_resiliencia_general(
        self, api_base_url, sample_article, sample_fragment
    ):
        """
        ...
        failure_scenarios = [
            ...
            },
            {
                ...
                "data": {
                    **sample_fragment,
                    "texto_original": "Test con 特殊字符 🚀 \x00 \n\r",
                },
        ...
        for scenario in failure_scenarios:
            ...
            try:
                if scenario.get("concurrent_requests"):
                    ...
                    async with aiohttp.ClientSession() as session:
                        ...
                        for i in range(scenario["concurrent_requests"]):
                            data = {
                                **scenario["data"],
                                "id_fragmento": f"concurrent_{i}",
                            }
                            task = session.post(
                                f"{api_base_url}/{scenario['endpoint']}",
                                json=data,
                                timeout=aiohttp.ClientTimeout(
                                    total=scenario["timeout"]
                                ),
                            )
                            tasks.append(task)
                        ...
                        if success_rate > 0.5:
                            print(
                                f"  ✅ Manejó concurrencia: {successful}/{len(results)} exitosos"
                            )
                            result.add_recovery_attempt(
                                True,
                            ...
                            print(
                                f"  ❌ Fallo bajo concurrencia: {successful}/{len(results)} exitosos"
                            )
                    ...
                    async with aiohttp.ClientSession() as session:
                        async with session.post(
                            ...
                            if response.status == 400:  # Error esperado
                                print(
                                    f"  ✅ JSON corrupto manejado correctamente (HTTP 400)"
                                )  # noqa: F541
                                result.add_recovery_attempt(
                                    True,
                                    time.time() - start_time,
                                ...
                                print(f"  ❌ Respuesta inesperada para JSON corrupto")  # noqa: F541
                                result.add_recovery_attempt(
                    ...
                    async with aiohttp.ClientSession() as session:
                        async with session.post(
                            ...
                            if response.status in [
                                ...
                                print(
                                    f"  ✅ Escenario manejado: HTTP {response.status}"
                                )
                                result.add_recovery_attempt(
                                    True,
                                    time.time() - start_time,
                ...
                if scenario["name"] == "Timeout de red":
                    print(f"  ✅ Timeout manejado correctamente")  # noqa: F541
                    result.add_recovery_attempt(
                        True,
                        time.time() - start_time,
                        {"scenario": scenario["name"], "timeout_handled": True},
                    )
                else:
                    print(f"  ❌ Timeout inesperado")  # noqa: F541
                    result.add_recovery_attempt(False, time.time() - start_time)
                ...
                result.add_recovery_attempt(
                    False,
                    time.time() - start_time,
                    {"scenario": scenario["name"], "exception": type(e).__name__},
                )
        ...
        try:
            ...
            if final_response["status"] == 200:
                print(
                    "  ✅ Sistema completamente operativo después de todos los escenarios"
                )
                result.system_state_changes.append(
                    "Sistema resiliente - todos los escenarios superados"
                )
            else:
                print("  ⚠️  Sistema parcialmente degradado")
                result.system_state_changes.append(
                    "Sistema parcialmente degradado post-tests"
                )
        ...
        # Assertions
        assert resilience_score >= 60, (
            f"Score de resiliencia muy bajo: {resilience_score:.1f}%"
        )
        assert result.successful_recoveries >= len(failure_scenarios) * 0.5, (
            "Menos del 50% de escenarios manejados"
        )
...
def generate_recovery_report(results: List[RecoveryTestResult]):
    ...
    overall_rate = (
        (total_recoveries / total_attempts * 100) if total_attempts > 0 else 0
    )

    print(f"\nRESUMEN GENERAL:")  # noqa: F541
