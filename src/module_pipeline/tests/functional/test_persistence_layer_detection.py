...
def test_persistence_layer_article_detection():
    ...
    es_articulo = resultado_pipeline_articulo.get('metadatos', {}).get('es_articulo_completo')
    assert es_articulo == True, "Debe detectar artículo desde metadatos del pipeline"
    print("✓ Artículo detectado correctamente desde metadatos del pipeline")
    
    # Test 2: Verificar información complementaria
    tipo_original = resultado_pipeline_articulo.get('metadatos', {}).get('tipo_contenido_original')
    assert tipo_original == "ArticuloProcesableItem", f"Tipo original incorrecto: {tipo_original}"
    ...
    assert fragmento_id.startswith("ART-"), f"ID debe tener formato ART-: {fragmento_id}"
    
    articulo_id_numerico = int(fragmento_id.replace("ART-", ""))
    assert articulo_id_numerico == 123, f"ID numérico incorrecto: {articulo_id_numerico}"
...
def test_persistence_layer_fragment_detection():
    ...
    es_articulo = resultado_pipeline_fragmento.get('metadatos', {}).get('es_articulo_completo')
    assert es_articulo == False, "Debe detectar fragmento desde metadatos del pipeline"
    print("✓ Fragmento detectado correctamente desde metadatos del pipeline")
    
    # Test 2: Verificar información complementaria
    tipo_original = resultado_pipeline_fragmento.get('metadatos', {}).get('tipo_contenido_original')
    assert tipo_original == "FragmentoProcesableItem", f"Tipo original incorrecto: {tipo_original}"
    print(f"✓ Tipo original correcto: {tipo_original}")
    ...
    assert not fragmento_id.startswith("ART-"), f"ID de fragmento no debe tener formato ART-: {fragmento_id}"
...
def test_persistence_layer_legacy_compatibility():
    ...
    es_articulo_moderno = resultado_pipeline_legacy.get('metadatos', {}).get('es_articulo_completo')
    ...
    es_articulo_legacy = fragmento_legacy.get('metadata_adicional', {}).get('es_articulo_completo', False)
    assert es_articulo_legacy == True, "Debe funcionar detección legacy"
...
def test_persistence_layer_rpc_routing():
    ...
    for caso in casos_routing:
        if caso['es_articulo'] and caso['id_formato'].startswith("ART-"):
            ...
            print(f"  - Sin extracción ID numérico")
