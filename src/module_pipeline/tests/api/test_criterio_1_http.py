...
def test_criterio_1_http():
    ...
    try:
        print(f"1. Enviando artículo al pipeline...")  # noqa: F541
        ...
        print(f"2. Respuesta inicial recibida:")  # noqa: F541
        ...
        if job_id:
            ...
            if status_response.status_code == 200:
                ...
                if job_status.get("result"):
                    ...
                    print(f"\n5. Resultado del procesamiento:")  # noqa: F541
                    print(f"   - Éxito: {resultado.get('exito', False)}")
                    print(
                        f"   - Fase completada: {resultado.get('fase_completada', 0)}/7"
                    )

                    if resultado.get("persistencia"):
                        persist = resultado["persistencia"]
                        print(f"\n6. PERSISTENCIA EN SUPABASE:")  # noqa: F541
                        print(f"   - Exitosa: {persist.get('exitosa', False)}")
                        print(
                            f"   - Fragmento ID: {persist.get('fragmento_id', 'N/A')}"
                        )
                        print(f"   - Hechos: {persist.get('hechos_insertados', 0)}")
                        print(
                            f"   - Entidades: {persist.get('entidades_insertadas', 0)}"
                        )
