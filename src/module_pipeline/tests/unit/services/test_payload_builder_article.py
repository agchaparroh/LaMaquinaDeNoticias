...
def test_payload_builder_with_article_model():
    ...
    assert len(payload_data) == len(metadatos_articulo) + len(
        procesamiento_articulo
    ) + len(elementos_extraidos)
    ...
    for article_field, payload_field in field_mapping.items():
        assert payload_field in payload_data, (
            f"Campo {payload_field} no encontrado en payload"
        )
