...
import pytest
from groq import APIConnectionError, APIStatusError, Groq, RateLimitError  # noqa: F401
...
def test_send_prompt_with_system_prompt(mock_groq_client):
    ...
    with patch("src.services.groq_service.settings", MockSettingsValid()):
        ...
        response = service.send_prompt(
            prompt="Pregunta", system_prompt="Eres un asistente útil"
        )
...
def test_logging_occurs_during_operations(mock_logger, mock_groq_client):
    ...
    with patch("src.services.groq_service.settings", MockSettingsValid()):
        ...
        assert any(
            "inicializado" in str(call) for call in mock_logger.info.call_args_list
        )
        assert any(
            "Enviando prompt" in str(call) for call in mock_logger.info.call_args_list
        )
...
def test_groq_service_basic_workflow():
    ...
    with patch("src.services.groq_service.settings", MockSettingsValid()), patch(
        "src.services.groq_service.Groq"
    ) as MockGroq:
        # Instanciar servicio
        service = GroqService()
