...
class TestPayloadBuilder(unittest.TestCase):
    ...
    def test_construir_payload_articulo_completo_ok(self):
        ...
        self.assertEqual(
            payload.titular, MOCK_METADATOS_ARTICULO_DATA["titular"]
        )  # Acceder directamente al campo 'titular'
        self.assertEqual(len(payload.hechos_extraidos), 1)
        self.assertEqual(
            payload.hechos_extraidos[0].id_temporal_hecho, "hecho_temp_001"
        )
    ...
    def test_construir_payload_articulo_solo_requeridos_ok(self):
        ...
        self.assertIsInstance(payload, PayloadCompletoArticulo)
        self.assertEqual(
            payload.url, MOCK_METADATOS_ARTICULO_DATA["url"]
        )  # Acceder directamente al campo 'url'
        self.assertEqual(
            payload.resumen_generado_pipeline,
            MOCK_PROCESAMIENTO_ARTICULO_DATA["resumen_generado_pipeline"],
        )  # Acceder directamente al campo 'resumen_generado_pipeline'
        self.assertEqual(
            len(payload.hechos_extraidos), 0
        )  # Debería ser lista vacía por default
    ...
    def test_construir_payload_articulo_datos_invalidos_metadatos(self):
        ...
        invalid_metadatos["url_original_articulo"] = (
            "esto-no-es-un-url-valido"  # Asumiendo HttpUrl, aunque el modelo actual usa str
        )
        ...
        del invalid_metadatos[
            "titular"
        ]  # Campo requerido, el nombre correcto es 'titular'
        with self.assertRaises(
            ValueError
        ) as context:  # Error de construcción por Pydantic
            self.builder.construir_payload_articulo(
                metadatos_articulo_data=invalid_metadatos,
                procesamiento_articulo_data=MOCK_PROCESAMIENTO_ARTICULO_DATA,
            )
        self.assertIn(
            "Error de validación Pydantic en payload del artículo",
            str(context.exception),
        )
        self.assertIn(
            "titular", str(context.exception).lower()
        )  # El campo en el modelo Pydantic es 'titular'
    ...
    def test_construir_payload_articulo_datos_invalidos_hecho(self):
        ...
        self.assertIn(
            "'loc': ('id_temporal_hecho',)", str(context.exception)
        )  # Verifica que el error es sobre el campo 'id_temporal_hecho'
        self.assertIn(
            "Field required", str(context.exception)
        )  # Verifica el tipo de error de Pydantic
    ...
    def test_construir_payload_fragmento_completo_ok(self):
        ...
        self.assertEqual(
            payload.indice_secuencial_fragmento,
            MOCK_METADATOS_FRAGMENTO_DATA["indice_secuencial_fragmento"],
        )  # Verificar un campo que sí existe
        self.assertEqual(
            payload.contenido_texto_original_fragmento,
            MOCK_METADATOS_FRAGMENTO_DATA["contenido_texto_original_fragmento"],
        )
        self.assertEqual(
            payload.resumen_generado_fragmento, "Resumen del fragmento de prueba."
        )
        self.assertEqual(len(payload.hechos_extraidos), 1)
        self.assertTrue(payload.model_dump_json())
    ...
    def test_construir_payload_fragmento_solo_requeridos_ok(self):
        ...
        self.assertEqual(
            payload.indice_secuencial_fragmento,
            MOCK_METADATOS_FRAGMENTO_DATA["indice_secuencial_fragmento"],
        )  # Verificar un campo que sí existe en FragmentoPersistenciaPayload
    ...
    def test_construir_payload_fragmento_datos_invalidos_metadatos(self):
        ...
        del invalid_metadatos_fragmento[
            "contenido_texto_original_fragmento"
        ]  # Campo requerido por FragmentoPersistenciaPayload
        ...
        self.assertIn(
            "Error de validación Pydantic en payload del fragmento",
            str(context.exception),
        )
        self.assertIn(
            "contenido_texto_original_fragmento", str(context.exception).lower()
        )
