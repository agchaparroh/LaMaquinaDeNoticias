...
def test_validador_corrige_tipos_invalidos_entidad_entidad():
    ...
    assert (
        datos_corregidos["entidad_relacion"][0]["tipo_relacion"] == "aliado_con"
    )  # ubicacion → aliado_con
    assert (
        datos_corregidos["entidad_relacion"][1]["tipo_relacion"] == "aliado_con"
    )  # mencionado → aliado_con
    assert (
        datos_corregidos["entidad_relacion"][2]["tipo_relacion"] == "miembro_de"
    )  # sin cambios
...
def test_validador_caso_real_ubicacion():
    ...
    assert (
        datos_corregidos["entidad_relacion"][0]["descripcion"]
        == "Plaza San Miguel está ubicada en San Miguel"
    )
...
def test_validador_rangos_numericos():
    ...
    assert (
        datos_corregidos["entidad_relacion"][0]["fuerza_relacion"] == 10
    )  # Ajustado de 15
    assert (
        datos_corregidos["entidad_relacion"][1]["fuerza_relacion"] == 1
    )  # Ajustado de -2
    assert (
        datos_corregidos["hecho_entidad"][0]["relevancia_en_hecho"] == 10
    )  # Ajustado de 20
    assert (
        datos_corregidos["contradicciones"][0]["grado_contradiccion"] == 5
    )  # Ajustado de 10
...
if __name__ == "__main__":
    ...
    print(
        "- Rangos numéricos (fuerza_relacion 1-10, relevancia 1-10, grado_contradiccion 1-5)"
    )
