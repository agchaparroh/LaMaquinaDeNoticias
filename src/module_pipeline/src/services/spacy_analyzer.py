...
from ..models.analisis import AnalisisComponentes  # noqa: E402
...
class SpacyAnalyzer:
    ...
    def _get_nlp_model(
        self, model_name: Optional[str] = None, prefer_large: bool = False
    ) -> Optional[Language]:
        ...
        if model_name not in self._model_cache:
            ...
            for fallback_model in fallback_models:
                if fallback_model != model_name:  # No intentar el mismo modelo otra vez
                    ...
                    if loaded_model:
                        logger.warning(
                            f"Usando modelo de fallback '{fallback_model}' en lugar de '{model_name}'"
                        )
            ...
            logger.error(
                f"No se pudo cargar modelo spaCy '{model_name}' ni ningún fallback"
            )
            return None
