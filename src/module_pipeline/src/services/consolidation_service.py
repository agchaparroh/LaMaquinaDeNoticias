...
from ..utils.similarity_algorithms import (
...
class ConsolidationService:
    ...
    def __init__(self, config: Optional[ConsolidationConfig] = None):
        ...
        logger.info(f"ConsolidationService inicializado con umbral: {self.config.similarity_threshold}")
    ...
    def consolidar_entidades(
        ...
        reduction_percent = (1 - len(entidades_consolidadas) / len(todas_entidades)) * 100 if todas_entidades else 0
        logger.info(f"Consolidación completada: {len(todas_entidades)} → {len(entidades_consolidadas)} "
    ...
    def _calcular_similitud_entidades(
        ...
        # Similitud de alias
        alias_overlap = len(alias1.intersection(alias2)) / max(len(alias1.union(alias2)), 1)
    ...
    def consolidar_hechos(
        ...
        logger.info(f"Total hechos después de consolidación: {len(hechos_consolidados)}")
    ...
    def _fusionar_hechos(
        self,
        grupo: List[HechoProcesado]
    ) -> HechoProcesado:
        """
    ...
    def consolidar_datos(
        self,
        datos_por_chunk: List[List[DatosCuantitativos]]
    ...
    def consolidar_citas(
        self,
        citas_por_chunk: List[List[CitaTextual]]
    ) -> List[CitaTextual]:
        """
        ...
        for dato in datos:
            if dato.metadata_dato.hecho_id_relacionado:
                dato.metadata_dato.hecho_id_relacionado = mapeo_ids.get(
                    "hechos", {}
                ).get(dato.metadata_dato.hecho_id_relacionado, 
        ...
        for cita in citas:
            ...
            if cita.metadata_cita.hecho_relacionado_id:
                cita.metadata_cita.hecho_relacionado_id = mapeo_ids.get(
                    "hechos", {}
                ).get(cita.metadata_cita.hecho_relacionado_id,
    ...
    def _consolidar_con_algoritmos_vectorizados(
        ...
        elapsed_time = time.time() - start_time
        logger.debug(f"Consolidación vectorizada de {len(entidades)} entidades tipo {tipo}: "
                    f"{len(entidades_consolidadas)} resultado, {elapsed_time:.3f}s")
        
        return entidades_consolidadas
    
    def _fusionar_entidades_grupo(self, entidades: List[EntidadProcesada]) -> EntidadProcesada:
        """
    ...
    def _update_performance_stats(self, elements_processed: int, elapsed_time: float) -> None:
        """
