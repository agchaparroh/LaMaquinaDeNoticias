...
class SupabaseService:
    ...
    def __init__(self):
        ...
        if not hasattr(self, "_initialized"):
            ...
            try:
                ...
                self.logger.info(
                    "Cliente Supabase inicializado exitosamente con optimizaciones"
                )
    ...
    def _validar_estructura_payload(
        self, payload: Union[Dict[str, Any], BaseModel], tipo: str
    ) -> Dict[str, Any]:
        """
        ...
        if payload is None:
            raise ValidationError(
                message="Payload no puede ser None", phase=ErrorPhase.GENERAL
            )
        ...
        if tipo == "articulo":
            ...
            campos_requeridos = [
                "indice_secuencial_fragmento",
                "contenido_texto_original_fragmento",
                "fecha_procesamiento_pipeline_fragmento",
                "estado_procesamiento_final_fragmento",
            ]

        campos_faltantes = [
            campo for campo in campos_requeridos if campo not in payload
        ]
        if campos_faltantes:
            raise ValidationError(
                message=f"Campos requeridos faltantes en payload {tipo}",
                validation_errors=[
                    {"field": campo, "error": "Missing required field"}
            ...
            if lista in payload and not isinstance(payload[lista], list):
                raise ValidationError(
                    ...
                    validation_errors=[
                        {
                            "field": lista,
                            "error": f"Expected list, got {type(payload[lista]).__name__}",
                        }
                    ],
                    phase=ErrorPhase.GENERAL,
                )
    ...
    def _validar_respuesta_insercion(
        self, payload: Dict[str, Any], respuesta: Dict[str, Any], tipo: str
    ) -> None:
        """
        ...
        elementos_enviados = {
            ...
            "relaciones": len(payload.get("relaciones_hechos", []))
            + len(payload.get("relaciones_entidades", [])),
        }
    ...
    )  # Según documentación: 1 reintento para conexión
    def insertar_articulo_completo(
        self, payload: Union[Dict[str, Any], BaseModel]
        ...
        try:
            ...
            response = self.client.rpc(
                "insertar_articulo_completo", {"datos_json": payload_dict}
            ).execute()
    ...
    def actualizar_articulo_procesado(
        self, payload: Union[Dict[str, Any], BaseModel]
    ) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            payload_dict = self._validar_estructura_payload(payload, "articulo")
            self.logger.info(
                f"DEBUG - Tipo de payload_dict después de validar: {type(payload_dict)}"
            )
            self.logger.info(
                f"DEBUG - payload_dict es None después de validar: {payload_dict is None}"
            )

            if not payload_dict.get("articulo_id") and not payload_dict.get("url"):
                raise ValueError(
                    "Se requiere articulo_id o url para actualizar el artículo"
                )

            # Log de identificadores
            if payload_dict.get("articulo_id"):
                self.logger.info(
                    f"Actualizando artículo por ID: {payload_dict['articulo_id']}"
                )
            else:
                self.logger.info(
                    f"Actualizando artículo por URL: {payload_dict['url'][:50]}..."
                )

            # Debug: Log completo del payload (temporal en INFO para debug)
            self.logger.info(
                f"DEBUG - Payload completo antes de RPC: {json.dumps(payload_dict, default=str)[:500]}..."
            )
            ...
            for campo, valor in payload_dict.items():
                if valor is None:
                    self.logger.info(
                        f"Eliminando campo {campo} con valor null del payload"
                    )
            ...
            def limpiar_nulls_profundo(obj):
                ...
                if isinstance(obj, dict):
                    return {
                        k: limpiar_nulls_profundo(v)
                        for k, v in obj.items()
                        if v is not None
                    }
                elif isinstance(obj, list):
                    return [
                        limpiar_nulls_profundo(item) for item in obj if item is not None
                    ]
                else:
                    return obj
            ...
            self.logger.info(
                f"DEBUG - Payload FINAL después de limpieza profunda: {json.dumps(payload_dict, default=str)}"
            )
            ...
            self.logger.info(
                f"Llamando RPC con parámetros: {json.dumps(rpc_params, default=str)[:200]}..."
            )

            # Llamar RPC
            response = self.client.rpc(
                "actualizar_articulo_procesado", rpc_params
            ).execute()
            ...
            if response.data:
                ...
                self.logger.warning(
                    "RPC actualizar_articulo_procesado no retornó datos"
                )
                return None
    ...
    def insertar_fragmento_completo(
        self, payload: Union[Dict[str, Any], BaseModel]
    ) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if isinstance(payload_dict, dict):
                ...
                elif "fragmento" in payload_dict and isinstance(
                    payload_dict["fragmento"], dict
                ):
                    fragmento_id = payload_dict["fragmento"].get("id_fragmento")

            if not fragmento_id:
                self.logger.warning(
                    "No se encontró id_fragmento en el payload, usando valor por defecto"
                )
            ...
            response = self.client.rpc(
                ...
                },  # Envolver en objeto con clave 'datos_json'
            ).execute()
            ...
            if response.data:
                ...
                if not isinstance(result, dict):
                    self.logger.error(
                        f"Respuesta RPC no es un diccionario: {type(result)}"
                    )
                    return None
                ...
                if "fragmento_id" not in result:
                    ...
                    if "status" in result and result["status"] == "error":
                        self.logger.error(
                            f"RPC retornó error: {result.get('mensaje', 'Sin mensaje')}"
                        )
                        self.logger.error(
                            f"Código SQL: {result.get('codigo_sql', 'Sin código')}"
                        )
                        return None
        ...
        try:
            ...
            if response.data:
                ...
                for row in response.data:
                    results.append(
                        (row["id"], row["nombre"], row["tipo"], row["score"])
                    )
                ...
                self.logger.debug(
                    f"No se encontraron entidades similares para '{nombre}'"
                )
                return []
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            response = self.client.table("entidades").select("id").limit(1).execute()  # noqa: F841
    ...
    def buscar_entidad_similar_cached(
        self, nombre_entidad: str, tipo_entidad: str, umbral_similitud: float = 0.8
        ...
        try:
            ...
            cache_key = (
                f"entidad_similar:{nombre_entidad}:{tipo_entidad}:{umbral_similitud}"
            )
            ...
            if cached_result is not None:
                self.logger.debug(
                    f"Cache hit para búsqueda de entidad: {nombre_entidad}"
                )
                return tuple(cached_result)  # Convertir a tuple para LRU cache

            # Ejecutar consulta
            result = self.buscar_entidad_similar(
                nombre_entidad, tipo_entidad, umbral_similitud
            )
            ...
            return tuple(
                self.buscar_entidad_similar(
                    nombre_entidad, tipo_entidad, umbral_similitud
                )
            )

    def batch_normalize_entities(
        self, entidades: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            for tipo, entidades_tipo in entidades_por_tipo.items():
                ...
                try:
                    response = self.client.rpc(
                        "normalizar_entidades_batch", batch_payload
                    ).execute()
                    batch_results = response.data if response.data else []

                    # Asignar resultados a posiciones originales
                    for (original_idx, entidad), resultado in zip(
                        entidades_tipo, batch_results
                    ):
                        if resultado:
                            resultados[original_idx] = {
                                **entidad,
                                "id_entidad_normalizada": resultado.get("id_entidad"),
                                "nombre_entidad_normalizada": resultado.get(
                                    "nombre_normalizado"
                                ),
                                "similitud_normalizacion": resultado.get(
                                    "similitud", 0.0
                                ),
                    ...
                    self.logger.warning(
                        f"Batch normalization falló para tipo {tipo}, usando método individual: {e}"
                    )
    ...
    def _normalize_single_entity(self, entidad: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            similares = self.buscar_entidad_similar_cached(
                entidad["nombre"], entidad.get("tipo", "UNKNOWN"), 0.8
            )
            if similares:
                mejor_match = similares[0]
                return {
