...
class GroqService:
    ...
    def __init__(self, api_key: str = None, model_id: str = None):
        ...
        try:
            ...
            logger.error(
                "GROQ_API_KEY no encontrada en settings.py. Asegúrate de que esté definida."
            )
            raise ValidationError(
                message="GROQ_API_KEY es requerida y no se encontró en la configuración.",
                validation_errors=[
                    {"field": "GROQ_API_KEY", "error": "Missing in settings"}
                ],
        ...
        try:
            ...
            logger.warning(
                f"GROQ_DEFAULT_MODEL_ID no encontrada en settings. Usando valor por defecto: {_hardcoded_default_model}"
            )
    ...
    @retry_groq_api(
        max_attempts=2, wait_seconds=5
    )  # Según documentación: máximo 2 reintentos
    def _create_chat_completion_with_retry(
        ...
        logger.debug(
            f"Intentando llamada a Groq API. Modelo: {model}, Timeout: {timeout}s"
        )
        try:
            ...
            logger.warning(
                f"Límite de tasa de Groq API excedido: {e}. Reintentando según configuración."
            )
            # El decorador @retry_groq_api manejará la conversión a GroqAPIError
            raise
        except APIStatusError as e:
            logger.error(
                f"Error de estado de Groq API: {e.status_code} - {e.response}. Mensaje: {e.message}"
            )
            # El decorador @retry_groq_api manejará la conversión a GroqAPIError
            raise
    ...
    def send_prompt(
        ...
        try:
            current_max_tokens = (
                ...
                else settings.GROQ_DEFAULT_MAX_TOKENS
            )
        except AttributeError:
            current_max_tokens = max_tokens if max_tokens is not None else _s_max_tokens
            if max_tokens is None:
                logger.debug(
                    f"GROQ_DEFAULT_MAX_TOKENS no en settings, usando {_s_max_tokens}"
                )  # noqa: E701
        ...
        try:
            current_temperature = (
                ...
                else settings.GROQ_DEFAULT_TEMPERATURE
            )
        except AttributeError:
            current_temperature = temperature if temperature is not None else _s_temp
            if temperature is None:
                logger.debug(
                    f"GROQ_DEFAULT_TEMPERATURE no en settings, usando {_s_temp}"
                )  # noqa: E701

        try:
            current_timeout = (
                timeout if timeout is not None else settings.GROQ_DEFAULT_TIMEOUT
            )
        except AttributeError:
            current_timeout = timeout if timeout is not None else _s_timeout
            if timeout is None:
                logger.debug(
                    f"GROQ_DEFAULT_TIMEOUT no en settings, usando {_s_timeout}"
                )  # noqa: E701
        ...
        try:
            ...
            logger.debug(
                f"Respuesta de Groq (primeros 100 chars): {response_content[:100]}..."
            )
