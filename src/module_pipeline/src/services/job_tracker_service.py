...
class JobTrackerService:
    ...
    def create_job(
        self, job_id: Optional[str] = None, metadata: Optional[Dict[str, Any]] = None
    ) -> str:
        """
    ...
    def update_status(
        ...
        if status not in valid_states:
            raise ValueError(
                f"Estado inválido: {status}. Debe ser uno de: {valid_states}"
            )
    ...
    def get_status(self, job_id: str) -> Optional[Dict[str, Any]]:
        ...
        with self._operation_lock:
            ...
            # Log de consulta
            self.logger.debug(
                f"Consulta de estado de job",  # noqa: F541
                job_id=job_id,
                status=job_info.status,
            )
    ...
    def delete_job(self, job_id: str) -> bool:
        ...
        with self._operation_lock:
            ...
            self.logger.info(
                f"Job eliminado",  # noqa: F541
                job_id=job_id,
                status=job_info.status,
                age_minutes=(datetime.utcnow() - job_info.created_at).total_seconds()
                / 60,
            )
    ...
    def cleanup_old_jobs(self, retention_minutes: Optional[int] = None) -> int:
        ...
        with self._operation_lock:
            ...
            if jobs_to_delete:
                self.logger.info(
                    f"Limpieza de jobs antiguos completada",  # noqa: F541
    ...
    def get_all_jobs(self, status_filter: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        with self._operation_lock:
            ...
            self.logger.debug(
                f"Consulta de todos los jobs",  # noqa: F541
    ...
    def set_retention_minutes(self, minutes: int):
        ...
        self.logger.info(
            f"Tiempo de retención actualizado",  # noqa: F541
