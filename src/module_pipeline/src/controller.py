...
from .models.entrada import (  # noqa: F401
...
from .services.job_tracker_service import JobStatus, get_job_tracker_service
...
from .utils.fragment_processor import FragmentProcessor  # noqa: F401
...
class PipelineController:
    ...
    async def process_article(self, articulo_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        campos_faltantes = [
            campo for campo in campos_requeridos if campo not in articulo_data
        ]
        if campos_faltantes:
            # Usar ValidationError personalizada en lugar de ValueError genérico
            from .utils.error_handling import ErrorPhase
            from .utils.error_handling import ValidationError as CustomValidationError

            raise CustomValidationError(
                message=f"Campos requeridos faltantes en articulo_data",  # noqa: F541
                validation_errors=[
        ...
        articulo_id = articulo_data.get(
            "url",
            f"articulo_{articulo_data['medio']}_{articulo_data['fecha_publicacion']}",
        )
        contenido = articulo_data["contenido_texto"]

        article_logger.debug(
            f"Artículo recibido",  # noqa: F541
        ...
        if articulo_data.get("articulo_id"):
            ...
            article_logger.warning(
                f"No se encontró articulo_id en datos, usando UUID: {id_articulo}"
            )
        ...
        if not titular:
            raise ValueError(
                "No se encontró campo 'titular' o 'titulo' en los datos del artículo"
            )
        ...
        articulo_data_procesable = {
            ...
            "id_articulo_fuente": articulo_data.get(
                "articulo_id"
            ),  # Mapear articulo_id a id_articulo_fuente
            "contenido_texto": contenido,
            "medio": articulo_data["medio"],
            "titular": titular,  # Usar el campo correcto según Supabase
            "area_geografica": articulo_data.get(
                "area_geografica", articulo_data.get("pais", "España")
            ),
        ...
        try:
            ...
            article_logger.debug(
                f"Artículo validado correctamente: ID={articulo.id_articulo}"
            )
        ...
        resultado = self._adaptar_resultado_7_fases(
            resultado_pipeline, articulo_data_procesable
        )
        ...
        resultado["metricas_contenido"] = {
            ...
            "tiempo_por_caracter_ms": (tiempo_procesamiento * 1000) / len(contenido)
            if len(contenido) > 0
            else 0,
        }
        ...
        if "advertencias" in resultado and len(resultado["advertencias"]) > 0:
            article_logger.warning(
                f"Artículo procesado con advertencias",  # noqa: F541
        ...
        if payload:
            ...
            # Si es dict, usar get()
                ...
                resumen = payload.get(
                    "resumen_generado_fragmento", "Procesado con 7 fases"
                )
            else:
        ...
        resultado["campos_generados"] = {
            ...
            "simplificacion_aplicada": resultado.get("flujo_adaptativo", {}).get(
        ...
        if resultado_pipeline.get("payload"):
            resultado["persistencia"] = self._persistir_resultado_7_fases(
                resultado_pipeline, articulo, article_logger
            )

        # Log final con estado de procesamiento
        if resultado.get("procesamiento_parcial", False):
            article_logger.info(
                f"Procesamiento de artículo completado CON ADVERTENCIAS",  # noqa: F541
                tiempo_segundos=tiempo_procesamiento,
                advertencias_count=len(resultado.get("advertencias", [])),
                persistencia_exitosa=resultado.get("persistencia", {}).get(
                    "exitosa", False
                ),
            )
        else:
            article_logger.info(
                f"Procesamiento de artículo completado exitosamente",  # noqa: F541
                tiempo_segundos=tiempo_procesamiento,
                persistencia_exitosa=resultado.get("persistencia", {}).get(
                    "exitosa", False
                ),
                elementos_procesados={
                    "hechos": resultado.get("metricas", {})
                    .get("conteos_elementos", {})
                    .get("hechos_extraidos", 0),
                    "entidades": resultado.get("metricas", {})
                    .get("conteos_elementos", {})
                    .get("entidades_extraidas", 0),
                    "fases_ejecutadas": resultado.get("fase_completada", 0),
    ...
    def _adaptar_resultado_7_fases(
        self, resultado_pipeline: Dict[str, Any], fragmento_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        if "fase_7" in resultados_fases:
            ...
            if hasattr(fase7, "metadata_normalizacion"):
                ...
                metricas["conteos_elementos"]["relaciones_detectadas"] = sum(
                    stats.values()
                )
    ...
    async def process_fragment(self, fragmento_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        from .models.entrada import FragmentoProcesableItem  # noqa: F811
        ...
        try:
            ...
            resultado = self._adaptar_resultado_7_fases(
                resultado_pipeline, fragmento_data
            )
            ...
            if resultado_pipeline.get("payload"):
                resultado["persistencia"] = self._persistir_resultado_7_fases(
                    resultado_pipeline, fragmento, fragment_logger
                )
            ...
            return {
                ...
                "error_type": type(e).__name__,
                "timestamp": datetime.utcnow().isoformat(),
                "metricas": {"tiempo_total_segundos": tiempo_total},
            }
    ...
    def _persistir_resultado_7_fases(
        ...
        try:
            ...
            payload = resultado_pipeline.get("payload")
            if not payload:
                return {"exitosa": False, "mensaje": "No hay payload para persistir"}
            ...
            if resultado_pipeline.get("metadatos", {}).get("es_articulo_completo"):
                ...
                hasattr(fragmento, "metadata_adicional")
                and fragmento.metadata_adicional
            ):
                # Fallback para compatibilidad con código legacy
                es_articulo = fragmento.metadata_adicional.get(
                    "es_articulo_completo", False
                )
                if es_articulo:
                    logger.info(
                        "Detectado artículo completo desde metadata_adicional (legacy)"
                    )
            else:
            ...
            if es_articulo:
                ...
                # Llamar al nuevo RPC
                resultado_persistencia = supabase_service.actualizar_articulo_procesado(
                    payload_dict
                )
                ...
                resultado_persistencia = supabase_service.insertar_fragmento_completo(
                    payload_dict
                )
    ...
    async def _process_fragment_4_phases(
        self, fragmento_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        with log_phase(
            "Fase1_Triaje", request_id, fragment_id=str(fragment_uuid)
        ) as fase1_logger:  # noqa: F821
            resultado_fase1 = None
            try:
                resultado_fase1 = ejecutar_fase_1(  # noqa: F821
                    id_fragmento_original=fragment_uuid,  # noqa: F821
                    texto_original_fragmento=fragmento.texto_original,  # noqa: F821
                )
                tiempo_fase1 = time.time() - tiempo_inicio_fase1
                metricas_pipeline["tiempos_fases"]["fase1"] = tiempo_fase1  # noqa: F821
                metricas_pipeline["tasas_exito"]["fase1"] = True  # noqa: F821

                fase1_logger.info(
                    f"Fase 1 completada exitosamente",  # noqa: F541
                    relevante=resultado_fase1.es_relevante,
                    tiempo_segundos=tiempo_fase1,
                    longitud_texto=len(fragmento.texto_original),  # noqa: F821
                )
            except Exception as e:
                tiempo_fase1 = time.time() - tiempo_inicio_fase1
                metricas_pipeline["tiempos_fases"]["fase1"] = tiempo_fase1  # noqa: F821
                metricas_pipeline["tasas_exito"]["fase1"] = False  # noqa: F821

                fase1_logger.error(
                    f"Error en Fase 1: {str(e)}", tiempo_segundos=tiempo_fase1
                )
                # Usar fallback - aceptar artículo según política
                from .models.procesamiento import ResultadoFase1Triaje

                fallback_data = handle_spacy_load_error_fase1(  # noqa: F821
                    article_id=str(fragment_uuid),  # noqa: F821
                    model_name=get_spacy_model_name(),
                    exception=e,
                )
                # Usar el texto original como texto para siguiente fase
                fallback_data["texto_para_siguiente_fase"] = fragmento.texto_original  # noqa: F821
                resultado_fase1 = ResultadoFase1Triaje(**fallback_data)
                advertencias.append(f"Fase 1 ejecutada con fallback: {str(e)}")  # noqa: F821
                fase1_logger.warning(
                    f"Fase 1 usando fallback para fragmento {fragment_uuid}"
                )  # noqa: F821

        # --- FASE 2: Extracción ---
        tiempo_inicio_fase2 = time.time()
        with log_phase(
            "Fase2_Extraccion", request_id, fragment_id=str(fragment_uuid)
        ) as fase2_logger:  # noqa: F821
            ...
            try:
                resultado_fase2 = ejecutar_fase_2(
                    ...
                    processor=processor,  # Pasar el processor para IDs coherentes  # noqa: F821
                )
                tiempo_fase2 = time.time() - tiempo_inicio_fase2
                metricas_pipeline["tiempos_fases"]["fase2"] = tiempo_fase2  # noqa: F821
                metricas_pipeline["tasas_exito"]["fase2"] = True  # noqa: F821
                metricas_pipeline["conteos_elementos"]["hechos_extraidos"] = len(
                    resultado_fase2.hechos_extraidos
                )  # noqa: F821
                metricas_pipeline["conteos_elementos"]["entidades_extraidas"] = len(
                    resultado_fase2.entidades_extraidas
                )  # noqa: F821

                fase2_logger.info(
                    f"Fase 2 completada exitosamente",  # noqa: F541
                ...
                tiempo_fase2 = time.time() - tiempo_inicio_fase2
                metricas_pipeline["tiempos_fases"]["fase2"] = tiempo_fase2  # noqa: F821
                metricas_pipeline["tasas_exito"]["fase2"] = False  # noqa: F821

                fase2_logger.error(
                    f"Error en Fase 2: {str(e)}", tiempo_segundos=tiempo_fase2
                )
                # Usar fallback - crear hecho básico del título
                fallback_data = handle_groq_extraction_error_fase2(
                    article_id=str(fragment_uuid),  # noqa: F821
                    titulo=fragmento.metadata_adicional.get("titular", "Sin título"),  # noqa: F821
                    medio=fragmento.metadata_adicional.get("medio", "Desconocido"),  # noqa: F821
                    exception=e,
                )
                ...
                hechos_fallback = [
                    HechoProcesado(  # noqa: F821
                        id_hecho=h["id_hecho"],
                        id_fragmento_origen=fragment_uuid,  # Campo requerido  # noqa: F821
                ...
                resultado_fase2 = ResultadoFase2Extraccion(  # noqa: F821
                    id_fragmento=fragment_uuid,  # noqa: F821
                ...
                advertencias.extend(fallback_data.get("advertencias", []))  # noqa: F821
                advertencias.append(f"Fase 2 ejecutada con fallback: {str(e)}")  # noqa: F821
                fase2_logger.warning(
                    f"Fase 2 usando fallback para fragmento {fragment_uuid}"
                )  # noqa: F821

        # --- FASE 3: Citas y Datos ---
        tiempo_inicio_fase3 = time.time()
        with log_phase(
            "Fase3_CitasDatos", request_id, fragment_id=str(fragment_uuid)
        ) as fase3_logger:  # noqa: F821
            ...
            try:
                ...
                metricas_pipeline["tiempos_fases"]["fase3"] = tiempo_fase3  # noqa: F821
                metricas_pipeline["tasas_exito"]["fase3"] = True  # noqa: F821
                metricas_pipeline["conteos_elementos"]["citas_extraidas"] = len(
                    resultado_fase3.citas_textuales_extraidas
                )  # noqa: F821
                metricas_pipeline["conteos_elementos"]["datos_cuantitativos"] = len(
                    resultado_fase3.datos_cuantitativos_extraidos
                )  # noqa: F821

                fase3_logger.info(
                    f"Fase 3 completada exitosamente",  # noqa: F541
                    citas_extraidas=len(resultado_fase3.citas_textuales_extraidas),
                    datos_cuantitativos=len(
                        resultado_fase3.datos_cuantitativos_extraidos
                    ),
                ...
                tiempo_fase3 = time.time() - tiempo_inicio_fase3
                metricas_pipeline["tiempos_fases"]["fase3"] = tiempo_fase3  # noqa: F821
                metricas_pipeline["tasas_exito"]["fase3"] = False  # noqa: F821

                fase3_logger.error(
                    f"Error en Fase 3: {str(e)}", tiempo_segundos=tiempo_fase3
                )
                # Usar fallback - continuar sin citas ni datos (no es crítico)
                fallback_data = handle_groq_citas_error_fase3(  # noqa: F821
                    article_id=str(fragment_uuid),  # noqa: F821
                    exception=e,
                )

                resultado_fase3 = ResultadoFase3CitasDatos(  # noqa: F821
                    id_fragmento=fragment_uuid,  # noqa: F821
                ...
                # Actualizar métricas con valores de fallback
                metricas_pipeline["conteos_elementos"]["citas_extraidas"] = 0  # noqa: F821
                metricas_pipeline["conteos_elementos"]["datos_cuantitativos"] = 0  # noqa: F821

                advertencias.extend(fallback_data.get("advertencias_citas_datos", []))  # noqa: F821
                advertencias.append(f"Fase 3 ejecutada con fallback: {str(e)}")  # noqa: F821
                fase3_logger.warning(
                    f"Fase 3 usando fallback para fragmento {fragment_uuid}"
                )  # noqa: F821

        # --- FASE 4: Normalización ---
        tiempo_inicio_fase4 = time.time()
        with log_phase(
            "Fase4_Normalizacion", request_id, fragment_id=str(fragment_uuid)
        ) as fase4_logger:  # noqa: F821
            ...
            try:
                resultado_fase4 = ejecutar_fase_4(
                    processor=processor,  # Pasar el processor primero como indica la firma  # noqa: F821
                    resultado_fase_1=resultado_fase1,
                    resultado_fase_2=resultado_fase2,
                ...
                metricas_pipeline["tiempos_fases"]["fase4"] = tiempo_fase4  # noqa: F821
                metricas_pipeline["tasas_exito"]["fase4"] = True  # noqa: F821

                # Contar elementos normalizados y relaciones
                relaciones_metadata = resultado_fase4.metadata_normalizacion.get(
                    "relaciones", {}
                )
                metricas_pipeline["conteos_elementos"]["entidades_normalizadas"] = len(
                    resultado_fase4.entidades_normalizadas
                )  # noqa: F821
                metricas_pipeline["conteos_elementos"]["relaciones_hecho_hecho"] = len(
                    relaciones_metadata.get("hecho_hecho", [])
                )  # noqa: F821
                metricas_pipeline["conteos_elementos"]["relaciones_entidad_entidad"] = (
                    len(relaciones_metadata.get("entidad_entidad", []))
                )  # noqa: F821
                metricas_pipeline["conteos_elementos"]["contradicciones"] = len(
                    relaciones_metadata.get("contradicciones", [])
                )  # noqa: F821

                fase4_logger.info(
                    f"Fase 4 completada exitosamente",  # noqa: F541
                    estado=resultado_fase4.estado_general_normalizacion,
                    entidades_normalizadas=len(resultado_fase4.entidades_normalizadas),
                    relaciones_detectadas=len(
                        relaciones_metadata.get("hecho_hecho", [])
                    )
                    + len(relaciones_metadata.get("entidad_entidad", [])),
                ...
                tiempo_fase4 = time.time() - tiempo_inicio_fase4
                metricas_pipeline["tiempos_fases"]["fase4"] = tiempo_fase4  # noqa: F821
                metricas_pipeline["tasas_exito"]["fase4"] = False  # noqa: F821

                fase4_logger.error(
                    f"Error en Fase 4: {str(e)}", tiempo_segundos=tiempo_fase4
                )
                ...
                for entidad in resultado_fase2.entidades_extraidas:
                    entidad_copia = EntidadProcesada(  # noqa: F821
                ...
                relaciones_fallback = handle_groq_relations_error_fase4(  # noqa: F821
                    article_id=str(fragment_uuid),  # noqa: F821
                    exception=e,
                )

                resultado_fase4 = ResultadoFase4Normalizacion(  # noqa: F821
                    id_fragmento=fragment_uuid,  # noqa: F821
                ...
                metricas_pipeline["conteos_elementos"]["relaciones_hecho_hecho"] = 0  # noqa: F821
                metricas_pipeline["conteos_elementos"]["relaciones_entidad_entidad"] = 0  # noqa: F821
                metricas_pipeline["conteos_elementos"]["contradicciones"] = 0  # noqa: F821

                advertencias.append(f"Fase 4 ejecutada con fallback: {str(e)}")  # noqa: F821
                fase4_logger.warning(
                    f"Fase 4 usando fallback para fragmento {fragment_uuid}"
                )  # noqa: F821

        # Calcular tiempo total de procesamiento
        tiempo_total_pipeline = time.time() - tiempo_inicio_total
        metricas_pipeline["tiempo_total_segundos"] = tiempo_total_pipeline  # noqa: F821

        # Calcular tasa de éxito general
        fases_exitosas = sum(
            1 for exito in metricas_pipeline["tasas_exito"].values() if exito
        )  # noqa: F821
        total_fases = len(metricas_pipeline["tasas_exito"])  # noqa: F821
        metricas_pipeline["tasa_exito_general"] = (
            fases_exitosas / total_fases if total_fases > 0 else 0.0
        )  # noqa: F821
        ...
        with self._metrics_lock:
            ...
            if len(advertencias) > 0:  # noqa: F821
                self.metrics["errores_totales"] += len(advertencias)  # noqa: F821

        # Retornar diccionario simple con todos los resultados
        resultado_pipeline = {
            "fragmento_id": fragmento.id_fragmento,  # noqa: F821
            "fragmento_uuid": str(fragment_uuid),  # noqa: F821
            "request_id": request_id,
            "fase_1_triaje": resultado_fase1.model_dump() if resultado_fase1 else {},
            "fase_2_extraccion": resultado_fase2.model_dump()
            if resultado_fase2
            else {},
            "fase_3_citas_datos": resultado_fase3.model_dump()
            if resultado_fase3
            else {},
            "fase_4_normalizacion": resultado_fase4.model_dump()
            if resultado_fase4
            else {},
            "procesamiento_exitoso": True,  # True aunque haya usado fallbacks
            "procesamiento_parcial": len(advertencias)
            > 0,  # Indica si hubo fallbacks  # noqa: F821
            "advertencias": advertencias,  # Lista de advertencias acumuladas  # noqa: F821
            "timestamp": resultado_fase1.fecha_creacion.isoformat()
            if resultado_fase1
            else datetime.utcnow().isoformat(),
            "processor_stats": processor.get_stats(),  # Añadir estadísticas del processor  # noqa: F821
            "metricas": metricas_pipeline,  # Añadir métricas del pipeline  # noqa: F821
        }

        # Log de resumen del processor
        processor.log_summary()  # noqa: F821

        # Log final con métricas completas
        fragment_logger.info(
            "Pipeline de fragmento completado",
            tiempo_total_segundos=tiempo_total_pipeline,
            tasa_exito_general=metricas_pipeline["tasa_exito_general"],  # noqa: F821
            elementos_procesados={
                "hechos": metricas_pipeline["conteos_elementos"].get(
                    "hechos_extraidos", 0
                ),  # noqa: F821
                "entidades": metricas_pipeline["conteos_elementos"].get(
                    "entidades_extraidas", 0
                ),  # noqa: F821
                "citas": metricas_pipeline["conteos_elementos"].get(
                    "citas_extraidas", 0
                ),  # noqa: F821
                "datos_cuantitativos": metricas_pipeline["conteos_elementos"].get(
                    "datos_cuantitativos", 0
                ),  # noqa: F821
                "relaciones": metricas_pipeline["conteos_elementos"].get(
                    "relaciones_hecho_hecho", 0
                )  # noqa: F821
                + metricas_pipeline["conteos_elementos"].get(
                    "relaciones_entidad_entidad", 0
                ),  # noqa: F821
            },
            advertencias_count=len(advertencias),  # noqa: F821
        )
        ...
        if not resultado_fase2 or len(resultado_fase2.hechos_extraidos) == 0:
            fragment_logger.warning(
                "No hay datos suficientes para persistir. Omitiendo persistencia."
            )
        ...
        try:
            ...
            metadatos_fragmento = {
                ...
                ),  # E012 FIX: Añadir id_fragmento faltante  # noqa: F821
                "indice_secuencial_fragmento": fragmento.orden_en_articulo or 0,  # noqa: F821
                "titulo_seccion_fragmento": fragmento.metadata_adicional.get(
                    "titulo_seccion"
                ),  # noqa: F821
                "contenido_texto_original_fragmento": fragmento.texto_original,  # noqa: F821
                "num_pagina_inicio_fragmento": fragmento.metadata_adicional.get(
                    "pagina_inicio"
                ),  # noqa: F821
                "num_pagina_fin_fragmento": fragmento.metadata_adicional.get(
                    "pagina_fin"
                ),  # noqa: F821
            }
            ...
            estado_final = (
                "completado_ok"
                if resultado_fase1 and resultado_fase1.es_relevante
                else "completado_no_relevante"
            )
                ...
                hechos_extraidos_data = [
                    {
                        ...
                        "metadata_hecho": hecho.metadata_hecho.model_dump()
                        if hecho.metadata_hecho
                        else {},
                    }
                ...
                entidades_autonomas_data = [
                    {
                        ...
                        "id_temporal": str(
                            entidad.id_entidad
                        ),  # IMPORTANTE: Requerido por la función SQL
                        "nombre": entidad.nombre_entidad_normalizada or entidad.nombre,
                        "tipo": entidad.tipo,
                        "relevancia": entidad.relevancia,
                        "metadata_entidad": entidad.metadata_entidad.model_dump()
                        if entidad.metadata_entidad
                        else {},
                        "id_entidad_normalizada": str(entidad.id_entidad_normalizada)
                        if entidad.id_entidad_normalizada
                        else None,
            ...
            if resultado_fase3 and hasattr(
                resultado_fase3, "citas_textuales_extraidas"
            ):
                citas_textuales_data = [
                    {
                        ...
                        "metadata_cita": cita.metadata_cita.model_dump()
                        if cita.metadata_cita
                        else {},
                    }
                ...
                datos_cuantitativos_data = [
                    {
                        ...
                        "metadata_dato": dato.metadata_dato.model_dump()
                        if dato.metadata_dato
                        else {},
                    }
            ...
            if resultado_pipeline and "resultados_fases" in resultado_pipeline:
                ...
                if fase7 and hasattr(fase7, "metadata_normalizacion"):
                    relaciones_completas = fase7.metadata_normalizacion.get(
                        "relaciones_completas", {}
                    )
                    relaciones_estructurales = relaciones_completas.get(
                        "relaciones_estructurales", {}
                    )
                    relaciones_temporales = relaciones_completas.get(
                        "relaciones_temporales", {}
                    )

                    # Mapear al formato esperado
                    relaciones_metadata = {
                        "hecho_entidad": relaciones_estructurales.get(
                            "hecho_entidad", []
                        ),
                        "hecho_hecho": relaciones_temporales.get(
                            "hecho_relacionado", []
                        ),
                        "entidad_entidad": relaciones_estructurales.get(
                            "entidad_relacion", []
                        ),
                        "contradicciones": relaciones_temporales.get(
                            "contradicciones", []
                        ),
                    }
            ...
            for cont in relaciones_metadata.get("contradicciones", []):
                contradiccion_mapeada = {
                    ...
                    "hecho_contradictorio_id_temporal": str(
                        cont.get("hecho_contradictorio_id")
                    ),
            ...
            fragment_logger.info(
                f"Relaciones a persistir",  # noqa: F541
            ...
            if resultado_persistencia:
                ...
                fragment_logger.info(
                    f"Fragmento persistido exitosamente",  # noqa: F541
                    fragmento_id=resultado_persistencia.get("fragmento_id"),
                ...
                resultado_pipeline["persistencia"] = {
                    "exitosa": True,
                    "fragmento_id": resultado_persistencia.get("fragmento_id"),
                    "hechos_insertados": resultado_persistencia.get(
                        "hechos_insertados", 0
                    ),
                    "entidades_insertadas": resultado_persistencia.get(
                        "entidades_insertadas", 0
                    ),
                    "citas_insertadas": resultado_persistencia.get(
                        "citas_insertadas", 0
                    ),
                    "datos_insertados": resultado_persistencia.get(
                        "datos_insertados", 0
                    ),
                ...
                if (
                    relaciones_hechos_data
                    or relaciones_entidades_data
                    or contradicciones_detectadas_data
                ):
                    fragment_logger.info(
                        f"Relaciones persistidas exitosamente",  # noqa: F541
                ...
                tiempo_persistencia = time.time() - tiempo_inicio_persistencia
                fragment_logger.warning(
                    "RPC insertar_fragmento_completo no retornó datos"
                )
                resultado_pipeline["persistencia"] = {
                    "exitosa": False,
            ...
            advertencias.append(f"Error en persistencia: {str(e)}")  # noqa: F821
            resultado_pipeline["advertencias"] = advertencias  # noqa: F821
    ...
    async def _process_article_background(
        self, articulo_data: Dict[str, Any], job_id: str
    ) -> None:
        """
        ...
        try:
            ...
            job_tracker.update_status(
                ...
                metadata_update={
                    ...
                    "processing_time_seconds": resultado.get(
                        "tiempo_procesamiento_articulo", 0
                    ),
                    "elementos_procesados": {
                        "hechos": resultado.get("metricas", {})
                        .get("conteos_elementos", {})
            ...
            bg_logger.info(
                ...
                elementos_procesados=resultado.get("metricas", {}).get(
                    "conteos_elementos", {}
                ),
            )
    ...
    async def _process_fragment_background(
        self, fragmento_data: Dict[str, Any], job_id: str
    ) -> None:
        """
            ...
            job_tracker.update_status(
                ...
                metadata_update={
                    ...
                    "elementos_procesados": {
                        ...
                        .get("conteos_elementos", {})
                        .get("entidades_extraidas", 0),
                        "citas": resultado.get("metricas", {})
                        .get("conteos_elementos", {})
                        .get("citas_extraidas", 0),
                        "datos_cuantitativos": resultado.get("metricas", {})
                        .get("conteos_elementos", {})
                        .get("datos_cuantitativos", 0),
                    },
                    "persistencia_exitosa": resultado.get("persistencia", {}).get(
                        "exitosa", False
                    ),
                    "procesamiento_parcial": resultado.get(
                        "procesamiento_parcial", False
                    ),
                },
            )
