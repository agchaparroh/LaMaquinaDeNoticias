...
# Importar modelos
from ..models.metadatos import MetadatosHecho  # noqa: E402
from ..models.procesamiento import HechoProcesado  # noqa: E402
...
_PROMPT_HECHOS_PATH = (
    Path(__file__).resolve().parent.parent.parent / "prompts" / "Hechos.md"
)
...
def _cargar_prompt_hechos() -> str:
    ...
    if _PROMPT_HECHOS_TEMPLATE is None:
        ...
        with open(_PROMPT_HECHOS_PATH, encoding="utf-8") as f:
...
def ejecutar_fase_4_hechos(
    ...
    """
    logger.info(
        f"Iniciando Fase 4: Extracción de Hechos para fragmento {resultado_simplificacion.id_fragmento}"
    )
    ...
    try:
        ...
        if len(resultado_simplificacion.texto_simplificado) > 10000:
            ...
            logger.info(f"Usando modelo grande para extracción de hechos")  # noqa: F541
        ...
        hechos_procesados = _procesar_hechos_extraidos(
            hechos_raw, resultado_simplificacion.id_fragmento, fragment_processor
        )
        ...
        resultado = {
            ...
            "hechos_futuros": sum(
                1 for h in hechos_procesados if h.metadata_hecho.es_futuro
            ),
        }
...
async def _procesar_chunk_hechos_async(
    ...
    try:
        ...
        # Procesar hechos
        hechos_procesados = _procesar_hechos_extraidos(
            hechos_chunk, resultado_simplificacion.id_fragmento, fragment_processor
        )

        logger.debug(
            f"Chunk {chunk_index + 1}: {len(hechos_procesados)} hechos extraídos"
        )
...
def extraer_hechos_con_chunking(
    ...
    return {
        ...
        "tipos_hechos": _contar_tipos_hechos(todos_los_hechos),
        "hechos_futuros": sum(
            1 for h in todos_los_hechos if h.metadata_hecho.es_futuro
        ),
        "requiere_consolidacion": True,
    }
