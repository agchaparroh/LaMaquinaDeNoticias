...
# Importar modelos
from ..models.procesamiento import ResultadoFase1Triaje  # noqa: E402
from ..models.simplificacion import (  # noqa: E402
...
from ..utils.json_parser import parse_llm_json_response  # noqa: E402, F401
...
_PROMPT_SIMPLIFICACION_PATH = (
    Path(__file__).resolve().parent.parent.parent / "prompts" / "Simplificación.md"
)
...
def _cargar_prompt_simplificacion() -> str:
    ...
    if _PROMPT_SIMPLIFICACION_TEMPLATE is None:
        ...
        with open(_PROMPT_SIMPLIFICACION_PATH, encoding="utf-8") as f:
            _PROMPT_SIMPLIFICACION_TEMPLATE = f.read()

        logger.info(
            f"Prompt de simplificación cargado desde: {_PROMPT_SIMPLIFICACION_PATH}"
        )
...
def _calcular_fechas_relativas(fecha_articulo: Optional[str]) -> Dict[str, str]:
    ...
    try:
        ...
        return {
            ...
            "{{FECHA_INICIO_SEMANA_PASADA}}": (
                fecha - timedelta(days=fecha.weekday() + 7)
            ).strftime("%Y-%m-%d"),
            "{{FECHA_FIN_SEMANA_PASADA}}": (
                fecha - timedelta(days=fecha.weekday() + 1)
            ).strftime("%Y-%m-%d"),
        }
...
def _preparar_prompt_simplificacion(
    texto: str, fecha_articulo: Optional[str] = None
) -> str:
    """
...
def _analizar_transformaciones(
    texto_original: str, texto_simplificado: str
) -> Dict[str, int]:
    """
    ...
    siglas_expandidas = len(
        [s for s in siglas_original if f"({s})" in texto_simplificado]
    )
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            if attempt < max_retries - 1:
                ...
                continue
            raise GroqAPIError(
                f"Error al simplificar texto después de {max_retries} intentos: {str(e)}"
            )
...
def ejecutar_fase_2_simplificacion(
    ...
    """
    logger.info(
        f"Iniciando Fase 2: Simplificación para fragmento {resultado_triaje.id_fragmento}"
    )
    ...
    try:
        ...
        if (
            len(texto_original) > 10000
        ):  # Umbral para modelo grande - casos excepcionales
            modelo = "llama3-70b-8192"
            logger.info(
                f"Usando modelo grande debido a longitud del texto: {len(texto_original)} chars"
            )
        ...
        transformaciones = _analizar_transformaciones(
            texto_original, texto_simplificado
        )

        # Calcular reducción de complejidad (aproximación basada en longitud)
        reduccion = (
            (len(texto_original) - len(texto_simplificado)) / len(texto_original)
        ) * 100
        ...
        logger.info(
            f"DEBUG TRUNCATION: tokens_prompt={metadatos_llamada.get('tokens_prompt')}, "
        ...
        return ResultadoFase2Simplificacion(
            id_resultado_simplificacion=str(uuid4()),
            id_fragmento=resultado_triaje.id_fragmento,
            texto_simplificado=resultado_triaje.texto_para_siguiente_fase
            or "",  # Fallback al original
...
def simplificar_con_chunking(
    ...
    """
    logger.info(
        f"Simplificando {len(chunks)} chunks para fragmento {resultado_triaje.id_fragmento}"
    )
    ...
    transformaciones_totales = _analizar_transformaciones(
        texto_original_completo, texto_simplificado_completo
    )
