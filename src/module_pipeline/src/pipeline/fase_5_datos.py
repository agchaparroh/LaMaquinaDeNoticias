...
# Importar modelos
from ..models.metadatos import MetadatosDato  # noqa: E402
from ..models.procesamiento import (  # noqa: E402
    DatosCuantitativos,
    EntidadProcesada,
    HechoProcesado,
)
from ..models.simplificacion import ResultadoFase2Simplificacion  # noqa: E402
...
from ..utils.fragment_processor import FragmentProcessor  # noqa: E402
from ..utils.json_parser import parse_llm_json_response  # noqa: E402
from ..utils.validation import escape_html  # noqa: E402, F401
...
_PROMPT_DATOS_PATH = (
    Path(__file__).resolve().parent.parent.parent / "prompts" / "Datos.md"
)
...
def _cargar_prompt_datos() -> str:
    ...
    if _PROMPT_DATOS_TEMPLATE is None:
        ...
        with open(_PROMPT_DATOS_PATH, encoding="utf-8") as f:
...
def _preparar_contexto_referencias(
    ...
    for entidad in entidades:
        entidades_json.append(
            {"id": entidad.id_entidad, "nombre": entidad.nombre, "tipo": entidad.tipo}
        )

    return json.dumps(hechos_json, ensure_ascii=False, indent=2), json.dumps(
        entidades_json, ensure_ascii=False, indent=2
    )
...
async def _llamar_groq_datos_async(
    client: Any, prompt: str, modelo: str = "llama-3.1-8b-instant", max_retries: int = 3
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            logger.error(
                f"Error en llamada a Groq (intento {attempt + 1}/{max_retries}): {str(e)}"
            )
            if attempt < max_retries - 1:
                continue
            raise GroqAPIError(
                f"Error al extraer datos después de {max_retries} intentos: {str(e)}"
            )
...
def _procesar_datos_extraidos(
    ...
    for dato in datos_raw:
        try:
            # Extraer periodo
            periodo_raw = dato.get("periodo", {})
            fecha_inicio = periodo_raw.get("inicio")
            fecha_fin = periodo_raw.get("fin")

            # Crear objeto PeriodoReferencia si hay fechas
            periodo_obj = None
            if fecha_inicio or fecha_fin:
                from ..models.metadatos import PeriodoReferencia

                periodo_obj = PeriodoReferencia(  # noqa: F841
...
def ejecutar_fase_5_datos(
    ...
    """
    logger.info(
        f"Iniciando Fase 5: Extracción de Datos para fragmento {resultado_simplificacion.id_fragmento}"
    )
    ...
    try:
        ...
        hechos_json, entidades_json = _preparar_contexto_referencias(
            hechos_extraidos, entidades_extraidas
        )
        ...
        datos_procesados = _procesar_datos_extraidos(
            datos_raw, resultado_simplificacion.id_fragmento, fragment_processor
        )
...
async def _procesar_chunk_datos_async(
    ...
    try:
            ...
            [
                {
                    ...
                    "contenido": hecho.contenido,
                    "tipo": hecho.metadata_hecho.tipo_hecho
                    if hasattr(hecho, "metadata_hecho") and hecho.metadata_hecho
                    else "DESCONOCIDO",
                }
        ...
        prompt = _preparar_prompt_datos(
            ...
            fecha_publicacion=contexto_articulo.get(
        ...
        logger.debug(
            f"Chunk {chunk_index + 1}: {len(datos_procesados)} datos extraídos"
        )
...
async def extraer_datos_con_chunking_paralelo(
    ...
    for i in range(0, len(chunks), max_concurrent_chunks):
        ...
        logger.info(
            f"Procesando lote {metadatos_agregados['parallel_batches'] + 1}: chunks {i + 1}-{i + len(batch_chunks)}"
        )
        ...
        for j, chunk_text in enumerate(batch_chunks):
            ...
            hechos_chunk = (
                hechos_por_chunk[chunk_idx] if chunk_idx < len(hechos_por_chunk) else []
            )
            entidades_chunk = (
                entidades_por_chunk[chunk_idx]
                if chunk_idx < len(entidades_por_chunk)
                else []
            )
        ...
        try:
            ...
            for k, resultado in enumerate(resultados_lote):
                ...
                if metadatos.get("tokens_prompt"):
                    metadatos_agregados["tokens_prompt_total"] += metadatos[
                        "tokens_prompt"
                    ]
                if metadatos.get("tokens_respuesta"):
                    metadatos_agregados["tokens_respuesta_total"] += metadatos[
                        "tokens_respuesta"
                    ]
                metadatos_agregados["duracion_total_ms"] += metadatos.get(
                    "duracion_llamada_ms", 0
                )
...
def extraer_datos_con_chunking(
    ...
    for i, chunk in enumerate(chunks):
        ...
        try:
            ...
            hechos_json, entidades_json = _preparar_contexto_referencias(
                hechos_por_chunk[i], entidades_por_chunk[i]
            )
            ...
            # Procesar datos
            datos_procesados = _procesar_datos_extraidos(
                datos_chunk, resultado_simplificacion.id_fragmento, fragment_processor
            )
