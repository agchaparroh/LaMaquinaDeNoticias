...
# Importar modelos
from ..models.metadatos import MetadatosCita  # noqa: E402
...
_PROMPT_CITAS_PATH = (
    Path(__file__).resolve().parent.parent.parent / "prompts" / "Citas.md"
)
...
def _cargar_prompt_citas() -> str:
    ...
    if _PROMPT_CITAS_TEMPLATE is None:
        ...
        with open(_PROMPT_CITAS_PATH, encoding="utf-8") as f:
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            if attempt < max_retries - 1:
                continue
            raise GroqAPIError(
                f"Error al extraer citas después de {max_retries} intentos: {str(e)}"
            )
    ...
    for cita in citas_raw:
        try:
            ...
            cita_procesada = CitaTextual(
                ...
                relevancia=cita.get(
                    "relevancia", 3
                ),  # Default relevancia 3 si no viene
...
def ejecutar_fase_6_citas(
    ...
    """
    logger.info(
        f"Iniciando Fase 6: Extracción de Citas para fragmento {resultado_simplificacion.id_fragmento}"
    )

    # Verificar si debe ejecutarse
    conteo_citas = contexto_articulo.get("conteo_citas", 0) if contexto_articulo else 0
    es_entrevista = (
        contexto_articulo.get("es_entrevista", False) if contexto_articulo else False
    )
    ...
    try:
        ...
        citas_procesadas = _procesar_citas_extraidas(
            citas_raw, resultado_simplificacion.id_fragmento, fragment_processor
        )
...
def _vincular_nombres_entidades(
    citas: List[CitaTextual], entidades: List[EntidadProcesada]
) -> None:
    """
...
async def _procesar_chunk_citas_async(
    ...
    try:
        ...
        # Procesar citas
        citas_procesadas = _procesar_citas_extraidas(
            citas_chunk, resultado_simplificacion.id_fragmento, fragment_processor
        )

        logger.debug(
            f"Chunk {chunk_index + 1}: {len(citas_procesadas)} citas extraídas"
        )
