...
# Importar modelos
from ..models.entrada import ArticuloProcesableItem, FragmentoProcesableItem
...
from ..utils.logging_config import LogContext, get_logger, log_phase
...
class PipelineCoordinator:
    ...
    def ejecutar_pipeline_completo(
        ...
        if isinstance(contenido, ArticuloProcesableItem):
            ...
            if contenido.id_fragmento.startswith("ART-"):
                ...
                self.base_logger.info(
                    f"Procesando fragmento con ID de trazabilidad: {fragmento_id_str}"
                )
            else:
                ...
                try:
                    ...
                    fragmento_id_str = contenido.id_fragmento
                    self.base_logger.info(
                        f"Procesando fragmento con UUID legacy: {fragmento_id_str}"
                    )
                except ValueError:
                    self.base_logger.error(
                        f"ID de fragmento inválido: {contenido.id_fragmento}"
                    )
                    raise ValueError(
                        f"El ID del fragmento no es válido: {contenido.id_fragmento}"
                    )
            ...
            raise ValueError(
                f"Tipo de contenido no soportado: {type(contenido)}. "
        ...
        logger.info(
            f"Iniciando pipeline completo para {type(contenido).__name__}: {fragmento_id_str}"
        )
        ...
        try:
            # === FASE 1: TRIAJE ===
            with log_phase(
                "Fase1_Triaje", request_id, fragment_id=fragmento_id_str
            ) as phase_logger:
                ...
                phase_logger.info(
                    ...
                    justificacion=resultado_fase1.justificacion_triaje[:50] + "..."
                    if resultado_fase1.justificacion_triaje
                    else None,
                )
            ...
            if not resultado_fase1.es_relevante:
                logger.info(
                    f"Fragmento marcado como no relevante. Pipeline terminado.",  # noqa: F541
            ...
            if flujo_config["chunking_aplicado"]:
                with log_phase(
                    "Chunking", request_id, fragment_id=fragmento_id_str
                ) as phase_logger:
                    ...
                    chunks = self.chunking_service.dividir_en_chunks(
                        ...
                        resultado_fase1.metadatos_specificos_triaje.analisis_contenido
                        if resultado_fase1.metadatos_specificos_triaje
                        else {},
                    )
            ...
            if flujo_config["simplificacion_aplicada"]:
                with log_phase(
                    "Fase2_Simplificacion", request_id, fragment_id=fragmento_id_str
                ) as phase_logger:
                    ...
                    for idx, chunk_texto in enumerate(chunks):
                        ...
                        resultado_simplif = ejecutar_fase_2_simplificacion(
                            ...
                            fecha_articulo=contexto_articulo.get("fecha_publicacion")
                            if contexto_articulo
                            else None,
            ...
            for idx, resultado_simplif in enumerate(resultados_simplificacion):
                ...
                with log_phase(
                    ...
                    if "entidades_extraidas" in resultado_entidades:
                        chunk_resultado["entidades"] = resultado_entidades[
                            "entidades_extraidas"
                        ]
                        phase_logger.info(
                            f"Entidades extraídas: {len(chunk_resultado['entidades'])}"
                        )

                # === FASE 4: EXTRACCIÓN DE HECHOS ===
                with log_phase(
                    f"Fase4_Hechos_Chunk{idx}", request_id, fragment_id=fragmento_id_str
                ) as phase_logger:
                    ...
                    if "hechos_extraidos" in resultado_hechos:
                        ...
                        phase_logger.info(
                            f"Hechos extraídos: {len(chunk_resultado['hechos'])}"
                        )

                # === FASE 5: EXTRACCIÓN DE DATOS (CONDICIONAL) ===
                if flujo_config["fase_5_ejecutada"]:
                    with log_phase(
                        f"Fase5_Datos_Chunk{idx}",
                        request_id,
                        fragment_id=fragmento_id_str,
                    ) as phase_logger:
                        phase_logger.info(
                            f"Extrayendo datos cuantitativos del chunk {idx}"
                        )
                        ...
                        if "datos_extraidos" in resultado_datos:
                            chunk_resultado["datos"] = resultado_datos[
                                "datos_extraidos"
                            ]
                            phase_logger.info(
                                f"Datos extraídos: {len(chunk_resultado['datos'])}"
                            )

                # === FASE 6: EXTRACCIÓN DE CITAS (CONDICIONAL) ===
                if flujo_config["fase_6_ejecutada"]:
                    with log_phase(
                        f"Fase6_Citas_Chunk{idx}",
                        request_id,
                        fragment_id=fragmento_id_str,
                    ) as phase_logger:
                        ...
                        if "citas_extraidas" in resultado_citas:
                            chunk_resultado["citas"] = resultado_citas[
                                "citas_extraidas"
                            ]
                            phase_logger.info(
                                f"Citas extraídas: {len(chunk_resultado['citas'])}"
                            )
            ...
            if flujo_config["consolidacion_aplicada"] and len(resultados_por_chunk) > 1:
                with log_phase(
                    "Consolidacion_CrossChunk", request_id, fragment_id=fragmento_id_str
                ) as phase_logger:
                    ...
                    entidades_consolidadas = (
                        self.consolidation_service.consolidar_entidades(todas_entidades)
                    )
                    hechos_consolidados = self.consolidation_service.consolidar_hechos(
                        todos_hechos
                    )
                    datos_consolidados = self.consolidation_service.consolidar_datos(
                        todos_datos
                    )
                    citas_consolidadas = self.consolidation_service.consolidar_citas(
                        todas_citas
                    )
                ...
                entidades_consolidadas = [
                    ent for chunk in resultados_por_chunk for ent in chunk["entidades"]
                ]
                hechos_consolidados = [
                    hecho for chunk in resultados_por_chunk for hecho in chunk["hechos"]
                ]
                datos_consolidados = [
                    dato for chunk in resultados_por_chunk for dato in chunk["datos"]
                ]
                citas_consolidadas = [
                    cita for chunk in resultados_por_chunk for cita in chunk["citas"]
                ]

            # === FASE 7: NORMALIZACIÓN Y RELACIONES ===
            with log_phase(
                "Fase7_Normalizacion_Relaciones",
                request_id,
                fragment_id=fragmento_id_str,
            ) as phase_logger:
                ...
                texto_simplificado_completo = " ".join(
                    [r.texto_simplificado for r in resultados_simplificacion]
                )
                ...
                phase_logger.info(
                    ...
                    relaciones_detectadas=resultado_fase7.metadata_normalizacion.get(
            ...
            articulo_original_preserved = resultado["metadatos"].get(
                "articulo_original"
            )
            es_articulo_completo = resultado["metadatos"].get(
            ...
            logger.info(
                ...
                tipo_articulo_original=type(articulo_original_preserved).__name__
                if articulo_original_preserved
                else "None",
            )
    ...
    def _crear_payload_no_relevante(
        ...
        if es_articulo_completo and articulo_original is not None:
            ...
            resultado_procesamiento = {
                ...
                "fecha_ingesta_sistema": articulo_original.fecha_recopilacion.isoformat()
                if articulo_original.fecha_recopilacion
    ...
    def _determinar_flujo_adaptativo(
        self, resultado_fase1: ResultadoFase1Triaje
    ) -> Dict[str, bool]:
        """
        ...
        simplificacion_necesaria = (
            densidad_entidades > 10
        )  # Más de 10 entidades por 100 tokens
        ...
        self.base_logger.info("Flujo adaptativo determinado", **config)

        return config

    def _construir_entidades_del_hecho(
        self, hecho: HechoProcesado, relaciones_hecho_entidad: List[Dict]
    ) -> List[Dict]:
        """
    ...
    def _generar_payload_completo_7_fases(
            ...
            hechos_data.append(
                {
                    ...
                    "tipo_hecho": hecho.metadata_hecho.tipo_hecho
                    if hasattr(hecho.metadata_hecho, "tipo_hecho")
                    else "SUCESO",
                    "importancia": hecho.importancia,  # Ya es 1-10
                    "fecha_ocurrencia_inicio": fecha_inicio_iso,
                    "fecha_ocurrencia_fin": fecha_fin_iso,
                    "precision_temporal": hecho.metadata_hecho.precision_temporal
                    if hasattr(hecho.metadata_hecho, "precision_temporal")
                    else "desconocido",
                    "metadata": {
                        "pais": hecho.metadata_hecho.pais
                        if hasattr(hecho.metadata_hecho, "pais")
                        else [],
                        "region": hecho.metadata_hecho.region
                        if hasattr(hecho.metadata_hecho, "region")
                        else [],
                        "ciudad": hecho.metadata_hecho.ciudad
                        if hasattr(hecho.metadata_hecho, "ciudad")
                        else [],
                    ...
                    "es_evento_futuro": hecho.metadata_hecho.es_futuro
                    if hasattr(hecho.metadata_hecho, "es_futuro")
                    else None,
                    "estado_programacion": hecho.metadata_hecho.estado_programacion
                    if hasattr(hecho.metadata_hecho, "estado_programacion")
                    else None,
                    "detalle_complejo_hecho": hecho.metadata_hecho.model_dump()
                    if hasattr(hecho.metadata_hecho, "model_dump")
                    else {},
                    "entidades_del_hecho": self._construir_entidades_del_hecho(
                        hecho,
                        resultado_fase7.metadata_normalizacion.get(
            ...
            entidades_data.append(
                {
                    ...
                    "tipo": entidad.tipo,
                    "descripcion": f"Entidad extraída con relevancia {entidad.relevancia}",
                    "relevancia": entidad.relevancia,  # Ya es 1-10
                    "metadata": {  # Cambio: metadata_entidad → metadata
                        **(
                            entidad.metadata_entidad.model_dump()
                            if hasattr(entidad.metadata_entidad, "model_dump")
                            else {}
                        ),
            ...
            citas_data.append(
                {
                    ...
                    "cita": cita.cita,  # ACTUALIZADO: texto_cita → cita
                    "id_temporal_entidad_emisora": str(cita.entidad_emisora_id)
                    if cita.entidad_emisora_id
                    else None,
                    "id_temporal_hecho_contexto": str(cita.hecho_contexto_id)
                    if cita.hecho_contexto_id
                    else None,
                    "fecha_cita": cita.fecha_cita if cita.fecha_cita else None,
                    "contexto": cita.contexto,  # ACTUALIZADO: contexto_cita → contexto
                    "relevancia": cita.relevancia,  # Campo directo en CitaTextual
        ...
        return self.payload_builder.construir_payload_fragmento(
            ...
            resumen_generado_fragmento=resultado_fase7.resumen_normalizacion
            or "Fragmento procesado exitosamente con 7 fases",
    ...
    def _generar_payload_articulo_completo(
        ...
        resultado_procesamiento = {
            ...
            "resumen_generado_pipeline": resultado_fase7.resumen_normalizacion
            or "Artículo procesado exitosamente con 7 fases",
        ...
        palabras_clave = []
        for entidad in sorted(entidades, key=lambda e: e.relevancia, reverse=True)[:10]:
            if entidad.nombre_entidad_normalizada:
                palabras_clave.append(entidad.nombre_entidad_normalizada)
            else:
                palabras_clave.append(entidad.nombre)
        ...
        if (
            resultado_fase7
            and hasattr(resultado_fase7, "metadata_normalizacion")
            and resultado_fase7.metadata_normalizacion
        ):
            ...
            if relaciones_estructurales:
                ...
                for rel in relaciones_estructurales.get("entidad_relacion", []):
                    relaciones_entidades_data.append(
                        {
                            "entidad_origen_id": str(rel.get("entidad_origen_id", "")),
                            "entidad_destino_id": str(
                                rel.get("entidad_destino_id", "")
                            ),
            ...
            if relaciones_temporales:
                ...
                for rel in relaciones_temporales.get("hecho_relacionado", []):
                    relaciones_hechos_data.append(
                        {
                            "hecho_origen_id": str(rel.get("hecho_origen_id", "")),
                            "hecho_destino_id": str(rel.get("hecho_destino_id", "")),
                            "tipo_relacion": self._mapear_tipo_relacion_hecho(
                    ...
                    contradicciones_data.append(
                        {
                            "hecho_principal_id": str(
                                cont.get("hecho_principal_id", "")
                            ),
                            "hecho_contradictorio_id": str(
                                cont.get("hecho_contradictorio_id", "")
                            ),
                            "tipo_contradiccion": self._mapear_tipo_contradiccion(
                                cont.get("tipo_contradiccion", "")
                            ),
        ...
        for hecho_dict in hechos_data:
                    ...
                    hecho_dict["entidades_del_hecho"].append(
                        {
                            "id_temporal": str(
                                entidad_id
                            ),  # Cambio: id_temporal_entidad → id_temporal
            ...
            if not hecho_dict["entidades_del_hecho"]:
                ...
                hecho_original = next(
                    (h for h in hechos if str(h.id_hecho) == hecho_dict["id_temporal"]),
                    None,
                )
            ...
            citas_data.append(
                {
                    ...
                    if hasattr(cita.metadata_cita, "relevancia")
                    else 3,  # ACTUALIZADO
                    "nombre_entidad_emisora": cita.persona_citada,
                }
            )
            ...
            datos_data.append(
                {
                    ...
                    "valor_numerico": dato.valor_numerico,
                    "unidad": dato.unidad,
                    "ambito_geografico": dato.metadata_dato.ambito_geografico
                    if hasattr(dato.metadata_dato, "ambito_geografico")
                    else [],
                    "periodo_referencia_inicio": dato.periodo_referencia_inicio,
                    "periodo_referencia_fin": dato.periodo_referencia_fin,
                    "tendencia": dato.metadata_dato.tendencia
                    if hasattr(dato.metadata_dato, "tendencia")
                    else None,
                    # Campos temporales
                    "id_temporal_dato": str(dato.id_dato_cuantitativo),
                    # Campos adicionales para compatibilidad
                    "id_dato_cuantitativo": dato.id_dato_cuantitativo,
                    "hecho_id": dato.hecho_id,
                    ...
                    "valor_anterior": dato.metadata_dato.valor_anterior
                    if hasattr(dato.metadata_dato, "valor_anterior")
                    else None,
                    "variacion_absoluta": dato.metadata_dato.variacion_absoluta
                    if hasattr(dato.metadata_dato, "variacion_absoluta")
                    else None,
                    "variacion_porcentual": dato.metadata_dato.variacion_porcentual
                    if hasattr(dato.metadata_dato, "variacion_porcentual")
                    else None,
                }
            )
    ...
    def _mock_fase_2(
        self, resultado_fase1: ResultadoFase1Triaje, processor: FragmentProcessor
    ) -> ResultadoFase2Extraccion:
        ...
        if "anunció" in resultado_fase1.texto_para_siguiente_fase or "":
            ...
            hecho_mock = HechoProcesado(
                ...
                contenido="Mock: Se anunció algo importante",
    ...
    def _mock_fase_3(
        self, resultado_fase2: ResultadoFase2Extraccion, processor: FragmentProcessor
    ) -> ResultadoFase3CitasDatos:
        ...
        if resultado_fase2.hechos_extraidos:
            ...
            cita_mock = CitaTextual(
                ...
                cita="Mock: Esta es una cita de ejemplo",
                relevancia=3,
    ...
    def _mock_fase_4(
        self, resultado_fase3: ResultadoFase3CitasDatos, processor: FragmentProcessor
    ) -> ResultadoFase4Normalizacion:
...
if __name__ == "__main__":
    ...
    print(f"\n--- Test Pipeline Coordinator ---")  # noqa: F541
    ...
    if resultado.get("metadatos", {}).get("processor_stats"):
        ...
        print(f"  Stats Processor:")  # noqa: F541
