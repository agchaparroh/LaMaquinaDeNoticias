...
# Importar modelos
from ..models.metadatos import MetadatosEntidad  # noqa: E402
from ..models.procesamiento import EntidadProcesada  # noqa: E402
...
_PROMPT_ENTIDADES_PATH = (
    Path(__file__).resolve().parent.parent.parent / "prompts" / "Entidades.md"
)
...
def _cargar_prompt_entidades() -> str:
    ...
    if _PROMPT_ENTIDADES_TEMPLATE is None:
        ...
        with open(_PROMPT_ENTIDADES_PATH, encoding="utf-8") as f:
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            if attempt < max_retries - 1:
                continue
            raise GroqAPIError(
                f"Error al extraer entidades después de {max_retries} intentos: {str(e)}"
            )
...
def ejecutar_fase_3_entidades(
    ...
    """
    logger.info(
        f"Iniciando Fase 3: Extracción de Entidades para fragmento {resultado_simplificacion.id_fragmento}"
    )
    ...
    try:
        ...
        if not resultado_simplificacion.texto_simplificado:
            raise ValueError(
                "No hay texto simplificado disponible para extraer entidades"
            )
        ...
        if len(resultado_simplificacion.texto_simplificado) > 10000:
            ...
            logger.info(f"Usando modelo grande para extracción de entidades")  # noqa: F541
        ...
        entidades_procesadas = _procesar_entidades_extraidas(
            entidades_raw, resultado_simplificacion.id_fragmento, fragment_processor
        )
...
async def _procesar_chunk_entidades_async(
    ...
    try:
        ...
        # Procesar entidades
        entidades_procesadas = _procesar_entidades_extraidas(
            entidades_chunk, resultado_simplificacion.id_fragmento, fragment_processor
        )

        logger.debug(
            f"Chunk {chunk_index + 1}: {len(entidades_procesadas)} entidades extraídas"
        )
        ...
        logger.error(
            f"Error procesando chunk {chunk_index + 1} para entidades: {str(e)}"
        )
