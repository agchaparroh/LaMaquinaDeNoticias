...
from ..services.spacy_analyzer import SpacyAnalyzer, create_spacy_analyzer
from ..services.adaptive_flow_controller import AdaptiveFlowController, create_adaptive_flow_controller
...
def ejecutar_fase_1(
    ...
    try:
        ...
        logger.debug(f"Iniciando análisis de contenido con SpacyAnalyzer para fragmento {id_fragmento_original}")
        spacy_analyzer = create_spacy_analyzer(nlp_model)
        analisis_contenido = spacy_analyzer.analizar_contenido(texto_limpio, modelo_spacy_nombre)
        
        # Validar análisis de contenido
        es_valido, advertencias = spacy_analyzer.validar_metricas(analisis_contenido)
        if advertencias:
            logger.warning(f"Advertencias en análisis de contenido para {id_fragmento_original}: {advertencias}")
            notas_procesamiento.extend([f"Advertencia análisis: {adv}" for adv in advertencias])
        
        # --- NUEVA FUNCIONALIDAD: Control de Flujo Adaptativo ---
        logger.debug(f"Generando decisiones de flujo con AdaptiveFlowController para fragmento {id_fragmento_original}")
        ...
        if not flow_controller.validate_decision(decisiones_flujo, analisis_contenido):
            logger.warning(f"Decisiones de flujo incoherentes para fragmento {id_fragmento_original}")
            notas_procesamiento.append("Decisiones de flujo requieren revisión manual")
        
        logger.info(f"Análisis completado para fragmento {id_fragmento_original}: "
        ...
        logger.debug(f"Estadísticas de procesamiento para {id_fragmento_original}: {stats_procesamiento}")
        
        # Agregar información de análisis a las notas
        if decisiones_flujo.justificacion:
            notas_procesamiento.append(f"Decisiones de flujo: {decisiones_flujo.justificacion}")
        ...
        metadatos_fase1 = MetadatosFase1Triaje(
            ...
            analisis_contenido=analisis_contenido.model_dump() if analisis_contenido else None,
            decisiones_flujo=decisiones_flujo.model_dump() if decisiones_flujo else None,
