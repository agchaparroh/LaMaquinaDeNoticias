...
logger = get_logger("Fase1_Triaje")
from ..config import get_spacy_fallback_models, get_spacy_model_name  # noqa: E402
    ...
    if modelo not in _NLP_MODELS_CACHE:
        ...
        for fallback_model in fallback_models:
            if fallback_model != modelo:  # No intentar el mismo modelo otra vez
                ...
                if loaded_model:
                    logger.warning(
                        f"Usando modelo de fallback '{fallback_model}' en lugar de '{modelo}'"
                    )
...
def _try_load_spacy_model(modelo: str) -> Optional[Language]:
    ...
    try:
        ...
        logger.debug(
            f"No se pudo cargar el modelo spaCy '{modelo}'. "
...
def ejecutar_fase_1(
    ...
    resultado_final = ResultadoFase1Triaje(
        id_fragmento=id_fragmento_original,
        es_relevante=True
        if debug_force_relevant
        else (evaluacion_triaje.get("decision", "ERROR_TRIAGE") == "PROCESAR"),
        decision_triaje=evaluacion_triaje.get("decision", "ERROR_TRIAGE"),
        justificacion_triaje=evaluacion_triaje.get("justificacion")
        + " [DEBUG: Forzado como relevante]"
        if debug_force_relevant
        else evaluacion_triaje.get("justificacion"),
...
_PROMPT_TRIAGE_PATH = (
    Path(__file__).resolve().parent.parent.parent / "prompts" / "Importancia.md"
)
