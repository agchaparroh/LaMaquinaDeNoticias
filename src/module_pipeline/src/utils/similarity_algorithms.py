...
def calcular_similitud_compuesta(
    texto1: str, texto2: str, pesos: Optional[dict] = None
) -> float:
    """
    ...
    """
    if pesos is None:
        pesos = {"exacta": 0.3, "difusa": 0.3, "jaccard": 0.2, "coseno": 0.2}
    ...
    # Ponderar
    score_final = sum(
        scores[algo] * peso for algo, peso in pesos.items() if algo in scores
    )
...
def encontrar_grupos_similares(
    elementos: List[str], umbral: float = 0.85, algoritmo: str = "compuesta"
) -> List[List[int]]:
    """
...
def analizar_similitudes(texto1: str, texto2: str) -> dict:
    ...
    return {
        ...
        "coseno": similitud_coseno_palabras(texto1, texto2),
        "es_subsecuencia": detectar_subsecuencia(
            min(texto1, texto2, key=len), max(texto1, texto2, key=len)
        ),
...
class OptimizedSimilarityProcessor:
    ...
    def get_candidates_fast(
        self, query_text: str, min_threshold: float = 0.1
    ) -> Set[int]:
        """
    ...
    def text_to_vector(self, text: str) -> tuple:
        ...
        bigram_features = [
            count / len(bigrams) if bigrams else 0.0 for _, count in top_bigrams
        ]
    ...
    def vectorized_similarity_batch(
        self, texts: List[str], query_text: str, threshold: float = 0.7
        ...
        for idx in candidates:
            if idx < len(texts):
                ...
                if HAS_NUMPY:
                    ...
                    # Versión sin numpy
                    dot_product = sum(
                        a * b for a, b in zip(query_vector, candidate_vector)
                    )
    ...
    def consolidate_with_early_termination(
        self, elements: List[str], threshold: float = 0.85, max_comparisons: int = 10000
    ) -> List[List[int]]:
        """
        ...
        for i, element in enumerate(elements):
            if i in assigned or comparisons_made >= max_comparisons:
                if comparisons_made >= max_comparisons:
                    logger.warning(
                        f"Early termination: alcanzado límite de {max_comparisons} comparaciones"
                    )
                break
        ...
        logger.debug(
            f"Consolidación completada: {len(groups)} grupos, {comparisons_made} comparaciones"
        )
        return groups
...
def consolidar_elementos_optimizado(
    elementos: List[str], umbral: float = 0.85, max_comparaciones: int = 10000
) -> List[List[int]]:
    """
