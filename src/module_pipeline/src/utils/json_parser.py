...
class LLMJSONParseError(Exception):
    ...
    def __init__(
        self, message: str, original_response: str = "", cleaned_response: str = ""
    ):
...
def detect_truncation(text: str) -> Tuple[bool, str]:
    ...
    if open_braces > close_braces:
        return (
            True,
            f"Llaves desbalanceadas: {open_braces} abiertas vs {close_braces} cerradas",
        )

    if open_brackets > close_brackets:
        return (
            True,
            f"Corchetes desbalanceados: {open_brackets} abiertos vs {close_brackets} cerrados",
        )
...
def parse_llm_json_response(
    raw_response: str, attempt_repair: bool = True, strict: bool = False
) -> Dict[str, Any]:
    """
    ...
    if cleaned_response != raw_response:
        logger.info(
            f"Markdown detectado y limpiado. Longitud: {len(raw_response)} -> {len(cleaned_response)}"
        )
    ...
    error_msg = f"No se pudo parsear la respuesta JSON del LLM"  # noqa: F541
    if is_truncated:
