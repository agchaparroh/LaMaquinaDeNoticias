...
class SpacyModelConfig:
    ...
    fallback_models: List[str] = field(
        default_factory=lambda: ["es_core_news_sm", "es_core_news_lg"]
    )
...
class ProcessingConfig:
    ...
    def __post_init__(self):
        ...
        if not (0.0 <= self.consolidation_similarity_threshold <= 1.0):
            raise ValueError(
                "consolidation_similarity_threshold debe estar entre 0.0 y 1.0"
            )
...
def load_pipeline_config() -> PipelineConfig:
    ...
    fallback_models_str = _get_str_env(
        "PIPELINE_SPACY_FALLBACK_MODELS", "es_core_news_sm,es_core_news_lg"
    )
    fallback_models_list = [
        model.strip() for model in fallback_models_str.split(",") if model.strip()
    ]
    ...
    processing_config = ProcessingConfig(
        consolidation_similarity_threshold=_get_float_env(
...
def print_pipeline_config_summary(config: Optional[PipelineConfig] = None):
    ...
    print("⚡ PROCESAMIENTO:")
    print(
        f"  - Similarity threshold: {config.processing.consolidation_similarity_threshold}"
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        print()
        print(
            f"✅ Validación: {'VÁLIDA' if validate_pipeline_config(config) else 'INVÁLIDA'}"
        )
        ...
        print(
            f"  - should_chunk_content('chars', 0, 10000): {should_chunk_content('chars', 0, 10000)}"
        )
        print(
            f"  - should_chunk_content('entities', 50, 1000): {should_chunk_content('entities', 50, 1000)}"
        )
