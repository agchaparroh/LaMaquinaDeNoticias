...
def _get_int_env(var_name: str, default: int) -> int:
    ...
    try:
        ...
        print(
            f"⚠️  WARNING: Variable {var_name} no es un número válido, usando default: {default}"
        )
        return default
...
SUPABASE_KEY = _get_required_env(
    "SUPABASE_SERVICE_ROLE_KEY"
)  # Usar SERVICE_ROLE_KEY para bypasear RLS
...
JOB_CLEANUP_INTERVAL_MINUTES = _get_int_env(
    "JOB_CLEANUP_INTERVAL_MINUTES", 5
)  # Default: cada 5 minutos
JOB_MAX_STORED = _get_int_env(
    "JOB_MAX_STORED", 10000
)  # Límite máximo de jobs en memoria
...
if USE_SENTRY and not SENTRY_DSN:
    print(
        "⚠️  WARNING: SENTRY_ENABLED=true pero SENTRY_DSN no configurado. Deshabilitando Sentry."
    )
    USE_SENTRY = False
...
IMPORTANCE_MODEL_PATH = Path(
    os.getenv("IMPORTANCE_MODEL_PATH", "./models/importance_model.pkl")
)
...
ALERT_LATENCY_THRESHOLD_SECONDS = _get_float_env(
    "ALERT_LATENCY_THRESHOLD_SECONDS", 30.0
)  # 30 segundos
ALERT_THROTTLE_MINUTES = _get_int_env(
    "ALERT_THROTTLE_MINUTES", 1
)  # 1 minuto entre alertas del mismo tipo
ALERT_RETENTION_HOURS = _get_int_env(
    "ALERT_RETENTION_HOURS", 24
)  # Retener alertas por 24 horas
