...
def parsear_metadatos_entidad_desde_json(
    entidad_data: Dict[str, Any],
) -> MetadatosEntidad:
    """
...
def parsear_metadatos_cita_desde_json(cita_data: Dict[str, Any]) -> MetadatosCita:
    ...
    try:
        ...
        return MetadatosCita(
            fecha=cita_data.get("fecha"), contexto=contexto, relevancia=relevancia
        )
    except Exception as e:
...
def parsear_metadatos_dato_desde_json(dato_data: Dict[str, Any]) -> MetadatosDato:
    ...
    try:
        ...
        if isinstance(ambito_geografico, list):
            ambito_geografico = [
                escape_html(a) if isinstance(a, str) else a for a in ambito_geografico
            ]
        ...
        if dato_data.get("periodo"):
            ...
            if isinstance(periodo_data, dict):
                periodo = PeriodoReferencia(
                    inicio=periodo_data.get("inicio"), fin=periodo_data.get("fin")
                )
...
def ejemplo_parseo_fase2(respuesta_llm: str, fragmento_id) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"hechos": hechos_parseados, "entidades": entidades_parseadas}
    ...
    try:
        ...
        for cita_data in data.get("citas_textuales", []):
            ...
            cita_info = {
                "id_fragmento_origen": fragmento_id,
                "cita": cita_data["cita"],
                "entidad_emisora_id": cita_data.get(
                    "entidad_id"
                ),  # Mapear a UUID si existe
        ...
        return {"citas": citas_parseadas, "datos": datos_parseados}
