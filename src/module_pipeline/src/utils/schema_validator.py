...
class SchemaValidator:
        ...
        for idx, entidad in enumerate(entidades):
            ...
            for campo in campos_requeridos:
                if campo not in entidad:
                    self.errores.append(f"Entidad {entidad.get('id', idx)} sin campo {campo}")
                    valido = False
            ...
            if "alias" in entidad and not isinstance(entidad["alias"], list):
                self.errores.append(f"Alias no es array en entidad {entidad.get('id', idx)}")
                valido = False
            
            # Validar fechas
            for campo_fecha in ["fecha_nacimiento", "fecha_disolucion"]:
                if campo_fecha in entidad:
                    self.validar_fecha(entidad[campo_fecha], f"entidad {entidad.get('id', idx)}.{campo_fecha}")
        ...
        for idx, hecho in enumerate(hechos):
            ...
            for campo in campos_requeridos:
                if campo not in hecho:
                    self.errores.append(f"Hecho {hecho.get('id', idx)} sin campo {campo}")
                    valido = False
            ...
            if "fecha" in hecho:
                if not isinstance(hecho["fecha"], dict):
                    self.errores.append(f"Fecha no es objeto en hecho {hecho.get('id', idx)}")
                    valido = False
                else:
                    if "inicio" not in hecho["fecha"] or "fin" not in hecho["fecha"]:
                        ...
                        self.validar_fecha(hecho["fecha"]["inicio"], f"hecho {hecho.get('id', idx)}.fecha.inicio")
                        self.validar_fecha(hecho["fecha"]["fin"], f"hecho {hecho.get('id', idx)}.fecha.fin")
            ...
            for campo_array in ["pais", "region", "ciudad"]:
                if campo_array in hecho and not isinstance(hecho[campo_array], list):
                    self.errores.append(f"{campo_array} no es array en hecho {hecho.get('id', idx)}")
                    valido = False
            
            # Validar booleano
            if "es_futuro" in hecho and not isinstance(hecho["es_futuro"], bool):
                self.errores.append(f"es_futuro no es booleano en hecho {hecho.get('id', idx)}")
                valido = False
        ...
        for idx, dato in enumerate(datos):
            ...
            if "valor" in dato:
                if not isinstance(dato["valor"], (int, float)):
                    self.errores.append(f"Valor no numérico en dato {dato.get('id', idx)}: {dato['valor']}")
                    valido = False
    ...
    def validar_relaciones_estructurales(self, relaciones: Dict[str, Any]) -> bool:
        ...
        if "hecho_entidad" in relaciones:
            for idx, rel in enumerate(relaciones["hecho_entidad"]):
                # Validar referencias
                if "hecho_id" in rel and rel["hecho_id"] not in self.ids_vistos["hechos"]:
                    self.advertencias.append(
                        f"Relación hecho-entidad {idx} referencia hecho inexistente: {rel['hecho_id']}"
                    )
                if "entidad_id" in rel and rel["entidad_id"] not in self.ids_vistos["entidades"]:
                    self.advertencias.append(
                ...
                if "relevancia_en_hecho" in rel:
                    if not isinstance(rel["relevancia_en_hecho"], int) or rel["relevancia_en_hecho"] < 1 or rel["relevancia_en_hecho"] > 10:
                        self.errores.append(
        ...
        if "entidad_relacion" in relaciones:
            for idx, rel in enumerate(relaciones["entidad_relacion"]):
                # Validar referencias
                if "entidad_origen_id" in rel and rel["entidad_origen_id"] not in self.ids_vistos["entidades"]:
                    self.advertencias.append(
                        f"Relación entidad-entidad {idx} referencia origen inexistente: {rel['entidad_origen_id']}"
                    )
                if "entidad_destino_id" in rel and rel["entidad_destino_id"] not in self.ids_vistos["entidades"]:
                    self.advertencias.append(
                ...
                for campo_fecha in ["fecha_inicio", "fecha_fin"]:
                    if campo_fecha in rel:
                        self.validar_fecha(rel[campo_fecha], f"relación entidad-entidad {idx}.{campo_fecha}")
    ...
    def validar_relaciones_temporales(self, relaciones: Dict[str, Any]) -> bool:
        ...
        if "hecho_relacionado" in relaciones:
            for idx, rel in enumerate(relaciones["hecho_relacionado"]):
                # Validar referencias
                if "hecho_origen_id" in rel and rel["hecho_origen_id"] not in self.ids_vistos["hechos"]:
                    self.advertencias.append(
                        f"Relación hecho-hecho {idx} referencia origen inexistente: {rel['hecho_origen_id']}"
                    )
                if "hecho_destino_id" in rel and rel["hecho_destino_id"] not in self.ids_vistos["hechos"]:
                    self.advertencias.append(
        ...
        if "contradicciones" in relaciones:
            for idx, cont in enumerate(relaciones["contradicciones"]):
                # Validar referencias
                if "hecho_principal_id" in cont and cont["hecho_principal_id"] not in self.ids_vistos["hechos"]:
                    self.advertencias.append(
                        f"Contradicción {idx} referencia principal inexistente: {cont['hecho_principal_id']}"
                    )
                if "hecho_contradictorio_id" in cont and cont["hecho_contradictorio_id"] not in self.ids_vistos["hechos"]:
                    self.advertencias.append(
                ...
                if "grado_contradiccion" in cont:
                    if not isinstance(cont["grado_contradiccion"], int) or cont["grado_contradiccion"] < 1 or cont["grado_contradiccion"] > 5:
                        self.errores.append(
    ...
    def validar_pipeline_completo(self, resultado: Dict[str, Any]) -> Tuple[bool, List[str], List[str]]:
        """
        ...
        if "relaciones_estructurales" in resultado:
            valido &= self.validar_relaciones_estructurales(resultado["relaciones_estructurales"])
        
        if "relaciones_temporales" in resultado:
            valido &= self.validar_relaciones_temporales(resultado["relaciones_temporales"])
...
def validar_resultado_fase(fase: int, resultado: Dict[str, Any]) -> Tuple[bool, List[str]]:
    """
    ...
    if fase == 3:
        ...
        valido = True
        if "relaciones_estructurales" in resultado:
            valido &= validator.validar_relaciones_estructurales(resultado["relaciones_estructurales"])
        if "relaciones_temporales" in resultado:
            valido &= validator.validar_relaciones_temporales(resultado["relaciones_temporales"])
    else:
...
def asegurar_coherencia_ids(elementos: List[Dict[str, Any]], tipo: str) -> List[Dict[str, Any]]:
    """
    ...
    for idx, elemento in enumerate(elementos, 1):
        ...
        if "id" in elemento_copia:
            if elemento_copia["id"] != idx:
                logger.debug(f"Corrigiendo ID de {tipo}: {elemento_copia['id']} -> {idx}")
