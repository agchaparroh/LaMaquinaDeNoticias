...
from ..utils.config import get_logging_config  # noqa: E402, F401
from ..utils.error_handling import ErrorPhase, ErrorType  # noqa: E402, F401
...
class AlertManager:
    ...
    def _get_default_config_dir(self) -> Path:
        ...
        )  # src/monitoring -> src -> module_pipeline
    ...
    def _analyze_log_record(self, record: Dict[str, Any]):
        ...
        try:
            ...
            # Detectar problemas específicos del pipeline de 7 fases
            self._check_pipeline_health(record, extra)
    ...
    def _check_api_errors(
        self, record: Dict[str, Any], level: str, message: str, extra: Dict[str, Any]
    ):
        """Detecta errores específicos de APIs externas y del pipeline de 7 fases."""
        ...
        # Detectar errores específicos del pipeline de 7 fases
        self._check_pipeline_specific_errors(record, level, message, extra)
    ...
    def _check_pipeline_specific_errors(
        self, record: Dict[str, Any], level: str, message: str, extra: Dict[str, Any]
    ):
        """Detecta errores específicos del pipeline de 7 fases."""
        phase = extra.get("phase", extra.get("fase", ""))

        # Detectar errores de fases específicas
        if phase and any(fase in phase.lower() for fase in ["fase", "phase"]):
            self._trigger_alert(
                alert_type=AlertType.PHASE_FAILURE,
                severity=AlertSeverity.WARNING,
                title=f"Phase {phase} Failure",
                description=f"Error en {phase}: {message[:200]}",
                labels={
                    "phase": phase,
                    "pipeline_type": extra.get("pipeline_type", "unknown"),
                },
                annotations={
                    "message": message[:500],
        ...
        # Detectar errores de chunking
        if any(term in message.lower() for term in ["chunk", "chunking", "fragment"]):
            self._trigger_alert(
                alert_type=AlertType.CHUNKING_ERROR,
                severity=AlertSeverity.WARNING,
                title="Chunking System Error",
                description=f"Error en sistema de chunking: {message[:200]}",
                labels={"component": "chunking_service", "phase": phase or "unknown"},
                annotations={
                    "message": message[:500],
                    "chunks_count": str(extra.get("chunks_count", "unknown")),
                },
            )

        # Detectar errores de consolidación
        if any(
            term in message.lower()
            for term in ["consolidation", "consolidar", "duplicat"]
        ):
            self._trigger_alert(
                alert_type=AlertType.CONSOLIDATION_FAILURE,
                severity=AlertSeverity.WARNING,
                title="Consolidation Process Error",
                description=f"Error en consolidación: {message[:200]}",
                labels={
                    "component": "consolidation_service",
                    "phase": phase or "unknown",
                },
                annotations={
                    "message": message[:500],
                    "similarity_threshold": str(
                        extra.get("similarity_threshold", "unknown")
                    ),
                },
            )

    def _check_pipeline_health(self, record: Dict[str, Any], extra: Dict[str, Any]):
        """Verifica la salud general del pipeline de 7 fases."""
        # Detectar pipeline estancado
        pipeline_duration = extra.get("pipeline_duration_seconds") or extra.get(
            "duration_seconds"
        )
        if pipeline_duration and isinstance(pipeline_duration, (int, float)):
            # Si un pipeline tarda más de 5 minutos, es preocupante
            if pipeline_duration > 300:
                self._trigger_alert(
                    alert_type=AlertType.PIPELINE_STALL,
                    severity=AlertSeverity.CRITICAL,
                    title="Pipeline Execution Stalled",
                    description=f"Pipeline tardó {pipeline_duration:.1f}s (>5min) en completarse",
                    labels={
                        "duration_seconds": str(pipeline_duration),
                        "pipeline_type": extra.get("pipeline_type", "unknown"),
                    },
                    annotations={
    ...
    def _check_error_rate(self, record: Dict[str, Any], level: str):
        ...
        with self._lock:
            ...
            while (
                self._error_counts[minute_key]
                and self._error_counts[minute_key][0][0] < cutoff_time
            ):
            ...
            if len(self._error_counts[minute_key]) >= 10:  # Mínimo 10 eventos
                ...
                error_events = sum(
                    1 for _, is_err in self._error_counts[minute_key] if is_err
                )
    ...
    def _load_alerts(self):
        ...
        try:
            ...
            logger.error(
                f"Error cargando alertas: {e}", extra={"component": "AlertManager"}
            )
    ...
    def _save_alerts(self):
        ...
        try:
            ...
            logger.error(
                f"Error guardando alertas: {e}", extra={"component": "AlertManager"}
            )
    ...
    def get_active_alerts(self) -> List[Dict[str, Any]]:
        ...
        with self._lock:
            return [
                alert.to_dict() for alert in self._alerts.values() if not alert.resolved
            ]
    ...
    def get_all_alerts(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        with self._lock:
            alerts = sorted(
                self._alerts.values(), key=lambda x: x.timestamp, reverse=True
            )
    ...
    def test_alert_system(self):
        ...
        # Simular error de fase del pipeline
        logger.error(
            "Test: Fase 3 Entidades processing failed",
            extra={
                "component": "TestPipeline",
                "phase": "Fase3_Entidades",
                "request_id": "test-req-123",
            },
        )

        # Simular error de chunking
        logger.error(
            "Test: Chunking service failed to process chunks",
            extra={"component": "ChunkingService", "chunks_count": 5},
        )

        # Simular pipeline lento
        logger.warning(
            "Test: Pipeline execution taking too long",
            extra={
                "component": "PipelineController",
                "pipeline_duration_seconds": 350,
...
def setup_alert_endpoints(app):
    ...
    logger.info("Alert endpoints configurados", extra={"component": "AlertManager"})
