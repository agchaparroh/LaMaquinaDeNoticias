...
def main():
    ...
    parser.add_argument(
        "--integration", action="store_true", help="Solo tests de integración"
    )
    ...
    parser.add_argument(
        "--middlewares", action="store_true", help="Solo tests de middlewares"
    )
    parser.add_argument(
        "--error-handling", action="store_true", help="Solo tests de error handling"
    )
    parser.add_argument(
        "--pipelines", action="store_true", help="Solo tests de pipelines"
    )
    ...
    parser.add_argument(
        "--fast", action="store_true", help="Excluir tests marcados como lentos"
    )
    ...
    parser.add_argument(
        "--html", action="store_true", help="Generar reporte HTML de cobertura"
    )
    ...
    if args.coverage:
        cmd.extend(["--cov=scraper_core", "--cov-report=term-missing"])
    ...
    if exit_code == 0:
        print(
            f"{Colors.OKGREEN}{Colors.BOLD}✓ Todos los tests pasaron exitosamente!{Colors.ENDC}"
        )
        ...
        print(
            f"{Colors.FAIL}{Colors.BOLD}✗ Algunos tests fallaron. Revisa los errores arriba.{Colors.ENDC}"
        )
