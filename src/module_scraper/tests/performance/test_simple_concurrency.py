...
from scrapy.http import Request  # noqa: F401
...
class TestSimpleConcurrency:
    ...
    def test_multiple_spiders_basic(self):
        ...
        print(f"\n🔄 Resultados de Concurrencia:")  # noqa: F541
    ...
    def test_concurrent_pipeline_processing(self):
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            ...
            results = [f.result() for f in futures]  # noqa: F841

        print(f"\n⚡ Procesamiento Concurrente:")  # noqa: F541
        print(f"✅ Items procesados: {len(items_processed)}")
        print(f"✅ Sin errores de concurrencia")  # noqa: F541

        # Verificaciones
        assert len(items_processed) == 10, "No todos los items fueron procesados"
        assert all(item is not None for item in items_processed), (
            "Algunos items son None"
        )
