...
def main():
    ...
    cmd = [
        ...
        '-L', 'INFO',
        '-s', 'CLOSESPIDER_ITEMCOUNT=5',  # Solo 5 items para prueba
        '-s', 'DOWNLOAD_DELAY=2',
        '-s', 'ITEM_PIPELINES={}',  # Sin pipelines para prueba inicial
        '-o', 'test_output_v2.json'  # Salida a archivo JSON
    ]
    ...
    try:
        ...
        if os.path.exists('test_output_v2.json'):
            ...
            with open('test_output_v2.json', 'r', encoding='utf-8') as f:
                content = f.read()
