...
from scraper_core.spiders.base.base_article import BaseArticleSpider  # noqa: F401
from scrapy import Spider
from scrapy.http import Request, Response  # noqa: F401
...
class TestGeneratorCompliance:
    ...
    def get_spider_type(self, spider_class: Type[Spider]) -> str:
        ...
        if "rss" in name:
            ...
            if (
                "DOWNLOAD_HANDLERS" in settings
                and "playwright" in str(settings["DOWNLOAD_HANDLERS"]).lower()
            ):
                return "playwright"
            return "scraping"
    ...
    def test_metadata_requirements(self):
        ...
        from tests.test_spiders.test_universal_spider import (  # noqa: F401
            SpiderTestCase,
            TestUniversalSpider,
        )
        ...
        useful_metadata_fields = ["spider_type", "extraction_method"]  # noqa: F841
        ...
        for spider_class in spiders:
            ...
            for i, error_html in enumerate(error_scenarios):
                response = test_helper._create_mock_response(
                    f"https://example.com/test-{i}", error_html, spider
                )
...
class TestSpiderIntegration:
    ...
    def test_complete_extraction_flow(self):
        ...
        from tests.test_spiders.test_universal_spider import (
            SpiderTestCase,
            TestUniversalSpider,
        )
        ...
        spiders = [s for s in test_helper.get_all_spiders() if "test" not in s.name]
        ...
        # Configurar mocks
        with patch(
            "scraper_core.pipelines.validation.DataValidationPipeline.process_item"
        ) as mock_validation, patch(
            "scraper_core.pipelines.cleaning.DataCleaningPipeline.process_item"
        ) as mock_cleaning, patch(
            "scraper_core.pipelines.storage.SupabaseStoragePipeline.process_item"
        ) as mock_storage:
            ...
            for request in requests[:2]:  # Solo procesar los primeros 2
                article_response = test_helper._create_mock_response(
                    request.url, test_case.test_data["sample_article_html"], spider
                )
...
def print_compliance_report(report: Dict[str, Any]):
    ...
    print(
        f"Puntuación: {report['compliance_score']}/{report['max_score']} ({report['compliance_percentage']:.1f}%)"
    )

    if report["passed_checks"]:
        print(f"\n✓ Aspectos Técnicos Correctos:")  # noqa: F541
        for check in report["passed_checks"]:
            print(f"  - {check}")

    if report["issues"]:
        print(f"\n✗ Aspectos a Corregir:")  # noqa: F541
