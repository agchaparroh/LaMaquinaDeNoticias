...
class TestUniversalSpider:
    ...
    def get_all_spiders(self) -> List[Type[Spider]]:
        ...
        for importer, modname, ispkg in pkgutil.walk_packages(
            path=spiders.__path__, prefix=spiders.__name__ + ".", onerror=lambda x: None
        ):
            if ispkg:
                continue
            ...
            try:
                ...
                for name, obj in inspect.getmembers(module, inspect.isclass):
                    if (
                        ...
                        and obj is not BaseArticleSpider
                        and hasattr(obj, "name")
                        and obj.name not in self.EXCLUDED_SPIDERS
                    ):
            ...
            if test_case.test_data["spider_type"] == "rss":
                ...
                if hasattr(spider, "parse_feed"):
                    ...
                    assert len(requests) > 0, (
                        f"{spider.name} no generó requests del feed"
                    )
            else:
                ...
                assert len(requests) > 0, (
                    f"{spider.name} no generó requests de la sección"
                )
    ...
    def test_spider_section_filtering(self):
        ...
        for spider_class in spiders:
            ...
            if hasattr(spider, "_is_section_article"):
                # URLs de prueba
                section_url = (
                    f"https://example.com/{spider.target_section}/articulo-test"
                )
        ...
        for spider_class in spiders:
            ...
            if hasattr(spider, "parse_article"):
                ...
                if item:
                    ...
                    assert (
                        "spider_type" in metadata or "extraction_method" in metadata
    ...
    def test_spider_error_handling(self):
        ...
        for spider_class in spiders:
            ...
            empty_response = self._create_mock_response(
                "https://example.com/empty", "", spider
            )
            ...
            try:
                ...
                pytest.fail(
                    f"{spider.name} no maneja correctamente responses vacíos: {e}"
                )
...
class TestRSSSpiders:
    ...
    def test_rss_spider_handles_feed_parsing(self):
        ...
        spiders = [
            s
            for s in TestUniversalSpider().get_all_spiders()
