import gzip  # For decompressing HTML  # noqa: F401
import os
import unittest
...
from scraper_core.utils.supabase_client import (
    SupabaseClient,  # Direct import for test client
)
from scrapy.crawler import Crawler  # For spec in MagicMock
from scrapy.settings import Settings  # noqa: F401
from scrapy.spiders import Spider  # noqa: F401
...
if os.path.exists(test_env_path):
    ...
    print(
        "Warning: .env.test not found, ensure test Supabase credentials are set in environment"
    )
...
class TestSupabaseIntegration(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        try:
            ...
            bucket_exists = any(
                bucket.get("name") == cls.test_bucket_name for bucket in buckets
            )
            ...
            print(
                f"Warning: Could not ensure test bucket '{cls.test_bucket_name}' exists: {e}"
            )
    ...
    def tearDownClass(cls):
        ...
        print(f"--- tearDownClass: Starting cleanup for Supabase integration tests ---")  # noqa: F541
        if hasattr(cls, "supabase_client") and hasattr(cls, "test_bucket_name"):
            try:
                ...
                delete_response = (
                    cls.supabase_client.table("articulos")
                    .delete()
                    .eq("medio", "Test Medio Integration")
                    .execute()
                )
                print(
                    f"Cleanup: Attempted to delete test articles. Response: {hasattr(delete_response, 'data') and delete_response.data or 'No data in response'}"
                )

                # List and delete files from the test bucket
                try:
                    # List all files in the bucket
                    files_in_bucket = cls.supabase_client.storage.from_(
                        cls.test_bucket_name
                    ).list()
                    if files_in_bucket:
                        # Extract file names/paths
                        file_paths_to_delete = [
                            f["name"] for f in files_in_bucket if "name" in f
                        ]
                        if file_paths_to_delete:
                            # Delete files in batches
                            for file_path in file_paths_to_delete:
                                try:
                                    cls.supabase_client.storage.from_(
                                        cls.test_bucket_name
                                    ).remove([file_path])
                                except Exception as e:
                                    print(f"Failed to delete file {file_path}: {e}")
                            print(
                                f"Cleanup: Deleted {len(file_paths_to_delete)} files from bucket '{cls.test_bucket_name}'."
                            )
                except Exception as e:
                ...
                print(
                    f"Error during tearDownClass cleanup: {e}. Manual cleanup of test data may be required."
                )
        print(f"--- tearDownClass: Finished cleanup ---")  # noqa: F541
    ...
    def setUp(self):
        ...
        self.test_article_url = (
            f"http://example.com/test-article-integration-{uuid.uuid4()}"
        )
    ...
    def _get_sample_articulo_item(self):
        ...
        item["contenido_html"] = (
            "<html><body><h1>Test HTML Content for Integration</h1></body></html>"
        )
    ...
    def test_process_item_success_scenario(self):
        ...
        self.assertIsNotNone(
            self.pipeline.supabase_client,
            "Supabase client in pipeline should not be None",
        )

        adapter = self.pipeline.process_item(item, spider=None)

        self.assertIsNotNone(
            adapter.get("storage_path"), "storage_path should be set after processing."
        )
        ...
        self.assertNotIn(
            "error_detalle",
            adapter,
            "No error should be present for successful processing.",
        )

        try:
            # 1. Verify article metadata in 'articulos' table
            db_response = (
                self.supabase_client.table("articulos")
                .select("*")
                .eq("url", item["url"])
                .execute()
            )
            self.assertTrue(
                ...
                f"Article with URL {item['url']} not found in DB.",
            )
            article_from_db = db_response.data[0]
            self.assertEqual(article_from_db["titular"], item["titular"])
            self.assertEqual(
                article_from_db["storage_path"], adapter.get("storage_path")
            )
            ...
            file_bytes = self.supabase_client.storage.from_(
                self.test_bucket_name
            ).download(storage_path)
            self.assertIsNotNone(
                file_bytes,
                f"File {storage_path} not found in bucket {self.test_bucket_name}.",
            )
    ...
    def test_process_item_missing_required_fields(self):
        ...
        try:
            ...
            self.assertIn(
                "error_detalle",
                adapter,
                "Item with missing fields should have error_detalle",
            )
        except Exception as e:
    ...
    def test_process_item_database_error(self, mock_upsert):
        ...
        self.assertIn(
            "error_detalle",
            adapter,
            "Database error should be captured in error_detalle",
        )
    ...
    def test_process_item_storage_error(self, mock_upload):
        ...
        self.assertIn(
            "error_detalle",
            adapter,
            "Storage error should be captured in error_detalle",
        )
    ...
    def test_duplicate_article_handling(self):
        ...
        self.assertEqual(
            len(db_response.data), 1, "Should only have one record for duplicate URL"
        )
