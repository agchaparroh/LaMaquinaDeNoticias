...
def mock_crawler_fixture(tmp_path):
    ...
    settings["VALIDATION_REQUIRED_FIELDS"] = [
        "url",
        "titular",
        "medio",
        "fecha_publicacion",
    ...
    settings["VALIDATION_DATE_FORMATS"] = [
        "%Y-%m-%dT%H:%M:%S.%fZ",
        "%Y-%m-%dT%H:%M:%SZ",
        "%Y-%m-%d %H:%M:%S",
    ]
...
def test_valid_item_full_pipeline_flow(mock_crawler_fixture, valid_item_data_fixture):
    ...
    processed_item_validated = validation_pipeline.process_item(
        processed_item_cleaned, spider
    )
    assert processed_item_validated is not None
    assert "error_validation" not in processed_item_validated, (
        f"Valid item marked with error: {processed_item_validated.get('error_validation')}"
    )
    ...
    mock_upsert_execute_result.data = [
        {"id_primary": "mock-db-id-456", "url": item["url"]}
    ]
    mock_supabase_client.table.return_value.upsert.return_value.execute = MagicMock(
        return_value=mock_upsert_execute_result
    )
    ...
    with patch.object(
        storage_pipeline, "_generate_url_hash", return_value=url_hash_mock
    ), patch.object(
        storage_pipeline,
        "_upload_html_content_with_retry",
        return_value=expected_storage_path,
    ) as mock_upload_html:
    ...
    assert (
        upload_args[0] == url_hash_mock
    )  # Check the key used for upload (the mocked hash)
...
def dirty_item_data_fixture():
    ...
    for req_field in [
        ...
        if (
            req_field not in data or not data[req_field]
        ):  # Check if field is missing or empty
            data[req_field] = f"Default value for {req_field}"  # Add default if missing
    if not data["titular"].strip():
        data["titular"] = "Default Dirty Title Test"  # noqa: E701
    if not data["medio"].strip():
        data["medio"] = "Default Dirty Medio Test"  # noqa: E701
    return data


def test_dirty_item_cleaning_and_validation_flow(
    mock_crawler_fixture, dirty_item_data_fixture
):
    spider = mock_crawler_fixture.spider
    ...
    mock_crawler_fixture.settings["CLEANING_STRIP_HTML_FIELDS"] = [
        "titular",
        "contenido_texto",
        "medio",
        "seccion",
    ]
    mock_crawler_fixture.settings["CLEANING_NORMALIZE_WHITESPACE_FIELDS"] = [
        "titular",
        "contenido_texto",
        "medio",
        "seccion",
    ...
    mock_crawler_fixture.settings["CLEANING_NORMALIZE_LIST_STR_FIELDS"] = [
        "etiquetas_fuente"
    ]
    ...
    cleaning_pipeline.open_spider(
        spider
    )  # Ensure settings are loaded if pipeline uses open_spider

    processed_item_cleaned = cleaning_pipeline.process_item(item, spider)
    assert processed_item_cleaned is not None

    assert processed_item_cleaned["titular"] == "A Dirty Title Test", (
        f"Titular cleaning failed: got '{processed_item_cleaned['titular']}'"
    )
    assert processed_item_cleaned["medio"] == "Test Dirty Medio Test", (
        f"Medio cleaning failed: got '{processed_item_cleaned['medio']}'"
    )
    assert "<b>" not in processed_item_cleaned["contenido_texto"], (
        "HTML <b> tag not stripped"
    )
    assert "  " not in processed_item_cleaned["contenido_texto"], (
        "Extra spaces not normalized"
    )
    assert processed_item_cleaned["etiquetas_fuente"] == ["dirty_test", "test_tag"], (
        f"etiquetas_fuente cleaning failed: got '{processed_item_cleaned['etiquetas_fuente']}'"
    )
    ...
    assert processed_item_validated is not None, (
        "Validation pipeline dropped cleaned item"
    )
    assert "error_validation" not in processed_item_validated, (
        f"Cleaned item marked with validation error: {processed_item_validated.get('error_validation')}"
    )
    ...
    mock_upsert_result.data = [
        {"id_primary": "mock-dirty-id-101", "url": processed_item_validated["url"]}
    ]
    mock_supabase_client.table.return_value.upsert.return_value.execute = MagicMock(
    ...
    assert (
        upsert_call_args["titular"] == "A Dirty Title Test"
    )  # Check cleaned data sent to DB
...
def test_item_invalid_data_fails_validation(
    mock_crawler_fixture, item_invalid_url_and_missing_titular_fixture
):  # Use new fixture
    spider = mock_crawler_fixture.spider
    item = ArticuloInItem(
        item_invalid_url_and_missing_titular_fixture
    )  # Use new fixture
    ...
    error_summary_validation = " ".join(
        processed_item_validated["error_validation"]
    ).lower()

    assert "titular" in error_summary_validation, (
        "Validation error message does not mention missing 'titular'"
    )
    assert "url" in error_summary_validation and (
        "format" in error_summary_validation
        or "valid" in error_summary_validation
        or "scheme" in error_summary_validation
    ...
    with patch.object(
        storage_pipeline, "_upload_html_content_with_retry"
    ) as mock_upload_html:
        final_item = storage_pipeline.process_item(processed_item_validated, spider)
...
def test_storage_pipeline_handles_upsert_error_gracefully(
    mock_crawler_fixture, valid_item_data_fixture
):
    """
    ...
    assert "error_validation" not in item_validated, (
        "Item unexpectedly failed validation before storage test"
    )
    ...
    with patch.object(storage_pipeline, "logger") as mock_pipeline_logger, patch.object(
        storage_pipeline,
        "_upload_html_content_with_retry",
        return_value="mock/path.html",
    ) as mock_upload_html:
        processed_item_storage = storage_pipeline.process_item(item_validated, spider)

    assert processed_item_storage is not None, (
        "Storage pipeline dropped item on upsert error"
    )
    assert "error_detalle" in processed_item_storage, (
        "Item does not have 'error_detalle' after upsert failure"
    )
    assert "Simulated DB Upsert Error" in processed_item_storage["error_detalle"], (
        "Error message not correctly set"
    )
    ...
    assert str(simulated_db_error) in str(
        log_args[2]
    )  # Check if exception is in log context
    ...
    # Check stats
    actual_stat_value = mock_crawler_fixture.stats.get_value(
        "supabase/storage_errors", 0
    )
    assert actual_stat_value == 1, (
        f"Supabase storage error stat incorrect: expected 1, got {actual_stat_value}"
    )

    if hasattr(cleaning_pipeline, "close_spider"):
        cleaning_pipeline.close_spider(spider)  # noqa: E701
    validation_pipeline.close_spider(spider)
    storage_pipeline.close_spider(spider)


def test_storage_pipeline_handles_html_upload_error_gracefully(
    mock_crawler_fixture, valid_item_data_fixture
):
    """
    ...
    with patch.object(storage_pipeline, "logger") as mock_pipeline_logger, patch.object(
        storage_pipeline,
        "_upload_html_content_with_retry",
        side_effect=simulated_html_error,
    ) as mock_upload_call:
    ...
    assert not processed_item_storage.get("storage_path"), (
        "storage_path was set despite HTML upload error"
    )
    ...
    assert str(simulated_html_error) in str(
        log_args[2]
    )  # Check exception in log context
    ...
    assert actual_stat_value == 1, (
        f"Supabase storage error stat for HTML incorrect: expected 1, got {actual_stat_value}"
    )
