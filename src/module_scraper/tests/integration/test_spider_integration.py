...
class SimpleHtmlSpider(Spider):
    ...
    def parse(self, response):
        self.logger.info(
            f"SimpleHtmlSpider: Parsed URL: {response.url}, Status: {response.status}"
        )
...
async def test_simple_html_site_flow():
    ...
    assert stats.get("simple_html_parse_called", False) is True, (
        "Spider's parse method was not called"
    )
    assert stats.get("simple_html_items_scraped", 0) == 1, (
        "Did not scrape the expected number of items"
    )
    assert len(spider.items_scraped) == 1, "Spider did not collect the item"
    assert (
        spider.items_scraped[0]["title"] == "Herman Melville - Moby-Dick; or, The Whale"
    ), "Scraped title is incorrect"
    ...
    assert stats.get("finish_reason") == "finished", (
        "Spider did not finish successfully"
    )
...
class JavaScriptSiteSpider(Spider):
    ...
    async def parse(self, response):
        self.logger.info(
            f"JavaScriptSiteSpider: Parsed URL: {response.url}, Status: {response.status}"
        )
        ...
        try:
            ...
            self.logger.info(
                f"HTML content when selector failed: {await page.content()}"
            )

        quotes = response.xpath('//div[@class="quote"]')
        assert len(quotes) > 0, (
            "No quotes found. JS might not have executed or page structure changed."
        )
        ...
        self.crawler.stats.set_value(
            "js_site_quotes_scraped_count", len(self.quotes_scraped)
        )
...
async def test_javascript_site_playwright_activation():
    ...
    assert stats.get("js_site_parse_called", False) is True, (
        "Spider's parse method was not called"
    )
    assert stats.get("js_site_quotes_scraped_count", 0) > 0, (
        "No quotes were scraped from JS site"
    )
...
class RobotsTxtSpider(Spider):
    ...
    def parse(self, response):
        ...
        self.logger.error(
            f"RobotsTxtSpider: Parse was called for {response.url}, which should be disallowed!"
        )
    ...
    def error_handler(self, failure):
        ...
        self.logger.info(
            f"RobotsTxtSpider: Error handler caught failure: {failure.getErrorMessage()}"
        )
...
async def test_robots_txt_blocking():
    ...
    assert not spider.parse_called, (
        "Spider's parse method was called for a disallowed URL"
    )
    ...
    )  # At least the robots.txt request
    ...
    assert (
        stats.get("downloader/response_count", 0) <= 1
    )  # Should only be response for robots.txt, not for /deny
    ...
    assert stats.get("robotstxt/forbidden_count", 0) == 1, (
        "RobotsTxtMiddleware did not record a forbidden URL"
    )
    assert stats.get("finish_reason") == "finished"
...
class SpiderCrawlOnceTestSpider(Spider):
    ...
    def start_requests(self):
        ...
        yield Request(
            self.test_url, callback=self.parse, dont_filter=False
        )  # dont_filter=False is default
        yield Request(
            self.test_url, callback=self.parse, dont_filter=False
...
async def test_spider_crawl_once_prevention():
    ...
    crawl_once_db_path_str = settings.get(
        "CRAWL_ONCE_PATH", "tests/.scrapy_test/crawl_once"
    )  # Fallback if not in test_settings
    ...
    assert spider.parse_call_count == 1, (
        "Parse method called more than once for the same URL with CrawlOnce enabled"
    )
    assert len(spider.urls_parsed) == 1, "URL list contains more than one entry"
    assert (
        spider.urls_parsed[0] == SpiderCrawlOnceTestSpider.test_url
        if spider.urls_parsed
        else False
    )
    ...
    assert (
        stats.get("crawl_once/ignored", 0) == 1 or stats.get("offsite/filtered", 0) == 1
    )  # Check common stat keys
    ...
    assert (
        stats.get("downloader/response_count", 0) == 1
    )  # Only one actual download & response to spider
...
class NetworkErrorSpider(Spider):
    ...
    def parse(self, response):
        ...
        self.logger.info(
            f"NetworkErrorSpider: Parse called for {response.url} (UNEXPECTED for network error)"
        )
        self.parse_called = True
    ...
    def errback_default(self, failure):
        ...
        self.logger.info(
            f"NetworkErrorSpider: Default errback caught failure for {failure.request.url if failure.request else 'N/A'}: {failure.getErrorMessage()}"
        )
...
async def test_spider_network_error_handling():
    ...
    assert not spider.parse_called, (
        "Parse method was called despite expected network errors"
    )
    assert stats.get("network_error_test_parse_called_unexpectedly", 0) == 0

    assert len(spider.errors_received) == 1, (
        "Spider's errback was not called or called multiple times for one URL"
    )
    ...
    for stat_key in stats.keys():
        if "retry/reason_count" in stat_key and dns_error_key_part in stat_key:
            assert stats[stat_key] == settings.getint(
                "RETRY_TIMES", 2
            )  # Each retry for this reason
            found_dns_retry_reason = True
            break
    assert found_dns_retry_reason, (
        f"Retry reason for DNSLookupError not found or count incorrect in stats: {stats}"
    )
