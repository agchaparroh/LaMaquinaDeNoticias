...
class PlaywrightSpider(Spider):
    ...
    async def parse(self, response):
        ...
        assert "HeadlessChrome" in user_agent or "Playwright" in user_agent, (
            "User-Agent should indicate Playwright usage"
        )
...
async def test_crawl_once_duplicate_detection():
    ...
    )  # Fallback for older versions or different stat names
    ...
    )  # Only one response received and processed by spider
...
async def test_user_agent_rotation():
    ...
    assert stats.get("user_agent_parse_count", 0) == len(
        UserAgentRotationSpider.start_urls
    )
