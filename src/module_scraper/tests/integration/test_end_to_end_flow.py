import json  # For metadata if needed  # noqa: F401
...
class DataIntegritySpider(Spider):
    ...
    def parse(self, response):
        ...
        raw_text_parts = response.xpath(
            "//p/text() | //body/text()[normalize-space() and not(parent::h1) and not(parent::title)]"
        ).getall()
...
def reset_e2e_global_mocks(request):
    ...
    global \
        mock_supabase_upsert_args, \
        mock_supabase_upload_args, \
        mock_item_after_storage_pipeline
    mock_supabase_upsert_args = None
    mock_supabase_upload_args = None
    mock_item_after_storage_pipeline = None
...
@pytest.mark.asyncio
async def test_data_integrity_e2e_flow(
    tmp_path, reset_e2e_global_mocks
):  # Added reset_e2e_global_mocks for clarity, though autouse=True means it runs anyway
    """
    ...
    with patch.object(
        SupabaseStoragePipeline, "client", new_callable=MagicMock
    ) as mock_client_instance, patch.object(
        SupabaseStoragePipeline,
        "_upload_html_content_with_retry",
        side_effect=capture_upload_and_return_path,
    ) as mock_upload_method, patch.object(
        SupabaseStoragePipeline, "process_item", side_effect=capture_item_after_storage
    ) as mock_process_item_storage:  # noqa: F841
    ...
    assert mock_supabase_upsert_args is not None, (
        "Mock for upsert was not called or args not captured."
    )
    assert mock_supabase_upload_args is not None, (
        "Mock for HTML upload was not called or args not captured."
    )
    assert mock_item_after_storage_pipeline is not None, (
        "Mock for storage pipeline process_item was not called or item not captured."
    )

    # --- Start of assertions ---
    assert mock_supabase_upsert_args is not None, (
        "Supabase upsert was not called (args not captured)."
    )
    assert mock_supabase_upload_args is not None, (
        "Supabase upload HTML was not called (args not captured)."
    )
    assert mock_item_after_storage_pipeline is not None, (
        "Storage pipeline did not process/return an item (or capture failed)."
    )
    ...
    assert "Herman Melville - Moby-Dick" in mock_supabase_upsert_args.get(
        "contenido_texto"
    ), "Key content missing from cleaned contenido_texto"
    assert mock_supabase_upsert_args.get("contenido_texto").startswith(
        "Herman Melville"
    ...
    assert (
        "<h1>Herman Melville - Moby-Dick; or, The Whale</h1>"
        in mock_supabase_upload_args.get("html_content")
    ), "Key H1 tag missing from HTML content intended for storage."
    assert (
        mock_supabase_upload_args.get("item_url") == DataIntegritySpider.start_urls[0]
    ...
    assert stats.get("downloader/request_count") == 1, (
        f"Expected 1 request, got {stats.get('downloader/request_count')}"
    )  # Assumes no redirects or retries for httpbin.org/html
...
class PlaywrightMetadataSpider(Spider):
    ...
    async def parse(self, response):
        ...
        try:
            ...
            item["metadata"] = {
                "playwright_info": "div.quote not found",
                "quote_count": 0,
            }
        else:
            quotes_count = len(await page.query_selector_all("div.quote"))
            item["metadata"] = {
                "playwright_info": "processed_with_playwright",
        ...
        item["contenido_texto"] = (
            f"Page content from {response.url} processed by Playwright. Title: {item['titular']}. Quotes found: {item['metadata'].get('quote_count', 0)}"
        )
...
async def test_playwright_metadata_e2e_flow(tmp_path, reset_e2e_global_mocks):
    ...
    def capture_upsert_pw(data_dict_for_db, **kwargs):
        ...
        mock_execute_response = MagicMock()
        # Ensure the 'url' in the response matches the item's URL if possible, or is a valid string
        url_in_item = data_dict_for_db.get(
            "url", "http://mockurl.com/default"
        )  # Use a default if 'url' isn't in data_dict_for_db
        mock_execute_response.data = [
            {"id_primary": "mock_pw_e2e_id_456", "url": url_in_item}
        ]
        return mock_execute_response
    ...
    def capture_upload_pw(url_hash_key, html_to_upload, item_url_for_log):
        ...
        year = datetime.now(timezone.utc).year
        month = datetime.now(timezone.utc).month  # noqa: E702
        return f"html_content/{year}/{month:02d}/{url_hash_key}.html"
    ...
    with patch.object(
        SupabaseStoragePipeline, "client", new_callable=MagicMock
    ) as mock_client_pw, patch.object(
        SupabaseStoragePipeline,
        "_upload_html_content_with_retry",
        side_effect=capture_upload_pw,
    ) as mock_upload_pw, patch.object(
        SupabaseStoragePipeline,
        "process_item",
        side_effect=capture_item_after_storage_pw,
    ) as mock_process_item_pw:  # noqa: F841
        # Corrected mock setup for the client:
        # The side_effect should be on the 'execute' method, not the return_value of execute.
        mock_client_pw.table.return_value.upsert.return_value.execute.side_effect = (
            capture_upsert_pw
        )
    ...
    assert mock_supabase_upsert_args is not None, (
        "Upsert not called for Playwright item"
    )
    assert mock_supabase_upload_args is not None, (
        "HTML upload not called for Playwright item"
    )
    assert mock_item_after_storage_pipeline is not None, (
        "Storage pipeline process_item not called for Playwright item"
    )
    ...
    assert (
        mock_supabase_upsert_args.get("url") == PlaywrightMetadataSpider.start_urls[0]
    )
...
class ErrorRecoverySpider(Spider):
    ...
    def parse(self, response):
        ...
        item["metadata"] = {
            ...
            "original_url_for_item": response.url,
        }
...
async def test_error_recovery_e2e_flow(tmp_path, reset_e2e_global_mocks):
    ...
    def capture_upsert_recovery(data_dict_for_db, **kwargs):
        ...
        execute_mock.data = [
            {"id_primary": mock_id, "url": url_in_item}
        ]  # Corrected: set data on the mock object directly
        return execute_mock
    ...
    with patch.object(
        SupabaseStoragePipeline, "client", new_callable=MagicMock
    ) as mock_client_recovery, patch.object(
        SupabaseStoragePipeline,
        "_upload_html_content_with_retry",
        side_effect=capture_upload_recovery,
    ) as mock_upload_recovery, patch.object(
        SupabaseStoragePipeline,
        "process_item",
        side_effect=capture_item_after_storage_recovery,
    ) as mock_process_item_recovery:  # noqa: F841
    ...
    assert len(captured_upserts) == 2, (
        f"Expected 2 items to be upserted, got {len(captured_upserts)}"
    )
    assert len(captured_uploads) == 2, (
        f"Expected 2 HTML contents to be uploaded, got {len(captured_uploads)}"
    )
    assert len(captured_final_items) == 2, (
        f"Expected 2 items from storage pipeline, got {len(captured_final_items)}"
    )

    # Sort items by URL to make assertions predictable if order isn't guaranteed
    captured_upserts.sort(key=lambda x: x.get("url", ""))
    captured_uploads.sort(
        key=lambda x: x.get("item_url", "")
    )  # Assuming item_url is the original URL
    captured_final_items.sort(key=lambda x: x.get("url", ""))

    # Item 1 (from url_always_ok: http://httpbin.org/html)
    item1_upsert = next(
        u for u in captured_upserts if u["url"] == ErrorRecoverySpider.url_always_ok
    )
    item1_upload = next(
        u
        for u in captured_uploads
        if u["item_url"] == ErrorRecoverySpider.url_always_ok
    )
    item1_final = next(
        i for i in captured_final_items if i["url"] == ErrorRecoverySpider.url_always_ok
    )

    assert "Herman Melville - Moby-Dick" in item1_upsert.get("titular")
    assert "Herman Melville - Moby-Dick" in item1_upload.get(
        "html_content"
    )  # Check raw HTML
    ...
    item2_upsert = next(
        u for u in captured_upserts if u["url"] == ErrorRecoverySpider.url_retry_once
    )
    item2_upload = next(
        u
        for u in captured_uploads
        if u["item_url"] == ErrorRecoverySpider.url_retry_once
    )
    item2_final = next(
        i
        for i in captured_final_items
        if i["url"] == ErrorRecoverySpider.url_retry_once
    )
    ...
    assert ErrorRecoverySpider.url_retry_once in item2_final.get(
        "storage_path", ""
    )  # Path might not contain full URL
