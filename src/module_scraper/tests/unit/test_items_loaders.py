...
import unittest
from datetime import date, datetime, timedelta  # Added timedelta, date  # noqa: F401
...
class TestProcessorFunctions(unittest.TestCase):
    ...
    def test_normalize_date(self):
        ...
        # ISO format
        date = normalize_date("2024-01-15T10:30:00Z")  # noqa: F811
        ...
        self.assertIsNone(
            normalize_date("ayer por la tarde")
    ...
    def test_normalize_date_relative_ayer(self, mock_datetime):
        ...
        expected_date = (self.MOCKED_NOW - timedelta(days=1)).replace(
    ...
    def test_normalize_date_relative_hace_dias(self, mock_datetime):
        ...
        # Test "hace 1 día"
        expected_date_1_day = (self.MOCKED_NOW - timedelta(days=1)).replace(
            hour=0, minute=0, second=0, microsecond=0
        )
        self.assertEqual(normalize_date("hace 1 día"), expected_date_1_day)
        self.assertEqual(
            normalize_date("hace 1 dia"), expected_date_1_day
        )  # without accent

        # Test "hace 5 días"
        expected_date_5_days = (self.MOCKED_NOW - timedelta(days=5)).replace(
            hour=0, minute=0, second=0, microsecond=0
        )
        self.assertEqual(normalize_date("hace 5 días"), expected_date_5_days)
        self.assertEqual(
            normalize_date("HACE 5 DIAS"), expected_date_5_days
        )  # uppercase, no accent, plural 's'
        self.assertEqual(
            normalize_date("hace 10 dias"),
            (self.MOCKED_NOW - timedelta(days=10)).replace(
                hour=0, minute=0, second=0, microsecond=0
            ),
        )
    ...
    def test_normalize_date_relative_hace_horas(self, mock_datetime):
        ...
        self.assertEqual(
            normalize_date("HACE 10 HORAS"), self.MOCKED_NOW - timedelta(hours=10)
        )
    ...
    def test_normalize_date_relative_hace_minutos(self, mock_datetime):
        ...
        self.assertEqual(
            normalize_date("HACE 30 MINUTOS"), self.MOCKED_NOW - timedelta(minutes=30)
        )
    ...
    def test_normalize_date_sets_midnight_for_standard_formats_without_time(self):
        ...
        self.assertEqual(
            dt_date_only_es.hour, 0
        )  # This depends on strptime default, usually 00:00:00
        self.assertEqual(dt_date_only_es.minute, 0)
        self.assertEqual(dt_date_only_es.second, 0)

        dt_date_only_text = normalize_date(
            "15 de Enero de 2024"
        )  # Assuming locale independent for month
    ...
    def test_generate_storage_path(self):
        ...
        item_dict = {"medio": "ABC", "titular": "Another Test"}
        path = generate_storage_path(item_dict)
...
class TestArticuloInItemLoader(unittest.TestCase):
    ...
    def test_storage_path_generation(self):
        ...
        self.assertIsNotNone(item["storage_path"])
        self.assertRegex(
            item["storage_path"], r"^[^/]+/\d{4}/\d{2}/\d{2}/[^/]+\.html\.gz$"
        )
