...
class TestSupabaseClient(unittest.TestCase):
    ...
    @patch.dict(
        os.environ,
        {"SUPABASE_URL": "http://test.supabase.co", "SUPABASE_KEY": "test_key"},
    )
    ...
    def test_initialization_success(self, mock_create_client):
        ...
        mock_create_client.assert_called_once_with(
            "http://test.supabase.co", "test_key"
        )
        self.assertIsNotNone(
            client.client, "Supabase internal client should be initialized."
        )
    ...
    def test_initialization_failure_missing_env_vars(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "Supabase URL and Key not found in environment variables."
        ):
            SupabaseClient()
    ...
    def test_get_client_not_initialized(self):
        ...
        with patch.dict(os.environ, {}, clear=True):
            ...
            sc_instance = SupabaseClient.__new__(
                SupabaseClient
            )  # Create instance without calling __init__
            sc_instance.client = None  # Ensure client is None
            sc_instance.logger = (
                MagicMock()
            )  # Mock logger to avoid errors on logger calls
    ...
    def test_upload_file_with_local_path_success(self, mock_create_client):
        ...
        mock_supabase_internal_client.storage.from_().upload.return_value = (
        ...
        with patch(
            ...
            result = client.upload_file(
                "test_bucket", "test/file.txt", local_file_path="dummy/path.txt"
            )
    ...
    def test_upload_file_with_file_content_success(self, mock_create_client):
        ...
        client.client.storage.from_("test_bucket").upload.assert_called_once_with(
            path="test/content.txt", file=file_bytes, file_options=file_options
        )
    ...
    def test_upload_file_no_source_provided(self, mock_create_client):
        ...
        with patch.object(client.logger, "error") as mock_logger_error:
            ...
            mock_logger_error.assert_called_with(
                "Either local_file_path or file_content must be provided for upload."
            )
    ...
    def test_upload_file_storage_exception(self, mock_create_client):
        ...
        mock_supabase_internal_client = MagicMock()
        mock_supabase_internal_client.storage.from_().upload.side_effect = Exception(
            "Storage error"
        )
        mock_create_client.return_value = mock_supabase_internal_client
        ...
        with patch.object(client.logger, "error") as mock_logger_error:
            result = client.upload_file(
                "test_bucket", "test/path.txt", file_content=b"data"
            )
            self.assertIsNone(result)
            mock_logger_error.assert_called_with(
                "Error uploading file to test_bucket/test/path.txt: Storage error"
            )
    ...
    def test_insert_data_success(self, mock_create_client):
        ...
        mock_supabase_internal_client = MagicMock()
        mock_supabase_internal_client.table().insert().execute.return_value = (
    ...
    def test_insert_data_no_data_provided(self, mock_create_client):
        ...
        with patch.object(client.logger, "warning") as mock_logger_warning:
            ...
            mock_logger_warning.assert_called_with(
                "No data provided for insertion into table test_table."
            )
    ...
    def test_insert_data_api_exception(self, mock_create_client):
        ...
        mock_supabase_internal_client = MagicMock()
        mock_supabase_internal_client.table().insert().execute.side_effect = Exception(
            "API error"
        )
        mock_create_client.return_value = mock_supabase_internal_client
        ...
        with patch.object(client.logger, "error") as mock_logger_error:
            ...
            self.assertIsNone(result)
            mock_logger_error.assert_called_with(
                "Error inserting data into test_table: API error"
            )
