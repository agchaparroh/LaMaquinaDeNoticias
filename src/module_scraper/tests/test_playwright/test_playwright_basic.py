...
class TestPlaywrightBasicFunctionality(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with patch("scraper_core.middlewares.playwright_custom_middleware.load_object"):
            self.middleware = PlaywrightCustomDownloaderMiddleware.from_crawler(
                self.crawler
            )
    ...
    def test_empty_content_detection(self):
        ...
        response = HtmlResponse(
            "http://test.com", body=b"<html><body>Loading...</body></html>"
        )
        self.assertTrue(self.middleware._is_content_empty(response, spider))
        ...
        response = HtmlResponse(
            "http://test.com", body=f"<html><body>{content}</body></html>".encode()
        )
    ...
    def test_playwright_error_detection(self):
        ...
        self.assertEqual(
            self.middleware._detect_playwright_error(response, spider), "timeout"
        )

        # Browser error
        response = HtmlResponse("http://test.com", body=b"playwright error occurred")
        self.assertEqual(
            self.middleware._detect_playwright_error(response, spider), "browser_error"
        )
    ...
    async def test_successful_response_passthrough(self):
        ...
        good_response = HtmlResponse(
            "http://test.com", body=f"<html><body>{good_content}</body></html>".encode()
        )
    ...
    def test_fallback_request_creation(self):
        ...
        fallback = self.middleware._create_fallback_request(
            request, spider, "test_reason"
        )
