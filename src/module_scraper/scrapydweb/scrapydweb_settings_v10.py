...
if not os.path.exists('/.dockerenv'):
    for directory in [DATA_PATH, SCRAPYDWEB_LOGS_DIR, SCRAPYDWEB_DATA_DIR, 
                      SCRAPYDWEB_DATABASE_DIR, SCRAPYDWEB_JOBS_DIR]:
...
LOCAL_SCRAPYD_LOGS_DIR = os.environ.get('SCRAPYDWEB_LOCAL_SCRAPYD_LOGS_DIR', 
                                        os.path.join(DATA_PATH, 'scrapyd_logs'))
...
TO_ADDRS = os.getenv('SMTP_TO', 'alertas@lamaquina.com').split(',') if os.getenv('SMTP_TO') else []
...
ENABLE_TELEGRAM_ALERT = os.environ.get('ENABLE_TELEGRAM_ALERT', 'False').lower() == 'true'
...
DASHBOARD_API_URL = os.getenv('DASHBOARD_API_URL', 'http://module_dashboard_review_backend:8004')
