...
class LoggingExampleSpider(BaseArticleSpider):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.logger.info(
            f"Spider {self.name} initialized with settings: {self.custom_settings}"
        )
    ...
    @log_exceptions(log_level=logging.ERROR)  # noqa: F821
    def parse_article(self, response: Response):
        ...
        if article_number == 3:
            ...
            self.logger.critical(
                ...
                extra={
                    "url": response.url,
                    "selector_results": loader.get_collected_values("titular"),
                },
            )
    ...
    def closed(self, reason):
        ...
        success_rate = (
            (len(self.successful_urls) / total_urls * 100) if total_urls > 0 else 0
        )
    ...
    def handle_error(self, failure):
        ...
        if failure.check(scrapy.exceptions.IgnoreRequest):
            error_type = "ignored"
            log_level = logging.INFO  # noqa: F821
        elif failure.check(scrapy.exceptions.NotSupported):
            error_type = "not_supported"
            log_level = logging.WARNING  # noqa: F821
        else:
            error_type = "general_failure"
            log_level = logging.ERROR  # noqa: F821
        ...
        self.logger.log(
            log_level, f"Request failed ({error_type}): {request.url} - {failure.value}"
        )
...
class LoggingExamplePipeline:
    ...
    def __init__(self):
        self.logger = logging.getLogger(__name__)  # noqa: F821
        self.structured_logger = StructuredLogger(__name__)
        self.processed_count = 0

    @log_execution_time
    @log_item_processing  # noqa: F821
