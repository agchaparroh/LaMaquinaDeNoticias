...
class RateLimitTestSpider(Spider):
    ...
    def handle_error(self, failure):
        ...
        url = request.url
        domain = urlparse(url).netloc  # noqa: F841

        logger.error(f"Request failed for {url}: {failure.value}")
    ...
    def closed(self, reason):
        ...
        for domain, times in self.request_times.items():
            if len(times) > 1:
                ...
                logger.info(
                    f"{domain}: Requests={len(times)}, Avg delay={avg_delay:.2f}s"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--domains", nargs="+", help="Specific domains to test (e.g., bbc.com cnn.com)"
    )
    parser.add_argument(
        "--show-config",
        action="store_true",
