...
class OptimizedSpiderMixin:
    ...
    def spider_closed(self, spider):
        ...
        self.logger.info(
            f"   📡 Requests: {total_requests} ({requests_per_second:.1f}/s)"
        )
    ...
    def handle_error_optimized(self, failure):
        ...
        if error_type == "temporary":
            self.logger.warning(
                f"⚠️  Temporary error for {request.url}: {failure.value}"
            )
    ...
    def _log_performance_update(self):
        ...
        self.logger.info(
            f"📈 Performance update: {rate:.1f} req/s, "
...
class OptimizedArticleSpider(scrapy.Spider, OptimizedSpiderMixin, LoggerMixin):
    ...
    def parse(self, response: Response) -> Iterator[Request]:
        ...
        unique_links = list(
            dict.fromkeys(article_links)
        )  # Remover duplicados preservando orden
        valid_links = [
            link for link in unique_links if self._is_valid_article_link(link)
        ]
        ...
        try:
            ...
            if self._validate_item_fast(item):
                ...
                if processing_time > 2:  # Más de 2 segundos es lento
                    ...
                    self.logger.warning(
                        f"⏱️  Slow article processing: {processing_time:.2f}s for {response.url}"
                    )
