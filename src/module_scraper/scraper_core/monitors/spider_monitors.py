...
try:
    ...
    print(
        f"ADVERTENCIA: Archivo de esquema no encontrado en {SCHEMA_FILE_PATH}. Usando esquema de validación vacío."
    )
    ARTICULO_SCHEMA = {}
...
class BasicStatsMonitor(Monitor, StatsMonitorMixin):
    ...
    def test_minimum_items_scraped(self):
        ...
        minimum_threshold = self.crawler.settings.getint(
            "SPIDERMON_MIN_ITEMS_SCRAPED", 1
        )  # Default a 1
    ...
    def test_maximum_critical_errors(self):
        ...
        max_critical_errors = self.crawler.settings.getint(
    ...
    def test_maximum_error_messages(self):
        ...
        max_error_messages = self.crawler.settings.getint(
...
class StructureChangeMonitor(Monitor, StatsMonitorMixin):
    ...
    def test_selectors_effectiveness(self):
        ...
        for field in critical_fields:
            empty_count = self.stats.get(
                f"spidermon/validation/fields/{field}_empty", 0
            )
...
class CriticalFieldsMonitor(Monitor, StatsMonitorMixin):
    ...
    def test_critical_fields_populated(self):
        ...
        for field, max_empty_ratio in critical_fields_thresholds.items():
            ...
            invalid_count = self.stats.get(
                f"spidermon/validation/fields/{field}_invalid", 0
            )
...
class ResponseTimeMonitor(Monitor, StatsMonitorMixin):
    ...
    def test_response_time_acceptable(self):
        ...
        max_acceptable = self.crawler.settings.getfloat(
    ...
    def test_response_time_variance(self):
        ...
        for status in [200, 301, 302, 304]:
            status_count = self.stats.get(
                f"downloader/response_status_count/{status}", 0
            )
            if status_count > 0:
...
class HTTPErrorRateMonitor(Monitor, StatsMonitorMixin):
    ...
    def test_http_error_rate_acceptable(self):
        ...
        self.assertFalse(error_rate > max_error_rate, msg=error_msg)
