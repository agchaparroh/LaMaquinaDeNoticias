    ...
    def _get_recommendations(self, failures: List[Dict[str, Any]]) -> List[str]:
        ...
        for failure in failures:
            ...
            if 'StructureChangeMonitor' in monitor:
                ...
                recommendations.append("• Verificar si el sitio web cambió su estructura HTML")
                ...
                recommendations.append("• Verificar si el sitio está bloqueando las solicitudes")
                ...
                recommendations.append("• Revisar si el sitio está experimentando problemas")
...
class SendWebhookAlert(Action):
    ...
    def _get_severity(self, failure: Dict[str, Any]) -> str:
        ...
        # Errores críticos
        if any(critical in monitor for critical in ['HTTPErrorRateMonitor', 'StructureChangeMonitor']):
            return 'critical'
        
        # Errores de advertencia
        elif any(warning in monitor for warning in ['ResponseTimeMonitor', 'CriticalFieldsMonitor']):
            return 'warning'
...
class LogStructuredAlert(Action):
    ...
    def run_action(self):
        ...
        if self.result.has_errors():
            if self._has_critical_errors():
                logger.error("SPIDERMON_ALERT_CRITICAL", extra={'structured_data': log_data})
            else:
                logger.warning("SPIDERMON_ALERT_WARNING", extra={'structured_data': log_data})
        else:
    ...
    def _prepare_log_data(self) -> Dict[str, Any]:
        ...
        log_data = {
            ...
            'monitors_status': {
                ...
                'failed': len([r for r in self.result.monitors_results if not r.passed]),
            },
            'spider_stats': {
                'items_scraped': stats.get('item_scraped_count', 0),
                'duration_seconds': stats.get('elapsed_time_seconds', 0),
    ...
    def _has_critical_errors(self) -> bool:
        ...
        for failure in self.result.failures:
            if any(monitor in failure.get('monitor', '') for monitor in critical_monitors):
                return True
        return False
