...
def get_optimized_settings(spider_type="default", domains=None):
    ...
    if domains:
        ...
        if len(domains) > 1:
            min_delay = min(
                DOMAIN_OPTIMIZATIONS.get(domain, {}).get("DOWNLOAD_DELAY", 1)
    ...
    if request_count > 100:  # Después de 100 requests
        ...
        if success_rate > 0.95:  # Muy buena tasa de éxito
            spider.crawler.engine.downloader.delay = max(
                0.5, spider.crawler.engine.downloader.delay * 0.8
            )
            spider.logger.info(
                f"High success rate ({success_rate:.2%}), reducing delay"
            )

        elif success_rate < 0.8:  # Tasa de éxito baja
            spider.crawler.engine.downloader.delay *= 1.5
            spider.logger.warning(
                f"Low success rate ({success_rate:.2%}), increasing delay"
            )
