...
from typing import Any, Dict, Iterator, List, Optional, Tuple, Union  # noqa: F401
...
from scrapy.utils.misc import arg_to_iter  # noqa: F401
...
class BaseCrawlSpider(BaseArticleSpider, CrawlSpider):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.custom_article_patterns = (
            kwargs.pop("article_patterns", "").split(",")
            if kwargs.get("article_patterns")
            else []
        )
        ...
        self.logger.info(
            f"Crawl spider initialized - max pages: {self.max_pages}, follow links: {self.follow_links}"
        )
    ...
    def parse_page(
        ...
        if self.pages_crawled >= self.max_pages:
            self.logger.info(
                f"Reached maximum page count ({self.max_pages}), stopping spider"
            )
    ...
    def parse_article(self, response: Response) -> Iterator[Dict[str, Any]]:
        ...
        article_data["referrer"] = response.request.headers.get("Referer", b"").decode(
            "utf-8"
        )
    ...
    def _is_article_page(self, response: Response) -> bool:
        ...
        paragraphs = response.css(
            "article p::text, .article-content p::text, .content p::text"
        ).getall()
    ...
    def _requests_to_follow(self, response: Response) -> Iterator[Request]:
        ...
        if self.pages_crawled >= self.max_pages:
            self.logger.info(
                f"Reached maximum page count ({self.max_pages}), not following more links"
            )
            return
    ...
    def from_crawler(cls, crawler, *args, **kwargs):
        ...
        crawler.signals.connect(
            spider._item_scraped, signal=scrapy.signals.item_scraped
        )
    ...
    def _item_scraped(self, item, response, spider):
        ...
        """
        self.logger.debug(
            f"Item scraped from {response.url} at depth {response.meta.get('depth', 0)}"
        )
