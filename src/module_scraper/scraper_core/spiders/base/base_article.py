...
class BaseArticleSpider(scrapy.Spider, LoggerMixin):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.sitemap_urls = list(
            arg_to_iter(kwargs.get("sitemap_urls", []))
        )  # For sitemap spiders
        ...
        if self.start_urls:
            for url in self.start_urls:
                yield self.make_request(
                    url, self.parse_article_list
                )  # Assumes a method to parse lists of articles
        elif self.feed_url:
            yield self.make_request(self.feed_url, self.parse_feed)
        elif self.sitemap_urls:
            for url in (
                self.sitemap_urls
            ):  # This part is usually handled by SitemapSpider itself
                yield self.make_request(
                    url, self.parse_sitemap_index_or_sitemap
                )  # Custom method for sitemap handling
        else:
    ...
    def parse_article_list(self, response: Response) -> Iterator[Request]:
        ...
        raise NotImplementedError(
            f"{self.name} must implement parse_article_list or override start_requests."
        )
    ...
    def parse_feed(self, response: Response) -> Iterator[Request]:
        ...
        raise NotImplementedError(
            f"{self.name} must implement parse_feed or override start_requests."
        )
    ...
    def parse_sitemap_index_or_sitemap(self, response: Response) -> Iterator[Request]:
        ...
        raise NotImplementedError(
            f"{self.name} should use SitemapSpider or implement sitemap parsing."
        )
    ...
    def parse_article(self, response: Response) -> Optional[ArticuloInItem]:
        ...
        if not collected_content_html:  # Si no se recolectó nada para contenido_html
            self.logger.warning(
                f"No content extracted for 'contenido_html' from {response.url} with standard request."
            )
            if (
                not is_playwright_retry
                and not was_original_playwright_request
                and self.settings.getbool("USE_PLAYWRIGHT_FOR_EMPTY_CONTENT", True)
            ):
                self.logger.info(
                    f"Retrying {response.url} with Playwright as 'contenido_html' was empty."
                )
        ...
        self.logger.info(
            f"Successfully parsed: {response.url}. Item will be passed to pipelines for validation and processing."
        )
        return article_item
    ...
    def _extract_metadata(self, response: Response) -> Dict[str, Any]:
        ...
        # OpenGraph
        metadata["og_title"] = response.xpath(
            "//meta[@property='og:title']/@content"
        ).get()
        metadata["og_description"] = response.xpath(
            "//meta[@property='og:description']/@content"
        ).get()
        metadata["og_url"] = response.xpath("//meta[@property='og:url']/@content").get()
        metadata["og_image"] = response.xpath(
            "//meta[@property='og:image']/@content"
        ).get()
        metadata["og_type"] = response.xpath(
            "//meta[@property='og:type']/@content"
        ).get()
        metadata["og_site_name"] = response.xpath(
            "//meta[@property='og:site_name']/@content"
        ).get()

        # Twitter Cards
        metadata["twitter_card"] = response.xpath(
            "//meta[@name='twitter:card']/@content"
        ).get()
        metadata["twitter_title"] = response.xpath(
            "//meta[@name='twitter:title']/@content"
        ).get()
        metadata["twitter_description"] = response.xpath(
            "//meta[@name='twitter:description']/@content"
        ).get()
        metadata["twitter_image"] = response.xpath(
            "//meta[@name='twitter:image']/@content"
        ).get()
        metadata["twitter_site"] = response.xpath(
            "//meta[@name='twitter:site']/@content"
        ).get()

        # Standard meta tags
        metadata["description"] = response.xpath(
        ...
        pub_date_str = (
            ...
            or response.xpath("//meta[@name='sailthru.date']/@content").get()
