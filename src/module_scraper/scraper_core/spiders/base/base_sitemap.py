...
class BaseSitemapSpider(BaseArticleSpider, SitemapSpider):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.logger.info(
            f"Sitemap spider initialized - crawling articles from last {self.days_back} days"
        )
            ...
            for url in self.sitemap_urls:
                if url.endswith("/robots.txt"):
                    yield self.make_request(
                        url, callback=self._parse_robots, priority=1
                    )
                else:
                    yield self.make_request(
                        url, callback=self._parse_sitemap, priority=0
                    )
        else:
            # Direct sitemap URLs
            for url in self.sitemap_urls:
                yield self.make_request(url, callback=self._parse_sitemap, priority=0)
            ...
            if "lastmod" in entry:
                try:
                    ...
                    lastmod = datetime.fromisoformat(
                        entry["lastmod"].replace("Z", "+00:00")
                    )

                    # Filter by date
                    if lastmod >= self.min_date:
                        self.logger.debug(
                            f"Including URL from {lastmod}: {entry['loc']}"
                        )
                        yield entry
                    else:
                        self.logger.debug(
                            f"Skipping old URL from {lastmod}: {entry['loc']}"
                        )
                except (ValueError, AttributeError) as e:
                    self.logger.warning(
                        f"Could not parse lastmod date: {entry.get('lastmod')} - {e}"
                    )
        ...
        for line in response.text.splitlines():
            ...
            if line.lower().startswith("sitemap:"):
                ...
                if self.sitemap_follow:
                    should_follow = any(
                        pattern in sitemap_url for pattern in self.sitemap_follow
                    )

                if should_follow:
                    self.logger.info(f"Found sitemap in robots.txt: {sitemap_url}")
                    yield self.make_request(
                        sitemap_url, callback=self._parse_sitemap, priority=0
                    )
                else:
                    self.logger.debug(
                        f"Skipping sitemap (not in follow list): {sitemap_url}"
                    )
