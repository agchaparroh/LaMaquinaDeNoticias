...
Medio: Infobae
Sección: america_latina
Fecha: 2025-07-01T19:40:00.123456
Estrategia: rss
...
class InfobaeAmericaLatinaSpider(scrapy.Spider):
    """
    Spider RSS para Infobae - america_latina
    ...
    name = "infobae_america_latina"
    allowed_domains = ["infobae.com", "www.infobae.com"]
    ...
    custom_settings = {
        ...
        "CRAWL_ONCE_PATH": f".scrapy/crawl_once/infobae_america_latina",  # noqa: F541
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.medio_info = {
            ...
            "tipo_medio": "revista",
            "medio_url_principal": "https://www.infobae.com/america/america-latina/",
    ...
    def get_rss_with_cloudscraper(self, url: str) -> str:
        ...
        try:
            ...
            if response.status_code == 200:
                self.logger.info(
                    f"✅ CloudScraper CHROME exitoso: {len(response.text)} chars"
                )
                return response.text
            else:
                self.logger.warning(
                    f"⚠️ CloudScraper CHROME status: {response.status_code}"
                )
                return None
    ...
    def get_article_with_cloudscraper(self, url: str) -> str:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.logger.warning(
                    f"⚠️ CloudScraper artículo status: {response.status_code}"
                )
                return None
    ...
    def _extract_content_from_html(
        self, html_content: str, article_data: Dict[str, Any]
    ):
        """
        ...
        try:
            ...
            if content:
                ...
                article_data["contenido_html"] = (
                    selector.css("article").get()
                    or selector.css("main").get()
                    or selector.css('[role="main"]').get()
                )
    ...
    def parse_rss(self, response):
        ...
        if response.status != 200:
            ...
            if response.status == 403:
                self.logger.warning(
                    "🚫 Scrapy bloqueado (403), intentando con CloudScraper..."
                )
    ...
    def _process_rss_content(self, rss_content: str):
        ...
        try:
            ...
            for entry in feed.entries[: self.max_articles]:
                ...
                if self._belongs_to_section(entry):
                    ...
                    if article_data.get("url"):
                        ...
                        self.logger.info(
                            f"📄 Procesando artículo: {article_data['url']}"
                        )
                        article_content = self.get_article_with_cloudscraper(
                            article_data["url"]
                        )
                        if article_content:
                            # Extraer contenido del HTML obtenido
                            self._extract_content_from_html(
                                article_content, article_data
                            )
                        else:
                            self.logger.warning(
                                f"⚠️ No se pudo obtener contenido completo del artículo"
                            )  # noqa: F541
            ...
            self.logger.info(
                ...
                f"({filter_rate:.1f}%) para sección 'america_latina'"
            )
    ...
    def _belongs_to_section(self, entry) -> bool:
        ...
        # Criterio 3: Por defecto incluir si no podemos determinar sección específica
        # Solo filtraremos los que definitivamente NO pertenecen a América Latina
        # Filtraremos específicamente en _is_section_article para ser más conservadores
        self.logger.debug(f"Artículo incluido por defecto (evaluando en filtro de URL)")  # noqa: F541
        return True
    ...
    def _is_section_article(self, url: str) -> bool:
        ...
        # Patrones específicos de Infobae para América Latina
        # Infobae usa rutas como /america/america-latina/, /america/mundo/
        # Nos interesa específicamente América Latina
        america_latina_patterns = [
            r"/america/america-latina/",  # Sección específica América Latina
            r"/america/mexico/",  # México
            r"/america/venezuela/",  # Venezuela
            r"/america/colombia/",  # Colombia
            r"/america/argentina/",  # Argentina
            r"/america/brasil/",  # Brasil
            r"/america/chile/",  # Chile
            r"/america/peru/",  # Perú
            r"/america/ecuador/",  # Ecuador
            r"/america/bolivia/",  # Bolivia
            r"/america/uruguay/",  # Uruguay
            r"/america/paraguay/",  # Paraguay
            r"/america/centroamerica/",  # Centroamérica
            r"/america/costa-rica/",  # Costa Rica
            r"/america/guatemala/",  # Guatemala
            r"/america/honduras/",  # Honduras
            r"/america/nicaragua/",  # Nicaragua
            r"/america/panama/",  # Panamá
            r"/america/el-salvador/",  # El Salvador
            r"/america/cuba/",  # Cuba
            r"/america/dominicana/",  # República Dominicana
            r"/america/haiti/",  # Haití
            r"/america/puerto-rico/",  # Puerto Rico
        ]

        # Verificar que pertenece a América Latina
        for pattern in america_latina_patterns:
            if re.search(pattern, url, re.IGNORECASE):
                self.logger.debug(f"URL aceptada por patrón {pattern}: {url}")
                return True

        # Si no encuentra patrón específico, rechazar para ser conservador
        self.logger.debug(f"URL no contiene patrón de América Latina: {url}")
        return False
    ...
    def extract_rss_data(self, entry) -> Dict[str, Any]:
        ...
        article_data = {
            ...
            "metadata": {
                ...
                "generation_date": "2025-07-01T19:40:00.123456",
                "frecuencia_minutos": 60,
            },
        }
    ...
    def parse_article(self, response):
        ...
        if response.status != 200:
            ...
            if response.status == 403:
                self.logger.warning(
                    f"🚫 Artículo bloqueado (403), intentando con CloudScraper: {response.url}"
                )
