...
Sección: region
Fecha: 2025-07-01T19:45:00.123456
Estrategia: rss
...
class Centroamerica360RegionSpider(scrapy.Spider):
    """
    Spider RSS para Centroamérica360 - region

    Utiliza feeds RSS para obtener artículos de noticias.
    Área geográfica: CENTROAMERICA
    Tipo de medio: revista
    """

    name = "centroamerica360_region"
    allowed_domains = ["centroamerica360.com", "www.centroamerica360.com"]
    ...
    custom_settings = {
        ...
        "CRAWL_ONCE_PATH": f".scrapy/crawl_once/centroamerica360_region",  # noqa: F541
        ...
        # Project: lamaquina
        # Spider: centroamerica360_region
        # Arguments: -a max_items=20
    }

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.rss_url = "https://www.centroamerica360.com/category/region/feed/"
        ...
        self.medio_info = {
            "medio": "Centroamérica360",
            "seccion": "region",
            "area_geografica": "CENTROAMERICA",
            "tipo_medio": "revista",
            "medio_url_principal": "https://www.centroamerica360.com/category/region/",
    ...
    def parse_rss(self, response):
        ...
        # Si el RSS parece mal formado, intentar con cloudscraper como alternativa
        try:
            # Intentar parsear primero para verificar formato
            test_feed = feedparser.parse(response.text)
            if test_feed.bozo and hasattr(test_feed, "bozo_exception"):
                self.logger.warning(
                    f"RSS mal formado detectado con Scrapy: {test_feed.bozo_exception}"
                )
                self.logger.info("🔄 Intentando obtener RSS con CloudScraper...")
                rss_content = self.get_rss_with_cloudscraper(self.rss_url)
                if rss_content:
                    # Procesar RSS obtenido con cloudscraper
                    for item in self._process_rss_content(rss_content):
                    ...
                    self.logger.warning(
                        "⚠️ CloudScraper también falló, usando RSS de Scrapy"
                    )
        except Exception as e:
            self.logger.error(f"Error verificando RSS: {e}")
    ...
    def _process_rss_content(self, rss_content: str):
        ...
        try:
            ...
            self.logger.info(
                ...
                f"({filter_rate:.1f}%) para sección 'region'"
            )
    ...
    def _belongs_to_section(self, entry) -> bool:
        ...
        # Criterio 3: Por defecto incluir todos los artículos del feed "region"
        # Ya que el feed RSS es específico para la sección región de Centroamérica
        self.logger.debug(f"Artículo incluido por defecto (feed específico de región)")  # noqa: F541
        return True
    ...
    def _is_section_article(self, url: str) -> bool:
        ...
        # Patrones específicos de Centroamérica360 para contenido regional
        # Centroamérica360 usa rutas como /region/ para contenido regional centroamericano
        regional_patterns = [
            r"/region/",  # /region/
            ...
            r"/panama/",  # Panamá
            r"/belice/",  # Belice
            r"/centroamerica/",  # Centroamérica general
        ]

        # Verificar que pertenece a contenido regional centroamericano
        for pattern in regional_patterns:
            if re.search(pattern, url, re.IGNORECASE):
                self.logger.debug(f"URL aceptada por patrón {pattern}: {url}")
                return True

        # También incluir por defecto ya que es feed específico de región
        self.logger.debug(
            f"URL incluida por defecto (feed específico de región): {url}"
        )
        return True
    ...
    def extract_rss_data(self, entry) -> Dict[str, Any]:
        ...
        article_data = {
            ...
            "metadata": {
                ...
                "generation_date": "2025-07-01T19:45:00.123456",
                "frecuencia_minutos": 60,
            },
        }
