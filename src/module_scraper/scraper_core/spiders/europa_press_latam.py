...
Medio: Europa Press
Sección: latam
Fecha: 2025-07-01T19:35:00.123456
Estrategia: rss
...
class EuropaPressLatamSpider(scrapy.Spider):
    """
    Spider RSS para Europa Press - latam

    Utiliza feeds RSS para obtener artículos de noticias.
    Área geográfica: HISPANOAMERICA
    Tipo de medio: agencia
    """

    name = "europa_press_latam"
    ...
    custom_settings = {
        ...
        "CRAWL_ONCE_PATH": f".scrapy/crawl_once/europa_press_latam",  # noqa: F541
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.medio_info = {
            ...
            "tipo_medio": "agencia",
            "medio_url_principal": "https://www.europapress.es/internacional/sudamerica-00407/",
    ...
    def _process_rss_content(self, rss_content: str):
        ...
        try:
            ...
            self.logger.info(
                ...
                f"({filter_rate:.1f}%) para sección 'latam'"
            )
    ...
    def _belongs_to_section(self, entry) -> bool:
        ...
        # Criterio 3: Por defecto incluir todos los artículos del feed de Sudamérica
        # Ya que el feed RSS es específico para Sudamérica/LATAM
        self.logger.debug(
            f"Artículo incluido por defecto (feed específico de Sudamérica)"
        )  # noqa: F541
        return True
    ...
    def _is_section_article(self, url: str) -> bool:
        ...
        # Patrones específicos de Europa Press para Internacional/Sudamérica
        # Europa Press usa /internacional/ para contenido internacional
        international_patterns = [
            r"/internacional/",  # /internacional/
            r"/sudamerica/",  # /sudamerica/
            r"/latinoamerica/",  # /latinoamerica/
            r"/america/",  # /america/
            r"/iberoamerica/",  # /iberoamerica/
        ]

        # Verificar que pertenece a la sección internacional/sudamericana
        for pattern in international_patterns:
            if re.search(pattern, url, re.IGNORECASE):
                self.logger.debug(f"URL aceptada por patrón {pattern}: {url}")
                return True

        # También incluir por defecto ya que es feed específico de Sudamérica
        self.logger.debug(
            f"URL incluida por defecto (feed específico de Sudamérica): {url}"
        )
        return True
    ...
    def extract_rss_data(self, entry) -> Dict[str, Any]:
        ...
        article_data = {
            ...
            "metadata": {
                ...
                "generation_date": "2025-07-01T19:35:00.123456",
                "frecuencia_minutos": 60,
            },
        }
