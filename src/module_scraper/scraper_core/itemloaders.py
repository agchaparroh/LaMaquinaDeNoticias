...
from itemloaders.processors import (  # noqa: F401
    Compose,
    Identity,
    Join,
    MapCompose,
...
def parse_and_format_date_processor(date_str_list):
    ...
    if parsed_date:
        ...
        if (
            ...
            now = datetime.now()
            if (
                abs((now - parsed_date.replace(tzinfo=None)).total_seconds()) < 86400
            ):  # 24 horas
...
def normalize_date(date_string: Union[str, datetime]) -> Optional[datetime]:
    ...
    original_date_str_cleaned = (
        date_string.strip()
    )  # Keep original for standard parsing
    lower_date_str = (
        original_date_str_cleaned.lower()
    )  # Use lowercased for relative matching
    ...
    try:
        ...
        tzinfos = {
            ...
            "IST": tz.gettz(
                "Asia/Kolkata"
            ),  # Indian Standard Time (más común en noticias)
    ...
    except Exception as e:  # Other dateparser errors  # noqa: F841
...
def generate_storage_path(item_dict: dict) -> str:
    ...
    titulo_slug = titulo_slug[
        : DEFAULT_SPIDER_VALUES.title_slug_max_length
    ]  # Limitar longitud
...
class ArticuloInItemLoader(ItemLoader):
    ...
    fecha_recopilacion_in = MapCompose(lambda x: x if x else datetime.now())

    # Estado: valor por defecto
    estado_procesamiento_in = MapCompose(lambda x: x if x else "pendiente")
