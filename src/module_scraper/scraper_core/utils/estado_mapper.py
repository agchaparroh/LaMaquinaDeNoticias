...
def map_estado_to_domain(estado: str, context: str = 'scraper') -> str:
    ...
    DOMAIN_STATES = {
        'pendiente', 'procesando', 'completado',
        'error_prefiltrado', 'error_relevancia',
        'error_extraccion', 'error_normalizacion', 'error_bd'
    }
...
def get_error_detail_for_estado(estado_original: str, error_message: Optional[str] = None) -> str:
    """
...
def map_tipo_medio_to_domain(tipo_medio: str) -> str:
    ...
    DOMAIN_TIPOS = {
        'diario', 'agencia', 'televisión', 'radio', 
        'digital', 'oficial', 'blog', 'otro'
    }
    ...
    if tipo_medio_lower not in TIPO_MEDIO_MAPPING and tipo_medio not in DOMAIN_TIPOS:
        logger.warning(
            f"Tipo de medio desconocido '{tipo_medio}'. "
            f"Mapeando a 'otro' por defecto."
        )
