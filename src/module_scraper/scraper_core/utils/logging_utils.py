    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            if execution_time > 1.0:  # Log if execution takes more than 1 second
                logger.warning(
                    "%s took %.2f seconds to execute", func.__qualname__, execution_time
                )
            else:
                logger.debug(
                    "%s executed in %.3f seconds", func.__qualname__, execution_time
                )
...
def log_item_processing(func: Callable) -> Callable:
    ...
    def wrapper(self, item, spider, *args, **kwargs):
        ...
        try:
            ...
            logger.debug(
                "Successfully processed item: %s", item.get("url", "Unknown URL")
            )
...
def log_response_stats(response, logger: logging.Logger):
    ...
    logger.debug(
        "Response stats: %s", json.dumps(stats, ensure_ascii=False, default=str)
    )
...
def log_item_stats(item: Dict[str, Any], logger: logging.Logger):
    ...
    logger.info("Item statistics: %s", json.dumps(stats, ensure_ascii=False))
