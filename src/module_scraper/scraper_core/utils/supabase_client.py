...
from .estado_mapper import (
    get_error_detail_for_estado,
    map_estado_to_domain,
    map_tipo_medio_to_domain,
)
...
class SupabaseClient:
    ...
    def __init__(self):
        if not hasattr(self, "_initialized"):  # Ensure __init__ runs only once
            ...
            if not self.supabase_url or not self.supabase_key:
                ...
                raise ValueError(
                    "Supabase URL and Service Role Key not found in environment variables."
                )
            ...
            self.articulos_table_name = os.getenv(
                "SUPABASE_ARTICULOS_TABLE", "articulos"
            )
            self.logger.info(
                f"SupabaseClient configured for simplified schema. Table: articulos='{self.articulos_table_name}'"
            )
    ...
    def get_client(self) -> Client:
        ...
        if not self.client:
            ...
            raise ConnectionError(
                "Supabase client not initialized. Call __init__ first."
            )
        return self.client
    ...
    def upload_to_storage(
        ...
        try:
            response = self.client.storage.from_(bucket_name).upload(
                path=file_path, file=file_content, file_options=file_options or {}
            )
    ...
    def create_bucket_if_not_exists(self, bucket_name: str) -> bool:
        ...
        try:
            ...
            # Create the bucket
            create_response = self.client.storage.create_bucket(bucket_name)  # noqa: F841
    ...
    def insert_data(
        ...
        if not data:
            self.logger.warning(
                f"No data provided for insertion into table {table_name}."
            )
            return None
        try:
    ...
    def upsert_articulo(
        self, articulo_item_data: Dict[str, Any]
    ) -> Optional[Dict[str, Any]]:
        """
        ...
        if missing_keys:
            self.logger.error(
                f"Missing required keys {missing_keys} in articulo_item_data for upsert_articulo."
            )
            return None
        ...
        db_articulo_data = {
            ...
            "tipo_medio": map_tipo_medio_to_domain(
                articulo_item_data.get("tipo_medio", "digital")
            ),
        ...
        db_articulo_data_cleaned = {
            k: v for k, v in db_articulo_data.items() if v is not None
        }

        # Log detallado de diagnóstico
        self.logger.info(f"=== UPSERT DIAGNÓSTICO ===")  # noqa: F541
        self.logger.info(f"URL: {db_articulo_data_cleaned.get('url')}")
        self.logger.info(
            f"Estado procesamiento: {db_articulo_data_cleaned.get('estado_procesamiento')}"
        )
        self.logger.info(f"Tipo medio: {db_articulo_data_cleaned.get('tipo_medio')}")
        self.logger.info(
            f"Fecha publicación: {db_articulo_data_cleaned.get('fecha_publicacion')}"
        )
        ...
        try:
            response = (
                self.client.table(self.articulos_table_name)
                .upsert(db_articulo_data_cleaned, on_conflict="url")
                .execute()
            )

            # Log detallado de la respuesta
            self.logger.info(f"=== RESPUESTA SUPABASE ===")  # noqa: F541
            self.logger.info(f"Response data existe: {response.data is not None}")
            self.logger.info(
                f"Response data length: {len(response.data) if response.data else 0}"
            )
            ...
            if response.data:
                self.logger.info(
                    f"✅ Article upserted successfully for URL: {db_articulo_data_cleaned['url']}"
                )
                self.logger.info(
                    f"ID del artículo guardado: {response.data[0].get('id', 'ID no disponible')}"
                )
                return response.data[0]
            else:
                self.logger.warning(
                    f"❌ Upsert returned no data for URL: {db_articulo_data_cleaned.get('url')}"
                )
            ...
            self.logger.error(
                f"Error upserting article for URL {db_articulo_data_cleaned.get('url')}: {e}"
            )
            return None
    ...
    def update_articulo_status(
        self, url: str, status: str, error_detail: Optional[str] = None
    ) -> bool:
        """
...
if __name__ == "__main__":
    ...
    if not SUPABASE_URL_TEST or not SUPABASE_KEY_TEST:
        logger.error(
            "SUPABASE_URL and SUPABASE_KEY must be set in environment for testing."
        )
    else:
        try:
