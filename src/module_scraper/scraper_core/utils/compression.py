...
def compress_html(
    html_content: Union[str, bytes], compression_level: int = 6, encoding: str = "utf-8"
) -> bytes:
    """
    ...
    if not 0 <= compression_level <= 9:
        raise ValueError(
            f"Compression level must be between 0 and 9, got {compression_level}"
        )
...
def decompress_html(
    compressed_content: bytes, encoding: str = "utf-8", return_bytes: bool = False
) -> Union[str, bytes]:
    """
...
def get_compression_info(compressed_data: bytes) -> dict:
    ...
    try:
        ...
        return {
            ...
            "compression_ratio": 1.0 - (len(compressed_data) / len(decompressed))
