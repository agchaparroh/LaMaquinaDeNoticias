...
from scrapy.exceptions import IgnoreRequest, NotConfigured  # noqa: F401
...
class PlaywrightCustomDownloaderMiddleware:
    ...
    def __init__(self, crawler):
        ...
        self.playwright_timeout = self.settings.getint(
            "PLAYWRIGHT_TIMEOUT", 30000
        )  # 30 segundos
        self.enable_fallback_on_error = self.settings.getbool(
    ...
    def spider_opened(self, spider):
        ...
        self.logger.info(
            f"Playwright configuration: max_retries={self.max_playwright_retries}, "
    ...
    def spider_closed(self, spider):
        ...
        self.logger.info(
            f"Playwright statistics - Empty content detections: {self.empty_content_detections}, "
        ...
        self.stats.set_value(
            "playwright/empty_content_detections", self.empty_content_detections
        )
        ...
        self.stats.set_value(
            "playwright/successful_recoveries", self.successful_recoveries
        )
    ...
    def _get_robust_playwright_config(self, request, spider) -> dict:
        ...
        config = {
            ...
            "playwright_page_coroutines": [
                # Esperar a que el contenido se cargue completamente
                {
                    ...
                    "args": ["networkidle"],
                    "kwargs": {"timeout": self.playwright_timeout},
                },
            ],
        ...
        if any(
            spa_indicator in url_lower
            for spa_indicator in ["react", "angular", "vue", "spa"]
        ):
    ...
    async def process_response(self, request, response, spider):
        ...
        if is_playwright_request:
            ...
            self.logger.info(
                f"Playwright successfully recovered content for {response.url}"
            )
    ...
    def _detect_playwright_error(self, response, spider) -> Optional[str]:
        ...
        if (
            ...
            and "playwright" in str(response.headers).lower()
        ):
            return "navigation"
        ...
        if (
            "out of memory" in response_text_lower
            or "resource exhausted" in response_text_lower
        ):
            return "resource_limit"

        return None

    def _handle_playwright_error(
        self, request, response, spider, error_type: str, retry_count: int
    ):
        """
        ...
        if retry_count >= self.max_playwright_retries:
            ...
            if self.enable_fallback_on_error:
                return self._create_fallback_request(
                    request, spider, "max_retries_exceeded"
                )
            else:
    ...
    def _should_check_empty_content(
        self, request, response, empty_retry_count: int
    ) -> bool:
        """
    ...
    def _handle_empty_content(self, request, response, spider, empty_retry_count: int):
        ...
        new_meta["playwright_retried"] = (
            True  # Para compatibilidad con BaseArticleSpider
        )
    ...
    def process_exception(self, request, exception, spider):
        ...
        if any(
            ...
            for keyword in ["playwright", "browser", "timeout", "target closed"]
        ):
            ...
            if retry_count < self.max_playwright_retries:
                ...
                if self.enable_fallback_on_error:
                    return self._create_fallback_request(
                        request, spider, f"exception: {exception}"
                    )
