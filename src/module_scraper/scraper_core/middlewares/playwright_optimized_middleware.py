...
class PlaywrightOptimizedMiddleware:
    ...
    def __init__(self, crawler):
        ...
        self.timeout = self.settings.getint(
            "PLAYWRIGHT_TIMEOUT", 20000
        )  # 20s en lugar de 30s
        self.enable_fallback = self.settings.getbool("PLAYWRIGHT_ENABLE_FALLBACK", True)
        self.enable_playwright = self.settings.getbool(
            "ENABLE_PLAYWRIGHT_MIDDLEWARE", False
        ...
        if middleware.enable_playwright:
            try:
                ...
                from scrapy_playwright.handler import (
                    ScrapyPlaywrightDownloadHandler,  # noqa: F401
                )
                ...
                middleware.logger.warning(
                    f"Playwright not available, disabling middleware: {e}"
                )
    ...
    def spider_opened(self, spider):
        if self.enable_playwright:
            self.logger.info(
                f"PlaywrightOptimizedMiddleware enabled for {spider.name} "
                f"(max_retries={self.max_retries}, timeout={self.timeout}ms)"
            )
        else:
            self.logger.info(
                f"PlaywrightOptimizedMiddleware disabled for {spider.name}"
            )

    def spider_closed(self, spider):
        if self.enable_playwright:
            self.logger.info(
                f"Playwright stats - Detections: {self.empty_content_detections}, "
                f"Retries: {self.retries}, Failures: {self.failures}, Recoveries: {self.recoveries}"
            )

            # Actualizar estadísticas
            self.stats.set_value(
                "playwright_optimized/empty_content_detections",
                self.empty_content_detections,
            )
    ...
    def _create_playwright_retry(self, request, response, spider):
        ...
        self.logger.info(
            f"Empty content detected, retrying with Playwright: {response.url}"
        )
    ...
    def process_exception(self, request, exception, spider):
        ...
        self.failures += 1
        exception_str = str(exception).lower()  # noqa: F841
