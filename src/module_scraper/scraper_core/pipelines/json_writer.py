...
from scrapy.exceptions import DropItem  # noqa: F401
...
class JsonWriterPipeline:
    ...
    def __init__(self, output_dir: str):
        ...
        logger.info(
            f"JsonWriterPipeline initialized. Output directory: {self.output_dir}"
        )
    ...
    def process_item(self, item, spider):
        ...
        if not isinstance(item, ArticuloInItem):
            logger.debug(
                f"Item is not ArticuloInItem, skipping JSON export: {type(item)}"
            )
        ...
        try:
            ...
            json_content = json.dumps(
                item_dict, ensure_ascii=False, indent=2, sort_keys=True
            )
