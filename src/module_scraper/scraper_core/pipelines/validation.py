...
class DataValidationPipeline:
    ...
    def from_crawler(cls, crawler):
        ...
        instance.min_content_length = crawler.settings.getint(
            "VALIDATION_MIN_CONTENT_LENGTH", 100
        )
        instance.min_title_length = crawler.settings.getint(
            "VALIDATION_MIN_TITLE_LENGTH", 10
        )

        # Load date format from settings
        instance.date_format = crawler.settings.get(
            "VALIDATION_DATE_FORMAT", "%Y-%m-%dT%H:%M:%S"
        )
        instance.drop_invalid_items = crawler.settings.getbool(
    ...
    def process_item(self, item, spider: Spider):
        ...
        if not isinstance(item, ArticuloInItem):
            logger.debug(
                f"Item is not ArticuloInItem, skipping validation: {type(item)}"
            )
            return item
        ...
        try:
            ...
            if self.drop_invalid_items:
                logger.warning(
                    f"Validation failed for {item_url} and VALIDATION_DROP_INVALID_ITEMS is True. Dropping item: {e}"
                )
                raise  # Re-raise to drop the item
            else:
                logger.warning(
                    f"Validation failed for {item_url} but VALIDATION_DROP_INVALID_ITEMS is False. Passing item with error details: {e}"
                )
        ...
        for field, expected_types in self.FIELD_TYPES.items():
            ...
            if isinstance(expected_types, tuple):
                ...
                if not isinstance(value, expected_types):
                    # Try type conversion
                    try:
                        if expected_types == str:  # noqa: E721
                            adapter[field] = str(value)
                        elif expected_types == bool:  # noqa: E721
                            adapter[field] = bool(value)
                        elif expected_types == list and isinstance(value, str):  # noqa: E721
                            # Convert comma-separated string to list
                            adapter[field] = [
                                x.strip() for x in value.split(",") if x.strip()
                            ]
                        elif expected_types == dict and isinstance(value, str):  # noqa: E721
                    ...
                    except (ValueError, json.JSONDecodeError) as e:  # noqa: F841
                        raise DataTypeError(
                            field=field,
                            expected_type=expected_types.__name__,
                            actual_type=type(value).__name__,
        ...
        try:
            ...
            if not all([parsed.scheme, parsed.netloc]):
                raise ValidationError(
                    f"Invalid URL format: missing scheme or netloc",  # noqa: F541
                    field="url",
                    item_url=item_url,
                )
            ...
            raise ValidationError(
                f"Invalid URL format: {e}", field="url", item_url=item_url
            )
                    ...
                    if match:
                        ...
                        try:
                            ...
                            dt = datetime.strptime(
                                normalized_value, "%Y-%m-%dT%H:%M:%S%z"
                            )
                            adapter[field] = dt.isoformat()
                            parsed = True
                        except ValueError:
                            try:
                                ...
                                dt = datetime.strptime(
                                    normalized_value, "%Y-%m-%dT%H:%M:%S.%f%z"
                                )
                                adapter[field] = dt.isoformat()
                                parsed = True
                            except ValueError:
