...
class DataCleaningPipeline:
    ...
    HTML_STRIP_FIELDS = ["titular", "contenido_texto", "resumen", "autor"]
    ...
    LIST_FIELDS = ["etiquetas_fuente", "categorias_asignadas"]
    ...
    def from_crawler(cls, crawler):
        ...
        instance.normalize_whitespace = crawler.settings.getbool(
            "CLEANING_NORMALIZE_WHITESPACE", True
        )
        instance.remove_empty_lines = crawler.settings.getbool(
            "CLEANING_REMOVE_EMPTY_LINES", True
        )
        instance.standardize_quotes = crawler.settings.getbool(
            "CLEANING_STANDARDIZE_QUOTES", True
        )
        instance.preserve_html_content = crawler.settings.getbool(
    ...
    def process_item(self, item, spider: Spider):
        ...
        try:
            ...
            adapter["error_detalle"] = (
                f"Cleaning error: {e}; {adapter.get('error_detalle', '')}"
            )
        except Exception as e:
            logger.error(
                f"Unexpected cleaning error for {item_url}: {e}", exc_info=True
            )
            adapter["error_detalle"] = (
                f"Unexpected cleaning error: {e}; {adapter.get('error_detalle', '')}"
            )
    ...
    def _strip_html_from_fields(self, adapter: ItemAdapter, item_url: str):
        ...
        for field in self.HTML_STRIP_FIELDS:
            ...
            try:
                ...
                logger.warning(
                    f"Failed to strip HTML from field '{field}' for {item_url}: {e}"
                )
    ...
    def _normalize_text_fields(self, adapter: ItemAdapter, item_url: str):
        ...
        for field in self.TEXT_NORMALIZE_FIELDS:
            ...
            try:
                ...
                logger.warning(
                    f"Failed to normalize text in field '{field}' for {item_url}: {e}"
                )
    ...
    def _standardize_dates(self, adapter: ItemAdapter, item_url: str):
        ...
        for field in date_fields:
            ...
            try:
                ...
                logger.warning(
                    f"Failed to standardize date in field '{field}' for {item_url}: {e}"
                )
    ...
    def _normalize_lists(self, adapter: ItemAdapter, item_url: str):
        ...
        for field in self.LIST_FIELDS:
            ...
            try:
                ...
                logger.warning(
                    f"Failed to normalize list field '{field}' for {item_url}: {e}"
                )
    ...
    def _clean_html_content(self, adapter: ItemAdapter, item_url: str):
        ...
        try:
            ...
            for comment in soup.find_all(
                text=lambda text: isinstance(text, type(soup.new_string("")))
            ):
