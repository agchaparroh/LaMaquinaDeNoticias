...
class ArticuloInItem(scrapy.Item):
    ...
    fuente = scrapy.Field()  # Nombre del spider/fuente de la noticia
    estado_procesamiento = scrapy.Field()  # Estado del artículo en el pipeline de procesamiento
    ...
    # Processing metadata
    fecha_recopilacion = Field()  # Auto-generated: cuándo se capturó (maps to DB)
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Set default values
        if 'fecha_recopilacion' not in self:
            self['fecha_recopilacion'] = datetime.now()
        ...
        if 'estado_procesamiento' not in self:
            self['estado_procesamiento'] = None
...
class ArticuloAdapter(dict):
    ...
    def __init__(self, item: ArticuloInItem):
        ...
        self['fuente'] = item.get('fuente')
        self['estado_procesamiento'] = item.get('estado_procesamiento')
        self['es_opinion'] = item.get('es_opinion', False)
        self['es_oficial'] = item.get('es_oficial', False)
        self['fecha_recopilacion'] = item.get('fecha_recopilacion', datetime.now())
        ...
        if isinstance(self['fecha_recopilacion'], datetime):
            self['fecha_recopilacion'] = self['fecha_recopilacion'].isoformat()
