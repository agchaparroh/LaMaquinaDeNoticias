...
class ArticuloInItem(BaseModel):
    ...
    medio_url_principal: Optional[str] = (
        None  # URL principal del medio (ej: https://elpais.com)
    )
    ...
    estado_procesamiento: Optional[str] = (
        "pendiente_connector"  # Estado: pendiente, procesando, etc.
    )
...
def prepare_articulo(articulo_data: dict) -> ArticuloInItem:
    ...
    if "articulo_id" in data and data["articulo_id"]:
        ...
        if data.get("url"):
            ...
            data["id"] = (
                f"art_{datetime.now().strftime('%Y%m%d%H%M%S')}_{id(data) % 1000:03d}"
            )
