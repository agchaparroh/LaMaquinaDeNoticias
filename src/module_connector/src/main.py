...
async def monitor_directory():
    ...
    logger.info(f"Directory structure verified:")  # noqa: F541
        ...
        try:
            ...
            try:
                ...
                files = [
                    ...
                    and os.path.isfile(os.path.join(SCRAPER_OUTPUT_DIR, f))
                ]
            ...
            for file_name in files:
                ...
                try:
                    ...
                    valid_articles, invalid_articles, has_valid = await process_file(
                        dest_path
                    )

                    if has_valid:
                        logger.info(
                            f"File {file_name} processed successfully with {len(valid_articles)} valid articles"
                        )
                        ...
                        if success_count > 0:
                            logger.info(
                                f"Successfully sent {success_count} articles to Pipeline API"
                            )

                        if failure_count > 0:
                            logger.warning(
                                f"Failed to send {failure_count} articles to Pipeline API"
                            )
                    else:
                        logger.warning(f"File {file_name} contains no valid articles")
                        if invalid_articles:
                            logger.warning(
                                f"Found {len(invalid_articles)} invalid articles in file"
                            )
...
async def process_file(
    file_path: str,
) -> Tuple[List[ArticuloInItem], List[Dict[str, Any]], bool]:
    """
    ...
    try:
        ...
        try:
            with gzip.open(file_path, "rt", encoding="utf-8") as f:
                content = f.read()
            logger.debug(
                f"Successfully read and decompressed file, size: {len(content)} characters"
            )
        ...
        for i, article_data in enumerate(articles_data):
            try:
                ...
                title_preview = (
                    article.titular[:50] + "..."
                    if len(article.titular) > 50
                    else article.titular
                )
        ...
        logger.info(
            f"Processing complete: {valid_count}/{total_articles} articles valid, {invalid_count} invalid"
        )

        if invalid_count > 0:
            logger.warning(
                f"File {os.path.basename(file_path)} contains {invalid_count} invalid articles"
            )
...
async def send_to_pipeline(
    session: aiohttp.ClientSession, article: ArticuloInItem
) -> bool:
    """
    ...
    try:
        ...
        title_preview = (
            article.titular[:30] + "..."
            if len(article.titular) > 30
            else article.titular
        )
        logger.info(
            f"Sending article to pipeline: '{title_preview}' (ID: {article_id})"
        )
        ...
        async with session.post(
            ...
            if status in [200, 202]:
                ...
                logger.info(
                    f"✅ Article successfully sent to pipeline (ID: {article_id})"
                )
                return True
                ...
                try:
                    ...
                    logger.error(
                        f"❌ Pipeline validation error for article {article_id}: {error_data}"
                    )
                except:  # noqa: E722
                    error_text = await response.text()
                    logger.error(
                        f"❌ Pipeline validation error for article {article_id}: {error_text}"
                    )
                return False
                ...
                logger.warning(
                    f"⚠️  Pipeline server error {status} for article {article_id} (will retry)"
                )
                ...
                logger.error(
                    f"❌ Unexpected response from pipeline for article {article_id}: {status}"
                )
                return False
        ...
        logger.warning(
            f"⚠️  Connection error for article {article_id} (will retry): {str(e)}"
        )
        raise
        ...
        if e.status in [500, 503]:
            logger.warning(
                f"⚠️  Server error {e.status} for article {article_id} (will retry)"
            )
            raise
        else:
        ...
        logger.error(
            f"❌ Unexpected error sending article {article_id} to pipeline: {str(e)}"
        )
        return False
...
async def send_articles_to_pipeline(articles: List[ArticuloInItem]) -> Tuple[int, int]:
    ...
    logger.info(
        f"Sending {len(articles)} articles to Pipeline API at {PIPELINE_API_URL}"
    )
    ...
    if success_count > 0:
        logger.info(
            f"✅ Pipeline API results: {success_count}/{total_articles} articles sent successfully ({success_rate:.1f}%)"
        )

    if failure_count > 0:
        logger.warning(
            f"❌ {failure_count}/{total_articles} articles failed to send ({(failure_count / total_articles * 100):.1f}%)"
        )
...
async def move_file(file_path: str, success: bool) -> None:
    ...
    try:
        ...
        logger.info(
            f"✅ File moved successfully: {file_name} -> {os.path.basename(dest_path)}"
        )
    ...
    try:
        ...
        try:
            ...
            files = [
                ...
                for f in all_files
                if f.endswith(".json.gz")
                and os.path.isfile(os.path.join(PIPELINE_PENDING_DIR, f))
            ]
        except PermissionError:
        ...
        for file_name in files:
            ...
            try:
                ...
                if valid_articles:
                    ...
                    if success:
                        logger.info(
                            f"✅ File {file_name}: {success_count}/{len(valid_articles)} articles sent successfully"
                        )
                    else:
                        logger.warning(
                            f"❌ File {file_name}: All {len(valid_articles)} articles failed to send"
                        )
                else:
                ...
                logger.info(
                    f"📊 File {file_name} summary: {valid_count}/{total_articles} valid articles, {invalid_count} invalid"
                )
        ...
        # Final summary
        success_rate = (
            (files_succeeded / files_processed * 100) if files_processed > 0 else 0
        )
        logger.info(
            f"🏁 Pending files processing complete: {files_succeeded}/{files_processed} successful ({success_rate:.1f}%)"
        )
...
async def main():
    ...
    logger.info(f"📊 Configuration:")  # noqa: F541
    ...
    try:
        ...
        if files_processed > 0:
            logger.info(
                f"📋 Startup processing complete: {files_succeeded}/{files_processed} files successful"
            )
        else:
