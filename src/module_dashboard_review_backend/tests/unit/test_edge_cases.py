...
from src.models.responses import ArticuloMetadata, HechoRelacionInfo, HechoResponse
...
class TestHechosServiceEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_get_relaciones_con_datos_inconsistentes(
        self, hechos_service, mock_supabase_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_relaciones_con_referencia_circular_compleja(
        self, hechos_service, mock_supabase_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_relaciones_con_ids_duplicados(
        self, hechos_service, mock_supabase_client
    ):
        ...
        expected_or_filter = (
            "hecho_origen_id.in.(1,1,2,2,1,3), hecho_destino_id.in.(1,1,2,2,1,3)"
        )
        mock_query.or_.assert_called_once_with(expected_or_filter)

    @pytest.mark.asyncio
    async def test_get_relaciones_con_lista_enormemente_grande(
        self, hechos_service, mock_supabase_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_hechos_for_revision_con_relaciones_malformadas(
        self, hechos_service, mock_supabase_client
    ):
...
class TestValidacionesFiltros:
    ...
    def test_filtro_fechas_iguales(self):
        ...
        filtro = HechoFilterParams(fecha_inicio=misma_fecha, fecha_fin=misma_fecha)
    ...
    def test_filtro_importancia_rango_invalido(self):
        ...
        assert (
            "importancia_max must be greater than or equal to importancia_min"
    ...
    def test_filtro_caracteres_especiales(self):
        ...
        assert filtro.tipo_hecho == "declaración_política"
        assert filtro.evaluacion_editorial == "verificado_✓"
