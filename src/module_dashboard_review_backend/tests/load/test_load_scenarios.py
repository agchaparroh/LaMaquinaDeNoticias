...
class TestLoadScenarios:
    ...
    def run_load_test(
        ...
        def make_request():
            ...
            try:
                ...
                if scenario == "read" or (
                    scenario == "mixed" and random.random() < 0.8
                ):
                    # Read request
                    params = {
    ...
    def test_sustained_normal_load(self, client, load_test_service):
        ...
        results = self.run_load_test(
            client, duration_seconds=30, rps_target=50, scenario="mixed"
        )
    ...
    def test_spike_load(self, client, load_test_service):
        ...
        results = {"phases": [], "total_requests": 0, "total_errors": 0}
        ...
        for phase_name, phase_results in results["phases"]:
            ...
            success_rate = (
                phase_results["successful_requests"] / phase_results["total_requests"]
            )
        ...
        spike_success_rate = (
            results["phases"][1][1]["successful_requests"]
            / results["phases"][1][1]["total_requests"]
        )
        assert spike_success_rate > 0.90, (
            "System should maintain >90% success during spike"
        )

        # Recovery should return to normal
        recovery_success_rate = (
            results["phases"][2][1]["successful_requests"]
            / results["phases"][2][1]["total_requests"]
        )
    ...
    def test_sustained_high_load(self, client, load_test_service):
        ...
        results = self.run_load_test(
            client, duration_seconds=60, rps_target=150, scenario="mixed"
        )
    ...
    def test_read_heavy_load(self, client, load_test_service):
        ...
        results = self.run_load_test(
            client, duration_seconds=30, rps_target=100, scenario="read"
        )
        ...
        p50 = sorted(results["response_times"])[
            int(len(results["response_times"]) * 0.50)
        ]
    ...
    def test_write_heavy_load(self, client, load_test_service):
        ...
        results = self.run_load_test(
            client, duration_seconds=30, rps_target=50, scenario="write"
        )
    ...
    def test_memory_leak_detection(self, client, load_test_service):
        ...
        results = self.run_load_test(
            client, duration_seconds=120, rps_target=30, scenario="mixed"
        )
