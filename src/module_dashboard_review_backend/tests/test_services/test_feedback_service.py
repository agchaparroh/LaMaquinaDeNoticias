...
def feedback_request():
    ...
    return ImportanciaFeedbackRequest(
        importancia_editor_final=8, usuario_id_editor="editor123"
    )
...
class TestFeedbackService:
    ...
    @pytest.mark.asyncio
    async def test_submit_importancia_feedback_hecho_exists_new_feedback(
        self, feedback_service, feedback_request
    ):
        ...
        with patch.object(
            SupabaseClient, "get_single_record", new_callable=AsyncMock
        ) as mock_get:
            ...
            with patch.object(
                SupabaseClient, "execute_with_retry", new_callable=AsyncMock
            ) as mock_execute:
                ...
                # Execute
                await feedback_service.submit_importancia_feedback(
                    hecho_id, feedback_request
                )

        # Verify hecho existence was checked
        mock_get.assert_called_once_with(table_name="hechos", record_id=hecho_id)
    ...
    @pytest.mark.asyncio
    async def test_submit_importancia_feedback_hecho_not_found(
        self, feedback_service, feedback_request
    ):
    ...
    @pytest.mark.asyncio
    async def test_submit_importancia_feedback_update_existing(
        self, feedback_service, feedback_request
    ):
    ...
    @pytest.mark.asyncio
    async def test_submit_importancia_feedback_database_error(
        self, feedback_service, feedback_request
    ):
    ...
    @pytest.mark.asyncio
    async def test_submit_importancia_feedback_with_valid_importance_range(
        ...
        feedback_min = ImportanciaFeedbackRequest(
            importancia_editor_final=1, usuario_id_editor="editor123"
        )

        # Test maximum value (10)
        feedback_max = ImportanciaFeedbackRequest(
            importancia_editor_final=10, usuario_id_editor="editor123"
        )
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                await feedback_service.submit_importancia_feedback(
                    hecho_id, feedback_min
                )
                await feedback_service.submit_importancia_feedback(
                    hecho_id, feedback_max
                )
    ...
    @pytest.mark.asyncio
    async def test_submit_evaluacion_editorial_success(
        self, feedback_service, evaluacion_request
    ):
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                # Execute
                await feedback_service.submit_evaluacion_editorial(
                    hecho_id, evaluacion_request
                )
    ...
    @pytest.mark.asyncio
    async def test_submit_evaluacion_editorial_hecho_not_found(
        self, feedback_service, evaluacion_request
    ):
    ...
    @pytest.mark.asyncio
    async def test_submit_evaluacion_editorial_valid_enum_values(
        self, feedback_service
    ):
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                await feedback_service.submit_evaluacion_editorial(
                    hecho_id, evaluacion_verified
                )
                await feedback_service.submit_evaluacion_editorial(
                    hecho_id, evaluacion_false
                )
    ...
    @pytest.mark.asyncio
    async def test_submit_evaluacion_editorial_database_error(
        self, feedback_service, evaluacion_request
    ):
        ...
        with patch.object(SupabaseClient, "get_client", return_value=mock_client):
            with patch.object(
                ...
                with patch.object(
                    ...
                    service = FeedbackService()
                    await service.submit_importancia_feedback(
                        hecho_id, feedback_request
                    )
