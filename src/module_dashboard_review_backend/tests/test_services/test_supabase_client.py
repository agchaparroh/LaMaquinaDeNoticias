...
class TestSupabaseClientInitialization:
    ...
    def test_singleton_pattern(self, mock_create_client, mock_settings):
        ...
        mock_create_client.assert_called_once_with(
            supabase_url="https://test.supabase.co", supabase_anon_key="test-key-123"
        )
    ...
    def test_reset_client(self, mock_create_client, mock_settings):
        ...
        # Get client
        client1 = SupabaseClient.get_client()  # noqa: F841

        # Reset client
        SupabaseClient.reset_client()

        # Get client again - should create new instance
        client2 = SupabaseClient.get_client()  # noqa: F841
...
class TestRetryLogic:
    ...
    async def test_successful_operation_no_retry(self):
        ...
        result = await SupabaseClient.execute_with_retry(
            mock_operation, "arg1", kwarg="value"
        )
...
class TestHelperMethods:
    ...
    async def test_get_single_record_found(self, mock_create_client, mock_settings):
        ...
        record = await SupabaseClient.get_single_record(
            table_name="hechos", record_id=123
        )
    ...
    async def test_get_single_record_not_found(self, mock_create_client, mock_settings):
        ...
        # Execute query
        record = await SupabaseClient.get_single_record(
            table_name="hechos", record_id=999
        )
    ...
    async def test_update_record_success(self, mock_create_client, mock_settings):
        ...
        result = await SupabaseClient.update_record(
            table_name="hechos", record_id=123, data={"status": "updated"}
        )
    ...
    async def test_update_record_failure(self, mock_create_client, mock_settings):
        ...
        with pytest.raises(Exception) as exc_info:
            await SupabaseClient.update_record(
                table_name="hechos", record_id=999, data={"status": "updated"}
            )
    ...
    async def test_count_records_with_filters(self, mock_create_client, mock_settings):
        ...
        count = await SupabaseClient.count_records(
            table_name="hechos", filters={"pais": "Mexico", "importancia": 8}
        )
    ...
    @pytest.mark.asyncio
    async def test_count_records_without_filters(
        ...
        # Execute count
        count = await SupabaseClient.count_records(table_name="hechos")
...
class TestIntegrationScenarios:
    ...
    async def test_retry_with_helper_method(self, mock_create_client, mock_settings):
        ...
        mock_query.eq.return_value = mock_query
        mock_query.limit.return_value = mock_query
        mock_query.execute.side_effect = [Exception("Temporary failure"), mock_result]
...
class TestEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_select_with_pagination_edge_cases(
        self, mock_create_client, mock_settings
    ):
        ...
        data, total = await SupabaseClient.select_with_pagination(
            table_name="hechos", limit=0, offset=0
        )
        ...
        data, total = await SupabaseClient.select_with_pagination(
            table_name="hechos", limit=10, offset=1000000
        )
    ...
    @pytest.mark.asyncio
    async def test_count_records_with_none_filters(
        self, mock_create_client, mock_settings
    ):
    ...
    @pytest.mark.asyncio
    async def test_update_record_with_custom_id_column(
        self, mock_create_client, mock_settings
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_single_record_with_special_characters(
        self, mock_create_client, mock_settings
    ):
        ...
        record = await SupabaseClient.get_single_record(
            table_name="users", record_id="test@example.com", columns="id,type"
        )
    ...
    @pytest.mark.asyncio
    async def test_select_with_pagination_without_ordering(
        self, mock_create_client, mock_settings
    ):
