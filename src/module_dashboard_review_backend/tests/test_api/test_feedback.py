...
from src.models.requests import EvaluacionEditorialRequest, ImportanciaFeedbackRequest
from src.models.responses import FeedbackResponse  # noqa: F401
...
def valid_feedback_payload():
    ...
    return {"importancia_editor_final": 8, "usuario_id_editor": "editor123"}
...
def invalid_feedback_payloads():
    ...
    return [
        ...
        {"importancia_editor_final": 0, "usuario_id_editor": "editor123"},
        # Importance out of range (too high)
        {"importancia_editor_final": 11, "usuario_id_editor": "editor123"},
        # Missing usuario_id_editor
        {"importancia_editor_final": 5},
        # Invalid type for importance
        {"importancia_editor_final": "eight", "usuario_id_editor": "editor123"},
        # Empty payload
        {},
    ]


def test_submit_importancia_feedback_success(
    mock_feedback_service, valid_feedback_payload
):
    ...
    assert (
        f"Feedback de importancia actualizado para hecho {hecho_id}" in data["message"]
    )
...
def test_submit_importancia_feedback_hecho_not_found(
    mock_feedback_service, valid_feedback_payload
):
...
def test_submit_importancia_feedback_boundary_values(mock_feedback_service):
    ...
    response = client.post(
        f"/dashboard/feedback/hecho/{hecho_id}/importancia_feedback",
        json={"importancia_editor_final": 1, "usuario_id_editor": "editor123"},
    )
    assert response.status_code == 200
    ...
    response = client.post(
        f"/dashboard/feedback/hecho/{hecho_id}/importancia_feedback",
        json={"importancia_editor_final": 10, "usuario_id_editor": "editor123"},
    )
    assert response.status_code == 200
...
def test_submit_importancia_feedback_generic_error(
    mock_feedback_service, valid_feedback_payload
):
...
def test_submit_importancia_feedback_long_usuario_id(mock_feedback_service):
    ...
    response = client.post(
        f"/dashboard/feedback/hecho/{hecho_id}/importancia_feedback",
        json={"importancia_editor_final": 5, "usuario_id_editor": long_usuario_id},
    )
...
def test_submit_importancia_feedback_special_characters(mock_feedback_service):
    ...
    for editor_id in special_ids:
        response = client.post(
            f"/dashboard/feedback/hecho/{hecho_id}/importancia_feedback",
            json={"importancia_editor_final": 7, "usuario_id_editor": editor_id},
        )
...
def test_submit_importancia_feedback_large_hecho_id(
    mock_feedback_service, valid_feedback_payload
):
...
def test_submit_importancia_feedback_null_body():
    ...
    response = client.post(
        f"/dashboard/feedback/hecho/{hecho_id}/importancia_feedback", json=None
    )
...
def test_submit_importancia_feedback_request_headers(
    mock_feedback_service, valid_feedback_payload
):
    ...
    response = client.post(
        f"/dashboard/feedback/hecho/{hecho_id}/importancia_feedback",
        json=valid_feedback_payload,
        headers={"X-User-Agent": "TestClient/1.0", "X-Request-ID": "test-request-123"},
    )
...
def test_submit_evaluacion_editorial_success(
    mock_feedback_service, valid_evaluacion_payload
):
    ...
    assert (
        call_args[0][1].justificacion_evaluacion_editorial
        == "Fact correctly extracted and verified against multiple sources"
    )


def test_submit_evaluacion_editorial_hecho_not_found(
    mock_feedback_service, valid_evaluacion_payload
):
...
def test_submit_evaluacion_editorial_generic_error(
    mock_feedback_service, valid_evaluacion_payload
):
    """Test handling of generic errors from service."""
    # Setup mock to raise generic exception
...
def test_submit_evaluacion_editorial_long_justification(mock_feedback_service):
    ...
    hecho_id = 123
    long_justification = (
        "This is a very detailed justification. " * 100
    )  # ~3700 characters
...
def test_submit_evaluacion_editorial_large_hecho_id(
    mock_feedback_service, valid_evaluacion_payload
):
    """Test with very large hecho_id."""
    # Setup mock
...
def test_submit_evaluacion_editorial_null_body():
    ...
    response = client.post(
        f"/dashboard/feedback/hecho/{hecho_id}/evaluacion_editorial", json=None
    )
    ...
    for i in range(5):
        response = client.post(
            ...
            json={
                "evaluacion_editorial": "verificado_ok_editorial"
                if i % 2 == 0
                else "declarado_falso_editorial",
