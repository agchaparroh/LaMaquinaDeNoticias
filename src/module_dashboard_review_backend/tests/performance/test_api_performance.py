...
class TestAPIPerformance:
    ...
    def test_hechos_revision_response_time(self, client, mock_hechos_service):
        ...
        stats = self.measure_response_time(
            client, "/dashboard/hechos_revision", iterations=100
        )
    ...
    def test_hechos_revision_with_filters_performance(
        self, client, mock_hechos_service
    ):
        ...
        for name, params in test_cases:
            stats = self.measure_response_time(
                client, "/dashboard/hechos_revision", params=params, iterations=50
            )
            ...
            assert stats["mean"] < 150, (
                f"{name}: Mean response time should be under 150ms"
            )
    ...
    def test_feedback_endpoints_performance(self, client):
        ...
        with patch("src.api.feedback.get_feedback_service") as mock:
            ...
            assert importance_stats["mean"] < 50, (
                "Mean response time should be under 50ms"
            )
    ...
    def test_response_size_impact(self, client, mock_hechos_service):
        ...
        for size in page_sizes:
            ...
            if size > 10:
                assert time_per_item < 2.0, (
                    "Time per item should be under 2ms for larger pages"
                )
    ...
    def test_concurrent_request_performance(self, client, mock_hechos_service):
        ...
        for num_concurrent in [1, 5, 10, 20]:
            ...
            with concurrent.futures.ThreadPoolExecutor(
                ...
                futures = [
                    executor.submit(make_request) for _ in range(num_concurrent * 5)
                ]
            ...
            assert avg_time < 200 * (1 + num_concurrent / 10), (
    ...
    def test_cache_effectiveness(self, client):
        ...
        with patch("src.api.dashboard.get_hechos_service") as mock:
            ...
            cold_stats = self.measure_response_time(
                client, "/dashboard/filtros/opciones", iterations=10
            )

            # Subsequent calls - potentially cached
            warm_stats = self.measure_response_time(
                client, "/dashboard/filtros/opciones", iterations=50
            )
    ...
    def test_error_handling_performance(self, client):
        ...
        with patch("src.api.dashboard.get_hechos_service") as mock:
            ...
            print(f"\n=== Error Handling Performance ===")  # noqa: F541
    ...
    def test_health_check_performance(self, client):
        ...
        # Basic health check
        basic_stats = self.measure_response_time(client, "/health", iterations=200)
        ...
        with patch(
            "src.services.supabase_client.SupabaseClient.execute_with_retry"
        ) as mock:
            mock.return_value = MagicMock()

            detailed_stats = self.measure_response_time(
                client, "/health/detailed", iterations=100
            )
            ...
            assert detailed_stats["mean"] < 50, (
                "Detailed health check should be under 50ms"
            )
            assert detailed_stats["p99"] < 100, (
                "Detailed health check P99 should be under 100ms"
            )
...
class TestDatabaseQueryPerformance:
    ...
    def mock_supabase_with_timing(self):
        ...
        with patch("src.services.supabase_client.SupabaseClient.get_client") as mock:
            ...
            def simulate_query_time(query_type, params=None):
                ...
                if params:
                    filter_count = len(
                        [k for k in params.keys() if k.startswith("filter_")]
                    )
    ...
    def test_query_optimization_opportunities(self, mock_supabase_with_timing):
        ...
        for name, params in test_scenarios:
            ...
            try:
                result = loop.run_until_complete(  # noqa: F841
                ...
                if duration > 100:
                    print(f"  ⚠️  Consider optimizing this query pattern")  # noqa: F541
...
class TestRelationshipsPerformance:
                            ...
                            relaciones.append(
                                {
                                    ...
                                    "descripcion_relacion": f"Relationship {j}"
                                    if j % 2 == 0
                                    else None,
    ...
    def measure_response_time_with_relationships(
        self, client, endpoint, params=None, iterations=50
    ):
        ...
        for _ in range(iterations):
            ...
            data = response.json()
            total_relations = sum(
                len(item.get("relaciones", [])) for item in data.get("items", [])
            )
    ...
    def test_endpoint_performance_with_relationships(
        self, client, mock_hechos_service_with_relationships
    ):
        ...
        for scenario_name, dataset_key in test_scenarios:
            ...
            stats = self.measure_response_time_with_relationships(
                client, "/dashboard/hechos_revision", iterations=30
            )
            ...
            assert stats["mean"] < 200, (
                f"Mean response time for {scenario_name} should be under 200ms"
            )
            assert stats["p95"] < 500, (
                f"P95 response time for {scenario_name} should be under 500ms"
            )
            ...
            if scenario_name == "No relationships":
                ...
                assert overhead < 500, (
                    f"Relationship overhead should be under 500ms (PRD requirement)"
                )  # noqa: F541

    @pytest.mark.performance
    def test_relationship_query_performance(
        self, mock_hechos_service_with_relationships
    ):
        ...
        with patch(
            "src.services.hechos_service.SupabaseClient.get_client"
        ) as mock_client:
            ...
            for test_name, hecho_ids, expected_relations in test_cases:
                mock_table.execute = lambda: mock_execute_with_timing(
                    expected_relations
                )
                ...
                assert avg_time < 1000, (
                    f"Relationship query should complete in under 1000ms"
                )  # noqa: F541

                # Efficiency check - should scale reasonably
                if len(hecho_ids) <= 100:
                    assert avg_time < 100, (
                        f"Small to medium queries should be under 100ms"
                    )  # noqa: F541

    @pytest.mark.performance
    def test_pagination_performance_with_relationships(
        self, client, mock_hechos_service_with_relationships
    ):
        ...
        for scenario_name, params in pagination_scenarios:
            ...
            stats = self.measure_response_time_with_relationships(
                client, "/dashboard/hechos_revision", params=params, iterations=20
            )
            ...
            assert stats["mean"] < 300, (
                f"Pagination {scenario_name} should be under 300ms"
            )
            assert stats["p95"] < 600, (
                f"Pagination P95 {scenario_name} should be under 600ms"
            )

    @pytest.mark.performance
    def test_memory_usage_with_relationships(
        self, client, mock_hechos_service_with_relationships
    ):
        ...
        for test_name, dataset in memory_tests:
            ...
            assert memory_increase < 50, (
                f"Memory increase should be under 50MB for {test_name}"
            )
