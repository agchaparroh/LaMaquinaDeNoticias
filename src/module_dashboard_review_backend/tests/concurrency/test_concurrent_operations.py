...
class TestConcurrentRequests:
    ...
    def test_concurrent_reads(self, client, mock_thread_safe_service):
        ...
        # Verify results
        print(f"\n=== Concurrent Read Test ===")  # noqa: F541
    ...
    def test_concurrent_writes(self, client, mock_thread_safe_service):
        ...
        with ThreadPoolExecutor(max_workers=num_threads) as executor:
            futures = [executor.submit(submit_feedback, i) for i in range(num_threads)]

            all_results = []
            for future in as_completed(futures):
                all_results.extend(future.result())

        print(f"\n=== Concurrent Write Test ===")  # noqa: F541
    ...
    def test_read_write_race_conditions(self, client, mock_thread_safe_service):
        ...
        original_submit_feedback = (
            feedback_service.submit_importancia_feedback.side_effect
        )
    ...
    def test_singleton_thread_safety(self):
        ...
        print(f"\n=== Singleton Thread Safety Test ===")  # noqa: F541
    ...
    def test_concurrent_filter_options(self, client):
        ...
        with patch("src.api.dashboard.get_hechos_service") as mock:
            ...
            async def mock_get_options():
                ...
                with call_lock:
                    call_times.append({"thread_id": thread_id, "start": start_time})
            ...
            with ThreadPoolExecutor(max_workers=10) as executor:
                futures = [executor.submit(get_options, i) for i in range(10)]

                all_results = []
                for future in as_completed(futures):
                    all_results.extend(future.result())

            print(f"\n=== Concurrent Filter Options Test ===")  # noqa: F541
            print(f"Total requests: {len(all_results)}")
    ...
    def test_request_ordering(self, client, mock_thread_safe_service):
        ...
        async def mock_get_with_order(params):
            ...
            with order_lock:
                request_order.append({"id": request_id, "time": time.time()})

            # Variable processing time based on request ID
            await asyncio.sleep(0.01 + (request_id % 5) * 0.01)

            with order_lock:
                completion_order.append({"id": request_id, "time": time.time()})
        ...
        def make_ordered_request(request_id):
            response = client.get(
                "/dashboard/hechos_revision", params={"request_id": request_id}
            )
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            futures = [
                executor.submit(make_ordered_request, i) for i in range(num_requests)
            ]

            results = [future.result() for future in as_completed(futures)]  # noqa: F841

        print(f"\n=== Request Ordering Test ===")  # noqa: F541
    ...
    def test_connection_pool_behavior(self, client):
        ...
        with patch("src.services.supabase_client.SupabaseClient.get_client") as mock:
            ...
            connection_pool = {"available": 5, "in_use": 0, "max_wait": 0}
            ...
            def acquire_connection():
                ...
                with pool_lock:
                    connection_pool["max_wait"] = max(
                        connection_pool["max_wait"], wait_time
                    )
            ...
            def mock_execute():
                wait_time = acquire_connection()  # noqa: F841
                try:
            ...
            print(f"\n=== Connection Pool Test ===")  # noqa: F541
    ...
    async def test_async_batch_processing(self):
        ...
        with patch("src.services.supabase_client.SupabaseClient.get_client") as mock:
            ...
            async def mock_batch_update(updates):
                batch_operations.append({"size": len(updates), "time": time.time()})
            ...
            print(f"\n=== Async Batch Processing Test ===")  # noqa: F541
