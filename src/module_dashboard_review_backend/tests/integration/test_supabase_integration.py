...
SKIP_INTEGRATION = not all(
    [os.getenv("TEST_SUPABASE_URL"), os.getenv("TEST_SUPABASE_ANON_KEY")]
)

pytestmark = pytest.mark.skipif(
    SKIP_INTEGRATION, reason="Test Supabase credentials not configured"
)
...
class TestRealSupabaseIntegration:
        ...
        if created_hechos or created_feedback:
            ...
            if created_feedback:
                for feedback_id in created_feedback:
                    try:
                        client.table("feedback_importancia_hechos").delete().eq(
                            "id", feedback_id
                        ).execute()
                    except:  # noqa: E722
                        pass
            ...
            if created_hechos:
                for hecho_id in created_hechos:
                    try:
                        client.table("hechos").delete().eq("id", hecho_id).execute()
                    except:  # noqa: E722
                        pass
    ...
    async def test_real_database_connection(self, test_client):
        ...
        print(
            f"Response time: {data['dependencies']['supabase'].get('response_time_ms', 'N/A')}ms"
        )
    ...
    async def test_real_hecho_retrieval(self, test_client, test_data_cleanup):
        ...
        response = test_client.get(
            ...
            params={"area_geografica": "Argentina", "importancia_min": 7},
        )
    ...
    async def test_real_feedback_submission(self, test_client, test_data_cleanup):
        ...
        test_hecho = await self.create_test_hecho(
            contenido="Hecho for feedback test", importancia=5
        )
        ...
        assert (
            result.data["justificacion_evaluacion_editorial"]
    ...
    async def test_real_pagination(self, test_client, test_data_cleanup):
        ...
        for i in range(25):
            hecho = await self.create_test_hecho(
                contenido=f"Pagination test hecho {i}", importancia=(i % 10) + 1
            )
            if hecho:
                created_hechos.append(hecho["id"])
        ...
        # Test first page
        response = test_client.get(
            "/dashboard/hechos_revision", params={"limit": 10, "offset": 0}
        )
        ...
        assert page1["pagination"]["has_next"] == True  # noqa: E712

        # Test second page
        response = test_client.get(
            "/dashboard/hechos_revision", params={"limit": 10, "offset": 10}
        )
        ...
        assert page2["pagination"]["has_prev"] == True  # noqa: E712
        ...
        print(f"✓ Pagination working correctly")  # noqa: F541
    ...
    async def test_real_filter_options(self, test_client, test_data_cleanup):
        ...
        print(f"Available filters:")  # noqa: F541
    ...
    async def test_real_concurrent_operations(self, test_client, test_data_cleanup):
        ...
        for i in range(10):
            hecho = await self.create_test_hecho(
                contenido=f"Concurrent test hecho {i}", importancia=5
            )
            if hecho:
                test_hechos.append(hecho)
                created_hechos.append(hecho["id"])
        ...
        for i, hecho in enumerate(test_hechos):
            task = asyncio.create_task(submit_feedback(hecho["id"], (i % 10) + 1))
            tasks.append(task)
        ...
        print(f"Concurrent feedback submissions:")  # noqa: F541
        print(f"  Total: {len(results)}")
        print(f"  Successful: {successes}")
        print(f"  Failed: {failures}")

        # Most should succeed
        assert successes > len(results) * 0.8, (
    ...
    async def test_real_error_handling(self, test_client):
        ...
        response = test_client.post(
            ...
            json={"importancia_editor_final": 8, "usuario_id_editor": "test"},
        )
    ...
    async def test_real_performance_metrics(self, test_client, test_data_cleanup):
        ...
        timings = {"simple_query": [], "filtered_query": [], "paginated_query": []}
        ...
        for offset in range(0, 50, 10):
            start = time.time()
            response = test_client.get(
                "/dashboard/hechos_revision", params={"limit": 10, "offset": offset}
            )
            duration = time.time() - start
...
class TestDatabaseConstraints:
    ...
    async def test_unique_constraints(self, test_client, test_data_cleanup):
        ...
        response = test_client.post(
            ...
            json={"importancia_editor_final": 7, "usuario_id_editor": "test_user"},
        )
        ...
        response = test_client.post(
            f"/dashboard/feedback/hecho/{hecho['id']}/importancia_feedback",
            json={"importancia_editor_final": 9, "usuario_id_editor": "test_user"},
        )
