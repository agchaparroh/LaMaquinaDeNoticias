...
# Import models
from ..models.requests import HechoFilterParams  # noqa: F401
from ..models.responses import FilterOptionsResponse, HechoResponse, PaginatedResponse
...
async def get_hechos_for_revision(
    ...
    medio: Optional[str] = Query(None, description="Filter by media source"),
    area_geografica: Optional[str] = Query(
        None, description="Filter by country of publication"
    ),
    importancia_min: Optional[int] = Query(
        None, ge=1, le=10, description="Minimum importance level (1-10)"
    ),
    importancia_max: Optional[int] = Query(
        None, ge=1, le=10, description="Maximum importance level (1-10)"
    ),
    ...
    try:
        ...
        hechos, total_count = await hechos_service.get_hechos_for_revision(
            filter_params
        )
        ...
        logger.error(f"Error retrieving hechos for revision: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=500,
...
async def get_filter_options(
    ...
    try:
        ...
        logger.error(f"Error retrieving filter options: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail="Error retrieving filter options")
