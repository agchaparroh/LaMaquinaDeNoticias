...
class FeedbackService:
    ...
    async def submit_importancia_feedback(
        self, hecho_id: int, feedback: ImportanciaFeedbackRequest
    ) -> None:
        """
        ...
        hecho = await SupabaseClient.get_single_record(
            table_name="hechos", record_id=hecho_id
        )

        if not hecho:
            logger.error(f"Hecho with ID {hecho_id} not found")
            raise ResourceNotFoundError(detail=f"Hecho with ID {hecho_id} not found")
        ...
        if existing_result.data and len(existing_result.data) > 0:
            ...
            # Insert new feedback
            logger.info(f"Creating new feedback for hecho {hecho_id}")
    ...
    async def submit_evaluacion_editorial(
        self, hecho_id: int, evaluacion: EvaluacionEditorialRequest
    ) -> None:
        """
        ...
        logger.info(f"Successfully updated editorial evaluation for hecho {hecho_id}")
