...
class SupabaseClient:
    ...
    def _initialize_client(cls) -> None:
        ...
        try:
            ...
            raise DatabaseConnectionError(f"Invalid Supabase configuration: {str(e)}")
        except Exception as e:
            logger.error(f"Failed to initialize Supabase client: {str(e)}")
            raise DatabaseConnectionError(f"Could not connect to Supabase: {str(e)}")

    @classmethod
    async def execute_with_retry(
        cls, operation: Callable[..., T], *args, **kwargs
    ) -> T:
        """
    ...
    async def select_with_pagination(
        ...
        logger.debug(
            f"Selected {len(data)} records from {table_name} (total: {total_count})"
        )
    ...
    async def get_single_record(
        cls, table_name: str, record_id: Any, id_column: str = "id", columns: str = "*"
    ) -> Optional[Dict]:
        """
    ...
    async def update_record(
        cls, table_name: str, record_id: Any, data: Dict, id_column: str = "id"
    ) -> Dict:
        """
        ...
        def _update_record():
            client = cls.get_client()
            return (
                client.table(table_name).update(data).eq(id_column, record_id).execute()
            )
    ...
    async def count_records(
        cls, table_name: str, filters: Optional[Dict] = None
    ) -> int:
        """
        ...
        logger.debug(f"Counted {count} records in {table_name} with filters: {filters}")
