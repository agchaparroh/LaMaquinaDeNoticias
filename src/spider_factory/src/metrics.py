...
class Metrics:
    ...
    async def increment_spider_generated(
        self, medio: str, seccion: str, strategy: str, tiempo_generacion: float
    ):
        """
        ...
        # Log para tracking
        logger.info(
            f"Spider generado: {medio}_{seccion} | Estrategia: {strategy} | Tiempo: {tiempo_generacion:.2f}s"
        )
    ...
    async def record_error(self, error_type: str, details: Dict[str, Any]):
        ...
        await self.redis.lpush(f"{self.prefix}:errors:recent", json.dumps(error_data))
    ...
    async def get_spider_success_rate(self) -> float:
        ...
        total_errors = int(
            await self.redis.get(f"{self.prefix}:errors:by_type:spider_generation") or 0
        )
    ...
    async def get_request_reduction(self) -> float:
        ...
        total_analyses = cache_hits + int(
            await self.redis.get(f"{self.prefix}:cache:misses") or 0
        )
    ...
    async def get_system_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "tiempo_reduccion": round(
                await self.calculate_time_reduction(), 2
            ),  # Target: 97%
            "tiempo_promedio_rss": round(
                await self.get_average_time("rss"), 2
            ),  # Target: <5s
            "tiempo_promedio_primera_vez": round(
                await self.get_average_time("first"), 2
            ),  # Target: ~20s
            "tiempo_promedio_cache": round(
                await self.get_average_time("cache"), 2
            ),  # Target: <2s
            # Precisión
            "precision_spiders": round(
                await self.get_spider_success_rate(), 2
            ),  # Target: >90%
            # Eficiencia
            "reduccion_requests": round(
                await self.get_request_reduction(), 2
            ),  # Target: 70%
            "cache_hit_rate": round(await self.get_cache_hit_rate(), 2),
            # Throughput
            "spiders_por_dia": await self.get_daily_spider_count(),  # Target: 200+
            "spiders_total": int(
                await self.redis.get(f"{self.prefix}:spiders:total") or 0
            ),
            # Adopción
            "porcentaje_adopcion": round(
                await self.get_adoption_rate(), 2
            ),  # Target: >80%
            # Desglose por estrategia
            "spiders_por_estrategia": {
                "rss": int(
                    await self.redis.get(f"{self.prefix}:spiders:by_strategy:rss") or 0
                ),
                "scraping": int(
                    await self.redis.get(f"{self.prefix}:spiders:by_strategy:scraping")
                    or 0
                ),
                "playwright": int(
                    await self.redis.get(
                        f"{self.prefix}:spiders:by_strategy:playwright"
                    )
                    or 0
                ),
            },
...
async def log_spider_metrics(
    ...
    """
    if success:
        await metrics.increment_spider_generated(
            medio, seccion, strategy, generation_time
        )
    else:
...
async def log_analysis_metrics(
    metrics: Metrics, url: str, strategy: str, analysis_time: float, from_cache: bool
):
    """
...
if __name__ == "__main__":
    ...
    async def test_metrics():
        ...
        await metrics.increment_spider_generated(
            "La Nación", "Economía", "scraping", 18.2
        )
