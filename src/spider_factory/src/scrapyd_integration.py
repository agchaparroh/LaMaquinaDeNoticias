            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    if result.get("status") == "ok":
                        ...
                        logger.info(f"Spider {spider_name} programado en Scrapyd con job_id: {job_id}")
                    ...
                    logger.error(f"Error HTTP {response.status_code} al contactar Scrapyd")
                    return {
                        "success": False,
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    finished_jobs = [
                        job for job in data.get("finished", [])
...
async def register_spider_in_scrapyd(
    ...
    if not medio or not seccion:
        ...
        if len(parts) >= 2:
            ...
            raise ValueError("No se puede determinar medio y sección del nombre del spider")
