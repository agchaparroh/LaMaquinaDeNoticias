...
class SpiderMigrator:
    ...
    def migrate_to_v2(self, file_path: Path, metadata: Dict[str, str], 
        ...
        try:
            ...
            new_name = f"{medio.lower().replace(' ', '_')}_{seccion.lower().replace(' ', '_')}"
    ...
    tipo_medio = "diario"  # TODO: Ajustar según corresponda (diario|revista|agencia)'''
            ...
            fields_to_add = [
                ...
                "item['metadata'] = {",
                "    'spider_type': '{}',".format(metadata.get('spider_type', 'scraping')),
            ...
            parse_method = re.search(r"def\s+parse(_item|_article|_news)?\s*\([^)]+\):", content)
            if parse_method:
                # Buscar el primer yield/return dentro del método
                yield_match = re.search(r"(yield\s+item|return\s+item)", content[parse_method.end():])
                if yield_match:
                    ...
                    for field in fields_to_add:
                        ...
                        if f"['{field_name}']" not in existing_content and f'["{field_name}"]' not in existing_content:
                            fields_text += field + "\n        "
                    
                    if fields_text.strip():
                        content = content[:insert_pos] + fields_text + "\n        " + content[insert_pos:]
            ...
            if 'custom_settings' not in content:
                ...
                if class_match:
                    ...
                    insert_match = re.search(r"tipo_medio\s*=\s*['\"][^'\"]+['\"]", content)
                    if insert_match:
    ...
    custom_settings = {{
        ...
        'CRAWL_ONCE_DEFAULT': False,
                        ...
                        content = content[:insert_pos] + custom_settings + content[insert_pos:]
            else:
    ...
    def validate_migration(self, file_path: Path) -> Dict[str, bool]:
        ...
        validations['medio_field'] = re.search(r"medio\s*=\s*['\"][^'\"]+['\"]", content) is not None
        validations['seccion_field'] = re.search(r"seccion\s*=\s*['\"][^'\"]+['\"]", content) is not None
        validations['area_geografica_field'] = re.search(r"area_geografica\s*=\s*['\"][^'\"]+['\"]", content) is not None
        validations['tipo_medio_field'] = re.search(r"tipo_medio\s*=\s*['\"][^'\"]+['\"]", content) is not None
        
        # Verificar que no use 'titulo'
        validations['titular_not_titulo'] = "item['titulo']" not in content and 'item["titulo"]' not in content
        
        # Verificar campos en items
        validations['item_medio'] = "item['medio']" in content or 'item["medio"]' in content
        validations['item_area_geografica'] = "item['area_geografica']" in content or 'item["area_geografica"]' in content
        validations['item_tipo_medio'] = "item['tipo_medio']" in content or 'item["tipo_medio"]' in content
        validations['item_seccion'] = "item['seccion']" in content or 'item["seccion"]' in content
        validations['item_metadata'] = "item['metadata']" in content or 'item["metadata"]' in content
...
def main():
    ...
    parser.add_argument('--dry-run', action='store_true', help='Simula migración sin modificar archivos')
    parser.add_argument('--medio', help='Nombre del medio (si no se puede detectar)')
    parser.add_argument('--seccion', help='Nombre de la sección (si no se puede detectar)')
    parser.add_argument('--validate-only', action='store_true', help='Solo valida sin migrar')
    ...
    if success and not args.dry_run:
        ...
        if all(validations.values()):
            ...
            print(f"\n⚠️  Migración completada pero hay validaciones pendientes")
            for check, passed in validations.items():
                if not passed:
                    print(f"  ❌ {check}")
