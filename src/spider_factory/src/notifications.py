...
class NotificationSystem:
    ...
    async def notify_structure_change(
        self, medio: str, url: str, cambios: Dict[str, Any]
    ):
        """
    ...
    async def notify_performance_degradation(
        self, component: str, current_value: float, threshold: float, metric_name: str
    ):
        """
    ...
    async def check_and_notify_system_health(self, metrics: Dict[str, Any]):
        ...
        kpi_checks = [
            (
                "Tiempo RSS",
                metrics.get("tiempo_promedio_rss", 0),
                5,
                metrics.get("tiempo_promedio_rss", 0) > 10,
            ),
            (
                "Tiempo Cache",
                metrics.get("tiempo_promedio_cache", 0),
                2,
                metrics.get("tiempo_promedio_cache", 0) > 5,
            ),
            (
                "Reducción de Tiempo",
                metrics.get("tiempo_reduccion", 0),
                97,
                metrics.get("tiempo_reduccion", 0) < 90,
            ),
            (
                "Precisión Spiders",
                metrics.get("precision_spiders", 100),
                90,
    ...
    async def _send_notification(
        self, title: str, message: str, notification_data: Dict[str, Any]
    ):
        """
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                response = await client.post(
                    self.slack_webhook, json=slack_message, timeout=10.0
                )
...
if __name__ == "__main__":
    ...
    async def test_notifications():
        ...
        for notif in history:
            print(
                f"- [{notif['type']}] {notif.get('priority', 'medium')}: {notif['timestamp']}"
            )
