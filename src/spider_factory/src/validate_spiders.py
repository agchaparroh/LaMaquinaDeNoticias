...
class SpiderValidator:
    ...
    def validate_spider_file(self, file_path: Path) -> Dict[str, any]:
        ...
        try:
            ...
            if not name_validation['valid']:
                result['errors'].append(f"Nombre inválido: {name_validation['message']}")
                result['valid'] = False
            ...
            for field, status in item_fields.items():
                if not status['present'] and field != 'metadata':  # metadata es opcional en algunos casos
            ...
            if not area_validation['valid']:
                result['errors'].append(f"Área geográfica inválida: {area_validation['value']}")
                result['valid'] = False
            ...
            if not tipo_validation['valid']:
                result['errors'].append(f"Tipo de medio inválido: {tipo_validation['value']}")
                result['valid'] = False
    ...
    def _validate_spider_name(self, content: str) -> Dict[str, any]:
        ...
        if '_' not in name:
            return {'valid': False, 'message': f"'{name}' no sigue formato medio_seccion"}
            
        # No debe tener caracteres especiales excepto _
        if not re.match(r'^[a-z0-9_]+$', name):
            return {'valid': False, 'message': f"'{name}' contiene caracteres no permitidos"}
    ...
    def _validate_class_fields(self, content: str) -> Dict[str, Dict]:
        ...
        for field in self.REQUIRED_CLASS_FIELDS:
            ...
            if match:
                ...
                if field == 'area_geografica' and value not in AREAS_GEOGRAFICAS_VALIDAS:
                    results[field]['warning'] = f"Área geográfica '{value}' no está en la lista válida"
                elif field == 'tipo_medio' and value not in ['diario', 'revista', 'agencia']:
                    results[field]['warning'] = f"Tipo de medio '{value}' debe ser: diario, revista o agencia"
            else:
    ...
    def _validate_custom_settings(self, content: str) -> Dict[str, any]:
        ...
        settings_match = re.search(r'custom_settings\s*=\s*{([^}]+)}', content, re.DOTALL)
        ...
        for setting, expected in self.REQUIRED_SETTINGS.items():
            ...
            if match:
                ...
                if expected == True and value != 'True':
                    ...
                    result['valid'] = False
                elif expected == False and value != 'False':
    ...
    def _validate_area_geografica(self, content: str) -> Dict[str, any]:
        ...
        value = match.group(1)
        return {
            'valid': value in AREAS_GEOGRAFICAS_VALIDAS,
            'value': value
        }
    ...
    def _validate_tipo_medio(self, content: str) -> Dict[str, any]:
        ...
        value = match.group(1)
        return {
            'valid': value in ['diario', 'revista', 'agencia'],
            'value': value
        }
    ...
    def _calculate_field_coverage(self) -> Dict[str, float]:
        ...
        coverage = {
            'class_fields': defaultdict(int),
            'item_fields': defaultdict(int)
        }
        ...
        for result in self.validation_results:
            if 'fields_status' in result:
                # Campos de clase
                for field, status in result['fields_status'].get('class_fields', {}).items():
                    if status['present']:
                        coverage['class_fields'][field] += 1
                        
                # Campos de items
                for field, status in result['fields_status'].get('item_fields', {}).items():
                    if status['present']:
        ...
        for category in coverage:
            for field in coverage[category]:
                coverage[category][field] = round(coverage[category][field] / total * 100, 1)
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if self.summary['common_issues'].get('uses_titulo', 0) > 0:
            recommendations.append(
                "Cambiar campo 'titulo' por 'titular' en items"
            )
...
def main():
    ...
    parser.add_argument(
        '--spiders-dir',
        type=Path,
        default=Path("/mnt/c/Users/DELL/Desktop/PruebaWindsurfAI/LaMaquinaDeNoticias/src/module_scraper/scraper_core/spiders"),
        help='Directorio con los spiders a validar'
    )
    parser.add_argument('--report', action='store_true', help='Genera reporte detallado')
    ...
    if args.spider:
        ...
        if args.report:
            ...
            # Guardar reporte
            output_path = args.output or Path(f"spider_validation_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
