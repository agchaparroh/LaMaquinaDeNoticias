...
class SiteSelectors(BaseModel):
    ...
    class Config:
        json_encoders = {str: lambda v: v if v else None}
...
class AnalysisResult(BaseModel):
    ...
    frecuencia_minutos: int = Field(
        60, description="Frecuencia de actualización en minutos"
    )
...
class SmartAnalyzer:
    ...
    async def analyze(self, request: SiteAnalysisRequest) -> AnalysisResult:
        ...
        if not request.force_analysis:
            cached_result = await self._get_cached_analysis(
                request.url, request.medio, request.seccion
            )
            if cached_result:
        ...
        if not request.force_analysis:
            ...
            if pattern_result:
                logger.info(
                    f"Aplicando patrón conocido para {domain}/{request.seccion}"
                )
    ...
    async def _check_rss(
        self, url: HttpUrl, domain: str, seccion: str = None
    ) -> Optional[HttpUrl]:
        """
        ...
        if cached:
            ...
            if data.get("has_rss"):
                ...
                logger.info(
                    f"RSS encontrado en cache para {domain}: {data['feed_url']} (fuente: {data.get('source', 'unknown')})"
                )
    ...
    async def _rss_phase_1_standard(
        self, url: HttpUrl, base_url: str, cache_key: str
    ) -> Optional[HttpUrl]:
        """Fase 1: Detección estándar (HTML + URLs comunes)"""
        try:
            ...
            if response.status_code == 200:
                ...
                if matches:
                    ...
                    if await self._validate_feed(feed_url):
                        await self._cache_positive_result(
                            cache_key, feed_url, "Fase 1: HTML link"
                        )
            ...
            for rss_path in standard_paths:
                ...
                if await self._validate_feed(feed_url):
                    await self._cache_positive_result(
                        cache_key, feed_url, f"Fase 1: {rss_path}"
                    )
                    return HttpUrl(feed_url)
    ...
    async def _rss_phase_2_section(
        self, base_url: str, seccion: str, cache_key: str
    ) -> Optional[HttpUrl]:
        """Fase 2: Búsqueda por sección específica"""
        ...
        try:
            for pattern in section_patterns:
                ...
                if await self._validate_feed(feed_url):
                    await self._cache_positive_result(
                        cache_key, feed_url, f"Fase 2: {pattern}"
                    )
                    return HttpUrl(feed_url)
        except Exception as e:
    ...
    async def _rss_phase_3_regional(
        self, base_url: str, domain: str, cache_key: str
    ) -> Optional[HttpUrl]:
        ...
        try:
            for pattern in regional_patterns:
                ...
                if await self._validate_feed(feed_url):
                    await self._cache_positive_result(
                        cache_key, feed_url, f"Fase 3: {pattern}"
                    )
                    return HttpUrl(feed_url)
        except Exception as e:
    ...
    async def _rss_phase_4_advanced(
        self, url: HttpUrl, base_url: str, cache_key: str
    ) -> Optional[HttpUrl]:
        """Fase 4: Búsqueda avanzada (texto HTML + parámetros GET)"""
        try:
            ...
            if response.status_code == 200:
                ...
                for href in matches:
                    ...
                    if await self._validate_feed(feed_url):
                        await self._cache_positive_result(
                            cache_key, feed_url, f"Fase 4: Texto HTML"
                        )  # noqa: F541
                        return HttpUrl(feed_url)
            ...
            for pattern in get_patterns:
                ...
                if await self._validate_feed(feed_url):
                    await self._cache_positive_result(
                        cache_key, feed_url, f"Fase 4: {pattern}"
                    )
                    return HttpUrl(feed_url)
    ...
    async def _validate_feed(self, feed_url: str) -> bool:
        ...
        try:
            ...
            response = await self.http_client.get(
                ...
                headers={
                    "User-Agent": "Mozilla/5.0 (compatible; SpiderFactory-RSS/2.0)"
                },
            )
    ...
    async def _cache_positive_result(self, cache_key: str, feed_url: str, source: str):
        ...
        cache_data = {
            ...
            "source": source,
            "confidence": "high"
            if "HTML link" in source or "Fase 1:" in source
            else "medium",
            "timestamp": datetime.now().isoformat(),
            "ttl_hours": ttl // 3600,
        }
    ...
    def get_rss_effectiveness_stats(self) -> Dict[str, float]:
        ...
        return {
            ...
            "phase_1_effectiveness": round(
                self.rss_metrics["phase_1_hits"] / total * 100, 2
            ),
            "phase_2_effectiveness": round(
                self.rss_metrics["phase_2_hits"] / total * 100, 2
            ),
            "phase_3_effectiveness": round(
                self.rss_metrics["phase_3_hits"] / total * 100, 2
            ),
            "phase_4_effectiveness": round(
                self.rss_metrics["phase_4_hits"] / total * 100, 2
            ),
    ...
    async def _get_cached_analysis(
        self, url: HttpUrl, medio: str, seccion: str
        ...
        if domain_data and section.encode() in domain_data:
            try:
                ...
                return AnalysisResult(
                    ...
                    notes=f"Patrón conocido aplicado",  # noqa: F541
    ...
    async def _analyze_with_firecrawl(
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                markdown_content = data.get("data", {}).get("markdown", "")  # noqa: F841
                ...
                selectors, sample_articles = self._extract_selectors_from_content(
                    html_content, str(url)
                )

                # Determinar estrategia y confianza
                strategy = (
                    AnalysisStrategy.PLAYWRIGHT
                    if needs_js
                    else AnalysisStrategy.SCRAPING
                )
    ...
    async def _basic_analysis(
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # Análisis muy básico
                needs_js = bool(
                    re.search(r"<script.*?(react|vue|angular)", content, re.IGNORECASE)
                )
                ...
                return AnalysisResult(
                    url=url,
                    domain=domain,
                    strategy=AnalysisStrategy.PLAYWRIGHT
                    if needs_js
                    else AnalysisStrategy.SCRAPING,
                ...
                for link in links[:5]:
                    ...
                    if href:
                        ...
                        if title and len(title) > 10:
                            sample_articles.append({"url": full_url, "title": title})
                break
    ...
    def _calculate_confidence(
        self, selectors: SiteSelectors, sample_articles: List[Dict[str, str]]
    ) -> float:
    ...
    async def _cache_analysis(self, url: HttpUrl, result: AnalysisResult):
        ...
        self.redis.setex(cache_key, TTL_7_DAYS, json.dumps(data))
        logger.info(f"Análisis cacheado para {url} con TTL de 7 días")

    async def _save_pattern(self, domain: str, section: str, result: AnalysisResult):
        """
...
async def test_analyzer():
    ...
    try:
        ...
        request = SiteAnalysisRequest(
            url="https://elpais.com/internacional", section_name="internacional"
        )

        result = await analyzer.analyze(request)
        print(f"Análisis completado:")  # noqa: F541
