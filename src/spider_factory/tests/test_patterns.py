...
from src.patterns import Pattern, PatternStorage  # noqa: F401
...
class TestPatternStorage:
    ...
    async def test_get_pattern_exists(self, mock_pattern_storage, mock_redis_client):
        ...
        mock_redis_client.exists = AsyncMock(return_value=True)  # noqa: F821
        mock_redis_client.hgetall = AsyncMock(return_value=pattern_data)  # noqa: F821
    ...
    @pytest.mark.asyncio
    async def test_get_pattern_not_exists(
        self, mock_pattern_storage, mock_redis_client
    ):
        """Test obtención de patrón no existente"""
        mock_redis_client.exists = AsyncMock(return_value=False)  # noqa: F821
    ...
    @pytest.mark.asyncio
    async def test_update_pattern_success(
        self, mock_pattern_storage, mock_redis_client
    ):
        """Test actualización de patrón con éxito"""
        # Configurar patrón existente
        pattern_data = {"success_count": "5", "failure_count": "1"}
        mock_redis_client.hgetall = AsyncMock(return_value=pattern_data)  # noqa: F821
        ...
        mock_redis_client.hincrby.assert_called_with(
            "pattern:example.com", "success_count", 1
        )

    @pytest.mark.asyncio
    async def test_update_pattern_failure(
        self, mock_pattern_storage, mock_redis_client
    ):
        ...
        mock_redis_client.hincrby.assert_called_with(
            "pattern:example.com", "failure_count", 1
        )

    @pytest.mark.asyncio
    async def test_pattern_failure_threshold(
        self, mock_pattern_storage, mock_redis_client
    ):
    ...
    async def test_list_patterns(self, mock_pattern_storage, mock_redis_client):
        ...
        pattern_keys = ["pattern:example.com", "pattern:test.com", "pattern:news.com"]
        mock_redis_client.keys = AsyncMock(return_value=pattern_keys)  # noqa: F821
    ...
    async def test_pattern_validation(self, mock_pattern_storage):
        ...
        is_valid = (
            pattern.selectors.title is not None
            and pattern.selectors.content is not None
        )
        assert not is_valid
    ...
    async def test_pattern_serialization(self, mock_pattern_storage):
        ...
        original_pattern = Pattern(
            domain="test.com",
            selectors=SiteSelectors(
                title="h1", content="article", date="time", author="span.author"
            ),
