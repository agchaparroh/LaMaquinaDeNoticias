...
for i, line in enumerate(config_content.split('\n'), 1):
    ...
    if not in_redis_manager_init and 'config.' in line and not line.strip().startswith('#'):
...
configs_found = 0
required_configs = [
    'CACHE_TTL_DAYS', 'CACHE_TTL_SECONDS', 'SPIDER_OUTPUT_PATH',
    'MAX_BATCH_SIZE', 'CONCURRENT_REQUESTS', 'BATCH_TIMEOUT',
    'RATE_LIMIT_REQUESTS', 'RATE_LIMIT_WINDOW', 'REDIS_MAX_CONNECTIONS'
]
...
print(f"{check_mark(configs_found == len(required_configs))} {configs_found}/{len(required_configs)} configuraciones encontradas")

# 3. Verificar validate_config
print("\n3. Verificando función validate_config...")
has_validate = 'def validate_config' in config_content
print(f"{check_mark(has_validate)} Función validate_config {'existe' if has_validate else 'NO existe'}")
...
print(f"{check_mark(logging_exists)} logging_config.py {'existe' if logging_exists else 'NO existe'}")
...
print(f"{check_mark(has_areas)} AREAS_GEOGRAFICAS_VALIDAS {'definida' if has_areas else 'NO definida'}")
...
all_good = (
    not bad_refs and 
    configs_found == len(required_configs) and
    has_validate and
    logging_exists and
    has_areas
)
