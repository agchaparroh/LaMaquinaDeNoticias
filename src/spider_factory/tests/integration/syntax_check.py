...
def find_python_files(root_dir):
    ...
    for root, dirs, files in os.walk(root_dir):
        ...
        dirs[:] = [d for d in dirs if d not in [
            ...
            'node_modules', '.git', 'htmlcov', '.pytest_cache'
        ]]
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.Import):
                    ...
                    if node.module and node.module.startswith(('httpx', 'redis', 'pydantic', 'tenacity')):
                        relative_path = os.path.relpath(file_path, root_dir)
...
def main():
    ...
    # Guardar resultados
    output_file = os.path.join(root_dir, f"syntax_check_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
