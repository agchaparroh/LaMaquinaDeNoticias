...
try:
    ...
    from src.batch_processor import BatchProcessor  # noqa: F401
...
class RealSitesTest:
    ...
    async def test_site_analysis(self, site: Dict) -> Dict:
        ...
        try:
            # Crear request
            request = AnalysisRequest(url=site["url"], section_name=site["section"])
            ...
            if os.getenv("FIRECRAWL_API_KEY"):
                ...
                if result.needs_javascript:
                    print(f"  ⚠️  Requiere JavaScript")  # noqa: F541
            ...
            print(f"  ❌ Error: {str(e)}")
            return {"site": site["name"], "success": False, "error": str(e)}
    ...
    async def test_spider_generation(self, site: Dict, analysis_result: Dict) -> bool:
        ...
        try:
            # Preparar datos
            site_info = SiteInfo(
                name=site["name"], url=site["url"], domain=site["url"].split("/")[2]
            )
    ...
    async def run_all_tests(self):
        ...
        for site in TEST_SITES:
            ...
            if analysis_result["success"]:
                generation_success = await self.test_spider_generation(
    ...
    def show_summary(self, batch_success: bool):
        ...
        for result in self.results:
            if result["success"]:
                ...
                print(
                    f"  {status} {result['site']}: {strategy} (conf: {confidence:.2f}) {spider}"
                )
            else:
                print(
                    f"  ❌ {result['site']}: {result.get('error', 'Error desconocido')}"
                )
