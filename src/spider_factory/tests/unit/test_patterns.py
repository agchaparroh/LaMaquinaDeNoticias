...
from src.patterns import Pattern, PatternStatus, PatternStorage
...
class TestPatternStorage:
    ...
    def test_get_pattern_by_domain_section(self, pattern_storage, mock_redis):
        ...
        # Ejecutar
        patterns = pattern_storage.get_patterns_by_domain_section(
            "example.com", "noticias"
        )
    ...
    def test_search_patterns(self, pattern_storage, mock_redis):
        ...
        results = pattern_storage.search_patterns(
            min_confidence=0.8, status=PatternStatus.ACTIVE
        )
    ...
    def test_pattern_versioning(self, pattern_storage, mock_redis, sample_pattern):
        ...
        new_selectors = SiteSelectors(
            title="h2.title-new", content="article.content-new"
        )

        # Ejecutar
        updated = pattern_storage.update_pattern(  # noqa: F841
            sample_pattern.id, selectors=new_selectors
        )
