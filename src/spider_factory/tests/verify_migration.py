...
def verify_migration_scripts():
    ...
    base_path = Path("/mnt/c/Users/DELL/Desktop/PruebaWindsurfAI/LaMaquinaDeNoticias/src/spider_factory/src")
    
    scripts = {
        "migrate_spider.py": {
            "classes": ["SpiderMigrator"],
            "methods": ["detect_spider_type", "backup_spider", "migrate_to_v2", "validate_migration"],
            "required_strings": ["AREAS_GEOGRAFICAS_VALIDAS", "custom_settings", "titular"]
        },
        "validate_spiders.py": {
            "classes": ["SpiderValidator"],
            "methods": ["validate_spider_file", "validate_all_spiders", "generate_compatibility_report"],
            "required_strings": ["REQUIRED_CLASS_FIELDS", "REQUIRED_ITEM_FIELDS", "REQUIRED_SETTINGS"]
        },
        "batch_migrate.py": {
    ...
    for script_name in scripts.keys():
        ...
        if script_path.exists():
            ...
            print(f"   {status} {script_name} {'es' if is_executable else 'no es'} ejecutable")
...
def main():
    ...
    if scripts_ok:
        ...
        print("\n📋 Uso de los scripts:")
        print("   1. validate_spiders.py --report  # Ver qué spiders necesitan migración")
