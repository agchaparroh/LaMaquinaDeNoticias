...
analysis_match = re.search(
    r"class AnalysisRequest\(BaseModel\):(.*?)(?=class|\Z)", api_content, re.DOTALL
)
if analysis_match:
...
analyze_match = re.search(
    r"async def analyze_site\(request: AnalysisRequest\):(.*?)(?=@app|\Z)",
    api_content,
    re.DOTALL,
)
if analyze_match:
...
generate_match = re.search(
    r"async def generate_spider\(request: GenerateSpiderRequest\):(.*?)(?=@app|\Z)",
    api_content,
    re.DOTALL,
)
if generate_match:
...
if "ScrapingStrategy" in api_content:
    print(
        f"{check_mark(False)} ERROR: Se encontró ScrapingStrategy (debe ser AnalysisStrategy)"
    )
else:
...
passed_checks = sum(
    [
        len(found_fields)
        == len(required_fields),  # Todos los campos en AnalysisRequest
        len(found_validators) == len(validators),  # Validadores retrocompatibilidad
        "/api/check-duplicate" in api_content
        and "@app.post" in api_content,  # Endpoint existe
