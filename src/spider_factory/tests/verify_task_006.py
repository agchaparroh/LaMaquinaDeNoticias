...
if files_exist["redis_pool.py"]:
    with open(os.path.join(BASE_PATH, "redis_pool.py")) as f:
...
if files_exist["metrics.py"]:
    with open(os.path.join(BASE_PATH, "metrics.py")) as f:
...
if files_exist["performance_metrics.py"]:
    with open(os.path.join(BASE_PATH, "performance_metrics.py")) as f:
        perf_content = f.read()
...
if files_exist["notifications.py"]:
    with open(os.path.join(BASE_PATH, "notifications.py")) as f:
...
passed_checks = sum(
    [
        ...
        "max_connections=50" in redis_pool_content
        if files_exist["redis_pool.py"]
        else False,
        "get_system_metrics" in metrics_content if files_exist["metrics.py"] else False,
        "RSS_TIME = 5" in perf_content
        if files_exist["performance_metrics.py"]
        else False,
        "notify_spider_failure" in notif_content
        if files_exist["notifications.py"]
        else False,
