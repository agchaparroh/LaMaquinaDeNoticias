...
from src.analyzer import SmartAnalyzer, AnalysisStrategy
from src.generator import SpiderGenerator
from src.patterns import PatternStorage
from src.metrics import Metrics
from src.models import AnalysisRequest, GenerateSpiderRequest
...
class TestIntegration:
    ...
    @pytest.mark.asyncio
    async def test_full_flow_rss_site(self, mock_analyzer, mock_generator, mock_redis_client):
        """Test flujo completo para sitio con RSS"""
        # 1. Análisis inicial encuentra RSS
        mock_analyzer.firecrawl.scrape = AsyncMock(return_value={
            'html': '''
            <html>
                <head>
    ...
    @pytest.mark.asyncio
    async def test_full_flow_scraping_site(self, mock_analyzer, mock_generator, mock_pattern_storage, mock_redis_client):
        """Test flujo completo para sitio de scraping"""
        # 1. No hay RSS, se hace análisis completo
        mock_analyzer.firecrawl.scrape = AsyncMock(side_effect=[
            {
                'html': '''
                <html>
                    <body>
    ...
    @pytest.mark.asyncio
    async def test_full_flow_cached_result(self, mock_analyzer, mock_generator, mock_redis_client):
    ...
    @pytest.mark.asyncio
    async def test_full_flow_with_pattern(self, mock_analyzer, mock_generator, mock_pattern_storage, mock_redis_client):
    ...
    @pytest.mark.asyncio
    async def test_batch_processing(self, mock_analyzer, mock_generator, mock_batch_sites):
    ...
    @pytest.mark.asyncio
    async def test_metrics_tracking(self, mock_analyzer, mock_generator, mock_metrics, mock_redis_client):
    ...
    @pytest.mark.asyncio
    async def test_error_recovery_flow(self, mock_analyzer, mock_generator, mock_redis_client):
