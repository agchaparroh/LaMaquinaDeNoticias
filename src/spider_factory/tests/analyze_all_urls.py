...
class NewsUrlAnalyzer:
                ...
                "extract": {
                    ...
                    "schema": {
                        ...
                        "properties": {
                            ...
                            "social_media_integration": {"type": "array", "items": {"type": "string"}},
                            ...
                            "third_party_scripts": {"type": "array", "items": {"type": "string"}},
                            ...
                            "monetization_methods": {"type": "array", "items": {"type": "string"}},
                            "content_categories": {"type": "array", "items": {"type": "string"}},
                            "multimedia_types": {"type": "array", "items": {"type": "string"}},
                            ...
                            "analytics_providers": {"type": "array", "items": {"type": "string"}},
                            "tag_managers": {"type": "array", "items": {"type": "string"}},
                            ...
                            "alternate_languages": {"type": "array", "items": {"type": "string"}},
    ...
    async def analyze_all_urls(self):
        ...
        file_path = Path("/mnt/c/Users/DELL/Desktop/PruebaWindsurfAI/LaMaquinaDeNoticias/src/spider_factory/tests/spiders de prueba/lista_urls")
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            ...
            for line in f:
                ...
                # Detectar categorías
                if line in ["Hispanidad", "Fuentes por País", "Fuentes de Análisis e Informes", "Historia y Cultura"]:
        ...
        for i in range(0, len(urls_to_analyze), batch_size):
            ...
            if i + batch_size < len(urls_to_analyze):
                print(f"\nPausando 5 segundos antes del siguiente lote...")
    ...
    def generate_report(self):
        ...
        report_path = Path("/mnt/c/Users/DELL/Desktop/PruebaWindsurfAI/LaMaquinaDeNoticias/src/spider_factory/tests/INFORME_EXHAUSTIVO_DESAFIOS_SPIDER_FACTORY.md")
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            f.write(f"**URLs exitosas:** {sum(1 for r in self.results if r['success'])}\n")
            f.write(f"**URLs con errores:** {sum(1 for r in self.results if not r['success'])}\n\n")
            ...
            for pattern, count in self.pattern_types.items():
                if count > 0:
                    ...
                    f.write(f"- **{pattern.replace('_', ' ').title()}:** {count} sitios ({percentage:.1f}%)\n")
            
            f.write("\n## LISTA EXHAUSTIVA DE DESAFÍOS IDENTIFICADOS\n\n")
            
            # Ordenar desafíos por frecuencia
            challenge_counts = [(k, len(v)) for k, v in self.challenges.items() if len(v) > 0]
            challenge_counts.sort(key=lambda x: x[1], reverse=True)
            
            for challenge_type, count in challenge_counts:
                f.write(f"### {challenge_type.replace('_', ' ').upper()} ({count} sitios)\n\n")
            ...
            for result in self.results:
                ...
                if result['success']:
                    ...
                    f.write(f"- RSS disponible: {'✅ Sí' if extract.get('has_rss') else '❌ No'}\n")
                    if extract.get('rss_urls'):
                        for rss in extract.get('rss_urls', []):
                            f.write(f"  - RSS URL: `{rss}`\n")
                    
                    f.write(f"- Requiere JavaScript: {'⚠️ Sí' if extract.get('requires_javascript') else '✅ No'}\n")
                    f.write(f"- Tiene paywall: {'🔒 Sí' if extract.get('has_paywall') else '✅ No'}\n")
                    f.write(f"- Requiere login: {'🔐 Sí' if extract.get('requires_login') else '✅ No'}\n")
                    f.write(f"- CMS detectado: {extract.get('cms_detected', 'No identificado')}\n")
                    f.write(f"- Framework SPA: {extract.get('spa_framework', 'Ninguno')}\n")
                    
                    # Selectores detectados
                    if any([extract.get('main_content_selector'), extract.get('article_selector'), 
                           extract.get('title_selector'), extract.get('date_selector')]):
                        f.write("\n**Selectores identificados:**\n")
                        if extract.get('main_content_selector'):
                            f.write(f"- Contenido principal: `{extract['main_content_selector']}`\n")
                    ...
                    f.write("\n**Detalles técnicos:**\n")
                    f.write(f"- Método de carga: {extract.get('content_loading_method', 'No especificado')}\n")
                    f.write(f"- Usa Cloudflare: {'⚠️ Sí' if extract.get('uses_cloudflare') else 'No'}\n")
                    f.write(f"- Usa CAPTCHA: {'🛡️ Sí' if extract.get('uses_captcha') else 'No'}\n")
                    f.write(f"- Scroll infinito: {'♾️ Sí' if extract.get('infinite_scroll') else 'No'}\n")
                    f.write(f"- Paginación AJAX: {'🔄 Sí' if extract.get('ajax_pagination') else 'No'}\n")
                    f.write(f"- Optimizado móvil: {'📱 Sí' if extract.get('mobile_optimized') else 'No'}\n")
                    f.write(f"- AMP habilitado: {'⚡ Sí' if extract.get('uses_amp') else 'No'}\n")
                    f.write(f"- PWA/Service Worker: {'🔧 Sí' if extract.get('uses_service_worker') else 'No'}\n")
                    
                    # APIs y endpoints
                    if extract.get('api_endpoints_found'):
                        f.write("\n**APIs detectadas:**\n")
                        for api in extract['api_endpoints_found'][:5]:  # Mostrar máximo 5
                            f.write(f"- `{api}`\n")
                    
                    # Métricas de rendimiento
                    f.write(f"\n**Rendimiento:**\n")
                    f.write(f"- Tiempo de respuesta: {result.get('response_time_ms', 'N/A')} ms\n")
                    f.write(f"- Tamaño Markdown: {result.get('markdown_length', 0):,} caracteres\n")
                    f.write(f"- Tamaño HTML: {result.get('html_length', 0):,} caracteres\n")
            ...
            f.write("| Sitio | RSS | JS | Paywall | Login | Cloudflare | Anti-Scraping | Complejidad |\n")
            f.write("|-------|-----|----|---------⁣|-------|------------|---------------|-------------|\n")
            ...
            for result in self.results:
                if result['success']:
                    ...
                    f.write(f"{'🛡️' if extract.get('anti_scraping_detected') else '✅'} | ")
            ...
            with open(json_path, 'w', encoding='utf-8') as json_file:
                json.dump({
                    ...
                    "challenges": {k: [{"name": n, "url": u} for n, u in v] 
                                  for k, v in self.challenges.items() if v},
...
"""
        ...
        return recommendations.get(challenge_type, """
...
""")
    ...
    def _generate_strategic_recommendations(self) -> str:
        ...
        rss_percentage = (len(self.challenges["has_rss"]) / total_sites * 100) if total_sites > 0 else 0
        js_percentage = (len(self.challenges["javascript_required"]) / total_sites * 100) if total_sites > 0 else 0
        paywall_percentage = (len(self.challenges["paywalls"]) / total_sites * 100) if total_sites > 0 else 0
...
async def main():
    ...
    try:
        ...
        print(f"Informe guardado en: INFORME_EXHAUSTIVO_DESAFIOS_SPIDER_FACTORY.md")
        print(f"Datos JSON guardados en: INFORME_EXHAUSTIVO_DESAFIOS_SPIDER_FACTORY.json")
