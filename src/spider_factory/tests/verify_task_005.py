...
if not bad_refs:
    print(
        f"{check_mark(True)} No hay referencias 'config.' (todas cambiadas a 'settings.')"
    )
else:
...
if "AREAS_GEOGRAFICAS_VALIDAS" in config_content:
    ...
    if areas_match:
        ...
        if area_count == expected:
            ...
            print(
                f"  {check_mark(False)} Contiene {area_count} áreas (esperadas {expected})"
            )
else:
