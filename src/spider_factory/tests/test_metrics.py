...
class TestMetrics:
    ...
    async def test_record_analysis(self, mock_metrics, mock_redis_client):
        ...
        await mock_metrics.record_analysis(
            domain="example.com", strategy="RSS", time_taken=5.2, from_cache=False
        )
    ...
    @pytest.mark.asyncio
    async def test_get_cache_hit_rate_no_analyses(
        self, mock_metrics, mock_redis_client
    ):
    ...
    async def test_get_average_time_saved(self, mock_metrics, mock_redis_client):
        ...
        mock_redis_client.lrange = AsyncMock(side_effect=[first_times, cache_times])
    ...
    async def test_get_summary(self, mock_metrics, mock_redis_client):
        ...
        mock_metrics.get_most_analyzed_domains = AsyncMock(
            return_value=[("example.com", 50), ("test.com", 30)]
        )
