...
class TestAPI:
    ...
    def test_analyze_endpoint_success(self, mock_analyzer_class, client):
        ...
        # Configurar mock
        mock_analyzer = Mock()
        mock_analyzer.analyze_site = AsyncMock(
    ...
    def test_analyze_endpoint_backward_compatibility(self, mock_analyzer_class, client):
        ...
        response = client.post(
            "/analyze", json={"url": "https://example.com", "name": "old_style_name"}
        )
    ...
    def test_generate_endpoint_success(
        self, mock_analyzer_class, mock_generator_class, client
    ):
        ...
        mock_generator.save_spider = Mock(
            return_value="/output/example_news_internacional.py"
        )
    ...
    def test_check_duplicate_endpoint(self, mock_redis_func, client):
        ...
        response = client.post(
            "/check-duplicate", json={"medio": "El País", "seccion": "Internacional"}
        )
        ...
        response = client.post(
            "/check-duplicate", json={"medio": "New Site", "seccion": "News"}
        )
    ...
    def test_analyze_invalid_request(self, client):
        ...
        # Sin URL
        response = client.post("/analyze", json={"medio": "Test"})
    ...
    def test_metrics_endpoint(self, mock_metrics_class, client):
        ...
        # Configurar mock
        mock_metrics = Mock()
        mock_metrics.get_all_metrics = AsyncMock(
            return_value={
                "analysis_count": 100,
                "spider_count": 50,
                "cache_hits": 30,
    ...
    def test_metrics_summary_endpoint(self, mock_metrics_class, client):
        ...
        # Configurar mock
        mock_metrics = Mock()
        mock_metrics.get_summary = AsyncMock(
            return_value={
                "total_analyses": 1000,
                "total_spiders": 500,
                "cache_hit_rate": 0.75,
    ...
    def test_metrics_performance_endpoint(self, mock_validator_class, client):
        ...
        # Configurar mock
        mock_validator = Mock()
        mock_validator.validate_kpis = AsyncMock(
            return_value={
                "rss_time_ok": True,
                "first_time_ok": True,
                "cache_time_ok": True,
