...
class SpiderMigrator:
    ...
    def migrate_spider(self, spider_file: Path) -> bool:
        ...
        try:
            ...
            with open(spider_file, encoding="utf-8") as f:
                content = f.read()

            # Guardar backup
            backup_path = (
                self.backup_dir
                / f"{spider_file.stem}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.py"
            )
    ...
    def _migrate_class_definition(self, content: str) -> str:
        ...
        content = re.sub(
            r"class (\w+)\(Spider\)", r"class \1(BaseArticleSpider)", content
        )
...
"""
    ...
    def _migrate_parse_methods(self, content: str) -> str:
        ...
        def replace_yield(match):
            ...
            item_code = (
                "loader = ArticuloLoader(item=ArticuloItem(), response=response)\n"
            )
                ...
                if ":" in line:
                    ...
                    if key_value:
                        ...
                        if len(parts) == 2:
                            ...
                            field_lines.append(
                                f"        loader.add_value('{mapped_field}', {value})"
                            )

            # Agregar campos requeridos
            field_lines.append("        loader.add_value('medio_url', response.url)")
            field_lines.append(
                "        loader.add_value('seccion', getattr(self, 'section_name', 'general'))"
            )
...
def main():
    ...
    parser.add_argument("--spider", "-s", help="Migrar solo un spider específico")
