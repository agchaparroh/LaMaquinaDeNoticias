...
class TestNginxConfiguration:
    ...
    def test_nginx_config_exists(self, nginx_config_file):
        ...
        assert nginx_config_file.exists(), (
            f"nginx.conf no encontrado en {nginx_config_file}"
        )
    ...
    def test_nginx_config_syntax(self, nginx_config_file, project_root):
        ...
        result = subprocess.run(
            [
                ...
                "--rm",
                "-v",
                f"{nginx_config_file}:/etc/nginx/nginx.conf:ro",
                "nginx:1.25-alpine",
                "nginx",
                "-t",
            ],
        ...
        assert result.returncode == 0, (
            f"Sintaxis inválida en nginx.conf: {result.stderr}"
        )
    ...
    def test_nginx_config_has_upstreams(self, nginx_config_file):
        ...
        for upstream in required_upstreams:
            assert upstream in config_content, (
                f"Upstream '{upstream}' no encontrado en nginx.conf"
            )
    ...
    def test_nginx_config_has_rate_limiting(self, nginx_config_file):
        ...
        rate_limit_zones = ["zone=api:10m rate=100r/m", "zone=general:10m rate=300r/m"]
    ...
    def test_health_check_script_in_dockerfile(self, project_root):
        ...
        assert "health-check.sh" in dockerfile_content, (
            "Script de health check no encontrado en Dockerfile"
        )
        assert "HEALTHCHECK" in dockerfile_content, (
            "HEALTHCHECK no configurado en Dockerfile"
        )
