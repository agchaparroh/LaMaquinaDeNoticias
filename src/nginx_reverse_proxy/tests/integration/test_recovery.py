...
class TestRecovery:
    ...
    def test_backend_recovery(
        ...
        print(
            f"  Tiempo de recuperación: {recovery_time}s"
            if recovery_time
            else "  No se recuperó"
        )
        ...
        assert recovery_time is not None, (
            "Nginx no se recuperó después de que el backend volvió"
        )
    ...
    def test_partial_upstream_failure(self, nginx_container_running, nginx_base_url):
        ...
        for name, stats in results.items():
            print(
                f"    {name}: {stats['success']}/10 exitosos ({stats['rate'] * 100:.0f}%)"
            )
    ...
    def test_nginx_reload_recovery(
        self, nginx_container_running, nginx_base_url, docker_client
    ):
        """
                ...
                try:
                    ...
                    if response.status_code != 200:
                        errors_during_reload.append(
                            {"time": datetime.now(), "status": response.status_code}
                        )
                except Exception as e:
                    errors_during_reload.append(
                        {"time": datetime.now(), "error": str(e)}
                    )
    ...
    def test_automatic_reconnection(
        ...
        successful_cycles = sum(
            1 for phase, status in results if phase == "recovered" and status == 200
        )
    ...
    def test_connection_pool_recovery(self, nginx_container_running, nginx_base_url):
        ...
        for i in range(100):
            try:
                ...
                response = session.get(
                    f"{nginx_base_url}/api/get", stream=True, timeout=1
                )
                if response.status_code == 200:
        ...
        try:
            ...
            print(
                f"  Respuesta con pool saturado: {response.status_code} en {slow_response_time:.2f}s"
            )
        except:  # noqa: E722
        ...
        if recovery_times:
            ...
            print(
                f"  Tiempo promedio después de recuperación: {avg_recovery_time:.2f}s"
            )
            assert avg_recovery_time < 0.5, (
                f"Respuestas muy lentas después de recuperación: {avg_recovery_time:.2f}s"
            )

    def test_rate_limit_state_persistence(
    ...
    def test_health_check_state_recovery(
        self, nginx_container_running, nginx_base_url, docker_client
    ):
        """
        ...
        print(
            f"  Estado inicial del health check: {'OK' if initial_health else 'FAIL'}"
        )
