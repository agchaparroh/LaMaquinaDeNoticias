...
class TestErrorHandling:
    ...
    def test_backend_down_handling(self, nginx_container_running, nginx_base_url):
        ...
        subprocess.run(
            ["docker", "stop", "mock_dashboard_backend"], capture_output=True
        )
        ...
        try:
            ...
            subprocess.run(
                ["docker", "start", "mock_dashboard_backend"], capture_output=True
            )
            time.sleep(2)
    ...
    def test_frontend_down_handling(self, nginx_container_running, nginx_base_url):
        ...
        subprocess.run(
            ["docker", "stop", "mock_dashboard_frontend"], capture_output=True
        )
        ...
        try:
            ...
            subprocess.run(
                ["docker", "start", "mock_dashboard_frontend"], capture_output=True
            )
            time.sleep(2)
    ...
    def test_connection_limit_handling(self, nginx_container_running, nginx_base_url):
        ...
        try:
            ...
            for i in range(100):
                ...
                response = session.get(f"{nginx_base_url}/nginx-health", stream=True)
