...
class TestLoad:
    ...
    def test_ramp_up_load(self, nginx_container_running, nginx_base_url):
        ...
        while current_users <= max_users:
            ...
            def user_simulation(user_id):
                local_metrics = {"requests": 0, "errors": 0, "response_times": []}
                ...
                while time.time() < stop_time:
                    try:
                        ...
                        response = session.get(
                            f"{nginx_base_url}{target_endpoint}", timeout=5
                        )
            ...
            with concurrent.futures.ThreadPoolExecutor(
                ...
                futures = [
                    executor.submit(user_simulation, i) for i in range(current_users)
                ]
                ...
                for future in concurrent.futures.as_completed(futures):
                    ...
                    level_metrics["response_times"].extend(
                        user_metrics["response_times"]
                    )
            ...
            level_metrics["error_rate"] = (
                level_metrics["errors"] / level_metrics["requests"]
                if level_metrics["requests"] > 0
                else 0
            )

            if level_metrics["response_times"]:
                level_metrics["avg_response_time"] = statistics.mean(
                    level_metrics["response_times"]
                )
                level_metrics["p95_response_time"] = sorted(
                    level_metrics["response_times"]
                )[int(len(level_metrics["response_times"]) * 0.95)]
            else:
            ...
            print(
                f"    Avg response time: {level_metrics['avg_response_time'] * 1000:.2f}ms"
            )
            print(
                f"    P95 response time: {level_metrics['p95_response_time'] * 1000:.2f}ms"
            )

            # Determinar si hemos alcanzado el punto de quiebre
            if (
                level_metrics["error_rate"] > 0.05
                or level_metrics["p95_response_time"] > 2.0
            ):
    ...
    def test_sustained_load(self, nginx_container_running, nginx_base_url):
        ...
        def sustained_user_load(user_id):
            ...
            while time.time() < stop_time:
                second_key = int(time.time() - start_time)  # noqa: F841
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=num_users) as executor:
            futures = [
                executor.submit(sustained_user_load, i) for i in range(num_users)
            ]
        ...
        if metrics["response_times"]:
            ...
            p99_response = sorted(metrics["response_times"])[
                int(len(metrics["response_times"]) * 0.99)
            ]
        ...
        print(f"\n  Resultados de carga sostenida:")  # noqa: F541
        ...
        assert error_rate < 0.02, (
            f"Tasa de error muy alta bajo carga sostenida: {error_rate * 100:.2f}%"
        )
        assert avg_response < 1.0, f"Tiempo de respuesta muy alto: {avg_response:.2f}s"
        assert stdev_response < avg_response, (
            "Respuestas muy inestables (alta desviación)"
        )
    ...
    def test_spike_load(self, nginx_container_running, nginx_base_url):
        ...
        for phase_name, num_users, duration in phases:
            ...
            with concurrent.futures.ThreadPoolExecutor(
                ...
                futures = [
                    executor.submit(spike_user_load, i) for i in range(num_users)
                ]
            ...
            phase_metrics["error_rate"] = (
                phase_metrics["errors"] / phase_metrics["requests"]
                if phase_metrics["requests"] > 0
                else 0
            )

            if phase_metrics["response_times"]:
                phase_metrics["avg_response"] = statistics.mean(
                    phase_metrics["response_times"]
                )
                phase_metrics["p95_response"] = sorted(phase_metrics["response_times"])[
                    int(len(phase_metrics["response_times"]) * 0.95)
                ]
            else:
        ...
        spike_degradation = (
            (spike_avg - baseline_avg) / baseline_avg if baseline_avg > 0 else 0
        )
    ...
    def test_mixed_load_pattern(self, nginx_container_running, nginx_base_url):
        ...
        def mixed_user_behavior(user_type, user_id):
            ...
            while time.time() < stop_time:
                ...
                if len(pattern["endpoints"]) > 1:
                    endpoint = (
                        pattern["endpoints"][0]
                        if user_id % 10 < pattern["ratio"] * 10
                        else pattern["endpoints"][1]
                    )
                else:
                ...
                try:
                    ...
                    local_metrics["requests"] += 1
                    local_metrics["requests_by_endpoint"][endpoint] = (
                        local_metrics["requests_by_endpoint"].get(endpoint, 0) + 1
                    )
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=100) as executor:
            ...
            for future in concurrent.futures.as_completed(all_futures):
                ...
                aggregated_metrics["response_times_by_type"][user_type].extend(
                    metrics["response_times"]
                )

        # Mostrar resultados
        print(f"\n  Resultados de carga mixta:")  # noqa: F541
        ...
        print(
            f"    Error rate global: {(aggregated_metrics['total_errors'] / aggregated_metrics['total_requests'] * 100):.2f}%"
        )
        ...
        # Validaciones
        error_rate = (
            aggregated_metrics["total_errors"] / aggregated_metrics["total_requests"]
        )
        assert error_rate < 0.02, (
            f"Error rate muy alto en carga mixta: {error_rate * 100:.2f}%"
        )
    ...
    def test_graceful_degradation(self, nginx_container_running, nginx_base_url):
        ...
        for stage_name, concurrent_users, duration in stages:
            ...
            with concurrent.futures.ThreadPoolExecutor(
                ...
                futures = [
                    executor.submit(overload_request, i)
            ...
            if stage_metrics["response_times"]:
                stage_metrics["avg_response"] = statistics.mean(
                    stage_metrics["response_times"]
                )
            else:
            ...
            stage_metrics["success_rate"] = (
                stage_metrics["success"] / total_requests if total_requests > 0 else 0
            )
            ...
            print(
                f"    Exitosos: {stage_metrics['success']} ({stage_metrics['success_rate'] * 100:.1f}%)"
            )
        ...
        for i, metrics in enumerate(degradation_metrics):
            if i > 0:
                ...
                if prev_success_rate > 0.5 and curr_success_rate < 0.1:
                    print(
                        f"    ⚠️ Colapso detectado entre {degradation_metrics[i - 1]['stage']} y {metrics['stage']}"
                    )
                else:
    ...
    def test_recovery_from_overload(self, nginx_container_running, nginx_base_url):
        ...
        for phase_name, num_users, duration in phases:
            ...
            phase_metrics["success_rate"] = (
                phase_metrics["success"] / phase_metrics["requests"]
                if phase_metrics["requests"] > 0
                else 0
            )
        ...
        if "baseline" in phase_results and "recovery" in phase_results:
            ...
            print(
                f"    Success rate: {baseline['success_rate'] * 100:.1f}% → {recovery['success_rate'] * 100:.1f}%"
            )
            print(
                f"    Avg response: {baseline['avg_response'] * 1000:.2f}ms → {recovery['avg_response'] * 1000:.2f}ms"
            )
