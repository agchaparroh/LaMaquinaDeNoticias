...
class TestAdvancedPerformance:
    ...
    def test_connection_pooling_efficiency(
        ...
        print(f"\nConnection Pooling Efficiency:")  # noqa: F541
    ...
    def test_static_vs_dynamic_performance(
        self, nginx_container_running, nginx_base_url
    ):
        """
        ...
        if static_times and dynamic_times:
            ...
            print(f"\nStatic vs Dynamic Performance:")  # noqa: F541
    ...
    def test_gzip_compression_ratio(self, nginx_container_running, nginx_base_url):
        ...
        for size in test_content_sizes:
            ...
            response_no_gzip = session.get(
                f"{nginx_base_url}/", headers=headers_no_gzip
            )
            uncompressed_size = len(response_no_gzip.content)

            # Request con compresión
            headers_gzip = {"Accept-Encoding": "gzip"}
            response_gzip = session.get(f"{nginx_base_url}/", headers=headers_gzip)
    ...
    def test_cache_hit_ratio(self, nginx_container_running, nginx_base_url):
        ...
        with requests.Session() as session:
            ...
            if response1.status_code == 200:
                ...
                response2 = session.get(f"{nginx_base_url}{static_resource}")  # noqa: F841
                ...
                if "ETag" in response1.headers:
                    ...
                    response3 = session.get(
                        f"{nginx_base_url}{static_resource}", headers=headers
                    )
    ...
    def test_requests_per_second(self, nginx_container_running, nginx_base_url):
        ...
        print(f"\nRequests Per Second Test:")  # noqa: F541
                ...
                bandwidth_results.append(
                    {
                        ...
                        "avg_size_kb": (total_bytes / request_count / 1024)
                        if request_count > 0
                        else 0,
                    }
                )
    ...
    def test_response_time_under_load(self, nginx_container_running, nginx_base_url):
        ...
        if response_times:
            ...
            print(f"\nResponse Time Under Load:")  # noqa: F541
            ...
            # Validaciones
            assert avg_time < 500, (
                f"Tiempo de respuesta promedio muy alto bajo carga: {avg_time:.2f}ms"
            )
