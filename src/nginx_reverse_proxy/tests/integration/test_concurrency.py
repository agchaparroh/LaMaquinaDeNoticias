...
class TestConcurrency:
    ...
    def test_concurrent_requests_handling(
        self, nginx_container_running, nginx_base_url
    ):
        """
        ...
        assert avg_response_time < 1.0, (
            f"Tiempo de respuesta muy alto: {avg_response_time:.2f}s"
        )
    ...
    def test_connection_limit_behavior(self, nginx_container_running, nginx_base_url):
        ...
        try:
            ...
            print(f"\nConnection Limit Test:")  # noqa: F541
            ...
            assert successful_connections > 50, "Muy pocas conexiones aceptadas"
    ...
    def test_rate_limiting_fairness(self, nginx_container_running, nginx_base_url):
        ...
        def client_requests(client_id):
            ...
            with requests.Session() as session:
                for i in range(requests_per_client):
                    try:
                        response = session.get(
                            f"{nginx_base_url}{endpoint}", headers=headers
                        )
        ...
        with ThreadPoolExecutor(max_workers=num_clients) as executor:
            futures = {
                executor.submit(client_requests, i): i for i in range(num_clients)
            }
        ...
        success_rates = [
            r["success"] / requests_per_client for r in client_results.values()
        ]
    ...
    def test_request_isolation(self, nginx_container_running, nginx_base_url):
        ...
        def make_isolated_request(request_id):
            ...
            try:
                response = requests.get(f"{nginx_base_url}{endpoint}", headers=headers)
        ...
        with ThreadPoolExecutor(max_workers=num_concurrent) as executor:
            futures = [
                executor.submit(make_isolated_request, i) for i in range(num_concurrent)
            ]

            for future in as_completed(futures):
                future.result()

        print(f"\nRequest Isolation Test:")  # noqa: F541
    ...
    def test_memory_usage_under_concurrency(
        self, nginx_container_running, nginx_base_url
    ):
        """
        ...
        print(
            f"  Memoria inicial: {initial_memory:.2f} MB"
            if initial_memory
            else "  No se pudo leer memoria inicial"
        )
        ...
        try:
            ...
            for i in range(max_connections):
                try:
                    ...
                    response = session.get(
                        f"{nginx_base_url}/nginx-health", stream=True, timeout=1
                    )
                ...
                if i % 100 == 0 and i > 0:
                    ...
                    try:
                        test_response = requests.get(
                            f"{nginx_base_url}/nginx-health", timeout=2
                        )
                        if test_response.status_code != 200:
                            print(
                                f"  Nginx dejó de responder después de {i} conexiones"
                            )
                            break
                    except:  # noqa: E722
            ...
            print(f"\nFile Descriptor Test:")  # noqa: F541
