...
class TestRealIntegration:
    ...
    def test_real_backend_integration(
        self, nginx_container_running, nginx_base_url, check_real_services
    ):
        """
        ...
        for endpoint in backend_endpoints:
            ...
            try:
                ...
                print(
                    f"    Response time: {results[endpoint]['response_time'] * 1000:.2f}ms"
                )
                print(f"    Content-Type: {results[endpoint]['content_type']}")
                print(
                    f"    CORS headers: {'Sí' if results[endpoint]['has_cors'] else 'No'}"
                )
        ...
        successful_endpoints = [
            ...
            if "status_code" in res and res["status_code"] < 500
        ]

        assert len(successful_endpoints) > 0, (
            "Ningún endpoint del backend real respondió correctamente"
        )
        ...
        assert len(api_endpoints_with_cors) > 0, (
            "Los endpoints API no tienen headers CORS"
        )

    def test_real_frontend_serving(
        self, nginx_container_running, nginx_base_url, check_real_services
    ):
        """
        ...
        for resource, expected_content_type in frontend_resources:
            ...
            try:
                ...
                print(
                    f"    Compressed: {'Sí' if results[resource]['compressed'] else 'No'}"
                )
    ...
    def test_full_stack_request_flow(
        ...
        if backend_available:
            ...
            response = requests.get(f"{nginx_base_url}/api/health", headers=headers)
            ...
            print(f"    Tiempo: {response.elapsed.total_seconds() * 1000:.2f}ms")
            print(
                f"    CORS: {'OK' if 'Access-Control-Allow-Origin' in response.headers else 'Missing'}"
            )
        ...
        if backend_available:
            ...
            response = requests.post(
                f"{nginx_base_url}/api/test", json=test_data, headers=headers
            )
    ...
    def test_docker_network_integration(self, nginx_container_running, docker_client):
        ...
        try:
            ...
            nginx_networks = list(
                nginx_container.attrs["NetworkSettings"]["Networks"].keys()
            )
            ...
            for service in services_to_check:
                try:
                    result = nginx_container.exec_run(
                        f"nslookup {service}", stderr=True
                    )
    ...
    def test_service_discovery(
        self, nginx_container_running, nginx_base_url, docker_client
    ):
        """
        ...
        for upstream_name, service_name, port in upstreams:
            ...
            result = nginx_container.exec_run(
                f"nc -zv {service_name} {port}", stderr=True
            )
    ...
    def test_cross_container_communication(
        self, nginx_container_running, docker_client
    ):
        """
        ...
        try:
            ...
            for container_id, container_info in connected_containers.items():
                ...
                if container_name != nginx_container_running:
                    ...
                    result = nginx_container.exec_run(
                        f"ping -c 1 -W 2 {container_name}", stderr=True
                    )
                    ...
                    if result.exit_code == 0:
                        ...
                        result = nginx_container.exec_run(
                            f"nc -zv {container_name} 80", stderr=True
                        )
                        if result.exit_code == 0:
