...
class TestDockerIntegration:
    ...
    def test_container_health_check(self, nginx_container_running):
        ...
        assert health_status in ["healthy", "starting"], (
            f"Container no está healthy: {health_status}"
        )
    ...
    def test_container_network_connectivity(self, nginx_container_running):
        ...
        network_result = subprocess.run(
            [
                ...
                nginx_container_running,
                "--format",
                "{{.NetworkSettings.Networks.lamacquina_network}}",
            ],
            capture_output=True,
            text=True,
        )
        ...
        assert (
            "NetworkID" in network_result.stdout
            or network_result.stdout.strip() != "<no value>"
        )
    ...
    def test_container_restart_recovery(self, nginx_container_running, nginx_base_url):
        ...
        subprocess.run(["docker", "restart", nginx_container_running], check=True)
    ...
    def test_container_resource_limits(self, nginx_container_running):
        ...
        assert result.stdout.strip() != "", (
            "No se pudieron obtener estadísticas del container"
        )
