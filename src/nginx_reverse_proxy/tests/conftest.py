...
def mock_backend_container():
    ...
    subprocess.run(
        ["docker", "network", "create", "lamacquina_network"], capture_output=True
    )
...
def mock_frontend_container():
    ...
    subprocess.run(
        [
            ...
            "sh",
            "-c",
            f"echo '{html_content}' > /usr/share/nginx/html/index.html",
        ],
        check=True,
    )
...
@pytest.fixture(scope="session")
def nginx_container_running(
    project_root, mock_backend_container, mock_frontend_container, nginx_container_name
):
    """
