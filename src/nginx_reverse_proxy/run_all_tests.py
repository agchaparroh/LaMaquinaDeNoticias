...
def main():
    parser = argparse.ArgumentParser(
        description="Ejecutar tests de nginx_reverse_proxy"
    )
    parser.add_argument(
        "--type",
        choices=[
    ...
    parser.add_argument(
        "--load-test", action="store_true", help="Ejecutar tests de carga con Locust"
    )
    ...
    if args.type == "all":
        ...
        test_commands.append(
            (f"{pytest_cmd} tests/integration/", "Tests de integración")
        )
    elif args.type == "performance":
        test_commands.append(
            (
                f"{pytest_cmd} tests/integration/test_performance.py tests/integration/test_advanced_performance.py",
        ...
        test_commands.append(
            (
                f"{pytest_cmd} tests/integration/test_concurrency.py",
        ...
        test_commands.append(
            (
                f"{pytest_cmd} tests/integration/test_recovery.py",
                "Tests de recuperación",
            )
        )
    elif args.type == "load":
        test_commands.append(
            (f"{pytest_cmd} tests/integration/test_load.py", "Tests de carga")
        )
    elif args.type == "real":
        test_commands.append(
            (
                f"{pytest_cmd} tests/integration/test_real_integration.py",
    ...
    print(f"\n{'#' * 60}")
    print(f"# Ejecutando tests de nginx_reverse_proxy")  # noqa: F541
