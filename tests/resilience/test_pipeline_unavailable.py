...
def test_connector_handles_pipeline_timeout():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        print(f"✅ Archivo de prueba creado con 2 artículos")  # noqa: F541
            ...
            for article in articles:
                ...
                for attempt in range(scenario.max_retries):
                    try:
                        ...
                        print(
                            f"   ⏱️ Intento {attempt + 1}/{scenario.max_retries}: TIMEOUT"
                        )
                        if attempt == scenario.max_retries - 1:
                            # Último intento falló
                            results["failed"] += 1
                            results["errors"].append(
                                f"Timeout after {scenario.max_retries} attempts"
                            )
                            print(f"   ❌ Agotados los reintentos para este artículo")  # noqa: F541
        ...
        assert len(scenario.retry_attempts) == 6, (
            f"Deberían ser {scenario.max_retries} intentos por cada uno de los 2 artículos"
        )

        # Verificar patrón de reintentos
        print(f"\n🔄 Patrón de reintentos:")  # noqa: F541
...
def test_connector_handles_connection_refused():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        print(f"\n✅ Connector manejó correctamente la conexión rechazada:")  # noqa: F541
...
def test_connector_handles_intermittent_failures():
    ...
    class IntermittentFailureScenario:
        ...
        async def simulate_intermittent_pipeline(self, article_data: dict):
            ...
            if self.call_count < self.success_after:
                print(
                    f"   ❌ Intento {self.call_count}: Error 503 (Service Unavailable)"
                )
                mock_response = Mock()
    ...
    async def process_with_intermittent_failure():
        ...
        print(f"📤 Procesando artículo con posibles fallos intermitentes...")  # noqa: F541
    ...
    print(f"\n✅ Sistema se recuperó correctamente:")  # noqa: F541
    print(f"   - Éxito después de {attempts} intentos")
    print(f"   - Resiliencia ante fallos temporales demostrada")  # noqa: F541
...
def test_connector_continues_after_failures():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        print(f"✅ Creados 3 archivos de prueba")  # noqa: F541

        # WHEN: Procesar con el segundo archivo fallando
        async def process_multiple_files():
            results = {"processed": [], "failed": [], "success": []}
            ...
            for i, filepath in enumerate(files):
                ...
                try:
                    ...
                    if i == 1:
                        ...
                        print(f"   ❌ Pipeline no disponible para este archivo")  # noqa: F541
                        results["failed"].append(filename)
                        scenario.move_to_error_directory(filepath)
                    else:
                        # Estos tienen éxito
                        print(f"   ✅ Archivo procesado exitosamente")  # noqa: F541
        ...
        print(f"\n📊 Resumen de procesamiento:")  # noqa: F541
        ...
        assert len(results["processed"]) == 3, (
            "Todos los archivos deberían intentar procesarse"
        )
        assert len(results["success"]) == 2, (
            "Dos archivos deberían procesarse exitosamente"
        )
        ...
        print(f"\n✅ El Connector continuó procesando a pesar de fallos individuales")  # noqa: F541
        print(f"   - Sistema resiliente: no se detuvo por fallos puntuales")  # noqa: F541
        print(f"   - Archivos problemáticos aislados en directorio de error")  # noqa: F541
...
def test_graceful_degradation_strategies():
    ...
    class DegradationStrategy:
        ...
        async def check_pipeline_health(self) -> bool:
            ...
            try:
                ...
                if self.circuit_breaker_open:
                    print(
                        "   🔴 Circuit breaker ABIERTO - Pipeline marcado como no disponible"
                    )
                    return False
        ...
        def add_to_queue(self, article):
            ...
            print(
                f"   📥 Artículo agregado a cola de respaldo (total: {len(self.queue)})"
            )
    ...
    print(
        f"   - Circuit breaker: {'ABIERTO' if final_strategy.circuit_breaker_open else 'CERRADO'}"
    )
