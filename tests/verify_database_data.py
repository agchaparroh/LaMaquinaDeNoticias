...
def verify_database_data():
    ...
    try:
                ...
                if count > 0:
                    ...
                    if sample_response.data:
                        ...
                        for i, record in enumerate(sample_response.data[:2], 1):
                            ...
                            if table == 'articulos':
                                print(f"      {i}. ID: {record.get('id')}, URL: {str(record.get('url', 'N/A'))[:50]}...")
                            elif table == 'hechos':
                                print(f"      {i}. ID: {record.get('id')}, Contenido: {str(record.get('contenido_hecho', 'N/A'))[:50]}...")
                            elif table == 'entidades':
                                print(f"      {i}. ID: {record.get('id')}, Nombre: {record.get('nombre', 'N/A')}, Tipo: {record.get('tipo', 'N/A')}")
                            elif table == 'citas_textuales':
                                print(f"      {i}. ID: {record.get('id')}, Cita: {str(record.get('texto_cita', 'N/A'))[:50]}...")
                            elif table == 'datos_cuantitativos':
                                print(f"      {i}. ID: {record.get('id')}, Indicador: {record.get('indicador', 'N/A')}, Valor: {record.get('valor', 'N/A')}")
                            else:
                                # Para otras tablas, mostrar ID y algunos campos
                                print(f"      {i}. ID: {record.get('id')}, Campos: {list(record.keys())[:5]}")
                        print()
        ...
        if total_records == 0:
            ...
            if articles > 0:
                if facts == 0 and entities == 0:
                    ...
                    print("   - Los artículos pueden no haber sido procesados completamente")
                else:
        ...
        try:
            ...
            recent_articles = supabase.table('articulos').select("id, url, fecha_procesamiento_pipeline").order('fecha_procesamiento_pipeline', desc=True).limit(5).execute()
        ...
        for rpc_name in rpc_functions:
            try:
                ...
                if rpc_name == 'buscar_entidad_similar':
                    test_response = supabase.rpc(rpc_name, {
