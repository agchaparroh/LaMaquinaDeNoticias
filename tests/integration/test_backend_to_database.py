...
def test_successful_integration():
    ...
    response = mock_supabase.rpc(
        "obtener_articulos_revision",
        {"limite": 10, "estado": "pendiente", "orden": "fecha_publicacion"},
    )
    ...
    elementos = (
        mock_supabase.table("elementos_extraidos")
        .select("*")
...
def test_handles_error():
    ...
    response = mock_supabase.rpc(
        "obtener_articulos_revision", {"estado": "invalid_status"}
    )
