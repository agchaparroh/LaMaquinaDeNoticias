...
def test_successful_integration():
    ...
    with patch("supabase.Client") as mock_client:
        ...
        mock_rpc.return_value = MockSupabaseResponse(
            data=[
                {"fragmento_uuid": processed_fragment["fragmento_id"], "success": True}
            ]
        )
        ...
        fragment_result = mock_rpc(
            "guardar_fragmento_procesado", {"p_fragmento": processed_fragment}
        ).execute()

        assert fragment_result.data[0]["success"] is True
        assert (
            fragment_result.data[0]["fragmento_uuid"]
            == processed_fragment["fragmento_id"]
        )

        # Paso 2: Normalizar y vincular entidades
        for entidad in processed_fragment["elementos_extraidos"]["entidades"]:
            entity_result = mock_normalize(
                "normalizar_entidad",
                {"p_nombre": entidad["texto"], "p_tipo": entidad["tipo"]},
            ).execute()

            assert entity_result.data[0]["entidad_id"] is not None
            assert (
                entity_result.data[0]["nombre_normalizado"] == "Ministerio de Economía"
            )
