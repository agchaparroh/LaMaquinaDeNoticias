...
def test_successful_integration():
    ...
    async def frontend_get_articles():
        response = await backend.get(
            "/dashboard/articulos",
            params={"pagina": 1, "por_pagina": 10, "estado": "todos"},
        )
    ...
    async def frontend_send_feedback():
        ...
        response = await backend.post(
            "/dashboard/feedback/art-001", json_data=feedback_data
        )
        return await response.json()
...
def test_handles_error():
    ...
    async def frontend_validation_error():
        ...
        response = await backend.post(
            "/dashboard/feedback/art-001", json_data=incomplete_feedback
        )
        return response.status, await response.json()
