...
NETWORK_DEPENDENCIES = {
    "lamacquina_connector": {
        ...
        "test_endpoints": {"lamacquina_pipeline": "module_pipeline:8003"},
    },
    "lamacquina_dashboard_review_backend": {"depends_on": [], "test_endpoints": {}},
    "lamacquina_chat_interface_backend": {"depends_on": [], "test_endpoints": {}},
    "lamacquina_dev_interface_backend": {"depends_on": [], "test_endpoints": {}},
...
def check_container_in_network(
    container_name: str, network_name: str = "lamacquina_network"
) -> Tuple[bool, str]:
    """
    ...
    try:
        ...
        if network_found:
            ...
            for net_name, net_info in networks.items():
                if network_name in net_name:
                    ...
                    return (
                        True,
                        f"✅ {container_name}: En red {net_name} (IP: {ip_address})",
                    )
        ...
        return (
            False,
            f"❌ {container_name}: Contenedor no encontrado o no está corriendo",
        )
...
def test_dns_resolution(from_container: str, to_hostname: str) -> Tuple[bool, str]:
    ...
    try:
        ...
        if result.returncode == 0 and "Address" in result.stdout:
            ...
            for line in lines:
                if (
                    "Address" in line and "#" not in line
                ):  # Ignorar la línea del servidor DNS
                    ip = line.split("Address:")[-1].strip()
                    return True, f"✅ {from_container} → {to_hostname}: Resuelto a {ip}"

        return (
            False,
            f"❌ {from_container} → {to_hostname}: No se pudo resolver el hostname",
        )
        ...
        try:
            ...
            return (
                False,
                f"❌ {from_container} → {to_hostname}: Error en prueba DNS - {str(e)}",
            )
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if result.stdout.strip() in [
                ...
                "404",
                "000",
            ]:  # 000 = conexión establecida pero sin respuesta HTTP
                return (
                    True,
                    f"✅ {from_container} → {to_endpoint}: Conexión HTTP establecida",
                )
            else:
                return (
                    False,
                    f"❌ {from_container} → {to_endpoint}: No se pudo conectar al puerto",
                )

    except Exception as e:
        return (
            False,
            f"❌ {from_container} → {to_endpoint}: Error en prueba de puerto - {str(e)}",
        )
...
def test_service_port_connectivity():
    ...
    assert all_connected, (
        "Algunos servicios no pueden conectar a puertos de sus dependencias"
    )
...
def test_reverse_proxy_can_reach_all_backends():
    ...
    for backend in backends:
        can_connect, message = test_port_connectivity(
            "lamacquina_reverse_proxy", backend
        )
        results.append(message)
        if not can_connect:
            all_reachable = False
