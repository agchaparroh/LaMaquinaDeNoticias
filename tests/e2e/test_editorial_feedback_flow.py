...
class EditorialFeedbackFlow:
    ...
    def dashboard_display_article(self) -> dict:
        ...
        display_data = {
            ...
            "metadata": {
                ...
                "hechos_sin_revisar": sum(
                    1
                    for h in self.article_data["hechos"]
                    if h["importancia_editor"] is None
                ),
                "promedio_confianza": sum(
                    h["confianza_extraccion"] for h in self.article_data["hechos"]
                )
                / len(self.article_data["hechos"]),
            },
    ...
    def editor_submit_feedback(self, hecho_id: str, nueva_importancia: int) -> dict:
        ...
        # Encontrar el hecho
        hecho = next(
            (h for h in self.article_data["hechos"] if h["id"] == hecho_id), None
        )
    ...
    def backend_update_database(self, hecho_id: str, nueva_importancia: int) -> dict:
        ...
        feedback_record = {
            ...
            "hecho_id": hecho_id,
            "importancia_automatica_original": next(
                h["importancia_automatica"]
                for h in self.article_data["hechos"]
                if h["id"] == hecho_id
            ),
            "importancia_editor_final": nueva_importancia,
...
def test_editorial_feedback_flow():
    ...
    print(
        f"   📈 Importancia: {hecho_target['importancia_automatica']} → {nueva_importancia}"
    )
    ...
    print(f"   ✅ Hecho actualizado en BD")  # noqa: F541
    ...
    hecho_actualizado = next(
        h
        for h in updated_display["articulo"]["hechos"]
        if h["id"] == hecho_target["id"]
    )
    assert hecho_actualizado["importancia_editor"] == nueva_importancia

    print(f"   ✅ Vista actualizada")  # noqa: F541
    print(f"   ✓ Hechos revisados: {updated_display['metadata']['hechos_revisados']}")
    print(
        f"   ✓ Hechos pendientes: {updated_display['metadata']['hechos_sin_revisar']}"
    )
    ...
    print(f"   - Feedback guardado para mejorar algoritmo")  # noqa: F541
...
def test_multiple_feedback_changes():
    ...
    for cambio in flow.feedback_history:
        print(
            f"   - Hecho {cambio['hecho_id']}: {cambio['valor_anterior']} → {cambio['valor_nuevo']}"
        )
