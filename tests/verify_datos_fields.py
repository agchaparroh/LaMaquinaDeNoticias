...
def main():
    ...
    for filepath in files_to_check:
        ...
        if isinstance(results, dict):
            ...
            if results["new_fields_found"]:
                print(f"✓ Campos nuevos (RPC) encontrados: {', '.join(results['new_fields_found'])}")
            else:
                print("⚠️  No se encontraron campos nuevos")
            
            # Verificar contexto de campos antiguos
            if results["old_fields_found"]:
                print(f"ℹ️  Campos antiguos encontrados: {', '.join(results['old_fields_found'])}")
    ...
    persistencia_result = next((r for r in all_results if r["file"] == "persistencia.py"), None)
    if persistencia_result:
        if all(field in persistencia_result["new_fields_found"] for field in ["indicador", "valor_numerico", "unidad", "id_temporal_hecho"]):
            print("✓ persistencia.py: Modelo DatoCuantitativoExtraidoItem tiene campos RPC correctos")
        else:
            print("✗ persistencia.py: Faltan algunos campos RPC en el modelo")
    
    # Verificar payload_builder.py
    payload_result = next((r for r in all_results if r["file"] == "payload_builder.py"), None)
    if payload_result:
        if payload_result["old_fields_found"] and payload_result["new_fields_found"]:
            print("✓ payload_builder.py: Tiene mapeo de campos antiguos a nuevos (correcto)")
        else:
            print("⚠️  payload_builder.py: Verificar mapeo de campos")
    
    # Verificar fase_5_datos.py
    fase5_result = next((r for r in all_results if r["file"] == "fase_5_datos.py"), None)
    if fase5_result:
    ...
    # Verificar test
    test_result = next((r for r in all_results if r["file"] == "test_rpc_datos_alignment.py"), None)
    if test_result:
