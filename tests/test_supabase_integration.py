...
class TestSupabaseIntegration(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        try:
            ...
            cls.pipeline.html_bucket_name = mock_crawler.settings.get(
                "SUPABASE_STORAGE_BUCKET"
            )

            logger.info(
                f"SupabaseStoragePipeline initialized for integration tests. Bucket: {cls.pipeline.html_bucket_name}"
            )

            # Ensure the test bucket exists
            cls.pipeline.open_spider(
                spider=None
            )  # 'spider' arg is not used by current open_spider logic for bucket creation
            logger.info(
                f"Ensured test bucket '{cls.pipeline.html_bucket_name}' exists."
            )
    ...
    def tearDownClass(cls):
        ...
        if hasattr(cls, "pipeline") and cls.pipeline:
            cls.pipeline.close_spider(
                spider=None
            )  # 'spider' arg is not used by current close_spider logic
    ...
    def _create_sample_item(self, unique_id: str) -> ArticuloInItem:
        ...
        item["contenido_html"] = (
            f"<html><body><h1>Test Article {unique_id}</h1><p>Some content.</p></body></html>"
        )
    ...
    def test_pipeline_processes_item_successfully(self):
        ...
        processed_item_adapter = self.pipeline.process_item(
            sample_item, spider=None
        )  # 'spider' is not used by process_item
        ...
        self.assertIn(
            "storage_path", processed_item, "storage_path not set in processed item"
        )
        self.assertTrue(
            processed_item["storage_path"].startswith(
                f"{sample_item['medio'].lower()}/"
            )
        )
        ...
        try:
            response = (
                self.supabase_client.client.table("articulos_beta")
                .select("url, titular")
            ...
            self.assertTrue(
                len(response.data) > 0,
                f"Article with URL {sample_item['url']} not found in DB.",
            )
