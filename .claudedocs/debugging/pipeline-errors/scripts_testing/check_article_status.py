...
def check_specific_article(client, article_id):
    ...
    try:
        response = client.table("articulos").select("*").eq("id", article_id).single().execute()
        ...
        if response.data:
            ...
            print(f"  - Fecha procesamiento: {articulo.get('fecha_procesamiento', 'N/A')}")
...
def check_processed_elements(client, article_id):
    ...
    try:
        ...
        # Hechos
        hechos = client.table("hechos").select("id, contenido").eq("articulo_id", article_id).execute()
        print(f"✅ Hechos: {len(hechos.data) if hechos.data else 0}")
        if hechos.data and len(hechos.data) > 0:
            for i, hecho in enumerate(hechos.data[:3], 1):  # Mostrar solo los primeros 3
                print(f"    {i}. {hecho.get('contenido', '')[:60]}...")
        
        # Entidades (directamente desde tabla entidades)
        entidades = client.table("entidades").select("id, nombre, tipo").eq("articulo_id", article_id).execute()
        print(f"✅ Entidades: {len(entidades.data) if entidades.data else 0}")
        if entidades.data and len(entidades.data) > 0:
            for i, ent in enumerate(entidades.data[:5], 1):  # Mostrar solo las primeras 5
                print(f"    {i}. {ent.get('nombre', 'N/A')} ({ent.get('tipo', 'N/A')})")
        
        # Citas
        citas = client.table("citas_textuales").select("id, cita").eq("articulo_id", article_id).execute()
        ...
        # Datos cuantitativos
        datos = client.table("datos_cuantitativos").select("id, valor, unidad").eq("articulo_id", article_id).execute()
...
def show_recent_articles(client, limit=5):
    ...
    try:
        ...
        ultimos = client.table("articulos").select(
            "id, titular, fecha_procesamiento, estado_procesamiento"
        ).eq("estado_procesamiento", "completado").order(
...
def show_pending_articles(client, limit=5):
    ...
    try:
        ...
        pendientes = client.table("articulos").select(
            "id, titular, fecha_recopilacion"
        ).eq("estado_procesamiento", "pendiente").order(
            "id", desc=True
        ).limit(limit).execute()
...
def main():
    ...
    pending_ids = show_pending_articles(client)
    
    # Ofrecer verificar uno específico si hay completados
    if completed_ids:
        print(f"\n💡 Para verificar un artículo específico:")
    ...
    try:
        ...
        for estado in estados:
            count = client.table("articulos").select("id", count='exact').eq("estado_procesamiento", estado).execute()
