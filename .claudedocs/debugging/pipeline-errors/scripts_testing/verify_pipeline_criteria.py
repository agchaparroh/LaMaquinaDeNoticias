    ...
    try:
        ...
        if response.status_code == 200:
            try:
                ...
                print(f"   ✅ Artículo enviado correctamente")  # noqa: F541
                print(f"   📝 Job ID: {job_id}")
                print(f"   🎯 Título: {article_data['titular'][:60]}...")
                return job_id, article_data["url"]
            except json.JSONDecodeError:
                print(f"   ✅ Artículo enviado (respuesta no JSON)")  # noqa: F541
            ...
            try:
                ...
                print(
                    f"   📄 Detalles: {json.dumps(error_data, indent=2, ensure_ascii=False)[:200]}..."
                )
            except:  # noqa: E722
        ...
        print(f"   ⏱️  Timeout después de 90s - procesamiento puede estar en progreso")  # noqa: F541
        return None, article_data["url"]
    except Exception as e:
        print(f"   ❌ Error: {e}")
        return None, None
...
def wait_and_verify_processing(job_id, test_url, max_wait_minutes=5):
    ...
    for i in range(checks):
        try:
            print(
                f"   🔍 Verificación {i + 1}/{checks} - esperando {check_interval}s..."
            )
            ...
            if test_url:
                ...
                if response.data and len(response.data) > 0:
                    ...
                    if estado == "completado":
                        ...
                        print(f"   ❌ Artículo falló en procesamiento")  # noqa: F541
            ...
            if recientes.data:
                completados = [
                    art
                    for art in recientes.data
                    if art.get("estado_procesamiento") == "completado"
                ]
                if completados:
                    print(
                        f"   ✅ {len(completados)} artículos procesados en últimos 5 min"
                    )

        except Exception as e:
            print(f"   ⚠️ Error en verificación: {e}")

    print(f"   ⏱️  Tiempo de espera agotado")  # noqa: F541
    return False, None


def verify_supabase_persistence(client):
    """Verifica persistencia correcta en Supabase"""
    print(f"\n4️⃣  Verificando persistencia en Supabase...")  # noqa: F541
    ...
    try:
        ...
        recientes = (
            ...
            .gte("fecha_procesamiento", diez_min_atras)
        ...
        if recientes.data:
            ...
            print(f"   ⚠️ No hay artículos procesados en los últimos 10 minutos")  # noqa: F541
            ...
            print(
                f"   📊 Total de artículos completados: {total_completados.count or 0}"
            )
...
def verify_error_handling():
    ...
    print(f"\n5️⃣  Verificando manejo de errores...")  # noqa: F541
    ...
    try:
        ...
        if response.status_code == 422:  # Validation error esperado
            ...
            print(
                f"   ⚠️ Respuesta inesperada para datos inválidos: {response.status_code}"
            )
            return False
...
def main():
    ...
    # Resumen final
    print(f"\n{'=' * 50}")
    print(f"📋 RESUMEN DE VERIFICACIÓN PRP")  # noqa: F541
    print(f"{'=' * 50}")
