...
PIPELINE_URL = os.getenv('PIPELINE_URL', 'http://localhost:8003/procesar_articulo')  # CORREGIDO: endpoint correcto
...
def get_latest_pending_article():
    ...
    try:
        ...
        ).eq('estado_procesamiento', 'pendiente').is_('contenido_texto', 'null').order('id', desc=True).limit(1).execute()
        ...
        ).eq('estado_procesamiento', 'pendiente').order('id', desc=True).limit(1).execute()
    ...
    try:
        ...
        if response.status_code == 200:
            try:
                ...
                print(f"\n✅ Procesamiento iniciado (respuesta no JSON)")
        ...
        print(f"\n⏱️  Timeout - El procesamiento puede estar en progreso")
