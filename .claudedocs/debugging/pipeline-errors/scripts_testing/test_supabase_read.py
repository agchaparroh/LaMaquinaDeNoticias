...
def test_connection_and_permissions(key_name, key_value):
    ...
    try:
        ...
        if total_articles == 0:
            print(f"   ⚠️  No hay artículos en la tabla")
            return True  # Conexión OK, pero sin datos
            
        # Test 2: Leer artículos recientes
        recent_response = client.table('articulos').select(
            "id, url, titular, estado_procesamiento, fecha_recopilacion"
        ).order('id', desc=True).limit(5).execute()
        
        if recent_response.data:
            print(f"   ✅ Lectura de artículos recientes: {len(recent_response.data)} encontrados")
            ...
            print(f"   ❌ No se pudieron leer artículos")
            return False
            
        # Test 3: Filtrar por estado
        try:
            pending_response = client.table('articulos').select(
                "id", count='exact'
            ).eq('estado_procesamiento', 'pendiente').execute()
            
            completed_response = client.table('articulos').select(
                "id", count='exact'  
        ...
        tables_to_check = ['hechos', 'entidades', 'citas_textuales', 'datos_cuantitativos']
        
        for table in tables_to_check:
            try:
                table_response = client.table(table).select("id", count='exact').limit(1).execute()
...
def test_write_permissions(key_name, key_value):
    ...
    try:
        ...
        print(f"   ✅ Cliente de escritura configurado correctamente")
        return True
...
def main():
    ...
    if SUPABASE_ANON_KEY:
        print(f"📖 Probando con ANON_KEY (usado por pipeline):")
        results['anon'] = test_connection_and_permissions("ANON_KEY", SUPABASE_ANON_KEY)
    
    # Test SERVICE_ROLE_KEY (permisos completos)
    if SUPABASE_SERVICE_KEY:
        print(f"\n🔑 Probando con SERVICE_ROLE_KEY (permisos completos):")
        results['service'] = test_connection_and_permissions("SERVICE_ROLE_KEY", SUPABASE_SERVICE_KEY)
