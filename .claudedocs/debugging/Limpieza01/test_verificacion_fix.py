...
def ejecutar_prueba():
    ...
    try:
        ...
        print("   Ejecute primero: docker-compose run --rm module_scraper scrapy crawl infobae_america_latina -a max_articles=1")
        return False
    
    # 2. Ejecutar el pipeline
    print("\n2. Ejecutando pipeline...")
    cmd = ["docker-compose", "run", "--rm", "module_pipeline", "python", "run_single_article.py", "test_article_relevante.json"]
    ...
    try:
        ...
        if errores_encontrados:
            ...
            # Buscar contadores
            import re
            entidades_match = re.search(r"entidades_insertadas['\"]?\s*:\s*(\d+)", output)
...
def main():
    ...
    print("Asegúrese de haber aplicado la migración SQL antes de ejecutar este script.\n")
