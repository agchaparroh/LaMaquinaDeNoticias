-- Migración: Corregir nombres de campos de entidades en actualizar_articulo_procesado
-- Fecha: 2025-01-19
-- Problema: La RPC busca campos con sufijo "_entidad" pero el pipeline envía sin sufijo
-- Solución: Actualizar la RPC para usar nombres de campos sin sufijo
...
BEGIN
    ...
    IF datos_json ? 'entidades_autonomas' THEN
        ...
        LOOP
            ...
            ELSE
                -- CAMBIO CRÍTICO: Usar campos sin sufijo "_entidad"
                ...
                VALUES (
                    v_entidad->>'nombre',              -- ✅ CORREGIDO: Sin sufijo
                    v_entidad->>'tipo',                -- ✅ CORREGIDO: Sin sufijo
                    v_entidad->>'descripcion',         -- ✅ CORREGIDO: Sin sufijo
                    CASE 
                        WHEN v_entidad ? 'alias' 
                        THEN ARRAY(SELECT jsonb_array_elements_text(v_entidad->'alias'))
                        ELSE NULL 
                    END,
                    COALESCE((v_entidad->>'relevancia_entidad_articulo')::INTEGER, 5), -- ✅ CORREGIDO: Campo correcto
            ...
            temp_entidad_id_map := temp_entidad_id_map || 
                jsonb_build_object(COALESCE(v_entidad->>'id_temporal_entidad', v_entidad->>'id')::TEXT, v_entidad_id::TEXT);
        END LOOP;
    END IF;
    ...
    IF datos_json ? 'datos_cuantitativos_extraidos' THEN
        ...
        LOOP
            ...
            -- MEJORA: Manejar campos faltantes con valores por defecto
            ...
            VALUES (
                v_hecho_id,
                v_articulo_id,
                COALESCE(v_dato->>'indicador_dato', v_dato->>'descripcion_dato'), -- ✅ Fallback a descripcion_dato
                COALESCE(v_dato->>'categoria_dato', 'general'),                    -- ✅ Valor por defecto
