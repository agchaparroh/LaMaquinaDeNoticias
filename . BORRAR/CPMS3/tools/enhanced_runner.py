...
class EnhancedRunner:
    ...
    def run(self) -> bool:
        ...
        if val_warnings and not self.options.get("force"):
            print(
                f"\n⚠️  Hay {len(val_warnings)} advertencias. ¿Continuar? (s/N): ",
                end="",
            )
        ...
        if self.options.get("save_processed"):
            ...
            with open(processed_path, "w", encoding="utf-8") as f:
                yaml.dump(
                    processed_plan, f, default_flow_style=False, allow_unicode=True
                )
...
def main():
    ...
    parser.add_argument("plan", help="Ruta al archivo execution_plan.yaml")

    parser.add_argument(
        "--force", "-f", action="store_true", help="Ejecutar incluso con advertencias"
    )
    ...
    parser.add_argument(
        "--save-processed", action="store_true", help="Guardar el plan procesado"
    )
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Solo validar, no ejecutar"
    )
