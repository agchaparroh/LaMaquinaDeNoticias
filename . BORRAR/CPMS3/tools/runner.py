...
class CPMS3Runner:
    def __init__(self, plan_path: str):
        ...
        self.staging = StagingManager(self.base_path, self.config.get('staging_dir', '.cpms3_staging'))
    ...
    def _action_modify_file(self, step: Dict):
        ...
        if content != original_content:
            ...
            if self.config.get('verbose', False):
                ...
                if lines_changed != 0:
                    print(f"   Líneas {'añadidas' if lines_changed > 0 else 'eliminadas'}: {abs(lines_changed)}")
        else:
        ...
        if expected_tests:
            ...
            if match:
                ...
                if passed != expected_tests:
                    raise ExecutionError(f"Se esperaban {expected_tests} tests, pero pasaron {passed}")
...
"""
    ...
    def _find_similar_lines(self, pattern: str, content: str, max_results: int = 3) -> List[str]:
    ...
    def _print_summary(self, success: bool):
        ...
        print(f"Pasos ejecutados: {len(self.executed_steps)}/{len(self.plan.get('steps', []))}")
        
        if self.strict_mode:
            print(f"🔒 Modo strict: Activado")
