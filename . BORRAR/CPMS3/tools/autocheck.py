...
class AutoCheck:
    ...
    def verify(self, verifications: List[Dict[str, Any]]) -> Tuple[bool, List[str]]:
        ...
        for check in verifications:
            ...
            self.results.append(
                {"type": check_type, "passed": passed, "detail": detail}
            )
    ...
    def _check_file_not_exists(self, params: Dict) -> Tuple[bool, str]:
        ...
        return (
            not_exists,
            f"Archivo {'NO existe' if not_exists else 'todavía existe'}: {path}",
        )
    ...
    def _check_file_contains(self, params: Dict) -> Tuple[bool, str]:
        ...
        return (
            contains,
            f"Archivo {'contiene' if contains else 'NO contiene'}: '{text[:50]}...'",
        )
    ...
    def _check_file_not_contains(self, params: Dict) -> Tuple[bool, str]:
        ...
        return (
            not_contains,
            f"Archivo {'NO contiene' if not_contains else 'todavía contiene'}: '{text[:50]}...'",
        )
    ...
    def _check_command_succeeds(self, params: Dict) -> Tuple[bool, str]:
        ...
        return (
            success,
            f"Comando {'exitoso' if success else 'falló'}: {command[:50]}...",
        )
    ...
    def _check_command_fails(self, params: Dict) -> Tuple[bool, str]:
        ...
        return (
            failed,
            f"Comando {'falló como esperado' if failed else 'NO falló'}: {command[:50]}...",
        )
    ...
    def _check_command_output(self, params: Dict) -> Tuple[bool, str]:
        ...
        return (
            contains,
            f"Output {'contiene' if contains else 'NO contiene'}: '{expected[:50]}...'",
        )
    ...
    def _check_test_passes(self, params: Dict) -> Tuple[bool, str]:
        ...
        result = subprocess.run(
            test_cmd, shell=True, capture_output=True, text=True, cwd=self.base_path
        )
    ...
    def _check_test_output(self, params: Dict) -> Tuple[bool, str]:
        ...
        return (
            contains,
            f"Test output {'contiene' if contains else 'NO contiene'}: '{expected}'",
        )
    ...
    def _check_permissions(self, params: Dict) -> Tuple[bool, str]:
        ...
        return (
            matches,
            f"Permisos {'correctos' if matches else 'incorrectos'}: {current} {'==' if matches else '!='} {expected}",
        )
    ...
    def _check_process(self, params: Dict) -> Tuple[bool, str]:
        ...
        result = subprocess.run(
            f"pgrep -f {process_name}", shell=True, capture_output=True
        )

        running = result.returncode == 0
        return (
            running,
            f"Proceso '{process_name}' {'ejecutándose' if running else 'NO encontrado'}",
        )
    ...
    def _check_port(self, params: Dict) -> Tuple[bool, str]:
        ...
        result = subprocess.run(
            f"nc -zv localhost {port}", shell=True, capture_output=True, text=True
        )
    ...
    def _check_api(self, params: Dict) -> Tuple[bool, str]:
        ...
        try:
            ...
            return (
                success,
                f"API {url}: status {response.status_code} {'==' if success else '!='} {expected_status}",
            )
        except Exception as e:
