...
def test_fase_3_entidades():
    ...
    try:
        ...
        print(
            f"   - id_fragmento en resultado_simplif: {resultado_simplif.id_fragmento}"
        )
        ...
        print(f"✅ Fase 3 ejecutada correctamente")  # noqa: F541
        ...
        for entidad in resultado.get("entidades_extraidas", []):
            if hasattr(entidad, "id_fragmento_origen"):
                print(
                    f"   - Entidad '{entidad.texto_entidad}' tiene id_fragmento_origen: {entidad.id_fragmento_origen}"
                )
            else:
                print(
                    f"   ❌ Entidad '{entidad.texto_entidad}' NO tiene id_fragmento_origen"
                )
                return False
