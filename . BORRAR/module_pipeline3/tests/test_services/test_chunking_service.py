...
class TestChunkingService:
    ...
    def config(self):
        ...
        return ChunkingConfig(
            max_chunk_size=100, overlap_size=20, context_window=10, min_chunk_size=30
        )
    ...
    def test_chunk_type_specific_strategies(self, service):
        ...
        chunks_quotes = service.create_chunks(
            text, chunk_type=ChunkType.QUOTES, preserve_boundaries=False
        )

        # Test estrategia ENTITIES
        chunks_entities = service.create_chunks(
            text, chunk_type=ChunkType.ENTITIES, preserve_boundaries=False
        )
    ...
    def test_calculate_optimal_chunk_size(self, service):
        ...
        config_quotes = service.calculate_optimal_chunk_size(10000, ChunkType.QUOTES)
        ...
        config_data = service.calculate_optimal_chunk_size(10000, ChunkType.DATA)
    ...
    def test_merge_overlapping_results(self, service):
        ...
        merged = service.merge_overlapping_results(results_by_chunk, overlap_info)
    ...
    def test_custom_config_per_operation(self, service):
        ...
        text = "A" * 500
        custom_config = ChunkingConfig(max_chunk_size=200, overlap_size=50)

        chunks = service.create_chunks(
            text, custom_config=custom_config, preserve_boundaries=False
        )
...
class TestTextChunk:
    ...
    def test_text_chunk_initialization(self):
        ...
        chunk = TextChunk(
            index=0, total=3, text="Texto de prueba", start_offset=0, end_offset=15
        )
...
class TestChunkingServiceIntegration:
    ...
    def test_real_spanish_text_chunking(self):
        ...
        """
            * 5
        )  # Repetir para hacer el texto más largo

        config = ChunkingConfig(
            max_chunk_size=500, overlap_size=50, respect_sentence_boundaries=True
        )
