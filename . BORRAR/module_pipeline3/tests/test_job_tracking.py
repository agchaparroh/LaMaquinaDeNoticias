...
def create_test_article():
    ...
    return {
        ...
        "contenido_texto": "Este es un contenido de prueba con más de 50 caracteres para validación. "
        * 3,
        "medio": "Test News",
        "area_geografica": "ES",
        "tipo_medio": "digital",
...
def test_job_tracking_system():
    ...
    try:
        ...
        response = requests.post(
            f"{BASE_URL}/procesar_articulo", json=article_data, headers=HEADERS
        )
            ...
            if job_id:
                ...
                if status_response.status_code == 200:
                    ...
                    if initial_status in valid_statuses:
                        ...
                        print_result(
                            False, f"Estado inicial inválido: {initial_status}"
                        )
                else:
                    print_result(
                        False,
                        f"Error al consultar estado: {status_response.status_code}",
                    )
            else:
    ...
    if test_job_id:
        try:
            ...
            if response.status_code == 200:
                ...
                present_fields = [
                    field for field in expected_fields if field in status_data
                ]
                ...
                if len(present_fields) == len(expected_fields):
                    ...
                    print_result(False, f"Campos faltantes en respuesta de estado")  # noqa: F541
            else:
                print_result(
                    False, f"Error al consultar estado: {response.status_code}"
                )
            ...
            if response.status_code == 200:
                ...
                if status_data.get("status") == "completed":
                    ...
                    if "result" in status_data:
                        print(
                            f"  - Fragmento ID: {status_data['result'].get('fragmento_id')}"
                        )
                        print(
                            f"  - Tiempo procesamiento: {status_data['result'].get('tiempo_procesamiento_segundos', 0):.2f}s"
                        )
                        elementos = status_data["result"].get("elementos_extraidos", {})
                        print(f"  - Elementos extraídos: {elementos}")
                else:
                    print_result(
                        False, f"Estado no es 'completed': {status_data.get('status')}"
                    )
            else:
                print_result(
                    False, f"Error al consultar estado: {response.status_code}"
    ...
    try:
        ...
        if response.status_code == 404:
            print_result(True, f"404 recibido correctamente para job inexistente")  # noqa: F541
            ...
            print_result(
                False, f"Código inesperado: {response.status_code} (esperado 404)"
            )
    ...
    try:
        response = requests.post(
            f"{BASE_URL}/procesar_articulo", json=invalid_article, headers=HEADERS
        )

        if response.status_code == 400:
            print_result(
                True, "Error 400 recibido correctamente para artículo inválido"
            )
            ...
            if response.text:
                try:
                    ...
                    if "detail" in error_data and isinstance(
                        ...
                        print_result(
                            True, "Validación funcionó correctamente (job no creado)"
                        )
                except:  # noqa: E722
                    print(f"  - Respuesta: {response.text}")
        else:
            print_result(
                False, f"Código inesperado: {response.status_code} (esperado 400)"
            )
...
if __name__ == "__main__":
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"{RED}✗ El servidor respondió con código: {response.status_code}{RESET}"
            )
