...
def verificar_flujo_id_fragmento():
    ...
    generacion = analizar_archivo(controller_path, r'"id_fragmento":\s*str\(uuid\.uuid4\(\)\)')
    if generacion:
    ...
    for fase in range(1, 8):
        fase_file = fases_dir / f"fase_{fase}_*.py"
        ...
        if archivos_fase:
            for archivo in archivos_fase:
                ...
                if usos:
                    print(f"✅ Fase {fase} ({archivo.name}): {len(usos)} referencias a id_fragmento")
                else:
                    print(f"⚠️  Fase {fase} ({archivo.name}): Sin referencias a id_fragmento")
    ...
    modelo_fragmento = analizar_archivo(persistencia_path, r'class FragmentoPersistenciaPayload')
    if modelo_fragmento:
        print(f"📍 FragmentoPersistenciaPayload definido en línea {modelo_fragmento[0][0]}")
        # Buscar campos del modelo
        with open(persistencia_path, 'r') as f:
            lineas = f.readlines()
            en_modelo = False
            ...
            for i, linea in enumerate(lineas):
                if 'class FragmentoPersistenciaPayload' in linea:
                    en_modelo = True
                elif en_modelo and 'class ' in linea and 'FragmentoPersistenciaPayload' not in linea:
                    break
    ...
    metodo_fragmento = analizar_archivo(builder_path, r'def construir_payload_fragmento')
    if metodo_fragmento:
        print(f"✅ Método construir_payload_fragmento encontrado en línea {metodo_fragmento[0][0]}")
    ...
    if alerts_path.exists():
        with open(alerts_path, 'r') as f:
            ...
            for alert in alerts_data.get('alerts', []):
                if 'id_fragmento' in alert.get('description', '') or \
                   'id_fragmento' in alert.get('annotations', {}).get('message', ''):
                    alertas_relevantes.append(alert)
            
            if alertas_relevantes:
                print(f"⚠️  Encontradas {len(alertas_relevantes)} alertas relacionadas con id_fragmento:")
                for alert in alertas_relevantes[:3]:  # Mostrar máximo 3
                    print(f"   - {alert.get('timestamp', 'N/A')}: {alert.get('description', 'N/A')[:80]}...")
            else:
