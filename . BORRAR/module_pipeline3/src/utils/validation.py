...
from pydantic import AfterValidator, BeforeValidator, field_validator  # noqa: F401
...
def _validate_non_empty_string(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        if not stripped:
            raise PydanticCustomError("empty_string", "String no puede estar vacío", {})
        return stripped
        ...
        for field in required_fields:
            if field not in data:
                ...
                validation_errors.append(
                    {"field": field, "message": f"El campo '{field}' no puede ser None"}
                )
...
def validate_confidence_score(value: float) -> float:
    ...
    if not 0.0 <= value <= 1.0:
        raise ValueError(
            f"Score de confianza debe estar entre 0 y 1, recibido: {value}"
        )
    return value
...
def validate_offset_range(
    ...
    if start is not None and end is not None:
        ...
        if end < start:
            raise ValueError(
                f"Offset final ({end}) no puede ser menor que el inicial ({start})"
            )
...
def validate_date_format(date_str: str, format: str = "%Y-%m-%d") -> str:
    ...
    try:
        ...
        raise ValueError(
            f"Formato de fecha inválido: {date_str}. Esperado: {format}"
        ) from e
...
def validate_wikidata_uri(uri: str) -> str:
    ...
    if not wikidata_pattern.match(uri):
        raise ValueError(
            f"URI de Wikidata inválida: {uri}. Debe seguir el formato https://www.wikidata.org/wiki/Qxxxxx"
        )
...
def validate_numeric_value(
    ...
    if min_value is not None and numeric_value < min_value:
        raise ValueError(
            f"Valor {numeric_value} es menor que el mínimo permitido {min_value}"
        )

    if max_value is not None and numeric_value > max_value:
        raise ValueError(
            f"Valor {numeric_value} es mayor que el máximo permitido {max_value}"
        )

    return numeric_value


def validate_date_optional(
    date_str: Optional[str], format: str = "%Y-%m-%d"
) -> Optional[str]:
    """
    ...
    raise ValueError(
        f"Formato de fecha inválido: {date_str}. Formatos soportados: ISO timestamps, YYYY-MM-DD, etc."
    )
