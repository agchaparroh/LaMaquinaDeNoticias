...
class MetricsCollector:
    ...
    def record_request_metric(
        ...
        self.logger.debug(
            f"Request metric recorded",  # noqa: F541
            method=method,
            path=path,
            status_code=status_code,
            duration_seconds=duration_seconds,
    ...
    def record_pipeline_metric(
        ...
        self.logger.debug(
            f"Pipeline metric recorded",  # noqa: F541
            pipeline_type=pipeline_type,
            duration_seconds=duration_seconds,
            success=success,
    ...
    def record_phase_metric(
        ...
        metric_data = {
            ...
            "chunking_used": chunks_processed is not None and chunks_processed > 1,
        }
        ...
        self.logger.debug(
            f"Phase metric recorded",  # noqa: F541
            phase_name=phase_name,
            duration_seconds=duration_seconds,
            success=success,
        )
    ...
    def get_aggregated_metrics(self) -> Dict[str, Any]:
        ...
        with self._operation_lock:
            ...
            for _, metric in recent_phases:
                ...
                # Nuevas métricas para el pipeline de 7 fases
                if metric.get("chunks_processed"):
                    phase_stats[phase]["chunks_total"] += metric["chunks_processed"]
                if metric.get("parallel_processing"):
                    phase_stats[phase]["parallel_count"] += 1
                if metric.get("chunking_used"):
                    phase_stats[phase]["chunking_count"] += 1
            ...
            for phase, stats in phase_stats.items():
                phase_success_rates[phase] = (
                    stats["success"] / max(stats["total"], 1)
                ) * 100
                phase_average_times[phase] = stats["total_time"] / max(
                    stats["total"], 1
                )
                # Estadísticas específicas del pipeline de 7 fases
                phase_chunking_stats[phase] = {
                    "chunking_usage_percent": (
                        stats["chunking_count"] / max(stats["total"], 1)
                    )
                    * 100,
                    "parallel_processing_percent": (
                        stats["parallel_count"] / max(stats["total"], 1)
                    )
                    * 100,
                    "average_chunks_per_execution": stats["chunks_total"]
                    / max(stats["chunking_count"], 1),
                }
            ...
            self._aggregated_metrics = {
                ...
                "phase_chunking_stats": phase_chunking_stats,
                "window_stats": {
                    "time_window_hours": self._time_window_hours,
        ...
        combined_metrics = {
            ...
            "integration_stats": {
                ...
                "data_consistency_check": {
                    "controller_fragments": controller_metrics.get(
                        "fragmentos_procesados", 0
                    ),
                    "collector_pipeline": collector_metrics["totals"][
                        "total_pipeline_processes"
                    ],
                    "difference": abs(
    ...
    def _cleanup_old_metrics(self):
        ...
        self.logger.debug(
            f"Metrics cleanup completed",  # noqa: F541
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._operation_lock:
            uptime_seconds = (
                datetime.utcnow() - self._total_counters["uptime_start"]
            ).total_seconds()
            ...
            return {
                ...
                "last_cleanup_ago_minutes": round(
                    (time.time() - self._last_cleanup) / 60, 1
                ),
...
if __name__ == "__main__":
    ...
    for i in range(5):
        collector.record_pipeline_metric(
            ...
            success=i < 4,
            elements_processed={
                "hechos": random.randint(1, 10),
                "entidades": random.randint(1, 5),
            },
            request_id=f"pipeline-{i}",
        )

    # Test 3: Métricas por fase (7 fases nuevas)
    print("3. Registrando métricas por fase...")
    phases = [
        "Fase1_Triaje",
        "Fase2_Simplificacion",
        "Fase3_Entidades",
        "Fase4_Hechos",
        "Fase5_Datos",
        "Fase6_Citas",
    ...
    for phase in phases:
        for i in range(3):
            # Simular chunking en algunas fases
            use_chunking = phase in ["Fase3_Entidades", "Fase4_Hechos"] and i % 2 == 0
            collector.record_phase_metric(
                ...
                chunks_processed=random.randint(2, 5) if use_chunking else None,
                parallel_processing=use_chunking and random.choice([True, False]),
            )
