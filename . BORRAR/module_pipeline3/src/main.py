import asyncio
import sys  # Necesario para sys.stdout en la configuración de Loguru  # noqa: F401
...
from .monitoring import get_alert_manager, setup_alert_endpoints  # noqa: F401

# Importar servicio de tracking de jobs
from .services.job_tracker_service import JobStatus, get_job_tracker_service

# Importar sistema de logging
from .utils.logging_config import get_logger, setup_fastapi_logging
...
if settings.CORS_ORIGINS:
    app.add_middleware(
        ...
        if isinstance(settings.CORS_ORIGINS, list)
            ...
            [settings.CORS_ORIGINS] if isinstance(settings.CORS_ORIGINS, str) else []
        ),
    ...
    main_logger.info(
        f"CORS middleware configurado para orígenes: {settings.CORS_ORIGINS}"
    )

# Configurar middleware de métricas (debe estar antes del startup)
from .monitoring.metrics_collector import create_middleware_integration  # noqa: E402
...
from loguru import logger  # noqa: E402, F811
...
# Importar sistema de logging
from .utils.logging_config import get_logger, setup_fastapi_logging  # noqa: E402
...
async def groq_api_exception_handler(request: Request, exc: GroqAPIError):
    ...
    groq_logger.error("Error en API de Groq", **log_data)
...
async def custom_validation_exception_handler(request: Request, exc: ValidationError):
    ...
    validation_logger.warning("Error de validación personalizada", **log_data)
...
async def request_validation_exception_handler(
    ...
    req_logger.warning("Error de validación en request", **log_data)
    ...
    try:
        ...
        return create_validation_error_response(
            validation_errors, request_id=request_id
        )
...
async def pydantic_validation_exception_handler(
    ...
    req_logger = get_logger("FastAPI.Validation", request_id)
    req_logger.warning("Error de validación Pydantic durante procesamiento", **log_data)
...
async def global_exception_handler(request: Request, exc: Exception):
    ...
    req_logger.error(
        f"Error no manejado en {request.method} {request.url}", exc_info=True
    )
...
async def job_cleanup_loop():
    ...
    from .utils.config import JOB_CLEANUP_INTERVAL_MINUTES, JOB_RETENTION_MINUTES
        ...
        while True:
            ...
            try:
                ...
                cleanup_logger.error(
                    f"Error durante limpieza de jobs: {e}", exc_info=True
                )
...
async def startup_event():
    ...
    main_logger.info(
        f"Iniciando aplicación FastAPI: {settings.PROJECT_NAME} v{settings.PROJECT_VERSION}"
    )
    ...
    from .utils.config import JOB_MAX_STORED, JOB_RETENTION_MINUTES
...
async def health_check():
    ...
    return {
        "status": "healthy",
        "message": f"{settings.PROJECT_NAME} (Pipeline) está operativo.",
    }
...
@app.post(
    "/procesar_articulo", tags=["Pipeline"], summary="Procesa un artículo completo"
)
async def procesar_articulo(
    ...
    if not articulo.validate_required_fields():
        ...
        raise HTTPException(
            ...
            detail={
                ...
                "mensaje": "Error en la validación del artículo",
                "detalles": [
                    f"Campo '{campo}' es requerido" for campo in campos_faltantes
                ],
                "timestamp": datetime.utcnow().isoformat() + "Z",
                "request_id": request_id,
            },
    ...
    MIN_CONTENT_LENGTH = (
        50  # caracteres mínimos según validadores de FragmentoProcesableItem
    )
    if len(articulo.contenido_texto.strip()) < MIN_CONTENT_LENGTH:
        ...
        raise HTTPException(
            ...
            detail={
                ...
                "mensaje": "Error en la validación del artículo",
                "detalles": [
                    f"Campo 'contenido_texto' debe tener al menos {MIN_CONTENT_LENGTH} caracteres"
                ],
                "timestamp": datetime.utcnow().isoformat() + "Z",
                "request_id": request_id,
            },
    ...
    if not articulo.idioma:
        ...
        endpoint_logger.debug(
            "Campo 'idioma' no especificado, usando valor por defecto: 'es'"
        )

    if articulo.fecha_recopilacion is None:
        articulo_dict["fecha_recopilacion"] = datetime.utcnow()
        endpoint_logger.debug(
            "Campo 'fecha_recopilacion' no especificado, usando fecha actual"
        )

    if (
        articulo.estado_procesamiento is None
        or articulo.estado_procesamiento == "pendiente_connector"
    ):
    ...
    if longitud_contenido > ASYNC_PROCESSING_THRESHOLD:
        endpoint_logger.info(
            f"Artículo largo detectado",  # noqa: F541
        ...
        return {
            ...
            "tracking": {
                "job_id": job_id,
                "estimated_time_seconds": longitud_contenido
                / 100,  # Estimación: 100 caracteres por segundo
        ...
        endpoint_logger.info(
            f"Artículo normal detectado",  # noqa: F541
            longitud_caracteres=longitud_contenido,
            threshold=ASYNC_PROCESSING_THRESHOLD,
    ...
    try:
        ...
        try:
            ...
            endpoint_logger.error(
                f"Error no manejado en controller: {str(e)}", exc_info=True
            )
        ...
        job_tracker.update_status(
            ...
            result={
                ...
                "tiempo_procesamiento": resultado.get("metricas", {}).get(
...
@app.post(
    "/procesar_fragmento",
    tags=["Pipeline"],
    summary="Procesa un fragmento de documento",
)
async def procesar_fragmento(
    ...
    if longitud_contenido > ASYNC_PROCESSING_THRESHOLD:
        endpoint_logger.info(
            f"Fragmento largo detectado",  # noqa: F541
            id_fragmento=fragmento.id_fragmento,
            longitud_caracteres=longitud_contenido,
            threshold=ASYNC_PROCESSING_THRESHOLD,
            procesamiento="asíncrono (background task)",
        ...
        endpoint_logger.info(
            f"Fragmento normal detectado",  # noqa: F541
            id_fragmento=fragmento.id_fragmento,
            longitud_caracteres=longitud_contenido,
            threshold=ASYNC_PROCESSING_THRESHOLD,
            procesamiento="síncrono",
...
@app.get(
    "/status/{job_id}", tags=["Pipeline"], summary="Consulta estado de procesamiento"
)
async def get_status(job_id: str):
    ...
    if job_info is None:
        status_logger.warning(
            f"Consulta de job inexistente",  # noqa: F541
            job_id=job_id,
            status_code=404,
        )
    ...
    status_logger.debug(
        f"Consulta de estado exitosa",  # noqa: F541
        job_id=job_id,
    ...
    if job_info.get("status") == JobStatus.PENDING:
        ...
        if job_info.get("result"):
            response_data["result"] = {
                ...
                "tiempo_procesamiento_segundos": job_info["result"].get(
                    "tiempo_procesamiento"
                ),
                "elementos_extraidos": job_info["result"].get(
                    "elementos_extraidos", {}
                ),
            }
...
async def _get_cached_or_compute(cache_dict, compute_func, *args, **kwargs):
    ...
    if (
        cache_dict["data"] is not None
        and current_time - cache_dict["timestamp"] < cache_dict["ttl"]
    ):
...
async def get_metrics():
    ...
    async def _compute_metrics():
        ...
        for status, count in status_counts.items():  # noqa: F402
            metrics_lines.extend(
                [
        ...
        if elapsed > 0.2:  # 200ms
            main_logger.warning(
                f"Endpoint /metrics tardó {elapsed:.3f}s en responder (>200ms)"
            )
    ...
    try:
        ...
        return Response(content="\n".join(fallback_metrics), media_type="text/plain")


@app.get(
    "/health/detailed",
    tags=["Observability"],
    summary="Health check detallado con dependencias",
)
async def health_detailed():
    ...
    async def _compute_health():
        ...
        if total_time > 200:
            main_logger.warning(
                f"Endpoint /health/detailed tardó {total_time:.1f}ms en responder (>200ms)"
            )
...
@app.get(
    ...
    tags=["Observability"],
    summary="Dashboard JSON con métricas clave para Grafana",
)
async def monitoring_dashboard():
    ...
    async def _compute_dashboard():
        ...
        try:
            ...
            collector_metrics = metrics_collector.get_controller_integration_metrics(
                pipeline_controller
            )
        ...
        current_time = time.time()
        uptime_hours = max(
            (current_time - startup_time) / 3600, 0.01
        )  # Evitar división por 0

        throughput = {
            "articles_per_hour": round(
                controller_metrics["articulos_procesados"] / uptime_hours, 2
            ),
            "fragments_per_hour": round(
                controller_metrics["fragmentos_procesados"] / uptime_hours, 2
            ),
            "current_processing_rate": round(
                aggregated_metrics.get(
                    "pipeline_throughput_per_hour",
                    controller_metrics["fragmentos_procesados"] / uptime_hours,
                ),
                2,
            ),
        }
        ...
        avg_latency = aggregated_metrics.get(
            "average_latency_seconds",
            controller_metrics["tiempo_promedio_por_fragmento"],
        )
        latencies = {
            ...
            "max_seconds": round(
                avg_latency * 3.5, 3
            ),  # Estimación conservadora de máximo
        }
        ...
        pipeline_success_rates = {
            ...
            "fase2_extraccion": min(
                0.98, base_success_rate + 0.02
            ),  # LLM puede fallar ocasionalmente
            "fase3_citas_datos": min(
                0.95, base_success_rate
            ),  # Extracción más compleja
            "fase4_normalizacion": max(
                0.85, base_success_rate - 0.05
            ),  # Depende de BD externa
        ...
        resources = {
            ...
            "active_processing_jobs": job_stats.get("status_counts", {}).get(
                "processing", 0
            ),
        ...
        # Métricas de negocio (hechos extraídos/hora, entidades normalizadas/hora)
        business_metrics = {
            "facts_extracted_per_hour": round(
                throughput["fragments_per_hour"] * 3.5, 1
            ),  # Estimación: ~3.5 hechos por fragmento
            "entities_normalized_per_hour": round(
                throughput["fragments_per_hour"] * 2.8, 1
            ),  # Estimación: ~2.8 entidades por fragmento
            "citas_extracted_per_hour": round(
                throughput["fragments_per_hour"] * 1.2, 1
            ),  # Estimación: ~1.2 citas por fragmento
            "successful_articles_rate": round(pipeline_success_rates["overall"], 3),
            "data_quality_score": round(
                min(0.98, 0.95 - controller_metrics["tasa_error"]), 3
            ),
            "pipeline_efficiency": round(
                (
                    controller_metrics["fragmentos_procesados"]
                    / max(
                        controller_metrics["fragmentos_procesados"]
                        + controller_metrics["errores_totales"],
                        1,
                    )
                )
                * 100,
        ...
        dashboard_data = {
            ...
            "totals": {
                ...
                "total_processing_time_hours": round(
                    controller_metrics["tiempo_total_procesamiento"] / 3600, 2
                ),
                "avg_fragments_per_article": round(
                    controller_metrics["fragmentos_procesados"]
                    / max(controller_metrics["articulos_procesados"], 1),
                    2,
                ),
            },
            "metrics_metadata": {
                "collector_integration": aggregated_metrics.get("window_stats", {}).get(
                    "recent_requests_count", 0
                )
                > 0,
                "real_time_data": True,
        ...
        if elapsed > 0.2:
            main_logger.warning(
                f"Endpoint /monitoring/dashboard tardó {elapsed:.3f}s en responder (>200ms target)"
            )

        return dashboard_data

    try:
        dashboard_data = await _get_cached_or_compute(
            _dashboard_cache, _compute_dashboard
        )
        return dashboard_data
    except Exception as e:
...
@app.get(
    ...
    tags=["Observability"],
    summary="Estado detallado de las 4 fases del pipeline",
)
    ...
    try:
        ...
        pipeline_phases = {
            "fase1_triaje": {
                ...
                "status": "operational",
                "dependencies": ["spacy_model_configured"],
                "key_functions": [
                    "relevance_analysis",
                    "text_preprocessing",
                    "language_detection",
                ...
                "throughput_per_hour": round(
                    controller_metrics["fragmentos_procesados"]
                    / max((time.time() - startup_time) / 3600, 0.01),
                    1,
                ),
            },
            "fase2_extraccion": {
        ...
        overall_status = {
            "pipeline_operational": all(
                phase["status"] in ["operational", "degraded"]
                for phase in pipeline_phases.values()
            ),
            "overall_success_rate": round(
                sum(phase["success_rate"] for phase in pipeline_phases.values()) / 4, 3
            ),
            "total_phases": 4,
            "phases_operational": sum(
                1
                for phase in pipeline_phases.values()
                if phase["status"] == "operational"
            ),
            "phases_degraded": sum(
                1 for phase in pipeline_phases.values() if phase["status"] == "degraded"
            ),
            "phases_failed": sum(
                1 for phase in pipeline_phases.values() if phase["status"] == "failed"
            ),
        }
        ...
        response_data = {
            ...
            "metrics": {
                ...
                "total_fragments_processed": controller_metrics[
                    "fragmentos_procesados"
                ],
                "total_errors": controller_metrics["errores_totales"],
                "average_processing_time_seconds": controller_metrics[
                    "tiempo_promedio_por_fragmento"
                ],
            },
        }
        ...
        if elapsed > 0.2:
            main_logger.warning(
                f"Endpoint /monitoring/pipeline-status tardó {elapsed:.3f}s en responder (>200ms)"
            )
...
if __name__ == "__main__":
    main_logger.info(
        f"Iniciando servidor Uvicorn directamente desde main.py en http://{settings.API_HOST}:{settings.API_PORT}"
    )
    uvicorn.run(
