...
class PipelineController:
    ...
    async def process_article(self, articulo_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if articulo_data.get("articulo_id"):
            ...
            article_logger.warning(
                f"No se encontró articulo_id en datos, usando UUID: {id_fragmento}"
            )
        ...
        article_logger.debug(
            "Artículo convertido a FragmentoProcesableItem con todos los metadatos"
        )
        ...
        try:
            ...
            article_logger.debug(
                f"Fragmento validado correctamente: ID={fragmento.id_fragmento}"
            )
        ...
        if resultado_pipeline.get("payload"):
            resultado["persistencia"] = self._persistir_resultado_7_fases(
                resultado_pipeline, fragmento, article_logger
            )
    ...
    def _persistir_resultado_7_fases(
        ...
        try:
            ...
            if (
                es_articulo
                and hasattr(fragmento, "id_fragmento")
                and fragmento.id_fragmento.startswith("ART-")
            ):
                try:
                    ...
                    logger.error(
                        f"No se pudo extraer ID numérico de: {fragmento.id_fragmento}"
                    )
                    return {
                        "exitosa": False,
            ...
            if es_articulo:
                ...
                resultado_persistencia = supabase_service.insertar_articulo_completo(
                    payload_dict
                )
            else:
    ...
    async def _process_fragment_4_phases(
        ...
        with log_phase(
            ...
            try:
                ...
                fallback_data = handle_groq_extraction_error_fase2(  # noqa: F821
                    article_id=str(fragment_uuid),  # noqa: F821
                ...
                entidades_autonomas_data = [
                    {
                        ...
                        "nombre": entidad.nombre_entidad_normalizada
                        or entidad.texto_entidad,
                        "tipo": entidad.tipo_entidad,
                        "relevancia_entidad_articulo": int(
                            entidad.relevancia_entidad * 10
                        ),
