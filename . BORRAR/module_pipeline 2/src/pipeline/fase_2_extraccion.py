...
from ..models.metadatos import MetadatosEntidad, MetadatosHecho
...
def _load_prompt_template() -> str:
    ...
    if _PROMPT_EXTRACCION_TEMPLATE is None:
        try:
            with open(_PROMPT_EXTRACCION_PATH, encoding="utf-8") as f:
                _PROMPT_EXTRACCION_TEMPLATE = f.read()
            logger.info(
                f"Plantilla de prompt de extracción cargada desde: {_PROMPT_EXTRACCION_PATH}"
            )
        except FileNotFoundError:
            logger.error(
                f"No se encontró el archivo de prompt de extracción en: {_PROMPT_EXTRACCION_PATH}"
            )
            raise ErrorFase2(
                f"Archivo de prompt de extracción no encontrado: {_PROMPT_EXTRACCION_PATH}"
            )
...
def _llamar_groq_api_extraccion(
    ...
    try:
        ...
        logger.error(
            f"Fallo al cargar plantilla de prompt para extracción. Artículo: {article_id_log}. Error: {e_prompt}"
        )
        raise GroqAPIError(
            f"Fallo al cargar plantilla de prompt para extracción: {e_prompt}",
            phase=ErrorPhase.FASE_2_EXTRACCION,
            article_id=config.get("article_id"),
        ) from e_prompt
    ...
    logger.debug(
        f"Variables del prompt - Título: '{titulo_documento[:50]}...', Fuente: '{fuente_tipo}', País: '{pais_origen}'"
    )
    ...
    logger.info(
        f"Enviando solicitud a Groq API para extracción (modelo: {config['model_id']}). Artículo: {article_id_log}"
    )
    logger.debug(
        f"Configuración Groq - Timeout: {config['timeout']}s, Temperature: {config['temperature']}, Max tokens: {config['max_tokens']}"
    )
    ...
    if hasattr(chat_completion, "usage"):
        logger.debug(
            f"Token usage - Prompt: {chat_completion.usage.prompt_tokens}, Completion: {chat_completion.usage.completion_tokens}, Total: {chat_completion.usage.total_tokens}"
        )

    respuesta_contenido = chat_completion.choices[0].message.content
    if not respuesta_contenido or not respuesta_contenido.strip():
        logger.warning(
            f"Groq API para extracción devolvió una respuesta vacía. Artículo: {article_id_log}"
        )
        raise GroqAPIError(
            "Respuesta vacía de Groq API para extracción.",
            phase=ErrorPhase.FASE_2_EXTRACCION,
            article_id=config.get("article_id"),
        )

    logger.info(
        f"Respuesta recibida de Groq API para extracción. Artículo: {article_id_log}"
    )
...
def _parsear_hechos_from_json(
    ...
    logger.debug(
        f"Parseando {len(hechos_raw)} hechos del JSON para fragmento {id_fragmento_origen}"
    )
    ...
    for hecho_raw in hechos_raw:
        try:
            ...
            descripcion_hecho = texto_hecho_sanitizado[
                :50
            ...
            fecha_inicio = fecha_obj.get("inicio")  # noqa: F841
            fecha_fin = fecha_obj.get("fin")  # noqa: F841
            ...
            try:
                ...
                logger.warning(
                    f"Confianza inválida para hecho {id_hecho_json}: {confianza}. Usando valor por defecto."
                )
                confianza = 0.8
            ...
            hecho_procesado = HechoProcesado(
                ...
                respuesta_llm_bruta={"id_original": id_hecho_json, "fecha": fecha_obj},
            )

            hechos_procesados.append(hecho_procesado)
            logger.trace(
                f"Hecho procesado: ID {hecho_procesado.id_hecho}, Tipo: {metadata_hecho.tipo_hecho}"
            )
    ...
    logger.info(
        f"Parseados {len(hechos_procesados)} hechos exitosamente para fragmento {id_fragmento_origen}"
    )
...
def _parsear_entidades_from_json(
    ...
    logger.debug(
        f"Parseando {len(entidades_raw)} entidades del JSON para fragmento {id_fragmento_origen}"
    )
    ...
    for entidad_raw in entidades_raw:
        try:
            ...
            if descripcion_raw:
                ...
                descripcion_estructurada = [
                    escape_html(parte.strip()) for parte in partes if parte.strip()
                ]
            ...
            try:
                ...
                logger.warning(
                    f"Relevancia inválida para entidad {id_entidad_json}: {relevancia}. Usando valor por defecto."
                )
                relevancia = 0.7
            ...
            if offset_inicio is not None or offset_fin is not None:
                try:
                    ...
                    logger.warning(
                        f"Offsets inválidos para entidad {id_entidad_json}: {e}"
                    )
                    offset_inicio = offset_fin = None
            ...
            logger.trace(
                f"Entidad procesada: ID {entidad_procesada.id_entidad}, Nombre: {entidad_procesada.texto_entidad}, Tipo: {entidad_procesada.tipo_entidad}"
            )

        except Exception as e:
            logger.warning(
                f"Error al parsear entidad {entidad_raw.get('id', 'N/A')}: {e}"
            )
            continue

    logger.info(
        f"Parseadas {len(entidades_procesadas)} entidades exitosamente para fragmento {id_fragmento_origen}"
    )
...
def ejecutar_fase_2(
    ...
    """
    logger.info(
        f"Iniciando Fase 2: Extracción para fragmento ID: {resultado_fase_1.id_fragmento}"
    )

    # Verificar que hay texto para procesar
    if (
        not resultado_fase_1.texto_para_siguiente_fase
        or not resultado_fase_1.texto_para_siguiente_fase.strip()
    ):
        logger.warning(
            f"No hay texto para procesar en Fase 2 para fragmento {resultado_fase_1.id_fragmento}"
        )
    ...
    if not groq_config.get("api_key"):
        logger.error(
            f"No se encontró GROQ_API_KEY. No se puede procesar fragmento {resultado_fase_1.id_fragmento}"
        )
        # Aplicar fallback
    ...
    try:
        ...
        texto_sanitizado_para_llm = escape_html(
            resultado_fase_1.texto_para_siguiente_fase
        )

        # Logging pre-llamada
        logger.debug(f"=== FASE 2 DEBUG - Pre-Groq API ===")  # noqa: F541
        logger.debug(f"Fragmento ID: {resultado_fase_1.id_fragmento}")
        logger.debug(
            f"Texto original length: {len(resultado_fase_1.texto_para_siguiente_fase)}"
        )
        logger.debug(f"Texto sanitizado length: {len(texto_sanitizado_para_llm)}")
        logger.debug(
            f"Primeros 100 chars del texto: {texto_sanitizado_para_llm[:100]}..."
        )
        ...
        logger.trace(
            f"Respuesta LLM (cruda) para extracción de {resultado_fase_1.id_fragmento}:\n{respuesta_llm_cruda}"
        )

        # Logging post-respuesta detallado
        logger.debug(f"=== FASE 2 DEBUG - Post-Groq API ===")  # noqa: F541
        logger.debug(
            f"Respuesta recibida - Tipo: {type(respuesta_llm_cruda)}, Length: {len(respuesta_llm_cruda) if respuesta_llm_cruda else 'None'}"
        )
        if respuesta_llm_cruda:
            logger.debug(f"Primeros 500 chars: {respuesta_llm_cruda[:500]}")
            logger.debug(
                f"Últimos 200 chars: {respuesta_llm_cruda[-200:] if len(respuesta_llm_cruda) > 200 else respuesta_llm_cruda}"
            )
            ...
            if stripped:
                ...
                logger.debug(
                    f"Primer carácter: '{first_char}', Último carácter: '{last_char}'"
                )
                logger.debug(
                    f"¿Empieza con {{?: {first_char == '{'}, ¿Termina con }}?: {last_char == '}'}"
                )
                # Buscar posibles problemas comunes
                if "```" in respuesta_llm_cruda:
                    logger.warning("La respuesta contiene markdown code blocks (```)")
                if respuesta_llm_cruda.startswith(
                    "Here"
                ) or respuesta_llm_cruda.startswith("Aquí"):
                    logger.warning(
                        "La respuesta parece tener texto explicativo al inicio"
                    )
        ...
        try:
            ...
            logger.error(
                f"Contenido que falló al parsear: '{respuesta_llm_cruda[:100]}...'"
            )
            logger.error(
                f"Tipo de contenido: {type(respuesta_llm_cruda)}, Longitud: {len(respuesta_llm_cruda) if respuesta_llm_cruda else 0}"
            )
            raise ProcessingError(
        ...
        if "hechos" not in respuesta_json and "entidades" not in respuesta_json:
            logger.warning(
                f"Respuesta JSON no contiene 'hechos' ni 'entidades' para fragmento {resultado_fase_1.id_fragmento}"
            )
        ...
        if not hechos_extraidos and not entidades_extraidas:
            ...
            logger.warning(
                f"No se extrajeron elementos del fragmento {resultado_fase_1.id_fragmento}"
            )

    except GroqAPIError as e:
        logger.error(
            f"GroqAPIError durante extracción para {resultado_fase_1.id_fragmento}: {e}"
        )
        error_dict = handle_generic_phase_error(
            article_id=str(resultado_fase_1.id_fragmento),
            phase=ErrorPhase.FASE_2_EXTRACCION,
        ...
        logger.error(
            f"Error inesperado durante extracción para {resultado_fase_1.id_fragmento}: {e}"
        )
        error_dict = handle_generic_phase_error(
            article_id=str(resultado_fase_1.id_fragmento),
            phase=ErrorPhase.FASE_2_EXTRACCION,
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            f"✅ Test exitoso: {len(resultado.hechos_extraidos)} hechos, {len(resultado.entidades_extraidas)} entidades"
        )
