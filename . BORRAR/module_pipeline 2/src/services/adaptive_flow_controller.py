...
class AdaptiveFlowController:
    ...
    def evaluate_content(self, analysis: AnalisisComponentes) -> FlowDecision:
        ...
        if decision.chunk_entities:
            justificaciones.append(f"Chunking entidades: {analysis.conteo_entidades} entidades detectadas")
        
        decision.chunk_facts = self._should_chunk_facts(analysis)
        if decision.chunk_facts:
            justificaciones.append(f"Chunking hechos: texto de {analysis.longitud_caracteres} caracteres")
        
        decision.chunk_quotes = self._should_chunk_quotes(analysis)
        if decision.chunk_quotes:
            justificaciones.append(f"Chunking citas: {'entrevista' if analysis.es_entrevista else f'{analysis.conteo_citas} citas'}")
        
        decision.chunk_data = self._should_chunk_data(analysis)
        if decision.chunk_data:
            justificaciones.append(f"Chunking datos: {analysis.conteo_datos} elementos numéricos")
        
        # Decisiones de ejecución de fases
        decision.execute_data_phase = analysis.conteo_datos >= self.thresholds.min_data_for_extraction
        if not decision.execute_data_phase:
            justificaciones.append("Fase de datos omitida: sin datos numéricos detectados")
        
        decision.execute_quotes_phase = analysis.conteo_citas >= self.thresholds.min_quotes_for_extraction
        ...
        if decision.use_large_model:
            justificaciones.append(f"Modelo grande recomendado: {analysis.conteo_tokens} tokens")
        
        # Configuración de chunking si aplica
        if any([decision.chunk_entities, decision.chunk_facts, decision.chunk_quotes, decision.chunk_data]):
        ...
        decision.justificacion = "; ".join(justificaciones) if justificaciones else "Procesamiento estándar"
        
        logger.info(
            "Decisiones de flujo generadas",
            chunk_any=any([decision.chunk_entities, decision.chunk_facts, decision.chunk_quotes, decision.chunk_data]),
            use_large_model=decision.use_large_model,
            skip_phases=2 - sum([decision.execute_data_phase, decision.execute_quotes_phase])
        )
    ...
    def _should_chunk_entities(self, analysis: AnalisisComponentes) -> bool:
        ...
        # Por densidad
        if analysis.densidad_entidades > self.thresholds.high_density_entities_threshold:
            return True
    ...
    def _should_chunk_data(self, analysis: AnalisisComponentes) -> bool:
        ...
        tipos_datos = sum([
            analysis.tiene_fechas,
            analysis.tiene_monedas,
            analysis.tiene_porcentajes
        ])
    ...
    def _should_use_large_model(self, analysis: AnalisisComponentes) -> bool:
        ...
        if (analysis.densidad_entidades > 15 or 
            analysis.densidad_numerica > 10):
            return True
    ...
    def _calculate_chunk_config(
        ...
        if decision.chunk_quotes and (analysis.es_entrevista or analysis.conteo_citas > 20):
    ...
    def validate_decision(self, decision: FlowDecision, analysis: AnalisisComponentes) -> bool:
        """
    ...
    def get_processing_stats(self, decision: FlowDecision) -> Dict[str, Any]:
        ...
        """
        return {
            "phases_to_execute": 5 + sum([decision.execute_data_phase, decision.execute_quotes_phase]),
            "chunking_phases": sum([
