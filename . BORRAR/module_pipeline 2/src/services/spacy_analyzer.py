...
class SpacyAnalyzer:
    ...
    def __init__(self, nlp_model: Optional[Language] = None):
        ...
        self.interview_pattern = re.compile(r'^(P:|R:|Pregunta:|Respuesta:|\-)', re.MULTILINE)
        self.number_pattern = re.compile(r'\b\d+(?:[.,]\d+)*\s*(?:%|€|\$|millones?|miles?|euros?|dólares?)')
        self.date_pattern = re.compile(r'\b\d{1,2}[/-]\d{1,2}[/-]\d{2,4}\b|\b\d{4}\b')
        self.money_pattern = re.compile(r'[€$]\s*\d+(?:[.,]\d+)*|(?:\d+(?:[.,]\d+)*\s*(?:euros?|dólares?))')
    ...
    def _set_cached_analysis(self, text_hash: str, analysis: AnalisisComponentes) -> None:
        """Guarda análisis en caché con límite de tamaño."""
        if len(self._analysis_cache) >= self._cache_max_size:
            # Eliminar 20% de entradas más antiguas (simple LRU aproximado)
            keys_to_remove = list(self._analysis_cache.keys())[:self._cache_max_size // 5]
    ...
    def analizar_contenido(
        ...
        if analisis.conteo_tokens > 0:
            analisis.densidad_entidades = (analisis.conteo_entidades / analisis.conteo_tokens) * 100
            analisis.densidad_numerica = (analisis.conteo_datos / analisis.conteo_tokens) * 100
    ...
    def _analizar_patrones_basicos(self, texto: str, analisis: AnalisisComponentes) -> AnalisisComponentes:
    ...
    def _analizar_con_spacy(self, texto: str, nlp: Language, analisis: AnalisisComponentes) -> AnalisisComponentes:
        ...
        for sent in doc.sents:
            sent_ents = [ent for ent in entidades if ent.start >= sent.start and ent.end <= sent.end]
        ...
        if analisis.conteo_oraciones > 0:
            analisis.promedio_tokens_por_oracion = total_tokens_oraciones / analisis.conteo_oraciones
    ...
    def _analizar_sin_spacy(self, texto: str, analisis: AnalisisComponentes) -> AnalisisComponentes:
        ...
        if analisis.conteo_oraciones > 0:
            analisis.promedio_tokens_por_oracion = analisis.conteo_tokens / analisis.conteo_oraciones
        
        # Detección básica de entidades (muy limitada)
        # Buscar palabras en mayúsculas que podrían ser nombres propios
        palabras_mayusculas = [p for p in palabras if p and p[0].isupper() and len(p) > 1]
        analisis.conteo_entidades = len(palabras_mayusculas) // 2  # Estimación conservadora
        ...
        for i, token in enumerate(tokens):
            if token.like_num or token.pos_ == "NUM":
                ...
                if i + 1 < len(tokens):
                    ...
                    if next_token.text.lower() in ['%', 'euros', 'dólares', 'millones', 'miles', '€', '$']:
                        count += 1
                    elif next_token.pos_ == "NOUN" and any(unit in next_token.text.lower() 
                                                          for unit in ['metro', 'kilo', 'litro', 'año']):
                        count += 1
        
        return count
    
    def sugerir_estrategia_chunking(self, analisis: AnalisisComponentes) -> Dict[str, Any]:
        """
        ...
        sugerencias["razon"] = "; ".join(razones) if razones else "contenido dentro de límites normales"
