...
class ErrorPhase(str, Enum):
    ...
    SIMPLIFICACION = "fase_2_simplificacion"  # Nueva fase 2
    FASE_2_EXTRACCION = "fase_2_extraccion"  # Ahora será fase 3
    FASE_3_CITAS_DATOS = "fase_3_citas_datos"  # Ahora será fases 5-6
    FASE_4_NORMALIZACION = "fase_4_normalizacion"  # Ahora será fase 7
...
def create_error_response(
    ...
    if status_code is None:
        if isinstance(error, ValidationError) or isinstance(
...
def create_validation_error_response(
    validation_errors: List[str], request_id: Optional[str] = None
) -> JSONResponse:
    """
...
def format_error_for_logging(
    ...
    if isinstance(error, PipelineException):
        log_data.update(
            {
                "level": "ERROR"
                if error.error_type != ErrorType.PROCESSING_ERROR
                else "WARNING",
    ...
    def decorator(func):
        ...
        @retry(
            stop=stop_after_attempt(
                max_attempts + 1
            ),  # +1 porque incluye el intento inicial
            wait=wait_strategy,
            ...
            while attempts <= connection_retries:
                ...
                try:
                    ...
                    if attempts <= connection_retries:
                        ...
                        logger.error(
                            f"Supabase RPC falló después de {connection_retries} reintentos"
                        )
...
class SpaCyModelLoadFallbackLog(PipelineException):
    def __init__(
        self, message, article_id=None, model_name=None, original_exception_message=None
    ):
        super().__init__(
            message,
            error_type=ErrorType.PROCESSING_ERROR,
            ...
            details={
                ...
                "reason": "SpaCy model load failed, fallback used",
                "original_error": original_exception_message,
            },
            article_id=article_id,
        )
        # La clase padre ya inicializa timestamp
...
class GroqRelevanceFallbackLog(PipelineException):
    def __init__(
        ...
        super().__init__(
            ...
            phase=ErrorPhase.FASE_1_TRIAJE,
            details={
                "reason": "Groq API for relevance failed, fallback used",
                "text_cleaned_excerpt": text_cleaned_excerpt,
                "original_error": original_exception_message,
            },
            article_id=article_id,
        )
...
class GroqTranslationFallbackLogEvent(PipelineException):
    def __init__(
        ...
        super().__init__(
            message,
            error_type=ErrorType.PROCESSING_ERROR,  # This will make format_error_for_logging use WARNING
            phase=ErrorPhase.FASE_1_TRIAJE,
            details={
                "reason": "Groq translation failed, using original text",
...
class GroqExtractionFallbackLog(PipelineException):
    def __init__(
        ...
        super().__init__(
            ...
            phase=phase,
            details={
                "reason": "Groq API extraction failed, using fallback data",
                "original_error": original_exception_message,
            },
            article_id=article_id,
        )
...
class NormalizationFallbackLog(PipelineException):
    def __init__(
        ...
        super().__init__(
            ...
            details={
                "reason": "Entity normalization failed, treating as new entity",
...
def handle_spacy_load_error_fase1(
    article_id: Optional[str], model_name: str, exception: Exception
) -> Dict[str, Any]:
    """
...
def handle_groq_relevance_error_fase1(
    article_id: Optional[str], text_cleaned: str, exception: Exception
) -> Dict[str, Any]:
    """
    ...
    log_details = format_error_for_logging(
        log_event, context={"original_exception_type": type(exception).__name__}
    )
...
def handle_groq_translation_fallback_fase1(
    ...
    log_details = format_error_for_logging(
        ...
        context={
            "original_language": original_language,
            "original_exception_type": type(exception).__name__ if exception else "N/A",
        },
    )
...
def handle_generic_phase_error(
    article_id: Optional[str], phase: ErrorPhase, step_failed: str, exception: Exception
) -> Dict[str, Any]:
    """
    ...
    ):  # It will already have phase, error_type, details
...
def handle_groq_extraction_error_fase2(
    ...
    log_details = format_error_for_logging(
        log_event, context={"titulo": titulo, "medio": medio}
    )
    logger.bind(**log_details).error(log_details.get("message"))
    ...
    hecho_basico = {
        ...
        "metadata_hecho": {"tipo_hecho": "SUCESO", "es_fallback": True},
    }
    ...
    entidad_basica = {
        ...
        "metadata_entidad": {"tipo": "ORGANIZACION", "es_fallback": True},
    }
    ...
    return {
        ...
        "advertencias": [
            f"Extracción degradada: {str(exception) if exception else 'Error en Groq API'}"
        ],
    }


def handle_json_parsing_error_fase2(
    article_id: Optional[str], json_response: str, exception: Exception
) -> Dict[str, Any]:
    """
...
def handle_groq_citas_error_fase3(
    article_id: Optional[str], exception: Exception = None
) -> Dict[str, Any]:
    """
    ...
    logger.bind(**log_details).warning(
        log_details.get("message")
    )  # WARNING porque no es crítico

    return {
        "citas_textuales_extraidas": [],
        "datos_cuantitativos_extraidos": [],
        "advertencias_citas_datos": [
            "Extracción de citas y datos omitida por fallo en API"
        ],
    }


def handle_normalization_error_fase4(
    article_id: Optional[str], entidades: List[Any], exception: Exception = None
) -> List[Any]:
    """
    ...
    log_details = format_error_for_logging(
        log_event, context={"num_entities": len(entidades)}
    )
    logger.bind(**log_details).warning(log_details.get("message"))
