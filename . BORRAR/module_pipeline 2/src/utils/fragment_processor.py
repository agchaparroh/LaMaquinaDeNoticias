...
class FragmentProcessor:
    ...
    def next_hecho_id(self, descripcion_corta: str = None) -> int:
        ...
        self.logger.debug(
            f"Asignado ID hecho {current_id} en fragmento {self.id_fragmento}: {descripcion_corta or 'Sin descripción'}"
        )
        return current_id
    ...
    def next_entidad_id(self, nombre_entidad: str = None) -> int:
        ...
        self.logger.debug(
            f"Asignado ID entidad {current_id} en fragmento {self.id_fragmento}: {nombre_entidad or 'Sin nombre'}"
        )
        return current_id
    ...
    def next_cita_id(self, cita_preview: str = None) -> int:
        ...
        self.logger.debug(
            f"Asignado ID cita {current_id} en fragmento {self.id_fragmento}: {cita_preview or 'Sin preview'}"
        )
        return current_id
    ...
    def next_dato_id(self, descripcion_dato: str = None) -> int:
        ...
        self.logger.debug(
            f"Asignado ID dato {current_id} en fragmento {self.id_fragmento}: {descripcion_dato or 'Sin descripción'}"
        )
        return current_id
    ...
    def reset_counters(self) -> None:
        ...
        self.logger.warning(
            f"FragmentProcessor contadores reiniciados para fragmento {self.id_fragmento}, chunk {self.chunk_index}"
        )

    def log_summary(self, custom_logger: Optional["loguru.Logger"] = None) -> None:  # noqa: F821
        """
    ...
    def parse_global_reference(self, referencia_global: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(
                f"Error parseando referencia global '{referencia_global}': {e}"
            )
            raise
    ...
    @staticmethod
    def consolidate_ids_mapping(
        processors: List["FragmentProcessor"],
    ) -> Dict[int, int]:
        """
        ...
        consolidated_counters = {"hecho": 1, "entidad": 1, "cita": 1, "dato": 1}
...
def create_fragment_processor(
    id_fragmento: UUID, chunk_index: int = 0, total_chunks: int = 1
) -> FragmentProcessor:
    """
...
if __name__ == "__main__":
    ...
    print(
        f"IDs asignados - Hechos: {hecho1}, {hecho2} | Entidades: {entidad1}, {entidad2}"
    )
    ...
    # Test con chunking
    print(f"\n--- Test FragmentProcessor con chunking ---")  # noqa: F541
    ...
    print(f"\n--- Test Consolidación de IDs ---")  # noqa: F541
