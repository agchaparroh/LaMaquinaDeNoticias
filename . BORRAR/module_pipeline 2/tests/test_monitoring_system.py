...
from src.models.entrada import FragmentoProcesableItem  # noqa: F401
...
class TestMonitoringSystem:
    ...
    def sample_article(self):
        ...
        return {
            ...
            "contenido_texto": "Este es un contenido de prueba para el sistema de monitoreo. "
            * 10,
            "idioma": "es",
            "autor": "Test Author",
    ...
    def test_health_detailed_endpoint_responds_correctly(self, test_client):
        ...
        print(
            "✅ /health/detailed endpoint responde correctamente con checks completos"
        )
    ...
    def test_pipeline_status_endpoint_responds_correctly(self, test_client):
        ...
        print(
            "✅ /monitoring/pipeline-status endpoint responde con información de las 4 fases"
        )
    ...
    def test_metrics_generated_during_processing(
        self, mock_process, test_client, sample_article
    ):
        """
    ...
    def test_fragment_processing_generates_metrics(
        self, mock_process, test_client, sample_fragment
    ):
        """
        ...
        mock_process.return_value = {
            ...
            "metricas": {
                ...
                "conteos_elementos": {"hechos_extraidos": 3, "entidades_extraidas": 2},
            },
        }
    ...
    def test_response_times_under_200ms(self, test_client):
        ...
        for endpoint in endpoints:
            ...
            assert response_time_ms < 200, (
                f"Endpoint {endpoint} tardó {response_time_ms:.1f}ms (>200ms)"
            )
    ...
    @patch("src.controller.PipelineController.process_article")
    def test_load_test_with_10_articles(
        self, mock_process, test_client, sample_article
    ):
        """
        ...
        for i in range(10):
            ...
            test_article["contenido_texto"] = (
                f"Contenido del artículo #{i + 1} para test de carga. " * 15
            )
        ...
        assert successful_requests == 10, (
            f"Solo {successful_requests}/10 requests fueron exitosas"
        )

        average_response_time = total_response_time / 10
        assert average_response_time < 5.0, (
            f"Tiempo promedio de respuesta {average_response_time:.2f}s demasiado alto"
        )
        ...
        assert requests_processed >= 10, (
            f"Solo se registraron {requests_processed} requests en métricas"
        )
        ...
        print(
            f"✅ Test de carga completado: 10 artículos procesados en {total_response_time:.2f}s"
        )
    ...
    def test_monitoring_system_integration(self, test_client):
        ...
        print(f"\n🌐 ENDPOINTS DE MONITOREO:")  # noqa: F541
        ...
        all_endpoints_working = all(
            "✅" in status for status in endpoints_status.values()
        )
        ...
        assert overall_success, (
            "Sistema de monitoreo no cumple todos los criterios de éxito"
        )
    ...
    def test_performance_under_load(self, test_client):
        ...
        print(f"\n📈 TEST DE RENDIMIENTO BAJO CARGA:")  # noqa: F541
        ...
        assert success_rate >= 95, (
            f"Tasa de éxito {success_rate:.1f}% por debajo del 95%"
        )
        assert avg_response_time < 500, (
            f"Tiempo promedio {avg_response_time:.1f}ms demasiado alto"
        )
        assert max_response_time < 1000, (
            f"Tiempo máximo {max_response_time:.1f}ms inaceptable"
        )
