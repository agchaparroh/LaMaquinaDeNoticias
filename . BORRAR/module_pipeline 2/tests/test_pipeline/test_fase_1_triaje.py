...
from src.module_pipeline.src.models.procesamiento import (
    MetadatosFase1Triaje,
    ResultadoFase1Triaje,
)
from src.module_pipeline.src.pipeline.fase_1_triaje import (  # noqa: F401
    ErrorFase1,
    ejecutar_fase_1,
)
...
def test_ejecutar_fase_1_spacy_load_failure(mock_cargar_modelo_spacy):
    ...
    assert (
        "Fallo al cargar modelo spaCy" in resultado.justificacion_triaje
    )  # Updated message
    assert "Simulated spaCy load failure" in resultado.justificacion_triaje
    assert (
        resultado.texto_para_siguiente_fase == texto_original
    )  # Updated: original text

    assert isinstance(resultado.metadatos_specificos_triaje, MetadatosFase1Triaje)
    assert (
        resultado.metadatos_specificos_triaje.nombre_modelo_triaje
        == modelo_spacy_nombre
    )
    ...
    assert any(
        "SpaCy model loading failed" in nota
        for nota in resultado.metadatos_specificos_triaje.notas_adicionales
    )
...
@patch(
    "src.module_pipeline.src.pipeline.fase_1_triaje._limpiar_texto"
)  # Mock to control cleaned text
def test_ejecutar_fase_1_groq_api_key_missing(
    mock_limpiar_texto, mock_get_groq_config, mock_cargar_modelo_spacy
):
    """
    ...
    assert (
        resultado.decision_triaje == "FALLBACK_ACEPTADO_ERROR_LLM"
    )  # Fallback decision
    assert "GROQ_API_KEY no configurada" in resultado.justificacion_triaje
    assert (
        resultado.texto_para_siguiente_fase == cleaned_text_mock
    )  # Cleaned text should be passed
    ...
    assert resultado.metadatos_specificos_triaje.notas_adicionales is not None
    assert any(
        "GROQ_API_KEY no configurada" in nota
        for nota in resultado.metadatos_specificos_triaje.notas_adicionales
    )
...
def test_ejecutar_fase_1_groq_relevance_api_error(
    mock_limpiar_texto,
    mock_llamar_groq_triaje,
    ...
    """
    from src.module_pipeline.src.utils.error_handling import ErrorPhase, GroqAPIError

    mock_cargar_modelo_spacy.return_value = _get_mock_spacy_model()
    mock_get_groq_config.return_value = {
        "api_key": "fake_key",
        "model_id": "test_relevance_model",
    }
    ...
    simulated_groq_error = GroqAPIError(
        "Simulated Groq API Error during relevance", phase=ErrorPhase.FASE_1_TRIAJE
    )
    ...
    assert (
        resultado.metadatos_specificos_triaje.nombre_modelo_triaje
        == "test_relevance_model"
    )
    assert resultado.metadatos_specificos_triaje.notas_adicionales is not None
    assert any(
        "Groq API failed during relevance evaluation" in nota
        for nota in resultado.metadatos_specificos_triaje.notas_adicionales
    )
...
def test_ejecutar_fase_1_groq_translation_api_error(
    ...
    """
    from src.module_pipeline.src.utils.error_handling import (  # noqa: F401
        ErrorPhase,
        GroqAPIError,
    )
    ...
    mock_get_groq_config.return_value = {
        "api_key": "fake_key",
        "model_id": "test_translation_model",
    }
    ...
    - Elemento 2
    """
    mock_llamar_triaje.return_value = (
        "mocked_prompt_triaje",
        mock_triaje_response_content,
    )
    ...
    assert any(
        "Traducción de 'en' falló" in nota
        for nota in resultado.metadatos_specificos_triaje.notas_adicionales
    )
    assert any(
        "Se usará texto original" in nota
        for nota in resultado.metadatos_specificos_triaje.notas_adicionales
    )
...
@patch("src.module_pipeline.src.pipeline.fase_1_triaje._llamar_groq_api_triaje")
@patch(
    "src.module_pipeline.src.pipeline.fase_1_triaje._llamar_groq_api_traduccion"
)  # Mock translation too
def test_ejecutar_fase_1_successful_run_no_translation_needed(
    ...
    assert (
        resultado.justificacion_triaje
        == "Artículo muy relevante sobre política española."
    )
    ...
    assert (
        resultado.texto_para_siguiente_fase == expected_cleaned_text
    )  # No translation needed

    assert isinstance(resultado.metadatos_specificos_triaje, MetadatosFase1Triaje)
    assert resultado.metadatos_specificos_triaje.idioma_detectado_original == "es"
    assert (
        resultado.metadatos_specificos_triaje.notas_adicionales is None
