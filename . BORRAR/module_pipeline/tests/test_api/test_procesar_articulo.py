...
mock_pipeline_controller.process_article = AsyncMock(
    return_value={
        "fragmento_id": "test_fragment_001",
        "metricas": {
            "tiempo_total_segundos": 0.5,
            "conteos_elementos": {"hechos_extraidos": 3, "entidades_extraidas": 5},
        },
        "resultado": "procesado",
    }
)
...
def test_procesar_articulo_valido():
    ...
    timestamp = datetime.fromisoformat(
        response_json["timestamp"].replace("Z", "+00:00")
    )
...
def test_procesar_articulo_sin_campos_requeridos():
    ...
    assert (
        "validation" in response_json["mensaje"].lower()
        or "error" in response_json["error"]
    )
