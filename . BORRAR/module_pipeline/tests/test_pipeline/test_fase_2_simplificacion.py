...
from src.pipeline.fase_2_simplificacion import (
    ...
    _preparar_prompt_simplificacion,
...
class TestEjecutarFase2Simplificacion:
    ...
    def test_simplificacion_exitosa(self, mock_cargar_prompt, mock_groq_class, resultado_triaje_mock):
        ...
        mock_response = Mock()
        mock_response.choices = [Mock(message=Mock(content="El plan del Banco Central Europeo (BCE) será implementado el 11 de junio de 2025."))]
        ...
        resultado = ejecutar_fase_2_simplificacion(
            resultado_triaje_mock,
            fecha_articulo="2025-06-10",
    ...
    @patch('src.pipeline.fase_2_simplificacion.Groq')
    @patch('src.pipeline.fase_2_simplificacion._cargar_prompt_simplificacion')
    def test_uso_modelo_grande_para_texto_largo(self, mock_cargar_prompt, mock_groq_class, resultado_triaje_mock):
...
class TestSimplificarConChunking:
    ...
    @patch('src.pipeline.fase_2_simplificacion.Groq')
    @patch('src.pipeline.fase_2_simplificacion._cargar_prompt_simplificacion')
    def test_simplificar_multiples_chunks(self, mock_cargar_prompt, mock_groq_class, resultado_triaje_mock):
        ...
        assert resultado.texto_simplificado.count("\n\n") == 2  # 3 chunks = 2 separadores
        assert "Procesado en 3 chunks" in resultado.metadatos_simplificacion.advertencias_simplificacion
    
    @patch('src.pipeline.fase_2_simplificacion.Groq')
    @patch('src.pipeline.fase_2_simplificacion._cargar_prompt_simplificacion')
    def test_fallo_parcial_usa_original(self, mock_cargar_prompt, mock_groq_class, resultado_triaje_mock):
