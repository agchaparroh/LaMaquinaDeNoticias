...
def _load_prompt_template() -> str:
    ...
    if _PROMPT_CITAS_DATOS_TEMPLATE is None:
        try:
            with open(_PROMPT_CITAS_DATOS_PATH, encoding="utf-8") as f:
                _PROMPT_CITAS_DATOS_TEMPLATE = f.read()
            logger.info(
                f"Plantilla de prompt de citas y datos cargada desde: {_PROMPT_CITAS_DATOS_PATH}"
            )
        except FileNotFoundError:
            logger.error(
                f"No se encontró el archivo de prompt de citas y datos en: {_PROMPT_CITAS_DATOS_PATH}"
            )
            raise ErrorFase3(
                f"Archivo de prompt de citas y datos no encontrado: {_PROMPT_CITAS_DATOS_PATH}"
            )
...
def _formatear_json_paso_1(resultado_fase_2: ResultadoFase2Extraccion) -> str:
    ...
    """
    logger.debug(
        f"Formateando JSON_PASO_1 para fragmento {resultado_fase_2.id_fragmento}"
    )
    ...
    json_paso_1 = {"hechos": hechos_json, "entidades": entidades_json}
...
def _llamar_groq_api_citas_datos(
    ...
    try:
        ...
        logger.error(
            f"Fallo al cargar plantilla de prompt para citas y datos. Artículo: {article_id_log}. Error: {e_prompt}"
        )
        raise GroqAPIError(
            f"Fallo al cargar plantilla de prompt para citas y datos: {e_prompt}",
            phase=ErrorPhase.FASE_3_CITAS_DATOS,
            article_id=config.get("article_id"),
        ) from e_prompt
    ...
    logger.info(
        f"Enviando solicitud a Groq API para citas y datos (modelo: {config['model_id']}). Artículo: {article_id_log}"
    )
    ...
    if not respuesta_contenido or not respuesta_contenido.strip():
        logger.warning(
            f"Groq API para citas y datos devolvió una respuesta vacía. Artículo: {article_id_log}"
        )
        raise GroqAPIError(
            "Respuesta vacía de Groq API para citas y datos.",
            phase=ErrorPhase.FASE_3_CITAS_DATOS,
            article_id=config.get("article_id"),
        )

    logger.info(
        f"Respuesta recibida de Groq API para citas y datos. Artículo: {article_id_log}"
    )
...
def _parsear_citas_textuales_from_json(
    ...
    logger.debug(
        f"Parseando {len(citas_raw)} citas del JSON para fragmento {id_fragmento_origen}"
    )
    ...
    for cita_raw in citas_raw:
        try:
            ...
            descripcion_cita = cita_raw.get("cita", "")[
                :50
            ]  # Primeros 50 chars para debug
            ...
            # Sanitizar contexto
            contexto_sanitizado = (
                escape_html(cita_raw.get("contexto", ""))
                if cita_raw.get("contexto")
                else None
            )
            ...
            logger.trace(
                f"Cita procesada: ID {cita_procesada.id_cita}, Relevancia: {metadata_cita.relevancia}"
            )
    ...
    logger.info(
        f"Parseadas {len(citas_procesadas)} citas exitosamente para fragmento {id_fragmento_origen}"
    )
...
def _parsear_datos_cuantitativos_from_json(
    ...
    logger.debug(
        f"Parseando {len(datos_raw)} datos cuantitativos del JSON para fragmento {id_fragmento_origen}"
    )
    ...
    for dato_raw in datos_raw:
        try:
            ...
            if id_dato_json in ids_vistos:
                logger.warning(
                    f"ID duplicado {id_dato_json} en datos cuantitativos, saltando..."
                )
                continue
                ...
                try:
                    ...
                    if fecha_inicio and fecha_fin:
                        ...
                        if datetime.strptime(
                            ...
                            logger.warning(
                                f"Periodo inválido en dato {id_dato_json}: fecha inicio posterior a fecha fin"
                            )
                    ...
                    periodo_obj = PeriodoReferencia(inicio=fecha_inicio, fin=fecha_fin)
                except ValueError as e:
                    logger.warning(
                        f"Error validando periodo en dato {id_dato_json}: {e}"
                    )
            ...
            unidad_sanitizada = (
                escape_html(dato_raw.get("unidad", ""))
                if dato_raw.get("unidad")
                else None
            )
            ...
            logger.trace(
                f"Dato procesado: ID {dato_procesado.id_dato_cuantitativo}, Categoría: {metadata_dato.categoria}"
            )

        except Exception as e:
            logger.warning(
                f"Error al parsear dato cuantitativo {dato_raw.get('id', 'N/A')}: {e}"
            )
            continue

    logger.info(
        f"Parseados {len(datos_procesados)} datos cuantitativos exitosamente para fragmento {id_fragmento_origen}"
    )
...
def ejecutar_fase_3(
    ...
    """
    logger.info(
        f"Iniciando Fase 3: Citas y Datos para fragmento ID: {resultado_fase_2.id_fragmento}"
    )

    # Verificar que hay elementos para procesar
    if (
        not resultado_fase_2.hechos_extraidos
        and not resultado_fase_2.entidades_extraidas
    ):
        logger.warning(
            f"No hay hechos ni entidades para procesar en Fase 3 para fragmento {resultado_fase_2.id_fragmento}"
        )
        ...
        resultado_vacio = ResultadoFase3CitasDatos(
            ...
            advertencias_citas_datos=[
                "No se encontraron elementos previos para análisis"
            ],
        )
    ...
    if not groq_config.get("api_key"):
        logger.error(
            f"No se encontró GROQ_API_KEY. No se puede procesar fragmento {resultado_fase_2.id_fragmento}"
        )
        # Aplicar fallback
    ...
    try:
        ...
        if resultado_fase_1 and resultado_fase_1.texto_para_siguiente_fase:
            ...
            logger.debug(
                f"Usando texto original de fase 1 para fragmento {resultado_fase_2.id_fragmento}"
            )
        else:
        ...
        logger.trace(
            f"Respuesta LLM (cruda) para citas y datos de {resultado_fase_2.id_fragmento}:\n{respuesta_llm_cruda}"
        )
        ...
        ids_entidades_fase2 = {
            e.id_entidad for e in resultado_fase_2.entidades_extraidas
        }
        ...
        for cita in citas_extraidas:
            if (
                cita.id_entidad_citada
                and cita.id_entidad_citada not in ids_entidades_fase2
            ):
                advertencias.append(
                    f"Cita {cita.id_cita} referencia entidad inexistente: {cita.id_entidad_citada}"
                )
            # Validar hecho_id desde respuesta_llm_bruta
            hecho_id = (
                cita.respuesta_llm_bruta.get("hecho_id")
                if cita.respuesta_llm_bruta
                else None
            )
            if hecho_id and hecho_id not in ids_hechos_fase2:
                advertencias.append(
                    f"Cita {cita.id_cita} referencia hecho inexistente: {hecho_id}"
                )
        ...
        for dato in datos_extraidos:
            # Validar hecho_id desde respuesta_llm_bruta
            hecho_id = (
                dato.respuesta_llm_bruta.get("hecho_id")
                if dato.respuesta_llm_bruta
                else None
            )
            if hecho_id and hecho_id not in ids_hechos_fase2:
                advertencias.append(
                    f"Dato {dato.id_dato_cuantitativo} referencia hecho inexistente: {hecho_id}"
                )

        # Generar resumen si no hay elementos extraídos
        if not citas_extraidas and not datos_extraidos:
            advertencias.append(
                "No se extrajeron citas ni datos cuantitativos del texto"
            )
            logger.warning(
                f"No se extrajeron elementos del fragmento {resultado_fase_2.id_fragmento}"
            )

    except GroqAPIError as e:
        logger.error(
            f"GroqAPIError durante extracción de citas y datos para {resultado_fase_2.id_fragmento}: {e}"
        )
        error_dict = handle_generic_phase_error(
            article_id=str(resultado_fase_2.id_fragmento),
            phase=ErrorPhase.FASE_3_CITAS_DATOS,
        ...
        logger.error(
            f"Error inesperado durante extracción de citas y datos para {resultado_fase_2.id_fragmento}: {e}"
        )
        error_dict = handle_generic_phase_error(
            article_id=str(resultado_fase_2.id_fragmento),
            phase=ErrorPhase.FASE_3_CITAS_DATOS,
    ...
    metadata_citas_datos = {
        ...
        "json_paso_1_elementos": len(resultado_fase_2.hechos_extraidos)
        + len(resultado_fase_2.entidades_extraidas),
...
if __name__ == "__main__":
    ...
    try:
        ...
        resultado = ejecutar_fase_3(
            resultado_fase2_mock, processor, resultado_fase1_mock
        )
        print(
            f"✅ Test exitoso: {len(resultado.citas_textuales_extraidas)} citas, {len(resultado.datos_cuantitativos_extraidos)} datos"
        )
