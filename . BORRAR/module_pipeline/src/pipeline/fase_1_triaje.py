...
from ..models.analisis import AnalisisComponentes, FlowDecision  # noqa: F401
...
from ..models.procesamiento import MetadatosFase1Triaje, ResultadoFase1Triaje
from ..services.adaptive_flow_controller import (  # noqa: F401
    AdaptiveFlowController,
    create_adaptive_flow_controller,
)

# Importar componentes de análisis y control de flujo
from ..services.spacy_analyzer import SpacyAnalyzer, create_spacy_analyzer  # noqa: F401
...
def _cargar_modelo_spacy(modelo: str = "es_core_news_lg") -> Optional[Language]:
    ...
    if not spacy:
        logger.warning(
            "spaCy no está instalado. No se puede cargar ningún modelo."
        )  # Changed to warning
        return None
    ...
    if modelo not in _NLP_MODELS_CACHE:
        try:
            ...
            logger.warning(
                f"No se pudo cargar el modelo spaCy '{modelo}'. "  # Changed to warning
                f"Asegúrate de que está descargado (ej: python -m spacy download {modelo})"
            )
            _NLP_MODELS_CACHE[modelo] = (
                None  # Marcar como no cargado para no reintentar innecesariamente
            )
        except Exception as e:
            logger.warning(
                f"Error inesperado al cargar el modelo spaCy '{modelo}': {e}"
            )  # Changed to warning
            _NLP_MODELS_CACHE[modelo] = None

    if (
        _NLP_MODELS_CACHE.get(modelo) is None and modelo != "es_core_news_lg"
    ):  # Evitar recursión infinita si el default falla
        logger.warning(
            f"Fallback: Intentando cargar modelo por defecto 'es_core_news_lg' en lugar de '{modelo}'."
        )
...
def _limpiar_texto(texto_original: str, nlp_model: Optional[Language]) -> str:
    ...
    if not nlp_model:
        logger.warning(
            "Modelo spaCy no disponible para limpieza. Devolviendo texto original con limpieza básica."
        )
    ...
    logger.debug(
        f"Limpiando texto con modelo: {nlp_model.meta.get('name', 'desconocido')}"
    )
    ...
    for token in doc:
        if token.is_space:
            ...
            and all(c == token.text[0] for c in token.text)
        ):
            ...
            tokens_limpios.append(
                token.text
            )  # Usar token.text para mantener el case original
    ...
    texto_limpio = re.sub(
        r"[ \t\r\f\v]+", " ", texto_semi_limpio
    )  # Reemplaza espacios horizontales múltiples por uno
    ...
    logger.trace(
        f"Texto original: '{texto_original[:100]}...' -> Texto limpio: '{texto_limpio[:100]}...'"
    )
...
def _detectar_idioma(texto_para_detectar: str, nlp_model: Optional[Language]) -> str:
    ...
    if not nlp_model:
        logger.warning(
            "Modelo spaCy no disponible para detección de idioma. Devolviendo 'und'."
        )
    ...
    logger.debug(
        f"Idioma identificado por el modelo '{nlp_model.meta.get('name', 'desconocido')}': {lang_code}"
    )
    logger.warning(
        f"Recordatorio: La detección de idioma con el modelo base '{nlp_model.meta.get('name', 'desconocido')}' "
...
def ejecutar_fase_1(
    ...
    try:
        ...
        fragmento_validado = FragmentoProcesableItem(
            ...
            id_articulo_fuente=str(
                id_fragmento_original
            ),  # Usando el mismo ID por ahora
        )
        texto_original_fragmento = (
            fragmento_validado.texto_original
        )  # Usar texto limpio
        logger.debug(
            f"Fragmento {id_fragmento_original}: Texto validado y limpiado por FragmentoProcesableItem"
        )
    except Exception as e:
        logger.warning(
            f"Error al validar FragmentoProcesableItem para {id_fragmento_original}: {e}"
        )
    ...
    try:
        ...
        if not nlp_model:  # Si el modelo (incluido el fallback) no se pudo cargar
            ...
            logger.warning(
                f"Fallo crítico: _cargar_modelo_spacy devolvió None para el modelo '{modelo_spacy_nombre}' para el fragmento {id_fragmento_original}."
            )  # Changed to warning
            # Simular una excepción para que el handler la capture de forma uniforme
            raise RuntimeError(
                f"Modelo spaCy '{modelo_spacy_nombre}' o fallback no disponible (devuelto como None)."
            )

        texto_limpio = _limpiar_texto(texto_original_fragmento, nlp_model)
        idioma_detectado = _detectar_idioma(
            texto_limpio, nlp_model
        )  # This is 'nlp_model.lang' or 'und'

        # --- NUEVA FUNCIONALIDAD: Análisis de Contenido con spaCy ---
        logger.debug(
            f"Iniciando análisis de contenido con SpacyAnalyzer para fragmento {id_fragmento_original}"
        )
        spacy_analyzer = create_spacy_analyzer(nlp_model)
        analisis_contenido = spacy_analyzer.analizar_contenido(
            texto_limpio, modelo_spacy_nombre
        )

        # Validar análisis de contenido
        es_valido, advertencias = spacy_analyzer.validar_metricas(analisis_contenido)
        if advertencias:
            logger.warning(
                f"Advertencias en análisis de contenido para {id_fragmento_original}: {advertencias}"
            )
            notas_procesamiento.extend(
                [f"Advertencia análisis: {adv}" for adv in advertencias]
            )

        # --- NUEVA FUNCIONALIDAD: Control de Flujo Adaptativo ---
        logger.debug(
            f"Generando decisiones de flujo con AdaptiveFlowController para fragmento {id_fragmento_original}"
        )
        flow_controller = create_adaptive_flow_controller()
        decisiones_flujo = flow_controller.evaluate_content(analisis_contenido)

        # Validar decisiones de flujo
        if not flow_controller.validate_decision(decisiones_flujo, analisis_contenido):
            logger.warning(
                f"Decisiones de flujo incoherentes para fragmento {id_fragmento_original}"
            )
            notas_procesamiento.append("Decisiones de flujo requieren revisión manual")

        logger.info(
            f"Análisis completado para fragmento {id_fragmento_original}: "
            f"{analisis_contenido.conteo_entidades} entidades, "
            f"{analisis_contenido.conteo_datos} datos, "
            f"{analisis_contenido.conteo_citas} citas"
        )

        # Obtener estadísticas de procesamiento
        stats_procesamiento = flow_controller.get_processing_stats(decisiones_flujo)
        logger.debug(
            f"Estadísticas de procesamiento para {id_fragmento_original}: {stats_procesamiento}"
        )

        # Agregar información de análisis a las notas
        if decisiones_flujo.justificacion:
            notas_procesamiento.append(
                f"Decisiones de flujo: {decisiones_flujo.justificacion}"
            )
        ...
        metadatos_fase1 = MetadatosFase1Triaje(
            ...
            # Agregar resultados de análisis de contenido
            analisis_contenido=analisis_contenido.model_dump()
            if analisis_contenido
            else None,
            decisiones_flujo=decisiones_flujo.model_dump()
            if decisiones_flujo
            else None,
            estadisticas_procesamiento=stats_procesamiento,
        )
        ...
        if idioma_detectado not in idiomas_soportados:
            logger.warning(
                f"Idioma detectado '{idioma_detectado}' no está en la lista de soportados para {id_fragmento_original}"
            )
            idioma_detectado = "und"

        if idioma_detectado == "und":
            logger.info(
                f"Idioma detectado como 'und' para fragmento {id_fragmento_original}. Asumiendo 'es' por defecto según documentación."
            )
            idioma_detectado = "es"
            notas_procesamiento.append(
                "Idioma asumido como 'es' por fallo en detección."
            )

        logger.debug(
            f"Fragmento {id_fragmento_original}: Texto limpio generado, Idioma para LLM: {idioma_detectado}"
        )

    except Exception as e_spacy:
        logger.warning(
            f"Excepción durante carga de spaCy o preprocesamiento inicial para {id_fragmento_original}: {e_spacy}"
        )
        ...
        current_notes = list(
            notas_procesamiento
        )  # Preserve any prior notes (e.g. from language detection if it ran partially)
        if fallback_dict.get("metadatos_specificos_triaje", {}).get("message"):
            current_notes.append(
                fallback_dict["metadatos_specificos_triaje"]["message"]
            )

        # For SpaCy failure, texto_limpio_utilizado is the placeholder from handler, idioma original is "und"
        # No content analysis or flow decisions available due to spaCy failure
        metadatos_error = MetadatosFase1Triaje(
            nombre_modelo_triaje=modelo_spacy_nombre,
            texto_limpio_utilizado=fallback_dict.get(
                "texto_para_siguiente_fase", "[PREPROCESAMIENTO_FALLIDO]"
            ),
            ...
            analisis_contenido=None,  # Analysis failed due to spaCy error
            decisiones_flujo=None,  # Flow decisions unavailable
            estadisticas_procesamiento=None,  # No stats available
        )
        ...
        resultado = ResultadoFase1Triaje(
            ...
            decision_triaje=fallback_dict[
                "decision_triaje"
            ],  # "FALLBACK_ACEPTADO_ERROR_PREPROCESO"
    ...
    if not groq_config.get("api_key"):
        logger.warning(
            f"No se encontró GROQ_API_KEY. Aplicando fallback para {id_fragmento_original}."
        )

        api_key_exception = RuntimeError(
            "GROQ_API_KEY no configurada. Artículo aceptado por política de fallback."
        )
        ...
        metadatos_fase1.nombre_modelo_triaje = groq_config.get(
            "model_id", "desconocido_por_no_api_key"
        )
        metadatos_fase1.notas_adicionales = (
            notas_procesamiento if notas_procesamiento else None
        )
        ...
        resultado = ResultadoFase1Triaje(
            ...
            puntuacion_triaje=fallback_dict["puntuacion_triaje"],
            confianza_triaje=fallback_dict["confianza_triaje"],
            texto_para_siguiente_fase=fallback_dict[
                "texto_para_siguiente_fase"
            ],  # This is texto_limpio via handler
    ...
    try:
        ...
        if not respuesta_llm_cruda or not isinstance(respuesta_llm_cruda, str):
            logger.error(
                f"Respuesta LLM inválida para {id_fragmento_original}: tipo {type(respuesta_llm_cruda)}"
            )
            raise GroqAPIError(
        ...
        if len(respuesta_llm_cruda.strip()) < 50:
            logger.warning(
                f"Respuesta LLM sospechosamente corta para {id_fragmento_original}: {len(respuesta_llm_cruda)} caracteres"
            )
            notas_procesamiento.append("Respuesta LLM más corta de lo esperado")

        logger.trace(
            f"Respuesta LLM (cruda) para triaje de {id_fragmento_original}:\n{respuesta_llm_cruda}"
        )
        evaluacion_triaje = _parsear_respuesta_triaje(respuesta_llm_cruda)

        if (
            evaluacion_triaje.get("decision") == "ERROR_TRIAGE"
        ):  # Check specific error decision from parser
            logger.warning(
                f"Parseo de respuesta LLM resultó en error para {id_fragmento_original}. Aplicando fallback. Justificación: {evaluacion_triaje.get('justificacion')}"
            )
        ...
        logger.error(
            f"GroqAPIError durante evaluación de relevancia para {id_fragmento_original}: {e_groq_relevance}"
        )
        fallback_dict = handle_groq_relevance_error_fase1(
            article_id=str(id_fragmento_original),
        ...
        metadatos_fase1.nombre_modelo_triaje = groq_config.get(
            "model_id", "desconocido_por_error_api"
        )
        if fallback_dict.get("metadatos_specificos_triaje", {}).get("message"):
            notas_procesamiento.append(
                fallback_dict["metadatos_specificos_triaje"]["message"]
        ...
        return resultado
    logger.debug(
        f"Evaluación de triaje parseada para {id_fragmento_original}: {evaluacion_triaje}"
    )

    # --- Subtarea 15.4: Implementación de Lógica de Traducción ---
    texto_para_siguiente_fase = (
        texto_limpio  # Default, use cleaned (original language) text
    )

    if (
        idioma_detectado
        and idioma_detectado.lower() != "es"
        and evaluacion_triaje.get("decision") != "DESCARTAR"
    ):
        if texto_limpio and texto_limpio.strip():
            logger.info(
                f"Traduciendo texto de '{idioma_detectado}' a 'es' para fragmento {id_fragmento_original}."
            )
            try:
                # Sanitizar texto antes de traducir
                texto_sanitizado_traduccion = escape_html(texto_limpio)
                texto_traducido_llm = (
                    _llamar_groq_api_traduccion(  # Use _llamar_groq_api_traduccion
                        config=groq_config_with_id,  # Use config with article_id
                ...
                logger.debug(
                    f"Texto traducido para {id_fragmento_original}: '{texto_traducido_llm[:100]}...'"
                )
                ...
                logger.warning(
                    f"GroqAPIError durante traducción para {id_fragmento_original}: {e_trans_groq}. Se usará texto original."
                )
            ...
            logger.debug(
                f"No se requiere traducción para {id_fragmento_original} (texto limpio vacío)."
            )
    else:
        logger.debug(
            f"No se requiere traducción para {id_fragmento_original} (idioma: {idioma_detectado}, decisión triaje: {evaluacion_triaje.get('decision')})."
        )

    # --- Subtarea 15.5: Ensamblaje final del objeto ResultadoFase1Triaje ---
    metadatos_fase1.nombre_modelo_triaje = groq_config.get(
        "model_id", "llama3-8b-8192"
    )  # Default if not set by error handling
    metadatos_fase1.notas_adicionales = (
        notas_procesamiento if notas_procesamiento else None
    )
    ...
    if puntuacion_triaje_val is not None:
        try:
            ...
            logger.warning(
                f"No se pudo convertir puntuacion_triaje '{puntuacion_triaje_val}' a float. Se usará None."
            )
    ...
    logger.info(
        f"Fase 1: Triaje completada para fragmento ID: {id_fragmento_original}. "
...
_PROMPT_TRIAGE_PATH = (
    Path(__file__).resolve().parent.parent.parent / "prompts" / "Prompt_1_filtrado.md"
)
...
def _load_prompt_template() -> str:
    ...
    if _PROMPT_TRIAGE_TEMPLATE is None:
        try:
            with open(_PROMPT_TRIAGE_PATH, encoding="utf-8") as f:
                _PROMPT_TRIAGE_TEMPLATE = f.read()
            logger.info(
                f"Plantilla de prompt de triaje cargada desde: {_PROMPT_TRIAGE_PATH}"
            )
        except FileNotFoundError:
            logger.error(
                f"No se encontró el archivo de prompt de triaje en: {_PROMPT_TRIAGE_PATH}"
            )
            raise ErrorFase1(
                f"Archivo de prompt de triaje no encontrado: {_PROMPT_TRIAGE_PATH}"
            )
        except Exception as e:
...
def _get_groq_config() -> Dict[str, Any]:
    ...
    return {
        ...
        "max_tokens": int(
            os.getenv("GROQ_API_MAX_TOKENS", "1000")
        ),  # Ajustado para triaje
...
def _llamar_groq_api_triaje(
    ...
    try:
        ...
        logger.error(
            f"Fallo al cargar plantilla de prompt para triaje. Artículo: {article_id_log}. Error: {e_prompt}"
        )
        raise GroqAPIError(
            f"Fallo al cargar plantilla de prompt para triaje: {e_prompt}",
            phase=ErrorPhase.FASE_1_TRIAJE,
            article_id=config.get("article_id"),
        ) from e_prompt

    prompt_formateado = (
        prompt_template.replace("{{TITULO}}", titulo)
        .replace("{{MEDIO}}", medio)
        .replace("{{PAIS}}", pais)
    ...
    logger.info(
        f"Enviando solicitud a Groq API para triaje (modelo: {config['model_id']}). Artículo: {article_id_log}"
    )
    ...
    if not respuesta_contenido or not respuesta_contenido.strip():
        logger.warning(
            f"Groq API para triaje devolvió una respuesta vacía. Artículo: {article_id_log}"
        )
        raise GroqAPIError(
            "Respuesta vacía de Groq API para triaje.",
            phase=ErrorPhase.FASE_1_TRIAJE,
            article_id=config.get("article_id"),
        )
    logger.info(
        f"Respuesta recibida de Groq API para triaje. Artículo: {article_id_log}"
    )
...
def _parsear_respuesta_triaje(respuesta_llm: str) -> Dict[str, Any]:
    ...
    parsed_data = {
        "exclusion": None,
        "exclusion_categoria": None,
        "tipo_articulo": "INDETERMINADO",  # Default
        "relevancia_geografica_score": None,
        "relevancia_geografica_pais": None,
        "relevancia_tematica_score": None,
        "relevancia_tematica_tema": None,
    ...
    try:
        ...
        elif respuesta_llm_stripped.startswith(
            "```"
        ):  # Handle cases like ```\n{\n...}\n```
        ...
        parsed_data["justificacion"] = data.get(
            "justificacion", "Sin justificación del LLM (JSON)."
        )
        parsed_data["tipo_articulo"] = data.get(
            "tipo_articulo", "INDETERMINADO"
        ).upper()
        parsed_data["total_puntuacion"] = data.get(
            "total_puntuacion"
        )  # Será validado a float después
        parsed_data["elementos_clave"] = data.get("elementos_clave", [])

        # Mapear puntuaciones individuales si están presentes
        parsed_data["relevancia_geografica_score"] = data.get(
            "relevancia_geografica_score"
        )
        parsed_data["relevancia_geografica_pais"] = data.get(
            "relevancia_geografica_pais"
        )
        ...
        parsed_data["complejidad_relacional_score"] = data.get(
            "complejidad_relacional_score"
        )
        ...
        for score_field in [
            "relevancia_geografica_score",
            "relevancia_tematica_score",
            "densidad_factual_score",
            "complejidad_relacional_score",
            "valor_informativo_score",
            "total_puntuacion",
        ]:
            val = parsed_data.get(score_field)
            if val is not None:
                try:
                    parsed_data[score_field] = (
                        float(val) if "." in str(val) else int(val)
                    )
                    # Validar rango de puntuaciones individuales (0-5)
                    if (
                        score_field != "total_puntuacion"
                        and parsed_data[score_field] is not None
                    ):
                        if not 0 <= parsed_data[score_field] <= 5:
                            logger.warning(
                                f"Puntuación '{score_field}' fuera de rango [0-5]: {parsed_data[score_field]}"
                            )
                            parsed_data[score_field] = max(
                                0, min(5, parsed_data[score_field])
                            )  # Clamp al rango
                except (ValueError, TypeError):
                    logger.warning(
                        f"Valor de puntuación JSON '{score_field}' ('{val}') no es numérico. Usando None."
                    )
        ...
        if parsed_data.get("total_puntuacion") is not None:
            if not 0 <= parsed_data["total_puntuacion"] <= 25:
                logger.warning(
                    f"Puntuación total fuera de rango [0-25]: {parsed_data['total_puntuacion']}"
                )
                parsed_data["total_puntuacion"] = max(
                    0, min(25, parsed_data["total_puntuacion"])
                )
        return parsed_data

    except json.JSONDecodeError:
        logger.debug(
            "Respuesta LLM para triaje no es un JSON válido, intentando parseo basado en regex."
        )
    ...
    # TIPO DE ARTÍCULO
    match = re.search(
        r"TIPO DE ARTÍCULO:\s*([A-ZÁÉÍÓÚÑ\s]+?)\n", respuesta_llm, re.IGNORECASE
    )
    if match:
    ...
    for key_text, (score_field, detail_field) in score_map.items():
        ...
        if match:
            try:
                ...
                logger.warning(
                    f"No se pudo parsear la puntuación REGEX para '{key_text}'. Valor: {match.group(1)}"
                )
    ...
    if match:
        try:
            ...
            logger.warning(
                f"No se pudo parsear la puntuación total. Valor: {match.group(1)}"
            )

    # DECISIÓN
    match = re.search(
        r"DECISIÓN:\s*(PROCESAR|CONSIDERAR|DESCARTAR)", respuesta_llm, re.IGNORECASE
    )
    if match:
    ...
    if elementos_match:
        ...
        parsed_data["elementos_clave"] = [
            line.strip()[1:].strip()
            for line in elementos_texto.split("\n")
...
def _llamar_groq_api_traduccion(
    ...
    if not Groq:
        logger.error(
            f"SDK de Groq no instalado para traducción. Artículo: {article_id_log}"
        )
        raise GroqAPIError(
            "SDK de Groq no instalado.",
            phase=ErrorPhase.FASE_1_TRIAJE,
    ...
    if not config.get("api_key"):
        logger.error(
            f"GROQ_API_KEY no configurada para traducción. Artículo: {article_id_log}"
        )
        raise GroqAPIError(
            "GROQ_API_KEY no configurada para traducción.",
            phase=ErrorPhase.FASE_1_TRIAJE,
            article_id=config.get("article_id"),
        )
    ...
    logger.info(
        f"Enviando solicitud a Groq API para traducción (modelo: {config['model_id']}). Artículo: {article_id_log}"
    )
    chat_completion = client.chat.completions.create(
        messages=[
            {
    ...
    if not respuesta_contenido or not respuesta_contenido.strip():
        logger.warning(
            f"Groq API para traducción devolvió una respuesta vacía. Idioma origen: {idioma_origen}. Artículo: {article_id_log}"
        )
        raise GroqAPIError(
            "Respuesta vacía o solo espacios de Groq API para traducción.",
            phase=ErrorPhase.FASE_1_TRIAJE,
            article_id=config.get("article_id"),
        )
    ...
    for prefijo in prefijos_error:
        if respuesta_limpia.lower().startswith(prefijo):
            logger.warning(
                f"Respuesta de traducción parece contener un mensaje de error para {article_id_log}: {respuesta_limpia[:50]}..."
            )
    ...
    logger.info(
        f"Respuesta de traducción recibida de Groq API. Idioma origen: {idioma_origen}. Artículo: {article_id_log}"
    )
