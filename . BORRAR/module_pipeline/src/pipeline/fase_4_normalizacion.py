...
from ..utils.json_parser import analyze_llm_response_format, parse_llm_json_response
...
_PROMPT_RELACIONES_PATH = (
    Path(__file__).resolve().parent.parent.parent / "prompts" / "Prompt_4_relaciones.md"
)
...
def _load_prompt_template() -> str:
    ...
    if _PROMPT_RELACIONES_TEMPLATE is None:
        try:
            with open(_PROMPT_RELACIONES_PATH, encoding="utf-8") as f:
                _PROMPT_RELACIONES_TEMPLATE = f.read()
            logger.info(
                f"Plantilla de prompt de relaciones cargada desde: {_PROMPT_RELACIONES_PATH}"
            )
        except FileNotFoundError:
            logger.error(
                f"No se encontró el archivo de prompt de relaciones en: {_PROMPT_RELACIONES_PATH}"
            )
            raise ErrorFase4(
                f"Archivo de prompt de relaciones no encontrado: {_PROMPT_RELACIONES_PATH}"
            )
...
def _get_groq_config() -> Dict[str, Any]:
    ...
    return {
        ...
        "max_tokens": int(
            os.getenv("GROQ_API_MAX_TOKENS", "6000")
        ),  # Límite ajustado para LLAMA 3.1 8B Instant
    ...
    for entidad in entidades:
        ...
        if cache_key in _NORMALIZATION_CACHE:
            ...
            try:
                ...
                logger.error(
                    f"Error al normalizar entidad '{entidad.texto_entidad}': {e}"
                )
                errores += 1
                continue
        ...
        if (
            not resultado.get("es_nueva", True)
            and resultado.get("score_similitud", 0) > umbral_similitud
        ):
            # Entidad encontrada y supera el umbral
            entidad.id_entidad_normalizada = UUID(
                str(resultado["id_entidad_normalizada"])
            )
            ...
            if resultado.get("uri_wikidata"):
                try:
                    ...
                    logger.warning(
                        f"URI de Wikidata inválida para entidad '{entidad.texto_entidad}': {e}"
                    )
    ...
    logger.info(
        ...
        extra={
            ...
            "cache_hits": len(
                [
                    k
                    for k in _NORMALIZATION_CACHE
                    if k in [f"{e.texto_entidad}:{e.tipo_entidad}" for e in entidades]
                ]
            ),
        },
    )
...
def _formatear_elementos_para_prompt(
    ...
    for entidad in resultado_fase_2.entidades_extraidas:
        entidad_dict = {
            ...
            "normalizada": entidad.nombre_entidad_normalizada
            if entidad.id_entidad_normalizada
            else None,
            "id_normalizada": str(entidad.id_entidad_normalizada)
            if entidad.id_entidad_normalizada
            else None,
        }
        entidades_json.append(entidad_dict)

    elementos_basicos = {"hechos": hechos_json, "entidades": entidades_json}
...
def _llamar_groq_api_relaciones(
    ...
    try:
        ...
        logger.error(
            f"Fallo al cargar plantilla de prompt para relaciones. Artículo: {article_id_log}. Error: {e_prompt}"
        )
        raise GroqAPIError(
            f"Fallo al cargar plantilla de prompt para relaciones: {e_prompt}",
            phase=ErrorPhase.FASE_4_NORMALIZACION,
            article_id=config.get("article_id"),
        ) from e_prompt
    ...
    prompt_formateado = (
        prompt_template.replace("{{TITULO_O_DOCUMENTO}}", titulo_documento)
    ...
    logger.info(
        f"Enviando solicitud a Groq API para relaciones (modelo: {config['model_id']}). Artículo: {article_id_log}"
    )
    ...
    if not respuesta_contenido or not respuesta_contenido.strip():
        logger.warning(
            f"Groq API para relaciones devolvió una respuesta vacía. Artículo: {article_id_log}"
        )
        raise GroqAPIError(
            "Respuesta vacía de Groq API para relaciones.",
            phase=ErrorPhase.FASE_4_NORMALIZACION,
            article_id=config.get("article_id"),
        )

    logger.info(
        f"Respuesta recibida de Groq API para relaciones. Artículo: {article_id_log}"
    )
...
def _parsear_relaciones_hecho_entidad(
    ...
    for rel in relaciones_json:
        ...
        if hecho_id not in ids_hechos_validos:
            logger.warning(
                f"Relación hecho-entidad con hecho_id inexistente: {hecho_id}"
            )
            continue
        if entidad_id not in ids_entidades_validas:
            logger.warning(
                f"Relación hecho-entidad con entidad_id inexistente: {entidad_id}"
            )
            continue
...
def _parsear_relaciones_hecho_hecho(
    relaciones_json: List[Dict[str, Any]], ids_hechos_validos: set
    ...
    for rel in relaciones_json:
        ...
        if hecho_origen_id not in ids_hechos_validos:
            logger.warning(
                f"Relación hecho-hecho con hecho_origen_id inexistente: {hecho_origen_id}"
            )
            continue
        if hecho_destino_id not in ids_hechos_validos:
            logger.warning(
                f"Relación hecho-hecho con hecho_destino_id inexistente: {hecho_destino_id}"
            )
            continue
        ...
        if hecho_origen_id == hecho_destino_id:
            logger.warning(
                f"Relación circular detectada: hecho {hecho_origen_id} consigo mismo"
            )
            continue
...
def _parsear_relaciones_entidad_entidad(
    relaciones_json: List[Dict[str, Any]], ids_entidades_validas: set
    ...
    for rel in relaciones_json:
        ...
        if entidad_origen_id not in ids_entidades_validas:
            logger.warning(
                f"Relación entidad-entidad con entidad_origen_id inexistente: {entidad_origen_id}"
            )
            continue
        if entidad_destino_id not in ids_entidades_validas:
            logger.warning(
                f"Relación entidad-entidad con entidad_destino_id inexistente: {entidad_destino_id}"
            )
            continue
        ...
        if entidad_origen_id == entidad_destino_id:
            logger.warning(
                f"Relación circular detectada: entidad {entidad_origen_id} consigo misma"
            )
            continue
        ...
        if tipo_relacion not in tipos_validos:
            logger.warning(
                f"Tipo de relación entidad-entidad inválido: {tipo_relacion}"
            )
            continue
        ...
        try:
            ...
            if fecha_inicio_validada and fecha_fin_validada:
                ...
                if datetime.strptime(
                    fecha_inicio_validada, "%Y-%m-%d"
                ) > datetime.strptime(fecha_fin_validada, "%Y-%m-%d"):
                    logger.warning(
                        f"Fechas inválidas en relación: inicio posterior a fin. Intercambiando."
                    )  # noqa: F541
                    fecha_inicio_validada, fecha_fin_validada = (
                        fecha_fin_validada,
                        fecha_inicio_validada,
                    )
        except ValueError as e:
...
def _parsear_contradicciones(
    contradicciones_json: List[Dict[str, Any]], ids_hechos_validos: set
) -> List[Contradiccion]:
    """
    ...
    for cont in contradicciones_json:
        ...
        if hecho_principal_id not in ids_hechos_validos:
            logger.warning(
                f"Contradicción con hecho_principal_id inexistente: {hecho_principal_id}"
            )
            continue
        if hecho_contradictorio_id not in ids_hechos_validos:
            logger.warning(
                f"Contradicción con hecho_contradictorio_id inexistente: {hecho_contradictorio_id}"
            )
            continue
...
def _procesar_relaciones_desde_json(
    respuesta_json: Dict[str, Any], resultado_fase_2: ResultadoFase2Extraccion
) -> Dict[str, Any]:
    """
    ...
    logger.debug(
        f"IDs válidos - Hechos: {ids_hechos_validos}, Entidades: {ids_entidades_validas}"
    )
    ...
    relaciones_hecho_hecho = _parsear_relaciones_hecho_hecho(
        respuesta_json.get("hecho_relacionado", []), ids_hechos_validos
    )

    relaciones_entidad_entidad = _parsear_relaciones_entidad_entidad(
        respuesta_json.get("entidad_relacion", []), ids_entidades_validas
    )

    contradicciones = _parsear_contradicciones(
        respuesta_json.get("contradicciones", []), ids_hechos_validos
    )
...
def ejecutar_fase_4(
    ...
    logger.info(
        f"Iniciando Fase 4: Normalización y Relaciones para fragmento ID: {id_fragmento}"
    )
    ...
    try:
        ...
        if resultado_fase_2.entidades_extraidas and supabase_service:
            logger.info(
                f"Normalizando {len(resultado_fase_2.entidades_extraidas)} entidades"
            )
            entidades_normalizadas = _normalizar_entidades_extraidas(
                resultado_fase_2.entidades_extraidas, supabase_service
            )
        ...
        if not groq_config.get("api_key"):
            logger.error(
                f"No se encontró GROQ_API_KEY. No se puede procesar fragmento {id_fragmento}"
            )
            # Aplicar fallback
            ...
            elementos_basicos, elementos_complementarios = (
                _formatear_elementos_para_prompt(resultado_fase_2, resultado_fase_3)
            )
            ...
            try:
                ...
                logger.trace(
                    f"Respuesta LLM (cruda) para relaciones de {id_fragmento}:\n{respuesta_llm_cruda}"
                )
                ...
                try:
                    ...
                    respuesta_json = parse_llm_json_response(
                        respuesta_llm_cruda, attempt_repair=True, strict=False
                    )

                    # Si el parseo devolvió un dict vacío, es un error parcial
                    if not respuesta_json:
                        logger.error(
                            "Parser devolvió diccionario vacío, no se pudieron extraer relaciones"
                        )
                        advertencias.append(
                            "No se pudo extraer JSON válido de la respuesta LLM para relaciones"
                        )
                        ...
                        if format_metrics["has_markdown"]:
                            logger.info(
                                "JSON parseado exitosamente después de limpiar markdown"
                            )
                        if format_metrics["is_truncated"]:
                            logger.warning(
                                f"JSON estaba truncado: {format_metrics['truncation_reason']}"
                            )
                            advertencias.append(
                                f"Respuesta LLM truncada: {format_metrics['truncation_reason']}"
                            )

                        # Procesar relaciones
                        relaciones_procesadas = _procesar_relaciones_desde_json(
                            respuesta_json, resultado_fase_2
                        )
                ...
                logger.error(
                    f"GroqAPIError durante extracción de relaciones para {id_fragmento}: {e}"
                )
                error_dict = handle_generic_phase_error(
                    article_id=str(id_fragmento),
                    phase=ErrorPhase.FASE_4_NORMALIZACION,
    ...
    num_normalizadas = len(
        [e for e in entidades_normalizadas if e.id_entidad_normalizada]
    )
    ...
    metadata_normalizacion = {
        ...
        "tokens_respuesta": len(respuesta_llm_cruda.split())
        ...
        "contradicciones_detectadas": len(
            relaciones_procesadas.get("contradicciones", [])
        ),
    ...
    if relaciones_procesadas:
        resultado.metadata_normalizacion["relaciones"] = {
            "hecho_entidad": [
                r.to_dict()
                for r in relaciones_procesadas.get("relaciones_hecho_entidad", [])
            ],
            "hecho_hecho": [
                r.to_dict()
                for r in relaciones_procesadas.get("relaciones_hecho_hecho", [])
            ],
            "entidad_entidad": [
                r.to_dict()
                for r in relaciones_procesadas.get("relaciones_entidad_entidad", [])
            ],
            "contradicciones": [
                c.to_dict() for c in relaciones_procesadas.get("contradicciones", [])
            ],
        }
...
if __name__ == "__main__":
    ...
    from ..models.metadatos import (
        MetadatosCita,
        MetadatosDato,
        MetadatosEntidad,
        MetadatosHecho,
    )
    from ..models.procesamiento import EntidadProcesada, HechoProcesado
    ...
    try:
        ...
        print(f"✅ Processor summary:")  # noqa: F541
