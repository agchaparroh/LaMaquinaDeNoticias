...
def _preparar_contexto_relaciones(
    ...
    contexto = {
        ...
        "entidades": [
            {"id": e.id_entidad, "nombre": e.texto_entidad, "tipo": e.tipo_entidad}
            for e in entidades
        ],
    }
    ...
    for entidad in entidades:
        try:
            ...
            resultado = normalizador.normalizar_entidad(
                ...
                metadata=entidad.metadata_entidad,
            )

            if resultado["encontrada"]:
                # Actualizar entidad con información normalizada
                entidad.id_entidad_normalizada = resultado["id_entidad"]
                entidad.nombre_entidad_normalizada = resultado["nombre_canónico"]
                entidad.uri_wikidata = resultado.get("uri_wikidata")
                entidad.similitud_normalizacion = resultado["similitud"]
            ...
            logger.error(
                f"Error normalizando entidad {entidad.texto_entidad}: {str(e)}"
            )
...
def ejecutar_fase_7_completa(
    ...
    try:
        ...
        error_info = handle_generic_phase_error(
            ...
            context={"hechos_count": len(hechos), "entidades_count": len(entidades)},
        )
