...
class ChunkingConfig(BaseModel):
    ...
    max_chunk_size: int = Field(default=3000, description="Tamaño máximo de cada chunk en caracteres")
    overlap_size: int = Field(default=200, description="Tamaño de superposición entre chunks")
    context_window: int = Field(default=100, description="Ventana de contexto adicional")
    min_chunk_size: int = Field(default=500, description="Tamaño mínimo para crear un chunk")
    respect_sentence_boundaries: bool = Field(default=True, description="Respetar límites de oraciones")
    respect_paragraph_boundaries: bool = Field(default=True, description="Respetar límites de párrafos")
    max_sentences_per_chunk: Optional[int] = Field(default=None, description="Máximo de oraciones por chunk")
...
class ChunkingService:
    ...
    def __init__(self, config: Optional[ChunkingConfig] = None, nlp_model: Optional[Language] = None):
        """
        ...
        for sent in doc.sents:
            ...
            if current_chars + sent_len > config.max_chunk_size and current_sentences:
                ...
                if config.overlap_size > 0 and len(current_sentences) > 1:
                    ...
                    current_start = chunks[-1].end_offset - sum(len(s) for s in overlap_sentences)
    ...
    def _create_simple_chunks(self, text: str, config: ChunkingConfig) -> List[TextChunk]:
        """
        ...
        while start < text_length:
            ...
            if end < text_length:
                ...
                if space_pos > start + chunk_size // 2:  # Si encontramos espacio en segunda mitad
                    end = space_pos
    ...
    def _enrich_chunks_with_context(
        ...
        for i, chunk in enumerate(chunks):
            ...
            if i > 0:
                chunk.context["previous_chunk_end"] = chunks[i-1].text[-50:] if len(chunks[i-1].text) > 50 else chunks[i-1].text
            
            if i < len(chunks) - 1:
                chunk.context["next_chunk_start"] = chunks[i+1].text[:50] if len(chunks[i+1].text) > 50 else chunks[i+1].text
