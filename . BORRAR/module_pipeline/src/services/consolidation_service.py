...
class ConsolidationService:
    ...
    def _calcular_similitud_entidades(
        ...
        nombre_sim = difflib.SequenceMatcher(
            None, entidad1.texto_entidad.lower(), entidad2.texto_entidad.lower()
        ).ratio()
        ...
        if entidad1.texto_entidad in alias2 or entidad2.texto_entidad in alias1:
            return 0.95
    ...
    def _fusionar_entidades(self, grupo: List[EntidadProcesada]) -> EntidadProcesada:
        ...
        if descripciones:
            entidad_base.metadata_entidad.descripcion_llm = " - ".join(
                set(descripciones)
            )
    ...
    def _calcular_similitud_hechos(
        ...
        tipo_bonus = (
            0.1
            if (
                hecho1.metadata_hecho.tipo_hecho_llm
                == hecho2.metadata_hecho.tipo_hecho_llm
            )
            else 0
        )
        ...
        fecha_bonus = 0
        if (
            hecho1.metadata_hecho.fecha_inicio_llm
            and hecho2.metadata_hecho.fecha_inicio_llm
        ):
            if (
                hecho1.metadata_hecho.fecha_inicio_llm
    ...
    def consolidar_datos(
        ...
        for dato in todos_datos:
            # Crear clave única
            clave = (dato.descripcion_dato.lower(), dato.valor_dato, dato.unidad_dato)
    ...
    def actualizar_referencias_cruzadas(
        ...
        for cita in citas:
            if cita.id_entidad_citada:
                cita.id_entidad_citada = mapeo_ids.get("entidades", {}).get(
                    cita.id_entidad_citada, cita.id_entidad_citada
                )
