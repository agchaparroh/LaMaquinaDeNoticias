...
class LoggingConfig:
    ...
    LOG_RETENTION_DAYS = {"development": 7, "staging": 30, "production": 90, "test": 1}
    ...
    def get_log_level(cls) -> str:
        ...
        if custom_level and custom_level.upper() in [
            "DEBUG",
            "INFO",
            "WARNING",
            "ERROR",
...
class PipelineLogger:
    ...
    def bind_request(self, request_id: str) -> "loguru.Logger":  # noqa: F821
        """
    ...
    def bind_component(
        self, component: str, request_id: Optional[str] = None
    ) -> "loguru.Logger":  # noqa: F821
        """
    ...
    def phase_context(self, phase: str, request_id: str, **extra):
        ...
        try:
            ...
            phase_logger.info(f"Completado {phase}", elapsed_seconds=elapsed, **extra)
...
def get_logger(component: str, request_id: Optional[str] = None) -> "loguru.Logger":  # noqa: F821
    """
...
class LogContext(BaseModel):
    ...
    def get_logger(self) -> "loguru.Logger":  # noqa: F821
        """Obtiene logger con contexto completo incluyendo span IDs."""
        context = {"request_id": self.request_id, "component": self.component}
    ...
    def create_child_span(
        self, child_phase: str, child_component: Optional[str] = None
    ) -> "LogContext":
...
def create_article_trace_context(
    request_id: str, fragment_id: str, component: str = "Pipeline"
) -> LogContext:
    """
...
def trace_phase(
    log_context: LogContext, phase_name: str, **extra_metadata
) -> LogContext:
    """
    ...
    span_logger.info(
        f"Iniciando span: {phase_name}", span_type="phase", **extra_metadata
    )
...
@contextmanager
def traced_phase(parent_context: LogContext, phase_name: str, **extra_metadata):
    """
...
@contextmanager
def log_phase_with_tracing(log_context: LogContext, phase_name: str, **extra):
    """
...
def test_logging_system():
    ...
    result = slow_function()  # noqa: F841

    # Test context manager
    pipeline_logger = PipelineLogger()
    with pipeline_logger.phase_context(
        "TestPhase", "TEST-002", fragment_id="frag-123"
    ) as ctx_logger:
    ...
    article_context = create_article_trace_context(
        request_id="REQ-123", fragment_id="FRAG-456", component="PipelineTest"
    )
    ...
    with traced_phase(
        article_context, "Fase1_Triaje_Test", test_metadata="valor_test"
    ) as fase1_context:
    ...
    with log_phase_with_tracing(
        article_context, "Fase2_Extraccion_Test", test_data="datos_prueba"
    ) as (fase2_logger, fase2_context):
